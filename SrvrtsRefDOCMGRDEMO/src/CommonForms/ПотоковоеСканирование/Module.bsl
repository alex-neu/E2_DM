...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	ИначеЕсли Источник = "TWAIN" И Событие = "EndBatch" Тогда
		...
		Если (ВторойПроходСканирования = Ложь) 
			...
			ПоказатьПредупреждение(
				ОписаниеОповещения, 
				НСтр("ru = 'Переверните и вставьте в сканер ту же пачку листов для сканирования оборотных сторон листов.'"));
			Возврат;
		КонецЕсли;	
	...
	ИначеЕсли Источник = "TWAIN" И Событие = "DiskSpaceLow" Тогда
		...
		ПоказатьПредупреждение(
			ОписаниеОповещения, 
			НСтр("ru = 'Недостаточно свободного места на диске. Потоковое сканирование прервано.'"));
...
Процедура ПриОткрытии(Отказ)
	...
	#Если ВебКлиент Тогда
		Отказ = Истина;
		ПоказатьПредупреждение(, НСтр("ru = 'В Веб-клиенте потоковое сканирование не поддерживается.'"));
		Возврат;
	#ИначеЕсли МобильныйКлиент Тогда
		Отказ = Истина;
		ПоказатьПредупреждение(, НСтр("ru = 'В Мобильном клиенте потоковое сканирование не поддерживается.'"));
		Возврат;
	#КонецЕсли
...
Процедура КаталогЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если РасширениеПодключено Тогда
		...
		ДиалогОткрытияФайла.Заголовок = НСтр("ru = 'Выберите путь каталога для загрузки изображений'");
...
Процедура ЗагрузитьСоСканера(Команда)
	...
	Если Не РаботаСоСканеромКлиентПовтИсп.ДоступнаКомандаСканировать() Тогда
		ПоказатьПредупреждение(, Нстр("ru = 'К вашему компьютеру не подключен ни один сканер.'"));
		Возврат;
	КонецЕсли;
...
Процедура Сканировать(Команда)
	...
	Если ТехнологияРаспознаванияШтрихкодов = ПредопределенноеЗначение("Перечисление.ТехнологииРаспознаванияШтрихкодов.Документооборот") Тогда
		Если Не ПроинициализироватьКомпонентуРаспознаванияШтрихкодов() Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не установлена компонента распознавания штрихкодов!'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ТипЗагрузки = ПредопределенноеЗначение("Перечисление.ТипЗагрузкиПотоковогоСканирования.ИзКаталога") Тогда
		Если ПустаяСтрока(КаталогЗагрузки) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Каталог загрузки не указан!'"), , "КаталогЗагрузки");
	...
	Если ТехнологияРаспознаванияШтрихкодов <> ПредопределенноеЗначение("Перечисление.ТехнологииРаспознаванияШтрихкодов.Документооборот") Тогда
		Если ПустаяСтрока(ИмяКомпьютераABBYY) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Имя компьютера с ABBYY Recognition Server не указано!'"), , "ИмяКомпьютераABBYY");
			ПоляНеЗаполнены = Истина;
		КонецЕсли;
		
		Если ПустаяСтрока(ИмяСценарияABBYYШтрихкоды) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Сценарий распознавания штрихкодов не указан!'"), , "ИмяСценарияABBYYШтрихкоды");
			ПоляНеЗаполнены = Истина;
		КонецЕсли;
		...
		Если ПустаяСтрока(ИмяСценарияABBYYТекст) Тогда
			Если РаспределениеИзображенийПоФайлам = ПредопределенноеЗначение("Перечисление.ВариантыРаспределенияИзображенийПоФайлам.РазделятьПустойСтраницей") Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Сценарий распознавания текста не указан!'"), , "ИмяСценарияABBYYТекст");
				ПоляНеЗаполнены = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТекущаяДатаСеансаСканирования <> Выборка.ДатаСеансаСканирования Тогда
			...
			Если Выборка.ТипЗагрузки = ПредопределенноеЗначение("Перечисление.ТипЗагрузкиПотоковогоСканирования.СоСканера") Тогда
				Представление = Представление + НСтр("ru = ' со сканера'"); 
			Иначе
				Представление = Представление + НСтр("ru = ' из каталога ""'"); 
...
Процедура УстановитьВидимостьНастроек()
	...
	Если ТипЗагрузки = ПредопределенноеЗначение("Перечисление.ТипЗагрузкиПотоковогоСканирования.ИзКаталога") Тогда
		УстановитьЗаголовокКоманды(НСтр("ru = 'Загрузить'"));
	...
	Иначе	
		УстановитьЗаголовокКоманды(НСтр("ru = 'Сканировать'"));
		Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.ГруппаНастройки;
...
Процедура ВыполнитьЗагрузкуИзКаталога()
	...
	Состояние(НСтр("ru = 'Подождите, идет сбор информации о каталоге...'"));
	...
	Для Каждого Файл Из МассивФайлов Цикл
		...
		Если Файл.ЭтоФайл() Тогда
			...
			Если РаботаСФайламиКлиентСервер.ЭтотФайлМожноРаспознать(
				РасширениеБезТочки, ИспользоватьImageMagickДляРаспознаванияPDF, ПрограммаРаспознавания) Тогда
...
Процедура РаспознаваниеШтрихКодов()
	...
	Состояние(НСтр("ru = 'Подождите, идет поиск документов по штрихкоду...'"));
...
Процедура РаспознаваниеШтрихКодовРеализация()
	...
	Для Каждого Строка Из Изображения Цикл
		...
		Состояние(НСтр("ru = 'Подождите, идет распознавание штрихкодов...'"), Счетчик * 100 / ЧислоСтрок);
		...
		Если ТипЗнч(КодВозврата) = Тип("Строка") Тогда
			...
			Если Найти(СтрокаВозврата, "Error.") = 1 Тогда // есть ошибка
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка распознавания пустого листа: %1'"),
	...
	Иначе	 // АББИ
		...
		Исключение
			...
			Исключение
				...
				Попытка
					ABBYY = Новый COMОбъект("ABBYYFineReaderServer14.Client"); //  ABBYY FineReader Server 14
				Исключение
				
					ПрерватьПроцессСканирования = Истина;
					ПоказатьПредупреждение(, НСтр("ru = 'Не установлен ABBYY Recognition Server.'"));
					Возврат Неопределено;
...
Функция РаспознатьШтрихКодОдногоФайла(ИмяФайлаКартинки, ПрерватьПроцессСканирования)
	...
	Если НРег(Файл.Расширение) = НРег(".pdf") Тогда
		...
		Если ПустаяСтрока(СтрокаПутей) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось преобразовать в картинки pdf файл: ""%1""'"),
				ИмяФайлаКартинки);
...
#Если Не МобильныйКлиент Тогда
	...
	Если ТехнологияРаспознаванияШтрихкодов = ПредопределенноеЗначение("Перечисление.ТехнологииРаспознаванияШтрихкодов.Документооборот") Тогда
		...
		Если Найти(СтрокаВозврата, "Error.") = 1 Тогда // есть ошибка
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка распознавания штрихкода: %1'"),
				СтрокаОшибкиКомпоненты);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
			Возврат Неопределено;
		КонецЕсли;        
	...
	Иначе	 // АББИ
		...
		Исключение
			ПрерватьПроцессСканирования = Истина;
			ПоказатьПредупреждение(, НСтр("ru = 'Не удается установить соединение с ABBYY Recognition Server.'"));
			Возврат Неопределено;
		КонецПопытки;	
...
Процедура РазмещениеВБазе(ВладельцыСНомерами)
	...
	Для Каждого ПараКлючЗначение Из ВсеВладельцы Цикл
		...
		Состояние(НСтр("ru = 'Идет размещение изображений в информационной базе...'"), Счетчик * 100 / ЧислоСтрок);
	...
	Если ТипЗагрузки = ПредопределенноеЗначение("Перечисление.ТипЗагрузкиПотоковогоСканирования.ИзКаталога") Тогда
		Элементы.ФормаПовторитьСканирование.Заголовок = НСтр("ru ='Повторить загрузку'");
	Иначе
		Элементы.ФормаПовторитьСканирование.Заголовок = НСтр("ru ='Повторить сканирование'");
	КонецЕсли;	
	...
	Иначе
		...
		ИначеЕсли ПоследниеДвеЦифры < 10 Или ПоследниеДвеЦифры > 20 Тогда
			Если ПоследняяЦифра = 1 Тогда
				Подпись = Нстр("ru = 'документ'");
			ИначеЕсли ПоследняяЦифра < 5 Тогда
				Подпись = НСтр("ru = 'документа'");
			Иначе
				Подпись = НСтр("ru = 'документов'");
			КонецЕсли;
		КонецЕсли;
...
Процедура ВнешнееСобытиеПродолжение1(Параметры) Экспорт
	...
	Если (ПродолжитьСканирование = Ложь) И ЗадаватьВопросОПродолженииСканирования И НЕ ВторойПроходСканирования Тогда	
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Продолжить сканирование?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
	КонецЕсли;
	ВыполнитьОбработкуОповещения(ОписаниеОповещения);
...
Процедура ВнешнееСобытиеПродолжение2(КодВозврата, Параметры) Экспорт
	...
	Если ВторойПроходСканирования Тогда
		...
		Если ИзображенияВторогоПроходаСканирования.Количество() <> Изображения.Количество() Тогда
			...
			ПоказатьПредупреждение(
				ОписаниеОповещения, 
				НСтр("ru = 'Количество отсканированных листов в первом и втором проходе сканирования отличается. Повторите процесс потокового сканирования.'"));
			Изображения.Очистить();
			Возврат;
		КонецЕсли;	
