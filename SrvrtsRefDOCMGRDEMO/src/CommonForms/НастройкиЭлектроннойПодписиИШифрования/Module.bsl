...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("СохранениеДанныхПользователя", Метаданные) Тогда
		ТекстОшибки = НСтр("ru = 'Отсутствует право сохранения данных. Обратитесь к администратору.'");
	...
	ПредЗначение = ПривилегированныйРежим();
	...
	УстановитьПривилегированныйРежим(ПредЗначение);
	
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		Элементы.ГруппаВизуализация.Доступность = Ложь;
		Элементы.СтраницаШтампЭП.Видимость = Ложь;
	КонецЕсли;
	...
	Если Пользователи.ЭтоПолноправныйПользователь() Тогда
		...
		Элементы.ПрограммыСертификатДляШифрования.Видимость = Истина;
	Иначе
		...
		Элементы.ПрограммыСертификатДляШифрования.Видимость = Ложь;
	КонецЕсли;
...
&НаКлиенте
Процедура ДействияПриОтправкеДанныхПоПочтеСЭлектроннойПодписьюПриИзменении(Элемент)
...
Процедура ПрограммыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ПрограммыСертификатДляШифрования
		И ЗначениеЗаполнено(Элемент.ТекущиеДанные.Ссылка)
		И Элемент.ТекущиеДанные.СертификатДляШифрованияОтсутствует = Истина Тогда
...
Процедура ЗаполнитьПрограммыИНастройки(ОбновитьПовтИсп = Ложь)
	...
	ДействияПриОтправкеПоПочтеСЭП              = ПерсональныеНастройки.ДействияПриОтправкеПоПочтеСЭП;
	...
	ТаблицаСертификатов = ЗапросСертификатов.Выполнить().Выгрузить();
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(Выборка.Ссылка) Тогда
			...
			ОбновитьЗначение(Строка.СертификатДляШифрования, "");
			ОбновитьЗначение(Строка.СертификатДляШифрованияОтсутствует, Ложь);
			
		Иначе
			
			СтрокаСертификатДляШифрования = "";
			Для Каждого СтрокаСертификата Из ТаблицаСертификатов Цикл
				Если СтрокаСертификата.Программа <> Выборка.Ссылка Тогда
					Продолжить;
				КонецЕсли;
		...
		Если СтрокаСертификатДляШифрования = "" Тогда
			ОбновитьЗначение(Строка.СертификатДляШифрования, НСтр("ru = 'Отсутствует. Кликните, чтобы добавить.'"));
			ОбновитьЗначение(Строка.СертификатДляШифрованияОтсутствует, Истина);
		Иначе
			ОбновитьЗначение(Строка.СертификатДляШифрования, СтрокаСертификатДляШифрования);
			ОбновитьЗначение(Строка.СертификатДляШифрованияОтсутствует, Ложь);
		КонецЕсли;
...
Процедура СохранитьНастройки()
	...
	СохраняемыеНастройки.Вставить("ДействияПриОтправкеПоПочтеСЭП",              ДействияПриОтправкеПоПочтеСЭП);
