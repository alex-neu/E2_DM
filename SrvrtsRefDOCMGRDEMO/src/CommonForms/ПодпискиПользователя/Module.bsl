...
Процедура ПодпискиУведомленийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ПодпискиУведомленийПоПочте И ДанныеСтроки.ЗапретИзмененияПоПочте Тогда
		ТекстПредупреждения = НСтр("ru = 'Настройка установлена Администратором системы и не может быть изменена.'");
	...
	ИначеЕсли Поле = Элементы.ПодпискиУведомленийПоPush И ДанныеСтроки.ЗапретИзмененияПоPush Тогда
		ТекстПредупреждения = НСтр("ru = 'Настройка установлена Администратором системы и не может быть изменена.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
	ИначеЕсли Поле = Элементы.ПодпискиУведомленийЧат И ДанныеСтроки.ЗапретИзмененияЧат Тогда
		ТекстПредупреждения = НСтр("ru = 'Настройка установлена Администратором системы и не может быть изменена.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
	КонецЕсли;
...
Процедура ПодпискиПоОбъектамВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле = Элементы.ПодпискиПоОбъектамЧатСтрокой Тогда
...
Процедура СпособыУведомленияПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Иначе
		Отказ = Истина;
		ТекстПредупреждения = НСтр("ru = 'Некорректный адрес почты или номер телефона.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
	КонецЕсли;
...
Процедура СохранитьНастройки()
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
		"НастройкиУведомлений", "УведомленияОкномПоказыватьВЦентреОповещений", 
		УведомленияОкномПоказыватьВЦентреОповещений,,ИмяПользователяИБ);
...
Процедура ПолучитьПодпискиПоОбъектам()
	...
	Для Каждого ДанныеПодписокПоОбъекту Из ДанныеПодписокПоОбъектам.Строки Цикл
		...
		Для Каждого ДанныеПодпискиПоВидуСобытия Из ДанныеПодписокПоОбъекту.Строки Цикл
			...
			Для Каждого ДанныеПодписки Из ДанныеПодпискиПоВидуСобытия.Строки Цикл
				Если ДанныеПодписки.СпособУведомления = Перечисления.СпособыУведомления.Чат Тогда
					СтрокаПодписки.Чат = ДанныеПодписки.Значение;
				КонецЕсли;
			КонецЦикла;
...
Процедура ПолучитьНастройки()
	...
	Для Каждого Подписка Из ДоступныеПодписки Цикл
		...
		НоваяСтрока.Чат = РегистрыСведений.НастройкиУведомлений.ПолучитьПодписку(
			Пользователь,
			Подписка.ВидСобытия,
			Перечисления.СпособыУведомления.Чат);
		НоваяСтрока.ЗапретИзмененияЧат = Не РазрешеноИзменятьНастройки
			И РегистрыСведений.НастройкиУведомлений.ПолучитьПодпискуПоУмолчанию(
				Подписка.ВидСобытия,
				Перечисления.СпособыУведомления.Чат);
	...
	УведомленияОкномПоказыватьВЦентреОповещений 
		= РаботаСУведомлениями.ИтоговаяНастройкаУведомленияОкномПоказыватьВЦентреОповещений(Пользователь);
