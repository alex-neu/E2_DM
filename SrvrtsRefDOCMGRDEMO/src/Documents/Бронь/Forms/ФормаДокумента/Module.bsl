...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.Ссылка.Пустая()
		...
		И МестнаяДатаНачала < ТекущаяДата() Тогда
		...
		ТекстВопроса = НСтр("ru = 'Указаная дата начала брони уже прошла. Забронировать?'");
...
Процедура ДатаОкончанияВремяАвтоПодбор(Элемент, Текст, ДанныеВыбора, Параметры, Ожидание, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		СтандартнаяОбработка = Ложь;
		РаботаСРабочимКалендаремКлиент.СформироватьДанныеВыбораВремени(Текст, ДанныеВыбора, МестнаяДатаОкончания);
	КонецЕсли;
...
Процедура ВесьДеньПриИзменении(Элемент)
	...
	Иначе
		...
		МестнаяДатаНачала = НачалоДня(МестнаяДатаНачала) + ОтображатьВремяС * 3600;
		МестнаяДатаОкончания = МестнаяДатаНачала + 3600;
...
Процедура ЗаполнитьСпискиВыбора()
	...
	Для Инд = 1 По 48 Цикл
		...
		Если МестнаяДатаНачала > ТекДата И МестнаяДатаНачала < ТекДата + 1800 Тогда
			Элементы.ДатаНачалаВремя.СписокВыбора.Добавить(МестнаяДатаНачала, Формат(МестнаяДатаНачала, "ДФ=ЧЧ:мм"));
		КонецЕсли;
	...
	Для Инд = 1 По 48 Цикл
		...
		Если МестнаяДатаОкончания > ТекДата И МестнаяДатаОкончания < ТекДата + 1800 Тогда
			Элементы.ДатаОкончанияВремя.СписокВыбора.Добавить(МестнаяДатаОкончания, Формат(МестнаяДатаОкончания, "ДФ=ЧЧ:мм"));
		КонецЕсли;
		ТекДата = ТекДата + 1800;
