...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ПолучателиТипАдреса.СписокВыбора.Добавить(
		НСтр("ru = 'Копия:'"), НСтр("ru = 'Копия:'"));
	Элементы.ПолучателиТипАдреса.СписокВыбора.Добавить(
		НСтр("ru = 'Скрытая копия:'"), НСтр("ru = 'Скрытая копия:'"));
	...
	Элементы.ОтправитьОбъекты.Видимость = ПисьмоОтправлено;
...
Процедура ПриОткрытии(Отказ)
	...
	// проверим ТребуетсяЗаполнитьВизуализацию
	НадоЗаполнить = Ложь;
	Для каждого ВложениеСтрока Из Вложения Цикл
		
		Если ВложениеСтрока.ТребуетсяЗаполнитьВизуализацию Тогда
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.HTML") Тогда
		...
		Пока СтрНайти(ТекстПисьмаДляЗаписи, "<script>") <> 0 Цикл 
			
			Если НЕ СтрНайти(ТекстПисьмаДляЗаписи, "</script>") <> 0 Тогда
				Прервать;
			КонецЕсли;
			
			НачалоСкрипта = СтрНайти(ТекстПисьмаДляЗаписи, "<script>");	
			КонецСкрипта = СтрНайти(ТекстПисьмаДляЗаписи, "</script>") + СтрДлина("</script>");
			
			ТекстПисьмаДляЗаписи = Лев(ТекстПисьмаДляЗаписи, НачалоСкрипта - 1) + 
				Прав(ТекстПисьмаДляЗаписи, СтрДлина(ТекстПисьмаДляЗаписи) - КонецСкрипта + 1);
		КонецЦикла;
...
Процедура ЗаполнитьНовоеПисьмо(Параметры)
	...
	ИначеЕсли Параметры.Свойство("Основание")
		...
		ИначеЕсли ТипЗнч(Параметры.Основание) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
			...
			ПредметыЗадачи = Мультипредметность.ПолучитьПредметыЗадачи(Параметры.Основание, Истина);
			ФайлыПредметовЗадачи = Новый Массив;
			Для каждого СтрокаПредметаЗадачи Из ПредметыЗадачи Цикл
				
				Если ТипЗнч(СтрокаПредметаЗадачи.Предмет) = Тип("СправочникСсылка.Файлы") Тогда
					ФайлыПредметовЗадачи.Добавить(СтрокаПредметаЗадачи.Предмет);
				Иначе	
					ФайлыПредмета = РаботаСФайламиВызовСервера.ПолучитьВсеПодчиненныеФайлы(СтрокаПредметаЗадачи.Предмет);
					Для каждого ФайлПредмета Из ФайлыПредмета Цикл
						Если ФайлыПредметовЗадачи.Найти(ФайлПредмета) = Неопределено Тогда
							ФайлыПредметовЗадачи.Добавить(ФайлПредмета);
						КонецЕсли;
					КонецЦикла; 
				КонецЕсли;
				
			КонецЦикла; 
			ДобавитьВложениеФайлы(ФайлыПредметовЗадачи);
			ФайлыЗадачи = РаботаСФайламиВызовСервера.ПолучитьВсеПодчиненныеФайлы(Параметры.Основание.БизнесПроцесс);
			ДобавитьВложениеФайлы(ФайлыЗадачи);
...
Процедура ПродолжениеОтправки0(Результат, Параметры) Экспорт
	...
	Если Результат <> Истина Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Письмо не было отправлено, 
			|так как некоторые из файлов письма заняты другим приложением.'"));
		Возврат;
	КонецЕсли;	
	Оповестить("ОбновитьСписокПоследних");
	...
	Если Результат.ДобавитьЗаместителей Тогда
		...
		Для Каждого ЗаместителиКлючИЗначение Из Результат.Заместители Цикл
			...
			Для Каждого Получатель Из Получатели Цикл
				...
				Если Получатель.ТипАдреса = НСтр("ru = 'Обратный адрес:'") Тогда
					Продолжить;
				КонецЕсли;
				...
				Если Получатель.Контакт = Сотрудник Тогда
					...
					Если ТекущийТипАдреса = НСтр("ru = 'Кому:'")
						Или ТипАдреса = НСтр("ru = 'Кому:'") Тогда
						ТипАдреса = НСтр("ru = 'Кому:'");
					ИначеЕсли ТекущийТипАдреса = НСтр("ru = 'Копия:'")
						Или ТипАдреса = НСтр("ru = 'Копия:'") Тогда
						ТипАдреса = НСтр("ru = 'Копия:'");
					ИначеЕсли ТекущийТипАдреса = НСтр("ru = 'Скрытая копия:'")
						Или ТипАдреса = НСтр("ru = 'Скрытая копия:'") Тогда
						ТипАдреса = НСтр("ru = 'Скрытая копия:'");
					КонецЕсли;
					
					Если Получатель.ТипАдреса = НСтр("ru = 'Кому:'") Тогда
						Получатель.ТипАдреса = НСтр("ru = 'Копия:'");
					КонецЕсли;
			...
			Для Каждого Заместитель Из Заместители Цикл
				...
				Если ТекущийТипАдреса = НСтр("ru = 'Кому:'")
					Или ТипАдреса = НСтр("ru = 'Кому:'") Тогда
					ТекущийТипАдреса = НСтр("ru = 'Кому:'");
				ИначеЕсли ТекущийТипАдреса = НСтр("ru = 'Копия:'")
					Или ТипАдреса = НСтр("ru = 'Копия:'") Тогда
					ТекущийТипАдреса = НСтр("ru = 'Копия:'");
				ИначеЕсли ТекущийТипАдреса = НСтр("ru = 'Скрытая копия:'")
					Или ТипАдреса = НСтр("ru = 'Скрытая копия:'") Тогда
					ТекущийТипАдреса = НСтр("ru = 'Скрытая копия:'");
				КонецЕсли;
...
Процедура ДобавитьАдресатов(Команда)
	...
	Иначе
		...
		Если ПолучателиТекущиеДанные <> Неопределено 
			...
			Если ПолучателиТекущиеДанные.ТипАдреса = НСтр("ru = 'Кому:'") Тогда
				НоваяСтрока.ТипАдреса = НСтр("ru = 'Копия:'");
			Иначе
...
Процедура ЗавершениеВыбораИзАдреснойКниги(Результат, Параметры) Экспорт
	...
	Если Не МножественныйВыбор Тогда
		...
		Если Результат.Количество() > 0 Тогда
			...
			ОбновитьНомерКартинкиАдресата(СтрокаДанных);
...
Процедура ДобавитьВПолучателиКопийИзПисьмаСервер(Письмо, КогоДобавить)
	...
	Если КогоДобавить = "Автор" Тогда
		...
		Если ВстроеннаяПочтаКлиентСервер.ЭтоВходящееПисьмо(Письмо) Тогда
			...
			ДобавитьУникальногоПолучателя(НСтр("ru = 'Копия:'"), Письмо.ОтправительАдресат);
		...
		ИначеЕсли ВстроеннаяПочтаКлиентСервер.ЭтоИсходящееПисьмо(Письмо) Тогда
			...
			ДобавитьУникальногоПолучателя(НСтр("ru = 'Копия:'"), Адресат);
	...
	ИначеЕсли КогоДобавить = "Получатели" Тогда
		...
		Для каждого Получатель Из Письмо.ПолучателиПисьма Цикл
			ДобавитьУникальногоПолучателя(НСтр("ru = 'Копия:'"), Получатель.Адресат);
		КонецЦикла;
...
Процедура ЗаполнитьПоШаблонуНаСервере(Результат)
	...
	Для каждого Строка Из ТаблицаПолучателиПисьма Цикл
		ДобавитьУникальногоПолучателя(НСтр("ru = 'Кому:'"), Строка.Адресат);
	КонецЦикла;
	Для каждого Строка Из ТаблицаПолучателиКопий Цикл
		ДобавитьУникальногоПолучателя(НСтр("ru = 'Копия:'"), Строка.Адресат);
	КонецЦикла;
	Для каждого Строка Из ТаблицаПолучателиСкрытыхКопий Цикл
		ДобавитьУникальногоПолучателя(НСтр("ru = 'Скрытая копия:'"), Строка.Адресат);
	КонецЦикла;
	Для каждого Строка Из ТаблицаПолучателиОтвета Цикл
		ДобавитьУникальногоПолучателя(НСтр("ru = 'Обратный адрес:'"), Строка.Адресат);
	КонецЦикла;
...
Процедура ИзменитьОтображаемыйШрифт(Команда)
	...
	#Если ВебКлиент Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Данная команда не поддерживается в веб-клиенте.'"));
		Возврат;
	#КонецЕсли
...
Процедура ВложенияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	#Если Не ВебКлиент Тогда
		...
		Если ТекущееВремя - ВремяОкончанияРедактированияПолучателей = 0 Тогда
			ПоказатьПредупреждение(,НСтр("ru = 'Откройте файл еще раз.'"));
			Возврат;
		КонецЕсли;
	#КонецЕсли
...
Процедура ЗавершениеУдаленияВложений(Ответ, Параметры) Экспорт
	...
	Если ТекущиеДанные.Расположение = "Файл" И ЗначениеЗаполнено(ТекущиеДанные.Ссылка) Тогда
	...
	Иначе
		...
		ИначеЕсли ПоследниеДвеЦифры < 10 Или ПоследниеДвеЦифры > 20 Тогда
			Если ПоследняяЦифра = 1 Тогда
				Подпись = Нстр("ru = 'файл'");
			ИначеЕсли ПоследняяЦифра < 5 Тогда
				Подпись = НСтр("ru = 'файла'");
			Иначе
				Подпись = НСтр("ru = 'файлов'");
			КонецЕсли;
		КонецЕсли;
...
Процедура ДобавитьВложениеФайлы(МассивФайлов, ИгнорироватьЗанятость = Ложь)
	...
	Для каждого ФайлИнфо Из ФайлыИнфо Цикл
		...
		СсылкаВизуализации = РаботаСФайламиВызовСервера.ПолучитьФайлВизуализации(
			ВложенияСтрока.Ссылка, УникальныйИдентификатор, ВложенияСтрока.ТребуетсяЗаполнитьВизуализацию);
		Если ЗначениеЗаполнено(СсылкаВизуализации) Тогда	
			ВложенияСтрока.Ссылка = СсылкаВизуализации;
		КонецЕсли;
...
Процедура ДобавитьФайлыСДискаПослеУстановкиРасширения(Результат, ПараметрыВыполнения) Экспорт
	...
	Иначе
		...
		Если РасширениеПодключено Тогда
			...
			ВыборФайла.Заголовок = НСтр("ru = 'Выбор файлов'");
...
Процедура ЗакончитьРедактированиеВложения(Команда)
	...
	Если Элементы.Вложения.ТекущиеДанные.Расположение <> "Файл" Тогда
		ВызватьИсключение НСтр("ru = 'Файл еще не записан'");
	КонецЕсли;
...
Процедура ДобавитьПолучателя(ТипАдреса, Адресат, СоответствиеДанныхАдресатов = Неопределено)
	...
	КонецЕсли;	
	ОбновитьНомерКартинкиАдресата(НоваяСтрока);
...
Процедура ЗаполнитьПолучателейКомуКопия()
	...
	ОбновитьНомераКартинокАдресатов(ТаблицаЗначений);
...
Процедура ЗагрузитьПолучателей()
	...
	Иначе	
		...
		Для каждого Строка Из Объект.ПолучателиПисьма Цикл
			ДобавитьПолучателя(НСтр("ru = 'Кому:'"), Строка.Адресат, СоответствиеДанныхАдресатов);
		КонецЦикла;
		Для каждого Строка Из Объект.ПолучателиКопий Цикл
			ДобавитьПолучателя(НСтр("ru = 'Копия:'"), Строка.Адресат, СоответствиеДанныхАдресатов);
		КонецЦикла;
		Для каждого Строка Из Объект.ПолучателиСкрытыхКопий Цикл
			ДобавитьПолучателя(НСтр("ru = 'Скрытая копия:'"), Строка.Адресат, СоответствиеДанныхАдресатов);
		КонецЦикла;
	КонецЕсли;
	...
	Если Элементы.Получатели.ТекущиеДанные <> Неопределено Тогда
		...
		Если Получатели.Количество() = Элементы.Получатели.ВыделенныеСтроки.Количество() Тогда
			...
			Если Получатели.Количество() = 1 Тогда
				...
				Строка.Представление = "";
				Строка.Внешний = Ложь;
				ОбновитьНомерКартинкиАдресата(Строка);
...
Процедура ПолучателиПредставлениеПриИзменении(Элемент)
	...
	Если Элементы.Получатели.ТекущиеДанные <> Неопределено 
		...
		ОбновитьНомерКартинкиАдресата(Элементы.Получатели.ТекущиеДанные);
...
Процедура ЗаполнитьСписокВыбора()
	...
	Пока Выборка.Следующий() Цикл
		...
		ВыбранныеКонтакты.Добавить(Выборка.Контакт);
	...
	Пока Выборка.Следующий() Цикл
		...
		ОписаниеОтсутствия = ОписанияОтсутствий[Выборка.Контакт];
...
Процедура ОтобразитьСвязиПисьма()
	...
	ПересылкаПисьмаСтрока = НСтр("ru = 'Нет'");
...
Функция ПолучитьТекстШаблона_ТипТекста(Шаблон, ТипТекста)
	...
	ТипТекста = ОбщегоНазначенияДокументооборотВызовСервера.ЗначениеРеквизитаОбъекта(
		Шаблон, "ТипТекста");
...
&НаКлиенте
Процедура ВыполнитьHTMLКоманду(Команда, Параметр = Неопределено)
	...
	Иначе
		HTMLДокумент.execCommand(Команда,, Параметр);
	КонецЕсли;
	Модифицированность = Истина;
...
Процедура ЗавершениеУдалитьВнешниеКартинки(Ответ, Параметры) Экспорт
	...
	Если МассивВнешнихСсылок.Количество() <> 0 Тогда
		...
		Для Каждого ОписаниеКартинки Из МассивВнешнихСсылок Цикл
			...
			НоваяСтрока = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Изображение: %1'"), НоваяСтрока);
	...
	Иначе
		ПоказатьПредупреждение(,НСтр("ru = 'Ссылок на картинки, подгружаемые из Интернет, в тексте письма не обнаружено.'"));
	КонецЕсли;
	...
	Если ЭлементСправочникаСпискиАдресов <> Неопределено Тогда
		...
		Если ЧислоДобавленных <> 0 Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Адресаты (%1 шт) добавлены в список адресов ""%2""'"), ЧислоДобавленных, ЭлементСправочникаСпискиАдресов);
...
Процедура ЗавершениеДобавленияВМоиКонтакты(ГруппаИзбранного, Параметры) Экспорт
	...
	Если ГруппаИзбранного <> Неопределено Тогда
		...
		Если ЧислоДобавленных <> 0 Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Адресаты (%1 шт) добавлены в группу избранного ""%2""'"), ЧислоДобавленных, ГруппаИзбранного);
...
Процедура ДобавитьВИзбранноеПользователяСервер(Знач ВыделенныеСтроки, ГруппаИзбранного, ЧислоДобавленных, Причина)
	...
	Для Каждого ИдСтроки Из ВыделенныеСтроки Цикл
		...
		Иначе	
			Причина = НСтр("ru = 'Не заполнен адресат.'")
		КонецЕсли;
...
Процедура ВставитьБезФорматирования(Команда)
	...
	Если Объект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.HTML") Тогда
		...
		ВыполнитьHTMLКоманду("PasteAsPlainText");
	    ЭтаФорма.ТекущийЭлемент = Элементы.ТекстПисьмаHTML;
...
Процедура ПоказатьФорматированноеСообщение()
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru = 'Некоторые получатели письма не добавлены'"));
...
Процедура ВывестиВидМаршрутизации(Форма)
	...
	Если Не ПисьмоОтправлено Тогда 
		...
		Если Не Форма.ИспользоватьВнутреннююМаршрутизацию Тогда 
			...
			Объект.ПричинаВидаМаршрутизации = НСтр("ru = 'Выключено использование внутренней маршрутизации'");
		...
		Иначе
			...
			Если ВнешниеАдресаты.Количество() = 0 Тогда 
				...
				Объект.ПричинаВидаМаршрутизации = 
					НСтр("ru = 'Это письмо будет отправлено напрямую, а не через почтовый сервер, так как все получатели этого письма работают с почтой в 1С:Документообороте.'"); 
			Иначе	
				Объект.ВидМаршрутизации = ВидМаршрутизацииВнешняя;
				
				Объект.ПричинаВидаМаршрутизации = НСтр("ru = 'Это письмо будет отправлено через почтовый сервер, так как следующие получатели являются внешними или не работают с почтой в 1С:Документообороте:'");
		...
		Если Объект.ВидМаршрутизации = ВидМаршрутизацииВнутренняя Тогда 
			Форма.ДекорацияВидМаршрутизацииПодсказка = НСтр("ru = 'Доставка внутренняя'");
		Иначе
			Форма.ДекорацияВидМаршрутизацииПодсказка = НСтр("ru = 'Доставка через почтовый сервер'");
		КонецЕсли;
...
Процедура ВывестиВидАдресатов(Форма)
	...
	Если Не Форма.ПриложениеЯвляетсяВебКлиентом Тогда
		...
		Если ВнешниеАдресаты.Количество() > 0 Тогда 
			Форма.ДекорацияВнешнийПодсказка = НСтр("ru = 'Это внешнее письмо'");
		Иначе
			Форма.ДекорацияВнешнийПодсказка = НСтр("ru = 'Это внутреннее письмо'");
		КонецЕсли;
...
Процедура ДекорацияВидМаршрутизацииЗаголовокИТекст(ТекстЗаголовка, ТекстСообщения)
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'Для данного письма невозможно сообщить, почему установлен такой вид маршрутизации, так как оно было отправлено до начала сохранения этих сведений.'");
...
Процедура ДобавитьПользователейВПолучатели(ПользователиДляДобавления)
	...
	Для Каждого ПользовательДляДобавленияКлючИЗначение Из ПользователиДляДобавления Цикл
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			Если НайденнаяСтрока.ТипАдреса = НСтр("ru = 'Копия:'")
				И ПользовательДляДобавленияКлючИЗначение.Значение = НСтр("ru = 'Кому:'") Тогда
				НайденнаяСтрока.ТипАдреса = НСтр("ru = 'Кому:'");
			КонецЕсли;
		КонецЦикла;
...
Процедура ЗаполнитьВизуализацию()
	...
	Для каждого ВложениеСтрока Из Вложения Цикл
		...
		Если ВложениеСтрока.ТребуетсяЗаполнитьВизуализацию Тогда
			...
			ВложениеСтрока.ТребуетсяЗаполнитьВизуализацию = Ложь;
			
			Файл = ВложениеСтрока.Ссылка;
			...
			Если ДанныеФайла.ТребуетсяСоздатьВизуализациюЭП
				...
				#Если Не ВебКлиент Тогда	
					...
					Если ЗначениеЗаполнено(ВизуализацияСсылка) Тогда
						ВложениеСтрока.Ссылка = ВизуализацияСсылка;
					КонецЕсли;	
				#КонецЕсли	
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОбновитьНомерКартинкиАдресата(СтрокаТаблицыПолучателей) 
	
	ДанныеДляНомераКартинки = ДанныеДляНомераКартинки(СтрокаТаблицыПолучателей.Контакт);
	СтрокаТаблицыПолучателей.НомерКартинки = НомерКартинкиАдресата(
...
&НаСервереБезКонтекста
Функция ДанныеДляНомераКартинки(Получатель)
	
	ДанныеДляНомераКартинки = Новый Структура("ОписаниеОтсутствия");
	ДанныеДляНомераКартинки.ОписаниеОтсутствия = Отсутствия.ПолучитьОписаниеТекущегоОтсутствияСотрудника(Получатель);
