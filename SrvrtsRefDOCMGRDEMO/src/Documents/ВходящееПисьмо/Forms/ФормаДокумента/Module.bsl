...
#Если МобильныйКлиент Тогда
	...
	ОбновитьДекорациюСкрытияТаблицыВложений();
#КонецЕсли
...
Процедура ВывестиВидМаршрутизации()
	...
	Если Не ПриложениеЯвляетсяВебКлиентом Тогда
		...
		Если Объект.ВидМаршрутизации = ВидМаршрутизацииВнутренняя Тогда 
			ДекорацияВидМаршрутизацииПодсказка = НСтр("ru = 'Доставка внутренняя'");
		Иначе
			ДекорацияВидМаршрутизацииПодсказка = НСтр("ru = 'Доставка через почтовый сервер'");
		КонецЕсли;
...
Процедура ВывестиВидАдресатов()
	...
	Если Не ПриложениеЯвляетсяВебКлиентом Тогда
		...
		Если ВнешниеАдресаты.Количество() > 0 Тогда 
			ДекорацияВнешнийПодсказка = НСтр("ru = 'Это внешнее письмо'");
		Иначе
			ДекорацияВнешнийПодсказка = НСтр("ru = 'Это внутреннее письмо'");
		КонецЕсли;
...
Процедура ДобавитьВМоиКонтактыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЧислоДобавленных <> 0 Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Адресаты (%1 шт) добавлены в группу избранного ""%2""'"), ЧислоДобавленных, Результат);
		ПоказатьОповещениеПользователя(НСтр("ru = 'Добавление в ""Избранное""'"),,Текст);
	ИначеЕсли Элементы.Получатели.ВыделенныеСтроки.Количество() = 1 Тогда
		ПоказатьПредупреждение(, Причина);
	КонецЕсли;
...
Процедура ДобавитьВИзбранноеПользователяСервер(Знач ВыделенныеСтроки, ГруппаИзбранного, ЧислоДобавленных, Причина)
	...
	Для Каждого ИдСтроки Из ВыделенныеСтроки Цикл
		...
		Если ДобавитьВМоиКонтактыСервер(Контакт, Адрес, ГруппаИзбранного) Тогда
			ЧислоДобавленных = ЧислоДобавленных + 1;
		Иначе	
			Причина = НСтр("ru = 'Данный адресат уже есть в группе избранного.'")
		КонецЕсли;
...
#Если МобильныйКлиент Тогда
	// На моб. клиенте доступно только сохранение.
	Возврат;
#КонецЕсли
...
Процедура СкопироватьСтрокиНаСервере(ЧислоФайлов)
	...
	ОбщегоНазначенияДокументооборот.СкопироватьФайлыВБуферОбменаИзТаблицыФайлы(
		Вложения, Элементы.Вложения.ВыделенныеСтроки);
...
Процедура ЗаполнитьПроектСтрокой(Проект)
	...
	Иначе
		ПроектСтрокой = НСтр("ru = 'Не указан'");
	КонецЕсли;	
...
Процедура ЗаполнитьПредметСтрокой(Предмет)
	...
	Иначе
		ПредметСтрокой = НСтр("ru = 'Не указан'");
	КонецЕсли;	
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	// Добавим команды в общее меню.
	Элементы.Переместить(Элементы.ПоказатьПараметрыПисьма, Элементы.МКПодменюСервис);
	Элементы.Переместить(Элементы.ФормаПоказатьИсходныйТекст, Элементы.МКПодменюСервис);
	Элементы.Переместить(Элементы.СвязиОбъекта, Элементы.МКПодменюСервис);
	Элементы.Переместить(Элементы.ЗадачиПоПредмету, Элементы.МКПодменюСервис);
	Элементы.Переместить(Элементы.ПротоколРаботы, Элементы.МКПодменюСервис);
	
	Элементы.Переместить(Элементы.ИзменитьКодировку, Элементы.МКПодменюНастройки);
	Элементы.Переместить(Элементы.ИзменитьШрифтОтображения, Элементы.МКПодменюНастройки);
	
	Элементы.ГруппаКоманды.Видимость = Ложь;
	// Установим нужный вид некоторых команд.
	Элементы.Ответить.Картинка = БиблиотекаКартинок.МККомандаОтветить;
	Элементы.ОтветитьВсем.Картинка = БиблиотекаКартинок.МККомандаОтветитьВсем;
	Элементы.Переслать.Картинка = БиблиотекаКартинок.МККомандаПереслать;
	Элементы.ПереместитьВКорзину.Картинка = БиблиотекаКартинок.МККомандаКорзина;
	Элементы.Контроль.Картинка = БиблиотекаКартинок.МККомандаКонтроль;
	Элементы.Контроль.Отображение = ОтображениеКнопки.Авто;
	Элементы.Контроль.Заголовок = НСтр("ru = 'Контроль'");
	
	Элементы.ГруппаДокумент.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	// Добавим вывод всех получателей письма.
	ГруппаПолучатели = Элементы.Добавить("ГруппаПолучатели", Тип("ГруппаФормы"), Элементы.ГруппаПисьмо);
	ГруппаПолучатели.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	Элементы.Переместить(ГруппаПолучатели, Элементы.ГруппаПисьмо, Элементы.ГруппаТелоПисьма);
	
	ДекорацияТекст = Элементы.Добавить("ПолучателиТекст", Тип("ДекорацияФормы"), ГруппаПолучатели);
	ДекорацияТекст.Вид = ВидДекорацииФормы.Надпись;
	ДекорацияТекст.Заголовок = НСтр("ru = 'Все получатели'");
	ДекорацияТекст.Подсказка = ДекорацияТекст.Заголовок;
	ДекорацияТекст.ЦветТекста = ЦветаСтиля.ЦветТекстаКнопки;
	ДекорацияТекст.Гиперссылка = Истина;
	ДекорацияТекст.УстановитьДействие("Нажатие", "Подключаемый_ПолучателиТекстНажатие");
	
	ТаблицаПолучатели = Элементы.Добавить("Получатели", Тип("ТаблицаФормы"), ГруппаПолучатели);
	ТаблицаПолучатели.ПутьКДанным = "Получатели";
	ТаблицаПолучатели.Шапка = Ложь;
	ТаблицаПолучатели.ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиЭлементаФормы.Нет;
	ТаблицаПолучатели.Видимость = Ложь;
	ТаблицаПолучатели.РастягиватьПоВертикали = Ложь;
	ТаблицаПолучатели.АвтоМаксимальнаяВысотаВСтрокахТаблицы = Ложь;
	ТаблицаПолучатели.Высота = 1;
	Если Получатели.Количество() < 7 Тогда
		ТаблицаПолучатели.ВариантУправленияВысотой = ВариантУправленияВысотойТаблицы.ПоСодержимому;
		ТаблицаПолучатели.ВысотаВСтрокахТаблицы = Получатели.Количество();
	Иначе
		ТаблицаПолучатели.ВариантУправленияВысотой = ВариантУправленияВысотойТаблицы.ВСтрокахТаблицы;
		ТаблицаПолучатели.ВысотаВСтрокахТаблицы = 7;
	КонецЕсли;
	ТаблицаПолучатели.КонтекстноеМеню.Доступность = Ложь;
	ТаблицаПолучатели.УстановитьДействие("Выбор", "Подключаемый_ПолучателиВыбор");
	
	ПолучателиТипАдреса = Элементы.Добавить("ПолучателиТипАдреса", Тип("ПолеФормы"), ТаблицаПолучатели);
	ПолучателиТипАдреса.Вид = ВидПоляФормы.ПолеВвода;
	ПолучателиТипАдреса.ПутьКДанным = "Получатели.ТипАдреса";
	ПолучателиТипАдреса.Ширина = 4;
	ПолучателиПолучателиАдресат = Элементы.Добавить("ПолучателиАдресат", Тип("ПолеФормы"), ТаблицаПолучатели);
	ПолучателиПолучателиАдресат.Вид = ВидПоляФормы.ПолеВвода;
	ПолучателиПолучателиАдресат.ПутьКДанным = "Получатели.ПочтовыйАдресат";
	...
	Иначе
		Если Вложения.Количество() >= 3 Тогда
			...
			// Добавим декорацию для скрытия вложений.
			ВложенияТекст = Элементы.Добавить("ВложенияТекст", Тип("ДекорацияФормы"),
								Элементы.ГруппаОтступВложенияРеквизиты);
			ВложенияТекст.Вид = ВидДекорацииФормы.Надпись;
			ВложенияТекст.Заголовок = Элементы.ВложенияПредставление.Заголовок; // Заголовок уже есть.
			ВложенияТекст.Подсказка = ВложенияТекст.Заголовок;
			ВложенияТекст.ЦветТекста = ЦветаСтиля.ЦветТекстаКнопки;
			ВложенияТекст.Гиперссылка = Истина;
			ВложенияТекст.УстановитьДействие("Нажатие", "Подключаемый_ВложенияТекстНажатие");
			
		КонецЕсли;
		
		Элементы.Переместить(Элементы.Вложения, Элементы.ГруппаОтступВложенияРеквизиты);
		Элементы.Вложения.ВертикальноеПоложениеВГруппе = ВертикальноеПоложениеЭлемента.Низ;
		Элементы.Вложения.АвтоМаксимальнаяШирина = Ложь;
		Элементы.Вложения.Шапка = Ложь;
		Элементы.Вложения.РастягиватьПоВертикали = Ложь;
		Элементы.Вложения.Высота = 1;
		Элементы.Вложения.АвтоМаксимальнаяВысота = Ложь;
		Элементы.Вложения.МаксимальнаяВысота = 1;
		Элементы.Вложения.АвтоМаксимальнаяВысотаВСтрокахТаблицы = Ложь;
		Элементы.Вложения.МаксимальнаяВысотаВСтрокахТаблицы = 4;
		Элементы.Вложения.ВариантУправленияВысотой = ВариантУправленияВысотойТаблицы.ПоСодержимому;
		Если Вложения.Количество() < 4 Тогда
			Элементы.Вложения.ВысотаВСтрокахТаблицы = Вложения.Количество();
		Иначе
			Элементы.Вложения.ВысотаВСтрокахТаблицы = 4;
		КонецЕсли;
		
		
		//Отключим пункты контекстного меню, кроме сохранения.
		Для Каждого Элемент Из Элементы.Вложения.КонтекстноеМеню.ПодчиненныеЭлементы Цикл
	...
	Элементы.ГруппаОтступВложенияРеквизиты.Ширина = 0;
	Элементы.ГруппаОтступВложенияРеквизиты.РастягиватьПоГоризонтали = Истина;
	
	// Перенесем реквизиты на вторую страницу
	Элементы.Страницы.ОтображениеСтраниц = ОтображениеСтраницФормы.Пролистывание;
	СтраницаРеквизиты = Элементы.Добавить("СтраницаРеквизиты", Тип("ГруппаФормы"), Элементы.Страницы);
	СтраницаРеквизиты.Вид = ВидГруппыФормы.Страница;
	Элементы.Переместить(Элементы.ГруппаВложенияРеквизиты, СтраницаРеквизиты);
	
	Элементы.ГруппаДокумент.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	// Добавим вывод всех получателей письма.
	ГруппаПереписка = Элементы.Добавить("ГруппаПереписка", Тип("ГруппаФормы"), Элементы.ГруппаДекорации);
	...
	ГруппаПереписка.Отображение = ОтображениеОбычнойГруппы.Нет;
	Элементы.Переместить(ГруппаПереписка, Элементы.ГруппаДекорации, Элементы.ПисемВПереписке);
	Элементы.Переместить(Элементы.ПисемВПереписке, ГруппаПереписка);
	Элементы.Переместить(Элементы.СтатусПисьма, ГруппаПереписка);
	Элементы.СтатусПисьма.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента.Право;
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ПолучателиТекстНажатие(Элемент)
...
&НаКлиенте
Процедура Подключаемый_ВложенияТекстНажатие(Элемент)
...
&НаКлиенте
Процедура ОбновитьДекорациюСкрытияТаблицыИсполнителей()
	
	ПолучателиТекст = Элементы.Найти("ПолучателиТекст");
	Если ПолучателиТекст = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьДекорациюСкрытияТаблицыВложений()
	
	ВложенияТекст = Элементы.Найти("ВложенияТекст");
	Если ВложенияТекст = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ПолучателиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
