...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Для Каждого Строка Из Результат Цикл
		...
		Если ЕстьКолонкаИзменена И Строка.Изменена Тогда
			РегистрыСведений.МП_ИзмененныеТрудозатраты.ИзменитьЗапись(Пользователь, Строка.ДатаДобавления, Строка.НомерДобавления);
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(Подразделение) Тогда 
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для пользователя %1 не указано Подразделение!'"),
			Строка(Пользователь));
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		МенеджерЗаписи.Записать();
		РегистрыСведений.МП_ИзмененныеТрудозатраты.УдалитьЗаписьОТрудозатрате(Пользователь, Выборка.ДатаДобавления, 
