...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПоказатьПредставлениеФайлаЭлектронногоДокумента(,, КонтекстОперации);
	...
	Если Объект.СостояниеЭДО = Перечисления.СостоянияВерсийЭД.ОшибкаПередачи Тогда
		Элементы.ГруппаОшибкиФормированияДокументаДО.Видимость = Истина;
	Иначе
		Элементы.ГруппаОшибкиФормированияДокументаДО.Видимость = Ложь;
	КонецЕсли;
...
&НаКлиенте
Процедура НадписьОшибкиФормированияДокументаДОНажатие(Элемент)
	
	Если Объект.СостояниеЭДО
			<> ПредопределенноеЗначение("Перечисление.СостоянияВерсийЭД.ОшибкаПередачи") Тогда
...
Процедура СохранитьЭДНаДиск(Команда)
	...
	СписокВерсийФайлов = ПолучитьСписокВерсийФайловДляСохранения();
	
	Если СписокВерсийФайлов.Количество() = 1 Тогда
		СохранитьВерсиюФайлаВместеСЭП(СписокВерсийФайлов[0].Значение);
	ИначеЕсли СписокВерсийФайлов.Количество() > 1 Тогда
		ОповещениеОВыборе = Новый ОписаниеОповещения("ОбработкаВыбораВерсииФайлаДляСохранения", ЭтаФорма);
		
		СписокВерсийФайлов.ПоказатьВыборЭлемента(ОповещениеОВыборе, "Выберите версию файла для сохранения");
	Иначе
		ПоказатьПредупреждение(, "Перед сохранением версии файла необходимо создать документы учета");
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработкаВыбораВерсииФайлаДляСохранения(Результат, ДопПараметры) Экспорт
	
	Если ЗначениеЗаполнено(Результат.Значение) Тогда
		СохранитьВерсиюФайлаВместеСЭП(Результат.Значение);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура СохранитьВерсиюФайлаВместеСЭП(ВерсияФайлаСсылка)
	
	РаботаСФайламиКлиент.СохранитьВместеСЭП(ВерсияФайлаСсылка, УникальныйИдентификатор);
...
Процедура УтвердитьЭД(Команда)
	...
	ПодключитьОбработчикОжидания("ПродолжитьУтверждениеДокумента", 0.1, Истина);
...
Процедура КомандаПодписатьОтправитьЭД(Команда)
	...
	ПодключитьОбработчикОжидания("ПодписатьОтправитьЭД", 0.1, Истина);
...
Процедура Завершить(Команда)
	...
	ДополнительныеПараметры.ЗаголовокФормы            = ВводСтрокиЗаголовок;
	ДополнительныеПараметры.Многострочность           = Истина;
	ДополнительныеПараметры.НазваниеКнопкиПоУмолчанию = НСтр("ru = 'Завершить'");
	ДополнительныеПараметры.Обязательность            = Истина;
	ОбменСКонтрагентамиСлужебныйКлиент.ПоказатьВводСтрокиБЭД(Оповещение, ДополнительныеПараметры);
...
Процедура ПоставитьВОчередьПриема(Команда)
	...
	ОбменСКонтрагентамиДОСлужебныйКлиент.ПоставитьЭДВОчередьПриема(Объект.Ссылка);
...
#Область КомандыСинхронизацииСДокументамиДО

&НаКлиенте
Процедура СоздатьДокументДО(Команда)
	
	ОбработкаОтветаНаВопрос = Новый ОписаниеОповещения("ОбработкаОтветаОСозданииДокументаДО", ЭтаФорма);
	
	ИсключенныеСостояния = Новый Массив;
	ИсключенныеСостояния.Добавить(
		ПредопределенноеЗначение("Перечисление.СостоянияВерсийЭД.Аннулирован"));
	ИсключенныеСостояния.Добавить(
		ПредопределенноеЗначение("Перечисление.СостоянияВерсийЭД.ЗакрытПринудительно"));
	ИсключенныеСостояния.Добавить(
		ПредопределенноеЗначение("Перечисление.СостоянияВерсийЭД.НеСформирован"));
	ИсключенныеСостояния.Добавить(
		ПредопределенноеЗначение("Перечисление.СостоянияВерсийЭД.ОбменЗавершен"));
	ИсключенныеСостояния.Добавить(
		ПредопределенноеЗначение("Перечисление.СостоянияВерсийЭД.ОбменЗавершенСИсправлением"));
	ИсключенныеСостояния.Добавить(
		ПредопределенноеЗначение("Перечисление.СостоянияВерсийЭД.ОжидаетсяАннулирование"));
	ИсключенныеСостояния.Добавить(
		ПредопределенноеЗначение("Перечисление.СостоянияВерсийЭД.Отклонен"));
	ИсключенныеСостояния.Добавить(
		ПредопределенноеЗначение("Перечисление.СостоянияВерсийЭД.ОшибкаПередачи"));
	
	Если ИсключенныеСостояния.Найти(Объект.СостояниеЭДО) <> Неопределено Тогда
		ПоказатьПредупреждение(,
			СтрШаблон(НСтр("ru = 'Документ находится в состоянии %1, в этом состоянии невозможно формировать документ ДО.'"),
				Объект.СостояниеЭДО), ,
			НСтр("ru = 'Недопустимое состояние обмена'"));
		
		Возврат;
	КонецЕсли;
	
	ТекстВопроса = НСтр("ru = 'Будет создан новый внутренний документ 1С:Документооборот, согласно настройкам ЭДО.
							|К нему будут прикреплены копии файлов электронного документа. Продолжить?'");
...
&НаКлиенте
Процедура ОбработкаОтветаОСозданииДокументаДО(Результат, ДопПараметры) Экспорт
	...
	Результат = ОбменСКонтрагентамиДОВызовСервера.СформироватьДокументыДОНаОснованииВходящихЭД(
		ПараметрыСоздания);
...
&НаКлиенте
Процедура ПоказатьДокументДО(Команда)
	
	КолвоОснований = Объект.ДокументыОснования.Количество();
	Если КолвоОснований = 0 Тогда
		Возврат;
	ИначеЕсли КолвоОснований = 1 Тогда 
		
		Если ЗначениеЗаполнено(Объект.ДокументыОснования[0].ДокументДО) Тогда
			ПоказатьЗначение(, Объект.ДокументыОснования[0].ДокументДО);
		ИначеЕсли ЗначениеЗаполнено(Объект.ДокументыОснования[0].Файл) Тогда
			ПоказатьЗначение(, Объект.ДокументыОснования[0].Файл);
		ИначеЕсли ЗначениеЗаполнено(Объект.ДокументыОснования[0].ВерсияФайла) Тогда
			ПоказатьЗначение(, Объект.ДокументыОснования[0].ВерсияФайла);
		Иначе
			ПоказатьЗначение(, Объект.ДокументыОснования[0].ДокументОснование);
		КонецЕсли;
		
	Иначе
		
		СпискокДокументовУчета = Новый СписокЗначений;
		
		Для каждого СтрОснования Из Объект.ДокументыОснования Цикл
			Если ЗначениеЗаполнено(СтрОснования.ДокументДО) Тогда
				СпискокДокументовУчета.Добавить(СтрОснования.ДокументДО, СтрОснования.ДокументДОПредставление);
			КонецЕсли;
		КонецЦикла;
		
		ОбработкаВыбора = Новый ОписаниеОповещения("ОбработатьВыборДокументаУчетаДляОтображения", ЭтаФорма);
		
		ПоказатьВыборИзСписка(ОбработкаВыбора, СпискокДокументовУчета, Элементы.ПоказатьДокументДО);
...
&НаКлиенте
Процедура ОбработатьВыборДокументаУчетаДляОтображения(Результат, ДопПараметры) Экспорт
...
Процедура ПосмотретьЭДМК(Команда)
	...
	#Если МобильныйКлиент Тогда
		Если Объект.ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.ПроизвольныйЭД") Тогда
			ТекущаяСтрокаДерева = ДеревоПодчиненныеЭД.НайтиПоИдентификатору(ИдентификаторТекущейСтроки);
			Если ТекущаяСтрокаДерева <> Неопределено И ТекущаяСтрокаДерева.ИмяРеквизита <> "ПрисоединенныйФайл" Тогда
...
Процедура ОтправитьПакетыЭД()
	...
	КонтекстОперации = ЭлектронноеВзаимодействиеСлужебныйКлиент.НовыйКонтекстОперации();
	ПакетыЭД = НайтиСоздатьПакетыЭлектронныхДокументов(ДеревоПодчиненныеЭД, ПрисоединенныйФайлСсылка, ТребуетсяПодпись, КонтекстОперации);
...
Функция НайтиСоздатьПакетыЭлектронныхДокументов(Знач ДеревоПодчиненныеЭД, Знач ПрисоединенныйФайлСсылка, Знач ПризнакПодписи, КонтекстОперации)
...
&НаСервере
Процедура ПоказатьПредставлениеФайлаЭлектронногоДокумента(ТекущаяСтрокаДерева = Неопределено, ПовторноеИспользование = Истина, КонтекстОперации = Неопределено)
...
Процедура ПодготовитьОтражениеВУчете()
	...
	Если Объект.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.СЧФДОПУПД
		ИЛИ Объект.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.КСЧФДИСУКД Тогда
		Если ЗначениеЗаполнено(Объект.ДополнительныеВидыЭД) Тогда
			Для каждого СтрокаВида Из Объект.ДополнительныеВидыЭД Цикл
				...
				КонецЦикла;
			КонецЦикла;
		ИначеЕсли Объект.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.СЧФДОПУПД Тогда
			СписокТиповДокументовДоп = ОбменСКонтрагентамиСлужебный.СписокОперацийВидаЭД(Перечисления.ВидыЭД.ТОРГ12Продавец);
		ИначеЕсли Объект.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.КСЧФДИСУКД Тогда
			СписокТиповДокументовДоп = ОбменСКонтрагентамиСлужебный.СписокОперацийВидаЭД(Перечисления.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель);
		КонецЕсли;
	ИначеЕсли Объект.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ДОПУПД Тогда
		Для каждого СтрокаВида Из Объект.ДополнительныеВидыЭД Цикл
			ДополнительныйСписок = ОбменСКонтрагентамиСлужебный.СписокОперацийВидаЭД(СтрокаВида.ВидЭД);
			Для каждого Элемент Из ДополнительныйСписок Цикл
...
Процедура НастроитьЭлементыОтраженияВУчете()
	...
	ЕстьПравоОбработкиДокумента = ОбменСКонтрагентамиСлужебный.ЕстьПравоОбработкиЭД();
...
Процедура СоздатьДокументыУчета(Знач СпособОбработки, КонтекстОперации)
	...
	КонецЕсли;
	ФайлДанныхСсылка = ОбменСКонтрагентамиСлужебный.ПолучитьДанныеЭД(ФайлЭлектронногоДокумента, КонтекстОперации);
...
Процедура ПроверитьВозможностьОтраженияВУчете()
	...
	// Для ДО в силу особенностей артитектуры программы метод отражения в учете БЭД не подходит.
	// Всю связь с документами ДО организуем собственными силами.
	ЕстьВозможностьОтраженияВУчете = Ложь;
	
КонецПроцедуры

#Область СинхронизацияСДокументамиДО

&НаКлиенте
Процедура НастроитьЭлементыДокументовДО()
	
	КолвоДокументов = Объект.ДокументыОснования.Количество();
	Если КолвоДокументов = 0 Тогда
		Элементы.ГруппаДокументыДООтраженногоВУчете.Видимость = Ложь;
		Элементы.СоздатьДокументДО.Видимость = Истина;
	ИначеЕсли КолвоДокументов = 1 Тогда
		Элементы.ГруппаДокументыДООтраженногоВУчете.Видимость = Истина;
		Элементы.СоздатьДокументДО.Видимость = Ложь;
		
		Элементы.ПоказатьДокументДО.Заголовок = Объект.ДокументыОснования[0].ДокументДОПредставление;
	Иначе
		Элементы.ГруппаДокументыДООтраженногоВУчете.Видимость = Истина;
		Элементы.СоздатьДокументДО.Видимость = Ложь;
		
		Элементы.ПоказатьДокументДО.Заголовок = СтрШаблон(НСтр("ru = 'Внутренние документы (%1)'"),
			КолвоДокументов);
	КонецЕсли;
...
Процедура ИзменитьВидимостьДоступностьНаСервере()
	...
	ЕстьПравоОбработки = ОбменСКонтрагентамиСлужебныйВызовСервера.ЕстьПравоВыполненияОбмена()
		И ОбменСКонтрагентамиСлужебный.ЕстьПравоОбработкиЭД();
	...
	Если ПараметрыВыводаПредупреждения = Неопределено Тогда
		...
		Иначе
			ПараметрыВыводаПредупреждения = ОбменСКонтрагентамиСлужебный.ПараметрыВыводаПредупреждения(Объект.Ссылка, КонтекстОперации);
		КонецЕсли;
	КонецЕсли;
	...
	Если СвойстваФайлаЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Входящий Тогда
		...
		Элементы.КомандаУтвердить.Видимость = ЕстьПравоОбработки И Объект.СостояниеЭДО = Перечисления.СостоянияВерсийЭД.НаУтверждении
			И НЕ СвойстваФайлаЭД.СтатусЭД = Перечисления.СтатусыЭД.Утвержден
			И НЕ (Объект.ТребуетсяПодтверждение И НЕ ОтправлятьВходящиеДокументыНаУтверждение);
	...
	ИначеЕсли СвойстваФайлаЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Исходящий Тогда
		...
		КомандаПодписиОтправки.Видимость = ЕстьПравоОбработки
			И (Объект.ТребуетсяПодтверждение ИЛИ ЭтоСлужебный)
			И ТребуетсяПодпись
	...
	Элементы.ФормаСоздатьПроизвольныйДокумент.Видимость = ЕстьПравоОбработки;
	...
	Элементы.ГруппаКомандАннулирование.Доступность = Элементы.ГруппаКомандАннулирование.Видимость;
	Элементы.КомандаОтклонитьАннулирование.Доступность = Элементы.ГруппаКомандАннулирование.Доступность;
	Элементы.КомандаПринятьАннулирование.Доступность = Элементы.ГруппаКомандАннулирование.Доступность;
	...
	Элементы.КомандаПоставитьВОчередьПриема.Доступность = Ложь;
	
	МассивПервичныхЭД = ОбменСКонтрагентамиСлужебный.ТипыПервичныхЭД();
	
	Если Объект.СостояниеЭДО = Перечисления.СостоянияВерсийЭД.ОшибкаПередачи
		И МассивПервичныхЭД.Найти(СвойстваФайлаЭД.ТипЭД) <> Неопределено
		И СвойстваФайлаЭД.СтатусЭД = Перечисления.СтатусыЭД.ОшибкаПередачи Тогда
		
		Элементы.КомандаПоставитьВОчередьПриема.Доступность = Истина;
		
	КонецЕсли;
	
	Элементы.КомандаПоставитьВОчередьПриема.Видимость = Элементы.КомандаПоставитьВОчередьПриема.Доступность;
...
Процедура ВыполнитьОбработкуОповещенияНаСервере()
	...
	ПоказатьПредставлениеФайлаЭлектронногоДокумента(
		ДеревоПодчиненныеЭД.НайтиПоИдентификатору(ИдентификаторТекущейСтроки), Ложь, КонтекстОперации);
...
Процедура ОформитьИнтерфейсДляМобильногоКлиентаНаСервере()
	...
	ВидимыеЭлементы = "ГруппаШапка, ГруппаСостояние, СостояниеЭД, СостояниеТекст, КартинкаОбновить,
...
&НаКлиенте
Процедура ПродолжитьУтверждениеДокумента()
...
&НаКлиенте
Функция ПолучитьСписокВерсийФайловДляСохранения()
	СписокВерсийФайлов = Новый СписокЗначений;
	
	Для каждого СтрОснования Из Объект.ДокументыОснования Цикл
		Если ЗначениеЗаполнено(СтрОснования.ВерсияФайла)
			И СписокВерсийФайлов.НайтиПоЗначению(СтрОснования.ВерсияФайла) = Неопределено Тогда
			
			СписокВерсийФайлов.Добавить(СтрОснования.ВерсияФайла, 
				СтрШаблон(НСтр("ru = '%1 (версия %2)'"), СтрОснования.ФайлПредставление, СтрОснования.ВерсияФайлаПредставление));
		КонецЕсли;
	КонецЦикла;
	
	Возврат СписокВерсийФайлов;
КонецФункции // ПолучитьСписокВерсийФайловДляСохранения()
...
Процедура ЗаполнитьВспомогательныеРеквизиты()
	...
	Запрос.Текст =
			...
			|	ВерсииФайлов.Представление КАК ВерсияФайлаПредставление,
			|	ЕСТЬNULL(Файлы.Ссылка, ЗНАЧЕНИЕ(Справочник.Файлы.ПустаяСсылка)) КАК Файл,
			|	ЕСТЬNULL(Файлы.Представление, """") КАК ФайлПредставление,
			|	ЕСТЬNULL(ВнутренниеДокументы.Ссылка, ЗНАЧЕНИЕ(Справочник.ВнутренниеДокументы.ПустаяСсылка)) КАК ДокументДО,
			|	ЕСТЬNULL(ВнутренниеДокументы.Представление, """") КАК ДокументДОПредставление
			|ИЗ
	...
	Запрос.УстановитьПараметр("ВерсииФайлов", Объект.ДокументыОснования.Выгрузить().ВыгрузитьКолонку("ДокументОснование"));
	
	ТЗВерсийФайловСПредставлениями = Запрос.Выполнить().Выгрузить();
	
	Для каждого СтрОснования Из Объект.ДокументыОснования Цикл
		СтрокаВерсии = ТЗВерсийФайловСПредставлениями.Найти(СтрОснования.ДокументОснование, "ВерсияФайла");
		
		Если СтрокаВерсии <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(СтрОснования, СтрокаВерсии);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
