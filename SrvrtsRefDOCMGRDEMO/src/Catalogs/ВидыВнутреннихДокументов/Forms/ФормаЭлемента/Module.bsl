...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РаботаСШаблонамиДокументовСервер.УстановитьТипыМестаИОтветственногоЗаХранение(ЭтаФорма);
	
	// Работа с аннулирование по ЭДО
	ИспользоватьОбменЭД = ПолучитьФункциональнуюОпцию("ИспользоватьОбменЭД");
	ЗаполнитьПараметрыАннулированияЭДО();
	НастроитьЭлементыКоммандАннулирования(ЭтаФорма);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если РаздельныйУчетДокументов И ЗначениеЗаполнено(ТекущийОбъект.Ссылка) Тогда
	...
	Если ТребуетсяНастроитьАннулированиеЭДО() Тогда
		ПараметрыЗаписи.Вставить("ТребуетсяНастроитьАннулированиеЭДО", Истина);
	КонецЕсли;
...
Процедура ЯвляетсяДоговоромПриИзменении(Элемент)
	...
	Если Объект.ЯвляетсяДоговором Тогда
		...
		Объект.ВариантУказанияОтменяющегоДокумента = ПредопределенноеЗначение(
...
Процедура ЯвляетсяЗаявкойНаОплатуПриИзменении(Элемент)
	...
	Если Объект.ЯвляетсяЗаявкойНаОплату Тогда
		...
		Объект.ВариантУказанияОтменяющегоДокумента = ПредопределенноеЗначение(
			"Перечисление.ВариантыУказанияОтменяющегоДокумента.ПустаяСсылка");
		...
		НаименованиеСтороны1.ДопустимыПользователи = Ложь;
		...
		НаименованиеСтороны2.ДопустимыПользователи = Истина;
...
&НаКлиенте
Процедура УчитыватьНедействующиеДокументыПриИзменении(Элемент)
	
	Если Объект.УчитыватьНедействующиеДокументы Тогда
		Объект.ВариантУказанияОтменяющегоДокумента = ПредопределенноеЗначение(
			"Перечисление.ВариантыУказанияОтменяющегоДокумента.Необязательное");
	Иначе
		Объект.ВариантУказанияОтменяющегоДокумента = ПредопределенноеЗначение(
...
&НаКлиенте
Процедура ВариантУказанияОтменяющегоДокументаПриИзменении(Элемент)
...
Процедура ПодписантНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ЗаголовокФормы = НСтр("ru = 'Выбор подписанта'");
...
Процедура ОтветственныйЗаХранениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	РаботаСАдреснойКнигойКлиент.ВыбратьОтветсвенногоЗаХранение(ЭтаФорма);
...
Процедура НоменклатураДелНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДелопроизводствоКлиент.НоменклатураДелНачалоВыбора(
		ЭтаФорма, Элемент, СтандартнаяОбработка, Контрагенты, Истина);
...
Процедура УтверждаетТекстНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	КонецЕсли;
	ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ru = 'Выбор утверждающего'"));
...
Процедура ПометитьШаблонНаУдаление(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, 
		СтрШаблон(НСтр("ru = 'Пометить ""%1"" на удаление?'"), ИмяШаблона), 
		РежимДиалогаВопрос.ДаНет,,КодВозвратаДиалога.Да);
...
&НаКлиенте
Процедура НастроитьАннулированиеЭДОВходящее(Команда)
	
	НастроитьАннулированиеЭДО(
		ПредопределенноеЗначение("Перечисление.НаправленияЭД.Входящий"));
	
КонецПроцедуры

&НаКлиенте
Процедура НастроитьАннулированиеЭДОИсходящее(Команда)
...
Процедура УстановитьДоступностьОпций(Форма)
	...
	ДоступностьГруппыКонтроляБумажныхДокументов = Не Объект.ЯвляетсяКомплектомДокументов
		И НЕ (Форма.РаздельныйУчетДокументов 
	...
	Если Форма.РаздельныйУчетДокументов Тогда 
	...
	УстановитьДоступностьОпции(Форма, "ВестиУчетПоСтатьямДДС",
		Не Объект.ЯвляетсяЗаявкойНаОплату);
	
	УстановитьДоступностьОпции(Форма, "ВариантУказанияОтменяющегоДокумента",
		Объект.УчитыватьНедействующиеДокументы);
...
Процедура ПослеЗаписиКлиент(ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("Закрыть")
		И ПараметрыЗаписи.Свойство("ТребуетсяНастроитьАннулированиеЭДО")
		И Не ПараметрыЗаписи.Свойство("ПропущенаНастройкаАннулированияЭДО") Тогда
		
		ОбработчикОтвета = Новый ОписаниеОповещения(
			"ПослеЗаписиКлиентПослеОтветаНастроекАннулирования",
			ЭтотОбъект,
			ПараметрыЗаписи);
		
		ПоказатьВопрос(ОбработчикОтвета,
			НСтр("ru = 'Для данного вида документов настроен обмен по ЭДО, но не настроены
				|правила создания документов аннулирования. Документы этого вида невозможно будет аннулировать.
				|Пропустить настройку создания документов аннулирования?'"),
			РежимДиалогаВопрос.ДаНет, , ,
			НСтр("ru = 'Не настроено аннулирование ЭДО.'"));
...
&НаКлиенте
Процедура ПослеЗаписиКлиентПослеОтветаНастроекАннулирования(Результат,
	...
	ПараметрыЗаписи.Вставить("ПропущенаНастройкаАннулированияЭДО", Истина);
	ПослеЗаписиКлиент(ПараметрыЗаписи);
...
Процедура ВыполнитьИзменениеОрганизации(ВидДокумента, Организация)
	...
	Исключение
		...
		Исполнители = РаботаСУведомлениями.СписокПолучателейУведомленийОПроблемах(
		...
		Для Каждого Пользователь Из Исполнители Цикл
...
Процедура ЗаполнитьСписокШаблоновВида(ВидДокумента = Неопределено)
	...
	Если Не МногоШаблонов Тогда 
		...
		Иначе 
			ЗаголовокРабочейГруппы = НСтр("ru = 'Доступен всем'");
...
Процедура ОбновитьВидимостьРеквизитов(Форма)
	...
	Если Объект.ВестиУчетПоНоменклатуреДел Или Форма.РаздельныйУчетДокументов Тогда
	...
	Если Форма.РаздельныйУчетДокументов 
		И ШаблонДокумента.ФормаДокумента = ПредопределенноеЗначение("Перечисление.ВариантыФормДокументов.Электронная") Тогда 
...
&НаКлиентеНаСервереБезКонтекста
Процедура НастроитьЭлементыКоммандАннулирования(Форма)
	
	Объект = Форма.Объект;
	
	Форма.Элементы.НастроитьАннулированиеЭДОИсходящее.Видимость = Истина;
	Форма.Элементы.НастроитьАннулированиеЭДОВходящее.Видимость = Истина;
	
	Если Не Форма.ИспользоватьОбменЭД Тогда
		Форма.Элементы.НастроитьАннулированиеЭДОИсходящее.Видимость = Ложь;
		Форма.Элементы.НастроитьАннулированиеЭДОВходящее.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	Если Не Объект.УчитыватьНедействующиеДокументы Тогда
		Форма.Элементы.НастроитьАннулированиеЭДОИсходящее.Видимость = Ложь;
		Форма.Элементы.НастроитьАннулированиеЭДОВходящее.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	Если Объект.ВариантУказанияОтменяющегоДокумента = ПредопределенноеЗначение(
			"Перечисление.ВариантыУказанияОтменяющегоДокумента.БезУказания")
		ИЛИ Объект.ВариантУказанияОтменяющегоДокумента = ПредопределенноеЗначение(
			"Перечисление.ВариантыУказанияОтменяющегоДокумента.ПустаяСсылка") Тогда
...
&НаСервере
Процедура ЗаполнитьПараметрыАннулированияЭДО()
	
	НастроеныПараметрыАннулированияЭДОИсходящие = Ложь;
	НастроеныПараметрыАннулированияЭДОВходящие = Ложь;
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	НастройкиСозданияДокументовАннулирования.Направление КАК Направление,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Направление = Перечисления.НаправленияЭД.Исходящий Тогда
			НастроеныПараметрыАннулированияЭДОИсходящие = Истина;
			
			Элементы.НастроитьАннулированиеЭДОИсходящее.Заголовок =
				СтрШаблон(НСтр("ru = 'Отдельным документом вида %1'"),
					Выборка.ВидДокументаАннулирования);
		КонецЕсли;
		
		
		Если Выборка.Направление = Перечисления.НаправленияЭД.Входящий Тогда
			НастроеныПараметрыАннулированияЭДОВходящие = Истина;
			
			Элементы.НастроитьАннулированиеЭДОВходящее.Заголовок =
				СтрШаблон(НСтр("ru = 'Отдельным документом вида %1'"),
					Выборка.ВидДокументаАннулирования);
		КонецЕсли;
	...
	Если Не НастроеныПараметрыАннулированияЭДОВходящие Тогда
		Элементы.НастроитьАннулированиеЭДОВходящее.Заголовок =
			НСтр("ru = 'Без отменяющего документа'");
	КонецЕсли;
	
	Если Не НастроеныПараметрыАннулированияЭДОИсходящие Тогда
		Элементы.НастроитьАннулированиеЭДОИсходящее.Заголовок =
			НСтр("ru = 'Без отменяющего документа'");
	КонецЕсли;
...
&НаКлиенте
Процедура НастроитьАннулированиеЭДО(Направление)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ОбработкаОтвета = Новый ОписаниеОповещения(
			"НастроитьАннулированиеЭДОПослеОтветаНаВопросЗаписи",
			ЭтотОбъект,
			ДополнительныеПараметры);
		
		ПоказатьВопрос(ОбработкаОтвета,
			НСтр("ru = 'Вид документов еще не записан, настройку можно произвести только после записи объекта.
				|Выполнить запись?'"),
			РежимДиалогаВопрос.ДаНет, , ,
			НСтр("ru = 'Запись объекта'"));
		Возврат;
	КонецЕсли;
	
	ОткрытьФормуНастроекАннулированияЭДО(Направление);
	
КонецПроцедуры

&НаКлиенте
Процедура НастроитьАннулированиеЭДОПослеОтветаНаВопросЗаписи(
	...
	ОткрытьФормуНастроекАннулированияЭДО(ДополнительныеПараметры.Направление);
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьФормуНастроекАннулированияЭДО(Направление)
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ВидДокумента", Объект.Ссылка);
	ПараметрыФормы.Вставить("Направление", Направление);
	ПараметрыФормы.Вставить("ОбязателенОтменяющийДокумент",
		Объект.ВариантУказанияОтменяющегоДокумента = ПредопределенноеЗначение(
			"Перечисление.ВариантыУказанияОтменяющегоДокумента.Обязательное"));
	
	ОбработчикЗакрытия = Новый ОписаниеОповещения(
		"ОбработатьЗакрытиеФормыНастроекАннулирования", ЭтаФорма);
	
	ОткрытьФорму("РегистрСведений.НастройкиСозданияДокументовАннулирования.Форма.НастройкаСозданияДокументаАннулирования",
		ПараметрыФормы, 
		ЭтаФорма, , , ,
		ОбработчикЗакрытия,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаКлиенте
Процедура ОбработатьЗакрытиеФормыНастроекАннулирования(Результат, ДополнительныеПараметры) Экспорт
...
&НаСервере
Функция ТребуетсяНастроитьАннулированиеЭДО()
	...
	Если Объект.ВариантУказанияОтменяющегоДокумента <> ПредопределенноеЗначение(
			"Перечисление.ВариантыУказанияОтменяющегоДокумента.Обязательное") Тогда
		
		Возврат Ложь;
	КонецЕсли;
	
	Если НастроеныПараметрыАннулированияЭДОВходящие
		И НастроеныПараметрыАннулированияЭДОИсходящие Тогда
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	ПравилаУчетаВидовЭДДО.ВидДокумента = &ВидДокумента
