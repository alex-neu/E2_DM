...
Процедура ПриОткрытии(Отказ)
	...
	Если ПредложитьЗаполнитьПоШаблону Тогда
		...
		ТекстВопроса = НСтр("ru = 'Заполнить контрольные точки по шаблону?'");
...
Процедура ЗаполнитьПоШаблону(Команда)
	...
	Если Не ЕстьШаблонПоВидуОбъекта Тогда
		Текст =	НСтр("ru = 'Отсутствует шаблон контрольных точек для вида проекта '");
	...
	Режим.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Да'"));
	Режим.Добавить(КодВозвратаДиалога.Нет, НСтр("ru = 'Нет'"));
...
Процедура ЗаполнитьПоШаблонуПродолжение(Ответ, Параметры) Экспорт
	...
	Состояние(НСтр("ru = 'Идет загрузка шаблона. Пожалуйста подождите...'"));
	...
	Состояние(НСтр("ru = 'Шаблон загружен.'"));
...
Процедура Выгрузить(Команда)
	...
	Если Диалог.Выбрать() Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Идёт сохранение списка контрольных точек в файл ""%1"". Пожалуйста подождите...'"),
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Список контрольных точек сохранен в файл ""%1""'"),
				Диалог.ПолноеИмяФайла);
		Состояние(Текст);
...
Процедура ЗагрузитьКТИзXMLПродолжение(Ответ, Параметры) Экспорт
	...
	ПараметрыДиалога.Вставить("Заголовок", НСтр("ru = 'Выберите файл'"));
	ПараметрыДиалога.Вставить("Фильтр", НСтр("ru = 'Файлы XML (*.xml)'") + "|*.xml|");
...
Процедура ЗагрузитьКТИзXMLЗавершение(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Идёт загрука списка контрольных точек из файла ""%1"". Пожалуйста подождите...'"),
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Список контрольных точек загружен из файла ""%1""'"),
			РезультатПомещенияФайлов.Имя);
	Состояние(Текст);
...
Процедура ЗаполнитьПоШаблонуНаСервере()
	...
	Если ВидОбъектаКТ = Неопределено Тогда
		...
		ВызватьИсключение НСтр("ru = 'Вид объекта контрольных точек не найден.'");
...
Процедура ЗапроситьОценки(Команда)
	...
	СтрокаСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Пожалуйста, подождите!%1Идет запрос оценок...'"),
		Символы.ПС);
	...
	Если КоличествоЗапрошенныхОценок > 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Оценки запрошены у ответственных за контрольные точки.'"));
	Иначе
		ПоказатьПредупреждение(,НСтр("ru = 'Нет контрольных точек нуждающихся в оценке ответственными.'"));
	КонецЕсли;
...
Процедура УдалитьГруппу()
	...
	Если Элементы.ГруппыКТ.ВыделенныеСтроки.Количество() > 1 Тогда
		ТекстВопроса = СтрШаблон(
			НСтр("ru = 'Пометить на удаление выбранные группы (%1) и входящие в них контрольные точки?'"),
			Элементы.ГруппыКТ.ВыделенныеСтроки.Количество());
	Иначе
		ТекстВопроса = НСтр("ru = 'Пометить на удаление выбранную группу и входящие в нее контрольные точки?'");
	КонецЕсли;	
	
	Режим = Новый СписокЗначений;
	Режим.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'ОК'"));
	Режим.Добавить(КодВозвратаДиалога.Нет, НСтр("ru = 'Отмена'"));
...
Процедура УдалениеГруппПродолжение(Ответ, Параметры) Экспорт
	...
	Состояние(НСтр("ru = 'Идет удаление групп и контрольных точек. Пожалуйста подождите...'"));
	
	УдалениеГруппПродолжениеСервер();
	
	Состояние(НСтр("ru = 'Группы и контрольные точки удалены.'"));
...
Процедура УдалениеГруппПродолжениеСервер()
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Изменение группы контрольной точки'"),
			...
			Если ИзменитьГруппуКТ(МассивКТ, СтрокаГруппыКТ.Группа) Тогда
				...
				Если МассивКТ.Количество() = 1 Тогда
					...
					ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Контрольная точка ""%1"" перенесена в группу ""%2""'"), МассивКТ[0], НоваяГруппаКТ);
					
					ПоказатьОповещениеПользователя(
						НСтр("ru = 'Контрольная точка перенесена.'"),
				...
				Иначе
					...
					ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Контрольные точки (%1 шт.) перенесены в группу ""%2""'"), МассивКТ.Количество(), НоваяГруппаКТ);
					
					ПоказатьОповещениеПользователя(
						НСтр("ru = 'Контрольные точки перенесены.'"),
						,
						ПолноеОписание,
						БиблиотекаКартинок.Информация32);
	...
	Для Каждого КонтрольнаяТочка Из МассивКТ Цикл
		Если Не ДокументооборотПраваДоступа.ПолучитьПраваПоОбъекту(КонтрольнаяТочка).Изменение Тогда 
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У вас нет права на изменение контрольной точки ""%1"".'"),
