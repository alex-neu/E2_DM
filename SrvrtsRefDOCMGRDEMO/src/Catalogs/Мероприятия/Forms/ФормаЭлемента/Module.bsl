...
Процедура СкопироватьСтрокиНаСервере(ЧислоФайлов)
	...
	ОбщегоНазначенияДокументооборот.СкопироватьФайлыВБуферОбменаИзСпискаФайлов(Элементы.Файлы.ВыделенныеСтроки);
...
Процедура ЗаполнитьПротоколНаОснованииПредыдущего(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.ПредыдущееМероприятие) Тогда
		ТекстСообщения = НСтр("ru = 'Не указано предыдущее мероприятие.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
...
Процедура ИсполнитьПротокол_Завершение(Результат, ДопПараметры) Экспорт
	...
	ПараметрыНаИсполнение.Мероприятие = Объект.Ссылка;
	ПараметрыНаИсполнение.ТипПротокола = Объект.ТипПротокола;
	ПараметрыНаИсполнение.МатериалПротокол = Объект.МатериалПротокол;
	ПараметрыНаИсполнение.РаздельноеИсполнение = РаздельноеИсполнениеПунктовПротокола;
	ПараметрыНаИсполнение.Владелец = ЭтотОбъект;
	ПараметрыНаИсполнение.ТаблицаПротокол = Объект.Протокол;
...
Процедура УстановитьДоступностьКоманд(ТекущиеДанные)
	...
	Иначе	
		...
		УстановитьДоступностьКоманды(Элементы.ФайлыКонтекстноеМеню.ПодчиненныеЭлементы.ФайлыКонтекстноеМенюРаспознать,
			РасширениеПоддерживается И ДоступноРаспознаваниеПоЗапросу И Редактирует.Пустая());
...
Процедура ЗаполнитьСписокТиповПредмета()
	...
	Если ПравоДоступа("Просмотр", Метаданные.Справочники.ВходящиеДокументы) Тогда
		СписокТиповПредмета.Добавить("Справочник.ВходящиеДокументы", НСтр("ru = 'Входящий документ'"));
	КонецЕсли;
	Если ПравоДоступа("Просмотр", Метаданные.Справочники.ИсходящиеДокументы) Тогда
		СписокТиповПредмета.Добавить("Справочник.ИсходящиеДокументы", НСтр("ru = 'Исходящий документ'"));
	КонецЕсли;
	Если ПравоДоступа("Просмотр", Метаданные.Справочники.ВнутренниеДокументы) Тогда
		СписокТиповПредмета.Добавить("Справочник.ВнутренниеДокументы", НСтр("ru = 'Внутренний документ'"));
	КонецЕсли;
