	...
	Для каждого СсылкаНаОбъект Из НаборСсылок Цикл
		...
		Попытка
			...
			Если Объект <> Неопределено Тогда
				...
				ОбработатьДанные_НоваяАрхитектураНастроекЭДО(Объект, Реквизиты, СвойстваПрофилейЭДО, КонверторФорматов, НастройкиПолученияВручную, Записать);
			КонецЕсли;
...
Функция ДанныеКОбработке_НоваяАрхитектураНастроекЭДО()
	...
	Если ОбменСКонтрагентамиСлужебный.ПрямойОбменДоступен() Тогда
...
Процедура ОбработатьДанные_НоваяАрхитектураНастроекЭДО(Объект, Реквизиты, СвойстваПрофилейЭДО, КонверторФорматов, НастройкиПолученияВручную, Записать)
	...
	ЭтоПрямойОбмен = ОбменСКонтрагентамиСлужебный.ЭтоПрямойОбмен(Объект.СпособОбменаЭД);
	...
	Если Объект.ИспользуетсяДляОтправки
		...
		Если Не ЗначениеЗаполнено(Набор) Тогда
			...
			Для каждого СтрокаНастроек Из Объект.ИсходящиеДокументы Цикл
				...
				Если ЭтоПрямойОбмен
					И (СтрокаНастроек.ИсходящийДокумент = Перечисления.ВидыЭД.СчетФактура
						ИЛИ СтрокаНастроек.ИсходящийДокумент = Перечисления.ВидыЭД.КорректировочныйСчетФактура) Тогда
					Продолжить;
				КонецЕсли;
...
Функция ОтправитьСообщения(НастройкаПриемаОтправки, Транспорт) Экспорт
	...
	ОбменСКонтрагентамиСлужебный.ОтправитьЭлектронныеДокументы();
