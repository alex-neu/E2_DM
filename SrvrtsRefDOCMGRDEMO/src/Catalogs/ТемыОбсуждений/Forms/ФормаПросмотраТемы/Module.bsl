...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИспользоватьАвтоматическоеПрочтение =
		ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
			"НастройкиПрочтения",
			"ИспользоватьАвтоматическоеПрочтениеФорума",
			Истина);
...
&НаКлиенте
Процедура ТекущееСообщениеТекстПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ТекущееСообщениеТекстСФайламиПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ТекущееСообщениеТекстСГолосованиемПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ТекущееСообщениеТекстСФайламиИГолосованиемПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
...
Процедура ПраваДоступа(Команда)
	...
	Если Не ЗначениеЗаполнено(Тема) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не указана тема.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбработкаОжиданияПрочтения()
	...
	Если ТекущиеДанныеСообщения <> Неопределено Тогда
		...
		Если ТекущиеДанныеСообщения.Свойство("ТекстСообщения")
			...
			ТекущееСообщениеТекст = РаботаСОбсуждениямиВызовСервера.ПредставлениеСообщенияHTML(
				ТекущееСообщение,
				Истина,
...
Процедура ЗадатьТему(НоваяТема)
	...
	Если ЗначениеЗаполнено(Тема) Тогда
		...
		Элементы.СписокСоздать.Заголовок = НСтр("ru = 'Ответить'");
		Элементы.ДеревоСообщенийСоздать.Заголовок = НСтр("ru = 'Ответить'");
	Иначе
		Элементы.СписокСоздать.Заголовок = НСтр("ru = 'Создать новую тему'");
...
Процедура ОбновитьЗаголовок()
	...
	ПредставлениеСообщенийТемы = НСтр("ru = 'Тема форума:'");
	
	Если ЗначениеЗаполнено(Тема) Тогда
		ПредставлениеЗаголовка = Строка(Тема);
	Иначе
		ПредставлениеЗаголовка = НСтр("ru = 'Новая тема'");
	КонецЕсли;
...
Процедура УстановитьДоступностьКоманд()
	...
	Элементы.ОтправитьОбъекты.Доступность = ЗначениеЗаполнено(ТекущееСообщение);
	Элементы.ОтправитьОбъектыДерево.Доступность = ЗначениеЗаполнено(ТекущееСообщение);
	
КонецПроцедуры

&НаСервере
Процедура УстановитьШрифтОбластиЧтения()
	
	РаботаСОбсуждениями.УстановитьПерсональнуюНастройку("ШрифтОбластиЧтения", ШрифтОбластиЧтения);
	ТекущееСообщениеТекст = РаботаСОбсуждениями.ПредставлениеСообщенияHTML(
		ТекущееСообщение,
		Истина,
		УникальныйИдентификатор);
