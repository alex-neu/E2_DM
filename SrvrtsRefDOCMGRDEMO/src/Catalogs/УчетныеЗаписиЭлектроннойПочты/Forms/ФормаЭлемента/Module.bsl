...
Процедура ВыполнитьПолучениеТокена()
	...
	Если СтрНайти(НРег(Объект.СерверВходящейПочты), "yandex.ru") <> 0 Тогда
		...
		АдресПолный = СтрШаблон(                                              
			"https://oauth.yandex.ru/authorize?client_id=%1&response_type=code&redirect_uri=%2&state=%3",
	...
	ИначеЕсли СтрНайти(НРег(Объект.СерверВходящейПочты), "mail.ru") <> 0 Тогда
		...
		АдресПолный = СтрШаблон(                                              
			"https://oauth.mail.ru/login?client_id=%1&response_type=code&scope=%2&redirect_uri=%3&state=%4",
			OAuthАвторизацияКлиент, 
