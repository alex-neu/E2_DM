...
Процедура ЗаполнитьДескрипторыОбъекта(ОбъектДоступа, ТаблицаДескрипторов, ПротоколРасчетаПрав = Неопределено) Экспорт
	...
	Если ПротоколРасчетаПрав <> Неопределено Тогда
		ЗаписьПротокола = Новый Структура("Элемент, Описание",
			ОбъектДоступа.Ссылка, НСтр("ru = 'Ответственные за обработку писем'"));
...
Функция ПроверитьПодключениеКСерверуИсходящейПочты(Профиль, АдресЭлектроннойПочты)
	...
	Тема = НСтр("ru = 'Тестовое сообщение 1С:Документооборот'");
	Тело = НСтр("ru = 'Это сообщение отправлено подсистемой электронной почты 1С:Документооборот'");
	ИмяОтправителяПисем = НСтр("ru = '1С:Документооборот'");
...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	Элемент.ИмяПользователя                  = НСтр("ru = '1С:Документооборот'");
...
Функция ПодобратьНастройкиПочты(АдресЭлектроннойПочты, Пароль, ДляОтправки, ДляПолучения, 
	ИзвестныеНастройкиПочтовогоСервера)
	...
	НайденныйПрофильPOP = ?(НастройкиПолучены И НайденныйПрофильIMAP = Неопределено 
		И ЗначениеЗаполнено(Профиль.АдресСервераPOP3), Профиль, Неопределено);
	...
	Если НайденныйПрофильIMAP <> Неопределено Тогда
		...
		Результат.Вставить("ИспользоватьЗащищенноеСоединениеДляВходящейПочты", 
			НайденныйПрофильIMAP.ИспользоватьSSLIMAP);
	КонецЕсли;
	...
	Если НайденныйПрофильPOP <> Неопределено Тогда
		...
		Результат.Вставить("ИспользоватьЗащищенноеСоединениеДляВходящейПочты", 
			НайденныйПрофильPOP.ИспользоватьSSLPOP3);
	КонецЕсли;
	...
	Если НайденныйПрофильSMTP <> Неопределено Тогда
		...
		Результат.Вставить("ИспользоватьЗащищенноеСоединениеДляИсходящейПочты", 
			НайденныйПрофильSMTP.ИспользоватьSSLSMTP);
	КонецЕсли;
	
	Результат.Вставить("ДляПолучения", 
		НайденныйПрофильIMAP <> Неопределено Или НайденныйПрофильPOP <> Неопределено);
...
Функция ПроверитьНастройкиПрофилей(ПрофильИсходящейПочты, ПрофильВходящейПочты, АдресЭлектроннойПочты) Экспорт
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			Если ОшибкаАутентификации(ТекстОшибки) Тогда
				ТекстСообщения.Добавить(
					НСтр("ru = 'Отправка тестового сообщения не выполнена: не удалось авторизоваться.'"));
			Иначе
				...
				Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
					МодульПолучениеФайловИзИнтернета = ОбщегоНазначения.ОбщийМодуль("ПолучениеФайловИзИнтернета");
					ДиагностикаСоединения = МодульПолучениеФайловИзИнтернета.ДиагностикаСоединения(
						ПрофильИсходящейПочты.АдресСервераSMTP);
	...
	Если ПрофильВходящейПочты <> Неопределено Тогда
		Протокол = ?(ЗначениеЗаполнено(ПрофильВходящейПочты.АдресСервераIMAP), 
			ПротоколИнтернетПочты.IMAP, ПротоколИнтернетПочты.POP3);
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			Если ОшибкаАутентификации(ТекстОшибки) Тогда
				ТекстСообщения.Добавить(
					НСтр("ru = 'Подключение к серверу входящей почты не выполнено: не удалось авторизоваться.'"));
			Иначе
				...
				Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
					МодульПолучениеФайловИзИнтернета = ОбщегоНазначения.ОбщийМодуль("ПолучениеФайловИзИнтернета");
					СерверВходящейПочты = ?(Протокол = ПротоколИнтернетПочты.IMAP, 
						ПрофильВходящейПочты.АдресСервераIMAP, ПрофильВходящейПочты.АдресСервераPOP3);
					ДиагностикаСоединения = МодульПолучениеФайловИзИнтернета.ДиагностикаСоединения(
						СерверВходящейПочты);
					ТехническиеПодробности.Добавить(ДиагностикаСоединения.ЖурналДиагностики);
				КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстСообщения) Тогда
		...
		Если ОшибкаАутентификации Тогда
			Рекомендации.Добавить(
				НСтр("ru = 'Проверьте правильность ввода логина и пароля, а также выбранный способ авторизации.'"));
		КонецЕсли;
...
Функция ИнформацияОПрограмме()
	...
	Результат.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Операционная система: %1'"), СистемнаяИнформация.ВерсияОС));
	Результат.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Платформа: %1 (%2)'"),
