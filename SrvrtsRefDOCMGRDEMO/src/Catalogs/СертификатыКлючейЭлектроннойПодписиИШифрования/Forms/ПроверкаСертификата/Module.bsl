...
Процедура Подключаемый_НадписьНажатие(Элемент)
	...
	ТекстОшибкиКлиент = "";
	ОписаниеОшибкиКлиент = ПроверкиНаКлиенте[ИмяОшибки];
	Если ТипЗнч(ОписаниеОшибкиКлиент) = Тип("Структура") Тогда
		
		Если ОписаниеОшибкиКлиент.Свойство("ТекстОшибкиПроверкиПодписи") Тогда
			ТекстОшибкиКлиент = ОписаниеОшибкиКлиент.ТекстОшибкиПроверкиПодписи;
		КонецЕсли;
		
		Если ОписаниеОшибкиКлиент.Свойство("ТекстОшибкиПроверкиСертификата") Тогда
			ТекстОшибкиКлиент = ОписаниеОшибкиКлиент.ТекстОшибкиПроверкиСертификата;
		КонецЕсли;
		
	ИначеЕсли ТипЗнч(ОписаниеОшибкиКлиент) = Тип("Строка") Тогда
		
		ТекстОшибкиКлиент = ОписаниеОшибкиКлиент;
...
Процедура УстановитьЭлемент(Форма, НачалоЭлемента, НаСервере,
	...
	Если ТипЗнч(ОписаниеОшибки) = Тип("Структура") Тогда
		...
		Если ОписаниеОшибки.Свойство("ТекстОшибкиПроверкиПодписи") Тогда
			ТекстОшибки = ОписаниеОшибки.ТекстОшибкиПроверкиПодписи;
		КонецЕсли;
		
		Если ОписаниеОшибки.Свойство("ТекстОшибкиПроверкиСертификата") Тогда
			ТекстОшибки = ОписаниеОшибки.ТекстОшибкиПроверкиСертификата;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		Если Не РезультатПроверки Тогда
			...
			ЭлементКартинка.Подсказка = ТекстОшибки;
			ЭлементНадпись.Гиперссылка = Истина;
			ЭлементНадпись.Заголовок = ТекстОшибки;
			
			ОписаниеИзвестнойОшибки = ОшибкаПоКлассификатору(ТекстОшибки);
	...
	Иначе
		...
		ЗначениеПроверки = ?(ТекстОшибки = Неопределено, Неопределено, Истина);
		Если (ОбъединятьРезультаты = "ОбъединятьПоИ"
			Или ОбъединятьРезультаты = "ОбъединятьПоИли") Тогда
			...
			КонецЕсли;
		Иначе
			ЭлементКартинка.Картинка = ?(ТекстОшибки = Неопределено,
				БиблиотекаКартинок.ПроверкаСертификатаНеВыполнялась,
				БиблиотекаКартинок.ПроверкаСертификатаУспех);
			ЭлементКартинка.Подсказка = ?(ТекстОшибки = Неопределено,
...
Функция ОшибкаПоКлассификатору(ОписаниеОшибки)
	...
	Если ТипЗнч(ОписаниеОшибки) = Тип("Структура") Тогда
		...
		Если ОписаниеОшибки.Свойство("ТекстОшибкиПроверкиПодписи") Тогда
			Возврат ЭлектроннаяПодписьСлужебный.ОшибкаПоКлассификатору(ОписаниеОшибки.ТекстОшибкиПроверкиПодписи);
		КонецЕсли;
		
		Если ОписаниеОшибки.Свойство("ТекстОшибкиПроверкиСертификата") Тогда
			Возврат ЭлектроннаяПодписьСлужебный.ОшибкаПоКлассификатору(ОписаниеОшибки.ТекстОшибкиПроверкиСертификата);
		КонецЕсли;
