...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НавигационнаяСсылка = "e1cib/list/Справочник.ВходящиеДокументы";
...
&НаКлиенте
Процедура СоздатьДокументВыполнить(Команда)
...
Процедура Обновить(Команда)
	...
	Если ТекущийЭлемент = Элементы.ДеревоКатегорий Тогда
...
Процедура ВыполнитьПоискПоРеквизитам(Команда)
	...
	ПараметрыОткрытия = Новый Структура("ВидПоиска, КлючНазначенияИспользования ", 
		ПредопределенноеЗначение("Перечисление.ВидыПоиска.ПоискВходящиеДокументы"),
		"ПоискИзСпискаВходящихДокументов");
...
Процедура ОткрытьОтчеты(Команда)
	...
	Раздел = ПредопределенноеЗначение("Перечисление.РазделыОтчетов.ВходящиеДокументыСписок");
	
	ЗаголовокФормы = НСтр("ru = 'Отчеты по входящим документам'");
		...
		Если ИзменитьРеквизитДокументов(
			...
			Если ЗначениеПеретаскивания.Количество() = 1 Тогда
				ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Входящий документ ""%1"" перенесен в ""%2""'"), ЗначениеПеретаскивания[0], ЗначениеРеквизита);
			...
			Иначе
				ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Входящие документы (%1 шт.) перенесены в ""%2""'"), ЗначениеПеретаскивания.Количество(), ЗначениеРеквизита);
...
Процедура СоздатьНовыйДокументПродолжение(Результат, Параметры) Экспорт
	...
	ОткрытьФорму(ИмяФормыДокумента, ПараметрыФормы, Элементы.Список, Новый УникальныйИдентификатор);
...
Функция ТЦСоздатьДокумент() Экспорт
	...
	СоздатьДокументВыполнить(Неопределено);
	Возврат Истина;
