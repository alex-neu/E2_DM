...
&НаКлиенте
Процедура ТекстСообщенияHTMLПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
...
Процедура ЗакончитьРедактированиеПервогоФайла(Команда)
	...
	ПараметрыОбновленияФайла.РедактируетТекущийПользователь = РедактируетТекущийПользователь;
...
Процедура УстановитьДоступностьФайловыхКоманд()
	...
	Если ФайлПрикрепленный <> Неопределено Тогда
		...
		Элементы.ЗакончитьРедактированиеПервый.Доступность = РедактируетТекущийПользователь;
		Элементы.РедактироватьПервый.Доступность = 
			(ФайлРедактирует.Пустая()
			ИЛИ РедактируетТекущийПользователь)
...
Процедура УстановитьРежимЧтения()
	...
	Если УстановитьРежимЧтения Тогда
		...
		ТекстСообщенияHTML = РаботаСОбсуждениями.ПредставлениеСообщенияHTML(
			Объект.Ссылка,
			Ложь,
			УникальныйИдентификатор);
		Элементы.ТекстСообщения.Видимость = Ложь;
		Элементы.ТекстСообщенияHTML.Видимость = Истина;
	КонецЕсли;
