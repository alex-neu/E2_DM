...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка) Экспорт 
	...
	Если ТипЗнч(ОснованиеЗаполнения) = Тип("СправочникСсылка.ВходящиеДокументы") Тогда
		...
		Заголовок = СтрШаблон(НСтр("ru = 'Ответ на ""%1""'"), РеквизитыОснования.Заголовок);
	...
	ИначеЕсли ТипЗнч(ОснованиеЗаполнения) = Тип("СправочникСсылка.Мероприятия") Тогда
		...
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отправка ""%1""'"), РеквизитыМероприятия.Наименование);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПолучитьФункциональнуюОпцию("ВестиУчетПоПроектам")
			И ПолучитьФункциональнуюОпцию("ИспользоватьВидыИсходящихДокументов")
			...
			Если Получатель = Получатель1 И Адресат = Адресат1 Тогда
				...
				Если ЗначениеЗаполнено(Адресат1) Тогда 
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Контрагент %1 и контактное лицо %2 в списке получателей указаны дважды.'"),
						Строка(Получатель1), 
						Строка(Адресат1));
				Иначе 
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Контрагент %1 в списке получателей указан дважды.'"),
						Строка(Получатель1));
				КонецЕсли;	
