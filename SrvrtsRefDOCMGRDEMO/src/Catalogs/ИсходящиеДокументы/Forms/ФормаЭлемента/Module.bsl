...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("УстановитьСвязь") Тогда 
		...
		Если ТипСвязи = Справочники.ТипыСвязей.ПереадресованДокументом
			...
			Объект.Заголовок = СтрШаблон(НСтр("ru = 'Переадресовано ""%1""'"), 
	...
	Если Параметры.Свойство("ВОтветНа") Тогда
		...
		Если ЗначениеЗаполнено(ВОтветНа) Тогда 
			...
			Если ЗначениеЗаполнено(РеквизитыВОтветНа.ИсходящаяДата) 
				...
				ВОтветНаСтрока = СтрШаблон(НСтр("ru = 'Исх. № %1 от %2'"), 
			...
			Если ДелопроизводствоКлиентСервер.ЭтоДокумент(ВОтветНа) Тогда
				Объект.Заголовок = СтрШаблон(НСтр("ru = 'Ответ на ""%1""'"), РеквизитыВОтветНа.Заголовок);
			КонецЕсли;
		КонецЕсли;
	...
	Если ИспользоватьСервисСПАРКРиски Тогда 
		...
		СПАРКРиски.ПриСозданииНаСервере(
			...
			ПараметрыПроцедуры);
	Иначе 
		Элементы.ПолучателиСводныйИндикатор.Видимость = Ложь;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	УИДЗамераВыполнениеКомандыЗаписать = ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Ложь, "ИсходящиеДокументыВыполнениеКомандыЗаписать");
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если РегламентированныйУчетОбращений Тогда 
		ЕстьПеренаправление = Ложь; ЕстьОтвет = Ложь;
		Для Каждого СтрВопрос Из ВопросыОбращения Цикл
			Если ЗначениеЗаполнено(СтрВопрос.РезультатРассмотрения) 
				И (Не ЗначениеЗаполнено(СтрВопрос.ОтветныйНаОбращениеДокумент)
				Или СтрВопрос.ОтветныйНаОбращениеДокумент = Объект.Ссылка) Тогда
				
				Если СтрВопрос.РезультатРассмотрения <> Перечисления.РезультатыРассмотренияОбращений.НаправленоВИнойОрган Тогда 
					ЕстьОтвет = Истина;
				КонецЕсли;
		...
		Если ЕстьПеренаправление И ЕстьОтвет Тогда
			ТекстОшибки = НСтр("ru = 'Документ не может быть ответным и сопроводительным одновременно. 
			|Поменяйте результат рассмотрения вопросов на ответные или сопроводительные значения.'");
			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,
				"ВопросыОбращения",, Отказ);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	РеквизитыСвязанногоДокументаИзменены = Ложь;
	РеквизитыДокументаИзменены1 = Ложь; РеквизитыДокументаИзменены2 = Ложь; СвязиУстановлены = Ложь;
	Если РегламентированныйУчетОбращений Тогда  
		Для Каждого СтрВопрос Из ВопросыОбращения Цикл 
			Если СтрВопрос.Доступно = Ложь Тогда 
				Продолжить;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(СтрВопрос.РезультатРассмотрения) И ЗначениеЗаполнено(СтрВопрос.ОтветныйНаОбращениеДокумент)
				И СтрВопрос.ОтветныйНаОбращениеДокумент = ТекущийОбъект.Ссылка Тогда 
				Если СтрВопрос.ЭтоПререадресация = Истина Тогда 
					СвязиДокументов.УдалитьСвязь(ТекущийОбъект.Ссылка, ВОтветНа,
						Справочники.ТипыСвязей.ПереадресацияДокументаПоВопросу, СтрВопрос.Вопрос);
				Иначе 
					СвязиДокументов.УдалитьСвязь(ТекущийОбъект.Ссылка, ВОтветНа, 
						Справочники.ТипыСвязей.ОтправленВОтветНа, СтрВопрос.Вопрос);
				КонецЕсли;
					
				РеквизитыДокументаИзменены1 = УстановитьРеквизитыПриИзмененииСвязи(ВОтветНа, СтрВопрос.РезультатРассмотрения,
					СтрВопрос.Вопрос, Объект.Получатели);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтрВопрос.РезультатРассмотрения) 
				И (Не ЗначениеЗаполнено(СтрВопрос.ОтветныйНаОбращениеДокумент)
				Или СтрВопрос.ОтветныйНаОбращениеДокумент = ТекущийОбъект.Ссылка) Тогда 
				Если СтрВопрос.РезультатРассмотрения = Перечисления.РезультатыРассмотренияОбращений.НаправленоВИнойОрган Тогда 
					СвязиДокументов.СоздатьСвязь(ТекущийОбъект.Ссылка, ВОтветНа, 
						Справочники.ТипыСвязей.ПереадресацияДокументаПоВопросу,,,,, СтрВопрос.Вопрос);
				Иначе 
					СвязиДокументов.СоздатьСвязь(ТекущийОбъект.Ссылка, ВОтветНа,
						Справочники.ТипыСвязей.ОтправленВОтветНа,,,,, СтрВопрос.Вопрос);
					СвязиУстановлены = Истина;
				КонецЕсли;
					
				РеквизитыДокументаИзменены2 = УстановитьРеквизитыПриИзмененииСвязи(ВОтветНа, СтрВопрос.РезультатРассмотрения,
					СтрВопрос.Вопрос, Объект.Получатели);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	РеквизитыСвязанногоДокументаИзменены = РеквизитыДокументаИзменены1 Или РеквизитыДокументаИзменены2;
	
	Если Не РегламентированныйУчетОбращений Или СвязиУстановлены = Ложь Тогда 
        ...
        Пока Индекс >= 0 Цикл
			...
			Попытка
				...
				ИОМВладелеца = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(
					Тип("СправочникСсылка.ИсходящиеДокументы"));
...
Процедура ПолучательПриИзменении(Элемент)
	...
	СкопироватьПолучателяИзРеквизитовВСписок();
...
Процедура ВОтветНаСтрокаПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(ВОтветНаСтрока) Тогда 
		...
		Модифицированность = Истина;
		Элементы.ГруппаВопросыОбращения.Видимость = Ложь;
		Элементы.ГруппаВОтветНа.ЦветФона = Новый Цвет();
	КонецЕсли;
...
Процедура ВОтветНаСтрокаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ЗаполнитьДанныеВответНа(Истина);
...
Процедура РезультатРассмотренияПриИзменении(Элемент)
	...
	ВопросыОбращения[0].РезультатРассмотрения = РезультатРассмотрения; 
...
Процедура ПолучателиПриИзменении(Элемент)
	...
	Если ИспользоватьСервисСПАРКРиски Тогда 
		ЭтотОбъект.ИндексыСПАРКРиски = Неопределено; // Сбросить полученные значения.
		ОбновитьОтображениеИндексыСПАРК(ТекущиеДанные.Получатель);
	КонецЕсли;
	// Конец ИнтернетПоддержкаПользователей.СПАРКРиски, ОбновитьОтображениеИндексов.
...
Процедура ПолучателиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	// ИнтернетПоддержкаПользователей.СПАРКРиски
	Если Поле = Элементы.ПолучателиСводныйИндикатор Тогда
		// Откроем подсказку
...
&НаКлиенте
Процедура СкопироватьПолучателяИзРеквизитовВСписок()
...
Процедура ПослеЗаписиКлиент(ПараметрыЗаписи)
	...
	Если РеквизитыСвязанногоДокументаИзменены И ЗначениеЗаполнено(ВОтветНа) Тогда 
		Оповестить("ИзмененыРеквизитыПриИзмененииСвязи", ВОтветНа, ЭтаФорма);
	КонецЕсли;
...
Процедура ПриИзмененииВидаДокумента()
	...
	РеквизитыВидДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ВидДокумента, 
		"НеобходимаПечатьШтрихкода, ИспользоватьЭтапыОбработкиДокумента, ОбязательноУказаниеПроекта");
	...
	Если Не Результат.Пустой() Тогда 
		...
		Пока Выборка.Следующий() Цикл
			Если ЗначениеЗаполнено(Выборка.ИсходящаяДата) И ЗначениеЗаполнено(Выборка.ИсходящийНомер) Тогда 
				ДанныеВыбора.Добавить(Выборка.Ссылка, СтрШаблон(НСтр("ru = '%1 (Исх. № %2 от %3)'"), 
...
&НаСервере
Процедура ЗаполнитьДанныеВответНа(ЭтоОбработкаВыбора = Ложь)
	...
	Если ЗначениеЗаполнено(ВОтветНа) Тогда 
		Если ДелопроизводствоКлиентСервер.ЭтоВходящийДокумент(ВОтветНа) Тогда 
			УстановитьПривилегированныйРежим(Истина);
			РеквизитыВОтветНа = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВОтветНа,
				"Заголовок, ИсходящаяДата, ИсходящийНомер, ВопросыОбращения, ВидДокумента.ЯвляетсяОбращениемОтГраждан,
				|Отправитель, Подписал, ВопросДеятельности, Проект");
		...
		Если РегламентированныйУчетОбращений И ЭтоВходящийДокумент
			И РеквизитыВОтветНа.ВидДокументаЯвляетсяОбращениемОтГраждан = Истина Тогда 
			ВопросыОбращения.Очистить();
			ВопросОбращенияТекст = Неопределено; ВопросОбращения = Неопределено;
			РезультатРассмотрения = Неопределено; ОтветныйНаОбращениеДокумент = Неопределено;
			Если Не РеквизитыВОтветНа.ВопросыОбращения.Пустой() Тогда 
				Номер = 1;
				Элементы.ГруппаВопросыОбращения.Видимость = Истина;
				ВыборкаВопросы = РеквизитыВОтветНа.ВопросыОбращения.Выбрать();
				Пока ВыборкаВопросы.Следующий() Цикл
					НоваяСтрока = ВопросыОбращения.Добавить();
					НоваяСтрока.Доступно = Истина;
					НоваяСтрока.Вопрос = ВыборкаВопросы.Вопрос;
					НоваяСтрока.РезультатРассмотрения = ВыборкаВопросы.РезультатРассмотрения;
					НоваяСтрока.ВопросТекст = СокрЛП(ВыборкаВопросы.Вопрос) + " (" + РаботаСОбращениямиВызовСервера.СформироватьКодВопроса(
						ВыборкаВопросы.Раздел, ВыборкаВопросы.Тематика,
						ВыборкаВопросы.Тема, ВыборкаВопросы.Вопрос) + ")";
					НоваяСтрока.ЭтоПререадресация = Ложь;
					
					Если ВыборкаВопросы.РезультатРассмотрения = Перечисления.РезультатыРассмотренияОбращений.НаправленоВИнойОрган Тогда 
						ОтветныйДокументПоВопросу = СвязиДокументов.ПолучитьСвязанныйДокумент(ВОтветНа,
							Справочники.ТипыСвязей.ПереадресованДокументомПоВопросу, ВыборкаВопросы.Вопрос);
						НоваяСтрока.ОтветныйНаОбращениеДокумент = ОтветныйДокументПоВопросу;
						НоваяСтрока.ЭтоПререадресация = Истина;
					Иначе 
						ОтветныйДокументПоВопросу = СвязиДокументов.ПолучитьСвязанныйДокумент(ВОтветНа,
							Справочники.ТипыСвязей.ОтправленОтвет, ВыборкаВопросы.Вопрос);
						НоваяСтрока.ОтветныйНаОбращениеДокумент = ОтветныйДокументПоВопросу;
					КонецЕсли;
					
					Если ЗначениеЗаполнено(НоваяСтрока.ОтветныйНаОбращениеДокумент)
						И НоваяСтрока.ОтветныйНаОбращениеДокумент <> Объект.Ссылка Тогда 
				...
				ВопросОбращения = ВопросыОбращения[0].Вопрос;
				РезультатРассмотрения = ВопросыОбращения[0].РезультатРассмотрения;
				ВопросОбращенияТекст = ВопросыОбращения[0].ВопросТекст;
				ОтветныйНаОбращениеДокумент = ВопросыОбращения[0].ОтветныйНаОбращениеДокумент;
				
				Если ВопросыОбращения.Количество() = 1 Тогда 
					Элементы.ОдинВопрос.Видимость = Истина;
					Элементы.ВопросыОбращения.Видимость = Ложь;
					Элементы.РезультатРассмотрения.Доступность = ВопросыОбращения[0].Доступно;
					
				Иначе 
					Элементы.ОдинВопрос.Видимость = Ложь;
					Элементы.ВопросыОбращения.Видимость = Истина;
				КонецЕсли;
				
				Элементы.ГруппаВОтветНа.ЦветФона = ЦветаСтиля["ЦветКалендаряФонТекущегоДня"];
		...
		Если ЭтоВходящийДокумент И ЭтоОбработкаВыбора Тогда 
			...
			Если Не ЗначениеЗаполнено(Объект.ВопросДеятельности) 
				И ЗначениеЗаполнено(РеквизитыВОтветНа.ВопросДеятельности) Тогда 
			...
			Если НесколькоПолучателей Тогда 
				МассивСтрок = Объект.Получатели.НайтиСтроки(Новый Структура("Получатель",
					РеквизитыВОтветНа.Отправитель));
				
				Если МассивСтрок.Количество() = 0 Тогда 
					Строка = Объект.Получатели.Добавить();
					
					Если ЗначениеЗаполнено(РеквизитыВОтветНа.Отправитель) И ЗначениеЗаполнено(РеквизитыВОтветНа.Подписал) Тогда
						Строка.Получатель = РеквизитыВОтветНа.Отправитель;
						Строка.Адресат = РеквизитыВОтветНа.Подписал;
					Иначе
						Строка.Получатель = РеквизитыВОтветНа.Отправитель;
			...
			Иначе 
				Если Не ЗначениеЗаполнено(Получатель) Тогда
					Если ЗначениеЗаполнено(РеквизитыВОтветНа.Отправитель) И ЗначениеЗаполнено(РеквизитыВОтветНа.Подписал) Тогда
						Получатель = РеквизитыВОтветНа.Отправитель;
						Адресат = РеквизитыВОтветНа.Подписал;
					Иначе
						Получатель = РеквизитыВОтветНа.Отправитель;
						Адресат = Неопределено;
					КонецЕсли;
...
Процедура УстановитьВидимостьПолучателей()
	...
	Если НесколькоПолучателей Тогда 
		...
		Элементы.ГруппаКому.Видимость = Ложь;
	...
	Иначе
		...
		Элементы.ГруппаКому.Видимость = Истина;
		Элементы.ДатаОтправки.Доступность = Отправлен;
		Элементы.ПолученОтвет.Видимость = ЗначениеЗаполнено(ПолученОтвет);
		Если ИспользоватьСВД Тогда
			УстановитьСтатусСВДПолучателя(Получатель);
...
Процедура УстановитьДоступностьПоЗапрету()
	...
	ИменаРеквизитов = Справочники.ИсходящиеДокументы.ИменаРеквизитовИзШаблона();
...
&НаСервереБезКонтекста
Функция УстановитьРеквизитыПриИзмененииСвязи(ДокументСсылка, Реузльтат, Вопрос, Получатели) Экспорт
	
	РеквизитыИзменены = Ложь;
	УстановитьПривилегированныйРежим(Истина);
	Если ТипЗнч(ДокументСсылка) = Тип("СправочникСсылка.ВходящиеДокументы") Тогда
		РеквизитыОбращения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка,
			"ВопросыОбращения, Отправитель");
		ТаблицаВопросыОбращения = РеквизитыОбращения.ВопросыОбращения;
		Выборка = ТаблицаВопросыОбращения.Выбрать();
		Пока Выборка.Следующий() Цикл 
			Если Выборка.Вопрос = Вопрос Тогда
				Если Выборка.РезультатРассмотрения <> Реузльтат Тогда 
					ЗаблокироватьДанныеДляРедактирования(ДокументСсылка);
					ДокументОбъект = ДокументСсылка.ПолучитьОбъект();
					Для Каждого СтрОбъекта Из ДокументОбъект.ВопросыОбращения Цикл 
						Если СтрОбъекта.Вопрос = Вопрос Тогда 
							СтрОбъекта.РезультатРассмотрения = Реузльтат;
							
							Если ЗначениеЗаполнено(Реузльтат) Тогда 
								Если Не ЗначениеЗаполнено(СтрОбъекта.ДатаОтвета) Тогда 
									СтрОбъекта.ДатаОтвета = ТекущаяДатаСеанса();
								КонецЕсли;
								
								// Заполним "ОрганДляПередачи" в обращении.
								Если Реузльтат = Перечисления.РезультатыРассмотренияОбращений.НаправленоВИнойОрган Тогда 
									Для Каждого СтрПолучатель Из Получатели Цикл 
										Если СтрПолучатель.Получатель <> РеквизитыОбращения.Отправитель Тогда 
											СтрОбъекта.ОрганДляПередачи = СтрПолучатель.Получатель;
											Прервать;
										КонецЕсли;
									КонецЦикла;
								КонецЕсли;
							...
							Иначе 
								СтрОбъекта.ДатаОтвета = Неопределено;
							КонецЕсли;
