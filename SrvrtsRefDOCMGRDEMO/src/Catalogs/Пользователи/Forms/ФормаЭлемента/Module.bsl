...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если УчетСамочувствияСотрудников Тогда
		...
		Иначе 
			СледитьЗаСамочувствием = РегистрыСведений.СотрудникиДляКонтроляСамочувствия.СотрудникНаКонтроле(
				Объект.Ссылка);
		КонецЕсли;
		
		Элементы.СледитьЗаСамочувствием.Доступность = Не Объект.Недействителен;
	Иначе 
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	// Копирование прав пока не используется.
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНесуществующаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтрШаблон(НСтр("ru = 'Роль ""%1"" не найдена в метаданных.'"), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтрШаблон(НСтр("ru = 'Роль ""%1"" в строке %%1 не найдена в метаданных.'"), Строка.Синоним));
			КонецЕсли;
			Если Строка.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтрШаблон(НСтр("ru = 'Роль ""%1"" недоступна для пользователей.'"), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтрШаблон(НСтр("ru = 'Роль ""%1"" в строке %%1 недоступна для пользователей.'"), Строка.Синоним));
			КонецЕсли;
		КонецЦикла;
...
Процедура ПереопределитьРедактированиеКонтактнойИнформацииВСервисе()
	...
	ЭлементEMail.АвтоОтметкаНезаполненного = ТребуетсяСинхронизацияССервисом;
...
Функция НачальноеОписаниеПользователяИБ()
	...
	// По умолчанию не показывать в списке выбора в целях усиления безопасности.
	//Если ОбщегоНазначения.РазделениеВключено() Тогда
