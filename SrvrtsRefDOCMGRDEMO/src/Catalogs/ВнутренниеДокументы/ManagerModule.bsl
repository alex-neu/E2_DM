...
Функция СоздатьВнутреннийДокумент(СтруктуруВнутреннегоДокумента) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Не удалось создать внутренний документ'",
...
Функция ИменаРеквизитовИзШаблона() Экспорт
	...
	Возврат "Адресат, ВидДокумента, Организация, ГрифДоступа, ВопросДеятельности, Папка, Подразделение,
...
Функция ПечатьКарточки(МассивОбъектов, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Если ПараметрыПечати.Свойство("РеквизитыКарточки") И ПараметрыПечати.РеквизитыКарточки Тогда
		...
		|	ВнутреннийДокумент.ВидДокумента.ВестиУчетСторон КАК ВестиУчетСторон,
		|	ВнутреннийДокумент.ВидДокумента.ИспользоватьПодписание КАК ИспользоватьПодписание,
		|	ВнутреннийДокумент.ВидДокумента.ИспользоватьУтверждение КАК ИспользоватьУтверждение,
		|	ВнутреннийДокумент.Подписал.Представление КАК Подписал,
		|	ВнутреннийДокумент.РезультатПодписания КАК РезультатПодписания,
		|	ВнутреннийДокумент.ДатаПодписания КАК ДатаПодписания
		|ИЗ
		|	Справочник.ВнутренниеДокументы КАК ВнутреннийДокумент
	...
	Запрос.Текст = Запрос.Текст +
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ГрифыУтверждения.Ссылка КАК Ссылка,
	|	ГрифыУтверждения.АвторУтверждения КАК АвторУтверждения,
	|	ГрифыУтверждения.ДатаУтверждения КАК ДатаУтверждения,
	|	ГрифыУтверждения.Результат КАК Результат
	|ИЗ
	|	Справочник.ВнутренниеДокументы.ГрифыУтверждения КАК ГрифыУтверждения
	|ГДЕ
	|	ГрифыУтверждения.Ссылка В(&МассивОбъектов)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ГрифыУтверждения.НомерСтроки;";
	
	Запрос.УстановитьПараметр("МассивОбъектов", МассивОбъектов);
	ИндексВыборкаГрифов = Индекс;
	...
	Пока ВыборкаЗаголовок.Следующий() Цикл
		...
		Если ПараметрыПечати.Свойство("РеквизитыКарточки") И ПараметрыПечати.РеквизитыКарточки Тогда 
			...
			Если ВыборкаРеквизиты.НайтиСледующий(ВыборкаЗаголовок.Ссылка, "Ссылка") Тогда
				...
				Если ВыборкаРеквизиты.ИспользоватьПодписание И Не ВыборкаРеквизиты.ВестиУчетСторон Тогда 
					ОбластьПодписаниеШапка.Параметры.Подписание = НСтр("ru = 'Подписание'");
					ТабличныйДокумент.Вывести(ОбластьПодписаниеШапка);
					
					ОбластьПодписаниеСтрока.Параметры.ПодписалЗаголовок = НСтр("ru = 'Подписан'");
					СтрокаПодпись = ДелопроизводствоКлиентСервер.ПолучитьСтрокуПодписи(ВыборкаРеквизиты, Ложь);
					ОбластьПодписаниеСтрока.Параметры.Результат = СтрокаПодпись;
					ТабличныйДокумент.Вывести(ОбластьПодписаниеСтрока);
					
				ИначеЕсли ВыборкаРеквизиты.ИспользоватьУтверждение Тогда 
					ОбластьПодписаниеШапка.Параметры.Подписание = НСтр("ru = 'Утверждение'");
					ТабличныйДокумент.Вывести(ОбластьПодписаниеШапка);
					
					ОбластьПодписаниеСтрока.Параметры.ПодписалЗаголовок = НСтр("ru = 'Утвержден'");
					ВыборкаГрифы = Результат[ИндексВыборкаГрифов].Выбрать();
					Если ВыборкаГрифы.НайтиСледующий(ВыборкаЗаголовок.Ссылка, "Ссылка") Тогда
						
						ВыборкаГрифы.Сбросить();
						Пока ВыборкаГрифы.НайтиСледующий(ВыборкаЗаголовок.Ссылка, "Ссылка") Цикл 
							
							СтрокаУтверждение = "";
							Если ВыборкаГрифы.Результат = ПредопределенноеЗначение("Перечисление.РезультатыУтверждения.Утверждено") Тогда
								РезультатУтверждения = НСтр("ru = 'Да'");
							Иначе
								РезультатУтверждения = НСтр("ru = 'Нет'");
							КонецЕсли;
							
							Если ЗначениеЗаполнено(ВыборкаГрифы.ДатаУтверждения) Тогда
								СтрокаУтверждение = СтрШаблон("%1 (%2, %3)",
									РезультатУтверждения, ВыборкаГрифы.АвторУтверждения, Формат(ВыборкаГрифы.ДатаУтверждения, "ДЛФ=D"));
							Иначе
								СтрокаУтверждение = СтрШаблон("%1 (%2)",
									РезультатУтверждения, ВыборкаГрифы.АвторУтверждения);
							КонецЕсли;
							
							ОбластьПодписаниеСтрока.Параметры.Результат = СтрокаУтверждение;
							ТабличныйДокумент.Вывести(ОбластьПодписаниеСтрока);
						КонецЦикла;
					Иначе 
						ОбластьПодписаниеСтрока.Параметры.Результат = НСтр("ru = 'Нет'");
						ТабличныйДокумент.Вывести(ОбластьПодписаниеСтрока);
					КонецЕсли;
