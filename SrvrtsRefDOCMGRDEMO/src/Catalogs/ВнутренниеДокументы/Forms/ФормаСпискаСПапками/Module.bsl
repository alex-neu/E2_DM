...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Список.Параметры.УстановитьЗначениеПараметра("ВестиУчетСканКопийОригиналовДокументов", 
		ВестиУчетСканКопийОригиналовДокументов);
	Список.Параметры.УстановитьЗначениеПараметра("РаздельныйУчетДокументов",
		ПолучитьФункциональнуюОпцию("РаздельныйУчетДокументов"));
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	ОбщегоНазначенияДокументооборотКлиентСервер.ПоказатьСкрытьКнопкуОчисткиОтбора(Элементы.ОтборСторона1, Сторона1);
	ОбщегоНазначенияДокументооборотКлиентСервер.ПоказатьСкрытьКнопкуОчисткиОтбора(Элементы.ОтборКонтрагент, Контрагент);
	ОбщегоНазначенияДокументооборотКлиентСервер.ПоказатьСкрытьКнопкуОчисткиОтбора(Элементы.ОтборСторона2, Сторона2);
...
Процедура СписокНоменклатураДелПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	ОбработатьПеретаскиваниеДокументов(
		ПараметрыПеретаскивания, "НоменклатураДел", ЭлементДерева.Ссылка, НСтр("ru = 'Номенклатура дел'"));
...
Процедура ОтборСторона1Очистка(Элемент, СтандартнаяОбработка)
	...
	ИначеЕсли Сторона1ДопустимыПользователи Тогда
		Сторона1 = ПредопределенноеЗначение("Справочник.Пользователи.ПустаяСсылка");
	Иначе
...
Процедура ОтборСторона1НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	КонецЕсли;
	СтандартнаяОбработка = Ложь;
	РаботаСоСпискамиДокументовКлиент.НачалоВыбораСтороны(Элемент,
		Сторона1,
		Сторона1ДопустимыКонтрагенты,
		Сторона1ДопустимыПользователи);
КонецПроцедуры

&НаКлиенте
Процедура ОтборСторона1АвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	Если ЗначениеЗаполнено(Текст) Тогда
		СтандартнаяОбработка = Ложь;
		ДанныеВыбора = РаботаСоСпискамиДокументов.ДанныеВыбораСтороны(Текст,
			Сторона1ДопустимыОрганизации,
			Сторона1ДопустимыКонтрагенты,
			Сторона1ДопустимыПользователи);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ОтборСторона1ОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	Если ЗначениеЗаполнено(Текст) Тогда 
		СтандартнаяОбработка = Ложь;
		ДанныеВыбора = РаботаСоСпискамиДокументов.ДанныеВыбораСтороны(Текст,
			Сторона1ДопустимыОрганизации,
...
Процедура ОтборСторона2Очистка(Элемент, СтандартнаяОбработка)
	...
	ИначеЕсли Сторона2ДопустимыПользователи Тогда
		Сторона2 = ПредопределенноеЗначение("Справочник.Пользователи.ПустаяСсылка");
	Иначе
...
Процедура ОтборСторона2НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	РаботаСоСпискамиДокументовКлиент.НачалоВыбораСтороны(Элемент,
		Сторона2,
		Сторона2ДопустимыКонтрагенты,
		Сторона2ДопустимыПользователи);
КонецПроцедуры

&НаКлиенте
Процедура ОтборСторона2АвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	Если ЗначениеЗаполнено(Текст) Тогда
		СтандартнаяОбработка = Ложь;
		ДанныеВыбора = РаботаСоСпискамиДокументов.ДанныеВыбораСтороны(Текст,
			Сторона2ДопустимыОрганизации,
			Сторона2ДопустимыКонтрагенты,
			Сторона2ДопустимыПользователи);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ОтборСторона2ОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	Если ЗначениеЗаполнено(Текст) Тогда 
		СтандартнаяОбработка = Ложь;
		ДанныеВыбора = РаботаСоСпискамиДокументов.ДанныеВыбораСтороны(Текст,
			Сторона2ДопустимыОрганизации,
...
Процедура РежимПросмотраПоПапкам(Команда)
	...
	Элементы.Папки.ТекущаяСтрока = ?(ТекущиеДанные = Неопределено, Неопределено, ТекущиеДанные.Папка);
...
&НаКлиенте
Процедура ОбработатьПеретаскиваниеДокументов(ПараметрыПеретаскивания, ИмяРеквизита, ЗначениеРеквизита, Синоним)
	...
	Для Каждого ДокументСсылка Из МассивДокументов Цикл
		...
		Если Не Делопроизводство.ПолеДокументаДоступноПоСостоянию(ИмяРеквизита, ДокументСсылка) Тогда 
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'В текущем состоянии запрещено изменять реквизит ""%1"" у документа ""%2"".'"),
				Синоним,
				Строка(ДокументСсылка));
		КонецЕсли;
		...
		Если Не Делопроизводство.ПолеДокументаДоступноДляИзменения(ИмяРеквизита, ДокументСсылка) Тогда 
			...
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Запрещено изменять реквизит ""%1"" у документа ""%2"".'"),
				Синоним,
				Строка(ДокументСсылка));
		КонецЕсли;
