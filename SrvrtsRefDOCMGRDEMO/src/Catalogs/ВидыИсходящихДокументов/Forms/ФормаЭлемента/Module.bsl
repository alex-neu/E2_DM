...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЕстьПравоНаСозданиеШаблона = ПравоДоступа("Добавление", Метаданные.Справочники.ШаблоныИсходящихДокументов);
	...
	Если Элементы.Найти("ФормаПолитикиДоступа") <> Неопределено Тогда
		...
		Если ОтключенныеРазрезы.Найти(ПланыВидовХарактеристик.ВидыДоступа.ВидыИсходящихДокументов) <> Неопределено Тогда
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не МногоШаблонов И Не НесколькоПолучателей Тогда 
		ШаблонДокумента.Получатели.Очистить();
		Если ЗначениеЗаполнено(Получатель) Или ЗначениеЗаполнено(СпособОтправки) Тогда
			ШаблонДокумента.Получатели.Добавить();
			ЗаполнитьЗначенияСвойств(ШаблонДокумента.Получатели[0], ЭтаФорма, "Получатель, Адресат, СпособОтправки");
		КонецЕсли;
	КонецЕсли;
	...
	Если Не МногоШаблонов 
		...
		Если ФайлыДобавленные.Количество() > 0 Тогда
			...
			Пока Индекс >= 0 Цикл
				...
				Если ТипЗнч(ШаблонДокумента.Ссылка) <> ТипЗнч(ДанныеФайла.Владелец) 
					Или ФайлТаблицы.ДобавленИзШаблона Тогда 
...
Процедура НоменклатураДелНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Для Каждого Строка Из ШаблонДокумента.Получатели Цикл
...
Процедура ПолучательНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("ОбщаяФорма.ВыборКонтрагентаКонтактноеЛицо", 
		Новый Структура("Получатель, Режим", Получатель, "КонтрагентыИСпискиРассылки"), Элемент);
...
Процедура ПолучателиПолучательНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("ОбщаяФорма.ВыборКонтрагентаКонтактноеЛицо",
		Новый Структура("Получатель, Режим",
			Получатель, "КонтрагентыИСпискиРассылки"),
		Элемент);
...
Процедура ПолучателиПолучательОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.СпискиРассылкиПоКонтрагентам") Тогда 
		СтандартнаяОбработка = Ложь;
		ЭлементКоллекции = ШаблонДокумента.Получатели.НайтиПоИдентификатору(Элементы.Получатели.ТекущаяСтрока);
		ЗаполнитьИсполнителейПоСпискуРассылки(ВыбранноеЗначение, ШаблонДокумента.Получатели.Индекс(ЭлементКоллекции));
	ИначеЕсли ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.КонтактныеЛица") Тогда 	
		
		ЭлементКоллекции = ШаблонДокумента.Получатели.НайтиПоИдентификатору(Элементы.Получатели.ТекущаяСтрока);
		ЗаполнитьИсполнителя(ВыбранноеЗначение, ШаблонДокумента.Получатели.Индекс(ЭлементКоллекции));
	ИначеЕсли ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		
		ЭлементКоллекции = ШаблонДокумента.Получатели.НайтиПоИдентификатору(Элементы.Получатели.ТекущаяСтрока);
		
		Если ЗначениеЗаполнено(ВыбранноеЗначение.Контрагент) И ЗначениеЗаполнено(ВыбранноеЗначение.КонтактноеЛицо) Тогда
			ЗаполнитьИсполнителя(ВыбранноеЗначение.КонтактноеЛицо, ШаблонДокумента.Получатели.Индекс(ЭлементКоллекции));
		Иначе
			ЗаполнитьИсполнителя(ВыбранноеЗначение.Контрагент, ШаблонДокумента.Получатели.Индекс(ЭлементКоллекции));
		КонецЕсли;			
...
Процедура ШаблоныВидаПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Копирование Тогда 
		...
		ОткрытьФорму("Справочник.ШаблоныИсходящихДокументов.ФормаОбъекта", ПараметрыФормы, Элементы.ШаблоныВида);
	Иначе 
		ОчиститьСообщения();
		ОткрытьФорму("Справочник.ШаблоныИсходящихДокументов.ФормаОбъекта", ПараметрыФормы,
...
Процедура ОдинПолучатель(Команда)
	...
	Если ШаблонДокумента.Получатели.Количество() > 1 Тогда 
...
Процедура ОдинПолучательПродолжение(Результат, Параметры)Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		Пока ШаблонДокумента.Получатели.Количество() > 1 Цикл
			ШаблонДокумента.Получатели.Удалить(1);
		КонецЦикла;
	ИначеЕсли Результат = КодВозвратаДиалога.Нет Тогда
		НесколькоПолучателей = Истина;
		Возврат;
	...
	Если ШаблонДокумента.Получатели.Количество() > 0 Тогда 	
		ЗаполнитьЗначенияСвойств(ЭтаФорма, ШаблонДокумента.Получатели[0], "Получатель, Адресат, СпособОтправки");
		ШаблонДокумента.Получатели.Очистить();
	КонецЕсли;
...
Процедура УстановитьДоступность(Форма)
	...
	ДоступностьГруппыКонтроляБумажныхДокументов = НЕ (Форма.РаздельныйУчетДокументов 
	...
	ДоступностьГруппыХраненияДокументов = НЕ (Форма.ИспользоватьМестаХранения 
		И Объект.ФормаДокумента = ПредопределенноеЗначение("Перечисление.ВариантыФормДокументов.Электронная"));
	УстановитьДоступностьОпции(Форма, "УчитыватьМестоХранения", ДоступностьГруппыХраненияДокументов);
	УстановитьДоступностьОпции(Форма, "УчитыватьОтветственногоЗаХранение", ДоступностьГруппыХраненияДокументов);
...
Процедура ЗаполнитьИсполнителейПоСпискуРассылки(СписокРассылки, НачальнаяСтрока = Неопределено)
	...
	Для Инд = 0 По ПолучателиКоличество - 1 Цикл
		Если НачальнаяСтрока = Неопределено Тогда 
			НоваяСтрока = ШаблонДокумента.Получатели.Добавить();
		Иначе
			НоваяСтрока = ШаблонДокумента.Получатели.Вставить(НачальнаяСтрока + Инд);
		КонецЕсли;	
	...
	КонецЦикла;	
	Если НачальнаяСтрока <> Неопределено Тогда 
		ШаблонДокумента.Получатели.Удалить(НачальнаяСтрока + ПолучателиКоличество);
	КонецЕсли;	
	
	Если Не НесколькоПолучателей Тогда 
		
		Если ШаблонДокумента.Получатели.Количество() = 1 Тогда 
			ЗаполнитьЗначенияСвойств(ЭтаФорма, ШаблонДокумента.Получатели[0], "Получатель, ПолучательЮрЛицо, Адресат, ПолученОтвет, Отправлен, ДатаОтправки, СпособОтправки, ВходящийНомер, ВходящаяДата");
		Иначе	
			НесколькоПолучателей = Истина;
			УстановитьВидимостьПолучателей();
		КонецЕсли;
...
Процедура ЗаполнитьИсполнителя(ВыбранноеЗначение, НачальнаяСтрока = Неопределено)
	...
	Если НачальнаяСтрока = Неопределено Тогда 
		НоваяСтрока = ШаблонДокумента.Получатели.Добавить();
	Иначе
		НоваяСтрока = ШаблонДокумента.Получатели.Вставить(НачальнаяСтрока);
	КонецЕсли;	
	...
	Если НачальнаяСтрока <> Неопределено Тогда 
		ШаблонДокумента.Получатели.Удалить(НачальнаяСтрока + 1);
	КонецЕсли;	
...
Функция ШаблонИзменен()
	...
	Если ШаблонДокумента.ВопросДеятельности <> ШаблонДокументаПриОткрытии.ВопросДеятельности
		...
		Или ОбщегоНазначенияДокументооборотКлиентСервер.ЕстьОтличияВТаблицах(
			...
			"Участник, Изменение")
		Или ОбщегоНазначенияДокументооборотКлиентСервер.ЕстьОтличияВТаблицах(
			ШаблонДокументаПриОткрытии.Получатели, 
			ШаблонДокумента.Получатели,
			"Получатель, Адресат, СпособОтправки")
		Или ОбщегоНазначенияДокументооборотКлиентСервер.ЕстьОтличияВТаблицах(
...
Функция ЕстьЗаполненныеРеквизитыШаблона()
	...
	Если ЗначениеЗаполнено(ШаблонДокумента.ВопросДеятельности)
		...
		Или ШаблонДокумента.Получатели.Количество() > 0
		...
		Или ШаблонДокумента.Категории.Количество() > 0 Тогда 
		Возврат Истина;
...
Процедура ЗаполнитьСписокШаблоновВида(ВидДокумента = Неопределено)
	...
	Запрос.Текст = 
		 ...
		 |ГДЕ
		 |	ШаблоныИсходящихДокументов.ВидДокумента = &ВидДокумента
		 |	И НЕ ШаблоныИсходящихДокументов.ПометкаУдаления";
	...
	Если Не МногоШаблонов Тогда 
		...
		СписокДопРеквизитов = РаботаСШаблонамиДокументовСервер.ПолучитьНаборДопРеквизитовДокумента(
			"ИсходящиеДокументы", ВидДокумента, ШаблонДокумента);
		...
		НесколькоПолучателей = (ШаблонДокумента.Получатели.Количество() > 1);
		Если Не НесколькоПолучателей И ШаблонДокумента.Получатели.Количество() > 0 Тогда 
			ЗаполнитьЗначенияСвойств(ЭтаФорма, ШаблонДокумента.Получатели[0], "Получатель, Адресат, СпособОтправки");
		КонецЕсли;
