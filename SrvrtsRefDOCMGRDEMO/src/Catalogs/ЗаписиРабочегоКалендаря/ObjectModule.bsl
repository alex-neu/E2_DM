	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("Предмет") И ЗначениеЗаполнено(ДанныеЗаполнения.Предмет) Тогда
			...
			ИначеЕсли ТипЗнч(ДанныеЗаполнения.Предмет) = Тип("ДокументСсылка.Бронь") Тогда 
				...
				РеквизитыБрони = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения.Предмет,
					"ДатаНачала, ДатаОкончания, ВесьДень,
					|ДатаНачалаПовторения, ДатаОкончанияПовторения, ИнтервалПовторения, КоличествоПовторов,
					|ПовторениеПоДнямМесяца, ПовторениеПоМесяцам, ПравилоОкончанияПовторения, ЧастотаПовторения,
					|ТипЗаписи, ИсключенияПовторения, ПовторениеПоДням");
				...
				КонецЕсли;
				ИсключенияПовторения.Загрузить(РеквизитыБрони.ИсключенияПовторения.Выгрузить());
				ПовторениеПоДням.Загрузить(РеквизитыБрони.ПовторениеПоДням.Выгрузить());
	...
	Если Связанная И Не ДополнительныеСвойства.Свойство("ИзменениеСвязаннойЗаписи") Тогда
		Если ЗначениеЗаполнено(Предмет) Тогда
			ТекстРекомендованногоДействия = СтрШаблон(
				...
				Предмет,
				ТипЗнч(Предмет))
		ИначеЕсли ЗначениеЗаполнено(СинхронизацияКалендарей.УзлыДляВыгрузки(ЭтотОбъект.Пользователь)) Тогда
	...
	Если НЕ ПометкаУдаления 
		...
		Если УстановитьНапоминание = Неопределено Тогда
			...
			Иначе
				...
				ИнтервалВремениСтрокой = НапоминанияПользователяДокументооборотКлиентСервер.ПредставлениеВремени(
		...
		Если УстановитьНапоминание Тогда
			...
			ИнтервалВремениНапоминания = 
				НапоминанияПользователяДокументооборот.ИнтервалВремени(ИнтервалВремениСтрокой);
