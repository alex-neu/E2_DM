...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтображатьПолеПользователь = РаботаСРабочимКалендаремСервер.ПолучитьПерсональнуюНастройку(
	...
	Элементы.Состояние.Доступность = РаботаСРабочимКалендаремСервер.ДоступноИзменениеКалендаря(
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не ОтсутствияКлиент.ПроверитьОтсутствиеПоЗаписиРабочегоКалендаря(ЭтаФорма, Отказ, ИнформацияОбОтсутствии) Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ПередЗаписьюПослеВопросаОВозможностиОтсутствия",
			ЭтотОбъект, ПараметрыЗаписи);
		ОтсутствияКлиент.ПоказатьПредупреждениеОбОтсутствииПоЗаписиРабочегоКалендаря(
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	РаботаСРабочимКалендаремСервер.УстановитьЦветЗаписиКалендаря(ТекущийОбъект.Ссылка, Цвет);
	РаботаСРабочимКалендаремСервер.УстановитьОтработанаЗаписьКалендаря(ТекущийОбъект.Ссылка, Отработана);
	ОбработатьИзменениеНапоминанияНаСервере();
...
Процедура ПользовательПриИзменении(Элемент)
	...
	Если РаботаСРабочимКалендаремСервер.ДоступноИзменениеКалендаря(ТекущийПользователь, Объект.Пользователь) Тогда
...
Процедура УстановитьВидимостьЭлементовФормы()
	...
	ТолькоПросмотр = Объект.Связанная
		И (ЗначениеЗаполнено(Объект.Предмет)
			Или ЗначениеЗаполнено(СинхронизацияКалендарей.УзлыДляВыгрузки(Объект.Пользователь)));
...
Функция ПолучитьНастройкиПовторения()
	...
	Если Объект.ЧастотаПовторения = Перечисления.ЧастотаПовторения.Ежемесячно
		И Не ЗначениеЗаполнено(Объект.ПовторениеПоДнямМесяца)
		И Объект.ПовторениеПоДням.Количество() = 1 Тогда
	...
	ИначеЕсли Объект.ЧастотаПовторения = Перечисления.ЧастотаПовторения.Ежегодно
		И Не ЗначениеЗаполнено(Объект.ПовторениеПоДнямМесяца)
		И Объект.ПовторениеПоДням.Количество() = 1 Тогда
...
Процедура ПрочитатьЦвет()
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка)
		...
		Цвет = РаботаСРабочимКалендаремСервер.ПолучитьЦветСобытияКалендаря(Параметры.ЗначениеКопирования);
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка)
		...
		Цвет = РаботаСРабочимКалендаремСервер.ПолучитьЦветСобытияКалендаря(Параметры.Основание.ПовторяющеесяСобытие);
...
Процедура ПрочитатьОтработана()
	...
	Отработана = РаботаСРабочимКалендаремСервер.ЗаписьКалендаряОтработана(Объект.Ссылка);
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	Элементы.ФормаПоказатьКалендарь.Видимость = Ложь;
	Элементы.ФормаСправочникЗаписиРабочегоКалендаряПечатьЗаписиКалендаря.Видимость = Ложь;
	
	Элементы.Переместить(Элементы.Описание, Элементы.ГруппаЗаписьКалендаря, Элементы.ГруппаДатаВремя);
	Элементы.Описание.Ширина = 0;
	Элементы.Описание.РастягиватьПоВертикали = Истина;
	
	Элементы.ГруппаДатаВремя.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	Элементы.Переместить(Элементы.ДатаОкончания, Элементы.ГруппаДатаОкончания, Элементы.ДатаОкончанияВремя);
	...
	Элементы.ДатаОкончания.Ширина = 0;
	...
	Элементы.Повторять.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	Элементы.ГруппаНастройкиВремени.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	
	Элементы.Отработана.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	
	Элементы.КоманднаяПанельЦвета.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Центр;
	Элементы.ГруппаЦвета.Отображение = ОтображениеГруппыКнопок.Обычное;
	
	Элементы.ДекорацияНапомнить.Видимость = Истина;
	Элементы.Переместить(Элементы.ИнтервалВремениСтрокой, Элементы.ГруппаНапоминаниеОтносительноДатыНачала,
		Элементы.УстановитьНапоминание);
	Элементы.ИнтервалВремениСтрокой.Ширина = 0;
	Элементы.ИнтервалВремениСтрокой.РастягиватьПоГоризонтали = Истина;
	Элементы.УстановитьНапоминание.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	
	Элементы.Переместить(Элементы.Предмет, Элементы.ГруппаЗаписьКалендаря, Элементы.ГруппаДанныеЗаписиКалендаря);
	Элементы.Переместить(Элементы.Состояние, Элементы.ГруппаЗаписьКалендаря, Элементы.ГруппаДанныеЗаписиКалендаря);
	Элементы.Состояние.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	Элементы.Состояние.Ширина = 0;
	Элементы.Состояние.РастягиватьПоГоризонтали = Истина;
	Элементы.Состояние.АвтоМаксимальнаяШирина = Ложь;
	Элементы.Состояние.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
