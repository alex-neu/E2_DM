...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ОбъектДляОтбораПоПравам) Тогда
		ТаблицаПрав = ДокументооборотПраваДоступа.ПолучитьПраваПользователейПоОбъекту(
...
Процедура СписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Для Каждого КлючИЗначение Из Строки Цикл
		...
		Если ДанныеСтроки.Свойство("Объект")
			...
			И ТипЗнч(ДанныеСтроки.Объект) = Тип("СправочникСсылка.Пользователи") Тогда
...
Процедура РезультатПоискаПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Для Каждого КлючИЗначение Из Строки Цикл
		...
		Если ДанныеСтроки.Свойство("ОбъектПоиска")
			И ЗначениеЗаполнено(ДанныеСтроки.ОбъектПоиска)
			И ТипЗнч(ДанныеСтроки.ОбъектПоиска) = Тип("СправочникСсылка.Пользователи") Тогда
...
&НаКлиенте
Процедура ВыбранныеАдресатыПослеУдаления(Элемент)
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	// Установим заголовок у ГруппаПолучатели и расстянем ее.
	...
	// Настроим команды.
	Элементы.ВыбратьАдресатовКоманда.Заголовок = "ОК";
		...
		Если Параметры.ВыбиратьЭлектронныеАдреса Тогда
			...
			ИначеЕсли Контакт.Метаданные().ТабличныеЧасти.Найти("КонтактнаяИнформация") <> Неопределено Тогда
				...
				Для Каждого АдресКонтакта ИЗ АдресаКонтакта Цикл
					...
					ВстроеннаяПочтаСервер.ПолучитьПочтовоеПредставлениеКонтакта(Контакт, АдресКонтакта.Представление);
					СтруктураАдресаКонтакта.ПолноеПредставление = 
					ПолучитьПолноеПредставлениеАдресата(АдресКонтакта.Представление, Контакт);
