...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечататьКарточкуЭД Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Справочник.ЭДПрисоединенныеФайлы.ПФ_MXL_КарточкаЭД_%1",
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ТаблицаЭДДляДопОбработки.Колонки.Добавить("ВладелецЭД");
	ТаблицаЭДДляДопОбработки.Колонки.Добавить("ФайлЭлектронногоДокумента");
	ТаблицаЭДДляДопОбработки.Колонки.Добавить("ЭлектронныйДокумент");
		...
		Попытка
			...
			Если ОбработкаВладельцаВыполнена И НастройкаОбработана Тогда
				Если ВыборкаВладельцевЭД.ОбработатьУтверждение Тогда
					Для каждого СтрокаТаблицы Из ТаблицаЭДДляДопОбработки Цикл
						СтрокаТаблицы.ФайлЭлектронногоДокумента = СтрокаТаблицы.СсылкаНаЭД;
						СтрокаТаблицы.ЭлектронныйДокумент = СтрокаТаблицы.ВладелецЭД;
					КонецЦикла;
...
Функция ПечатьКарточкиЭД(МассивОбъектов, ОбъектыПечати, ИмяМакета = "КарточкаЭД")
	...
	|	ЭДПрисоединенныеФайлыЭлектронныеЦифровыеПодписи.Объект КАК Ссылка,
	...
	|ГДЕ
	|	ЭДПрисоединенныеФайлыЭлектронныеЦифровыеПодписи.Объект В
	|			(ВЫБРАТЬ
	|				втЭД.Ссылка
	|			ИЗ
	|				втЭД)
	...
	|	ЭДПрисоединенныеФайлыЭлектронныеЦифровыеПодписи.Объект.ЭлектронныйДокументВладелец,
	|	ЭДПрисоединенныеФайлыЭлектронныеЦифровыеПодписи.Сертификат
	|ИЗ
	|	РегистрСведений.ЭлектронныеПодписи КАК ЭДПрисоединенныеФайлыЭлектронныеЦифровыеПодписи
	|ГДЕ
	|	ЭДПрисоединенныеФайлыЭлектронныеЦифровыеПодписи.Объект.ЭлектронныйДокументВладелец В
	|			(ВЫБРАТЬ
	|				втЭД.Ссылка
	|			ИЗ
	|				втЭД)
	|	И ЭДПрисоединенныеФайлыЭлектронныеЦифровыеПодписи.Объект.ТипЭлементаВерсииЭД В(&ТитулыПокупателя)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Справочник.ЭДПрисоединенныеФайлы.ПФ_MXL_КарточкаЭД_%1", ОбщегоНазначения.КодОсновногоЯзыка()));
