...
Процедура УстановитьДоступностьКоманд(РедактируетТекущийПользователь, Редактирует, ПодписанЭП, Зашифрован)
	...
	Элементы.СписокКонтекстноеМенюРаспознать.Доступность = 
		РасширениеПоддерживается И ДоступноРаспознаваниеПоЗапросу И Редактирует.Пустая();
...
Процедура ЗакончитьРедактирование(Команда)
	...
	Для Каждого ЭлементСписка Из Элементы.Список.ВыделенныеСтроки Цикл
		...
		Если Не ЗначениеЗаполнено(ДанныеСтроки.Редактирует)
			Или Не ДанныеСтроки.РедактируетТекущийПользователь Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СкопироватьСтрокиНаСервере(ЧислоФайлов)
	...
	ОбщегоНазначенияДокументооборот.СкопироватьФайлыВБуферОбменаИзСпискаФайлов(Элементы.Список.ВыделенныеСтроки);
