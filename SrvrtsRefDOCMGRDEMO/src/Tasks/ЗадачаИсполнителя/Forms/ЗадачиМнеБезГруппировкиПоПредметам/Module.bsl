...
Процедура ЗадачиМнеСортироватьСрокуИсполнения(Команда)
	...
	СортироватьСписокПоКолонке("СрокИсполнения");
...
Процедура ЗадачиМнеСортироватьДатеСоздания(Команда)
	...
	СортироватьСписокПоКолонке("Дата");
...
Процедура ЗадачиМнеСортироватьПоФлагу(Команда)
	...
	ПереключитьСортировку(ЗадачиМнеСортироватьПо, ЗадачиМнеНаправлениеСортировки, "НомерФлага");
	СортироватьСписокПоКолонке("НомерФлага");
...
Процедура УстановитьЗаголовок(ЗадачиМнеНовые, ЗадачиМнеПросроченные, ЗадачиМнеВсего)
	...
	Если Параметры.КлючОтбора = "НеПринятые" Тогда
		ТекстЗаголовка = НСтр("ru = 'Задачи мне (Новых: %1)'");
	ИначеЕсли Параметры.КлючОтбора = "Просроченные" Тогда
		ТекстЗаголовка = НСтр("ru = 'Задачи мне (Просроченных: %2)'");
	Иначе
		ТекстЗаголовка = НСтр("ru = 'Задачи мне (Новых: %1 / Просроченных: %2 / Всего: %3)'");
	КонецЕсли;
...
Процедура УстановитьТекущийОбъектВДеревеПоСсылке(ДеревоЭлемент, ДеревоРеквизит, ТекущийОбъект, ЗадачиМнеТекущийИндекс)
	...
	// Если задача найдена в дереве, то установим фокус на её позицию.
	Если Индекс <> -1 Тогда
	...
	Если КоличествоЭлементовВКорне = 1 Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура СортироватьСписокПоКолонке(ИмяКолонки)
	
	ЗадачиМнеСписок.Порядок.Элементы.Очистить();
	Для Каждого ПользовательскаяНастройка Из ЗадачиМнеСписок.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
		...
		ЭлементПорядка.ТипУпорядочивания = ?(ЗадачиМнеНаправлениеСортировки = "Возр",
...
Процедура УстановитьПометкуРежимуСортировки(Форма)
	...
	Форма.Элементы.ЗадачиМнеСортироватьСрокуИсполнения.Заголовок = НСтр("ru = 'Срок'");
	
	Форма.Элементы.ЗадачиМнеСортироватьДатеСоздания.Пометка = Ложь;
	Форма.Элементы.ЗадачиМнеСортироватьДатеСоздания.Заголовок = НСтр("ru = 'Дата создания'");
...
&НаКлиентеНаСервереБезКонтекста
Процедура ПереключитьСортировку(ЗадачиМнеСортироватьПо, ЗадачиМнеНаправлениеСортировки, ИмяКолонки)
...
Процедура ПолучитьРеквизитыИСвойстваЗадачиСогласованиеСогласовать(Задача, Знач РеквизитыЗадачи, ДополнительныеРеквизиты, СвойстваЭлементов)
	...
	Если ПодписыватьЭП Тогда
		...
		СвойстваЭлементов.Согласовано.Вставить("Заголовок", "Согласовано (ЭП)");
		...
		СвойстваЭлементов.СогласованоСЗамечаниями.Вставить("Заголовок", "Согласовано с замечаниями (ЭП)");
		...
		СвойстваЭлементов.НеСогласовано.Вставить("Заголовок", "Не согласовано (ЭП)");
	...
	Иначе
		СвойстваЭлементов.Вставить("Согласовано", Новый Соответствие);
		СвойстваЭлементов.Согласовано.Вставить("Заголовок", "Согласовано");
		СвойстваЭлементов.Согласовано.Вставить("Высота", 2);
		СвойстваЭлементов.Согласовано.Вставить("ВысотаЗаголовка", 0);
		
		СвойстваЭлементов.Вставить("СогласованоСЗамечаниями", Новый Соответствие);
		СвойстваЭлементов.СогласованоСЗамечаниями.Вставить("Заголовок", "Согласовано с замечаниями");
		...
		СвойстваЭлементов.Вставить("НеСогласовано", Новый Соответствие);
		СвойстваЭлементов.НеСогласовано.Вставить("Заголовок", "Не согласовано");
...
Процедура СформироватьДеревоЗадачМне(Дерево)
	...
	ТаблицаЗадач.Сортировать("Дата Возр");
	
	Если КлючНазначенияИспользования = "НеПринятыеЗадачиМне" Или КлючНазначенияИспользования = "ПросроченныеЗадачиМне" Тогда
		
		КоличествоЭлементов = ТаблицаЗадач.Количество();
		Для Индекс = 1 По КоличествоЭлементов Цикл
			
			СтрокаТаблицы = ТаблицаЗадач[КоличествоЭлементов - Индекс];
			Если КлючНазначенияИспользования = "НеПринятыеЗадачиМне"
				И СтрокаТаблицы.ПринятаКИсполнению
				И СтрокаТаблицы.Ссылка <> ЗадачиМнеТекущаяЗадача Тогда
				
				ТаблицаЗадач.Удалить(СтрокаТаблицы);
				
			ИначеЕсли КлючНазначенияИспользования = "ПросроченныеЗадачиМне"
				И Не СтрокаТаблицы.Просрочена Тогда
...
Функция ПредставлениеТочкиМаршрута(Задача)
	...
	Если Задача.ТочкаМаршрута = БизнесПроцессы.Утверждение.ТочкиМаршрута.Утвердить Тогда
		Возврат НСтр("ru = 'Утвердить / Подписать'");
	ИначеЕсли Задача.ТочкаМаршрута = БизнесПроцессы.Утверждение.ТочкиМаршрута.Ознакомиться Тогда
		Возврат НСтр("ru = 'Ознакомиться с результатом утверждения / подписи'");
	Иначе
...
Процедура ПродолжениеВыполненияКомандыЗадачаУтвержденоПослеПодписанияПредметовЭП(
	...
	Для Каждого Данные Из Результат.НаборДанных Цикл
		...
		ОбменСКонтрагентамиДОСлужебныйКлиент.УстановитьПризнакиПодписанияЭД(
			Данные.Представление, 
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Результат.ВыбранныйСертификат.Ссылка), 
			ПодписанныеДанные);
