...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	// Важность и автор процесса покажем после описания.
	Элементы.Переместить(Элементы.ГруппаВажностьИАвтор, Элементы.ГруппаСрокиАвторВажность,Элементы.ГруппаКомуИСрокИсполнения);
	// Сроки покажем после "Кому".
	Элементы.Переместить(Элементы.ГруппаСтарыйИНовыйСрок, Элементы.ГруппаСрокиАвторВажность);
	
	// Выровняем инициатора процесса ("Кому").
	Элементы.ИнициаторПроцесса.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
	Элементы.ИнициаторПроцесса.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	Элементы.ИнициаторПроцесса.РастягиватьПоГоризонтали = Истина;
	
	// Выровняем срок ответа.
	Элементы.ДекорацияПодписьСрока.Видимость = Ложь;
	Элементы.СрокИсполненияДата.Заголовок = НСтр("ru = 'Срок ответа (желаемый)'");

	Элементы.СрокИсполненияДата.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
	Элементы.СрокИсполненияДата.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	Элементы.СрокИсполненияДата.РастягиватьПоГоризонтали = Истина;
	Элементы.СрокИсполненияДата.Ширина = 0;
	
	Элементы.СрокИсполненияВремя.РастягиватьПоГоризонтали = Истина;
	Элементы.СрокИсполненияВремя.Ширина = 0;

	// Переместим и выравнием "Длительность переноса".
	Элементы.Переместить(Элементы.ДлительностьПереноса, Элементы.ГруппаСтарыйИНовыйСрок);
	Элементы.ДлительностьПереноса.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
	Элементы.ДлительностьПереноса.РастягиватьПоГоризонтали = Истина;
	
	// Добавим выделения для некоторых групп.
	Элементы.ГруппаСтарыйИНовыйСрок.Отображение = ОтображениеОбычнойГруппы.ОбычноеВыделение;
	Элементы.ГруппаКомуИСрокИсполнения.Отображение = ОтображениеОбычнойГруппы.ОбычноеВыделение;
	
	// Настроим картинки для кнопок выполнения.
	Если Элементы.ФормаСтартИЗакрыть.Видимость Тогда
		Элементы.ФормаСтартИЗакрыть.Картинка = БиблиотекаКартинок.ЗаписатьИЗакрыть;
		Элементы.ФормаСтартИЗакрыть.Отображение = ОтображениеКнопки.Картинка;
		
	ИначеЕсли Элементы.ФормаЗаписатьИЗакрыть.Видимость Тогда
	...
	ПараметрыАдаптациии.ЭлементЗадачиИПроцессы = Элементы.Задачи;
	ПараметрыАдаптациии.ЭлементЗадачиИПроцессыРезультатВыполнения = Элементы.ЗадачиРезультатВыполнения;
