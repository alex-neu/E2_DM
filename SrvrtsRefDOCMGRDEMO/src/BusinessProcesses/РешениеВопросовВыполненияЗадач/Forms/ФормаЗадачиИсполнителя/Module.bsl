...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПраваПоОбъекту.Изменение Тогда
		...
		Элементы.ОтправитьНаУточнение.Доступность = Ложь;
	...
	БизнесПроцессПредметаРассмотрения = ПредметРассмотрения.БизнесПроцесс;
	НаименованиеПроцессаПредметаРассмотренияГиперссылка = БизнесПроцессПредметаРассмотрения.Наименование;
		
	// Установка надписи о результате рассмотрения вопроса автором задачи
	Если Объект.БизнесПроцесс.ВидВопроса = Перечисления.ВидыВопросовВыполненияЗадач.ПереносСрока Тогда
		КоличествоПереносов = ПереносСроковВыполненияЗадачВызовСервера.КоличествоПереносовСрокаПоЗадачеИЗаявкеНаПеренос(
			ПредметРассмотрения,
			Объект.БизнесПроцесс);
		Если КоличествоПереносов = 0 Тогда
	...
	Исполнитель = Объект.БизнесПроцесс.ИнициаторПроцесса;
	СрокИсполнения = Объект.БизнесПроцесс.СрокИсполнения;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ВыполнитьЗадачу") И ПараметрыЗаписи.ВыполнитьЗадачу Тогда
		...
		БПОбъект.ОтправитьНаУточнение = ПараметрыЗаписи.ТребуетсяУточнение;
		РаботаСБизнесПроцессами.ЗаписатьПроцесс(БПОбъект, "ПростаяЗапись");
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ВыполнитьЗадачуФоново")
		...
		ПараметрыФоновогоВыполнения.Вставить("ОтправитьНаУточнение", ПараметрыЗаписи.ТребуетсяУточнение);
	КонецЕсли;
...
Процедура ПредметРассмотренияНажатие(Элемент, СтандартнаяОбработка)
	...
	ДанныеФормы = ПолучитьДанныеДляОткрытияФормы(ПредметРассмотрения);
	ОткрытьФорму(ДанныеФормы.ИмяФормы, ДанныеФормы.ПараметрыФормы, Элемент);
...
Процедура Ознакомился(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПродолжениеВыполненияЗадачиПослеВыбораФактическогоИсполнителя", ЭтаФорма, ТребуетсяУточнение);
...
&НаКлиенте
Процедура ОтправитьНаУточнение(Команда)
...
Процедура ПродолжениеВыполненияЗадачиПослеВыбораФактическогоИсполнителя(
	ВыбранныйФактическийИсполнитель, ТребуетсяУточнение) Экспорт
	...
	Если ТребуетсяУточнение И НЕ ЗначениеЗаполнено(Объект.РезультатВыполнения) Тогда
		Текст = НСтр("ru = 'При отправке на уточнение требуется указать комментарий.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст, , "Объект.РезультатВыполнения");
		Возврат;
	КонецЕсли;
	
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПродолжениеВыполненияЗадачиПослеПроверкиОтсутствия",
		ЭтотОбъект, ТребуетсяУточнение);
	Если ТребуетсяУточнение 
		И Не ОтсутствияКлиент.ПроверитьОтсутствиеПоЗадаче(ЭтаФорма, ОписаниеОповещения) Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ПродолжениеВыполненияЗадачиПослеПроверкиОтсутствия(Результат, ТребуетсяУточнение) Экспорт
	...
	ПараметрыЗаписи.Вставить("ТребуетсяУточнение", ТребуетсяУточнение);
...
Функция ПолучитьДанныеДляОткрытияФормы(ПредметРассмотрения)
	
	ДанныеФормы = БизнесПроцессы[ПредметРассмотрения.БизнесПроцесс.Метаданные().Имя].ФормаВыполненияЗадачи(ПредметРассмотрения, ПредметРассмотрения.ТочкаМаршрута);
	Возврат ДанныеФормы;
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	ЭлементыОсновное.Добавить(Элементы.ГруппаРезультатРассмотренияИПредметРассмотрения);
	...
	ПараметрыАдаптациии.СтраницыЭлементы.Добавить(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Элементы.Файлы));
	...
	ПараметрыАдаптациии.ЭлементПредметы = Элементы.Файлы;
	...
	Элементы.ОтправитьНаУточнение.Картинка = БиблиотекаКартинок.ЗадачаНеУспешноеВыполнение;
