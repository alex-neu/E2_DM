...
Процедура НаименованиеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	РаботаСБизнесПроцессамиКлиент.ВыбратьШаблонТекстаРеализация(ЭтаФорма, "Наименование",
		ПредопределенноеЗначение("Перечисление.ОбластиПримененияШаблоновТекстов.ПроцессПоручениеНаименование"));
...
Процедура ОписаниеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	РаботаСБизнесПроцессамиКлиент.ВыбратьШаблонТекстаРеализация(ЭтаФорма, "Описание",
		ПредопределенноеЗначение("Перечисление.ОбластиПримененияШаблоновТекстов.ПроцессПоручениеОписание"));
...
Процедура ЗаполнитьПоШаблону(Команда)
	...
	ШаблоныПоПредметам.ЗагрузитьЗначения(
		ШаблоныБизнесПроцессов.ШаблоныПоОбъектам(Предметы, Тип("СправочникСсылка.ШаблоныПоручения")));
	
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ЗаполнитьПоШаблонуПродолжение",
		ЭтотОбъект);
	ШаблоныБизнесПроцессовКлиент.ВыбратьШаблонБизнесПроцесса(
		"ШаблоныПоручения", ШаблоныПоПредметам, ОписаниеОповещения);
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	Элементы.Переместить(Элементы.Важность, Элементы.ГруппаУчастникиСрокиКонтрольныйСрокПроцесса, Элементы.ГруппаИсполнительСрокИсполнения);
	...
	ВыравниваниеЭлементаМК(Элементы.Исполнитель);
	ВыравниваниеЭлементаМК(Элементы.Проверяющий);
	ВыравниваниеЭлементаМК(Элементы.Контролер);
	ВыравниваниеЭлементаМК(Элементы.СрокИсполненияПредставление);
	...
	// Поправим заголовок у группы исполнители.
	Элементы.ГруппаИсполнительСрокИсполнения.Заголовок = НСтр("ru = 'Исполнитель'");
	...
	ПараметрыАдаптациии.ЭлементГруппаИсполнители = Элементы.ГруппаИсполнительСрокИсполнения;
...
Процедура ОбновитьСрокиИсполненияНаСервере() Экспорт
	...
	Если СрокиИсполненияПроцессов.ТребуетсяРасчетСроков(ЭтаФорма)
		...
		СрокиИсполненияПроцессов.РассчитатьСрокиПоручения(Объект, ПараметрыДляРасчетаСроков);
...
Процедура ПроверитьИзменениеСроковПроцесса()
	...
	СтруктураСроковДляПроверки.СрокиИсполнителей.Вставить(
		БизнесПроцессы.Поручение.ТочкиМаршрута.Выполнить, Объект.СрокИсполнения);
	
	СтруктураСроковДляПроверки.СрокиИсполнителей.Вставить(
		БизнесПроцессы.Поручение.ТочкиМаршрута.Проверить, Объект.СрокОбработкиРезультатов);
...
Процедура ОбработкаПроверкиЗаполненияНаСервереДляФоновогоСтарта(Отказ) Экспорт
	...
	ПроцессОбъект = РеквизитФормыВЗначение("Объект", Тип("БизнесПроцессОбъект.Поручение"));
