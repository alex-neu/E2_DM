...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не Отказ
		...
		ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, "УтверждениеВыполнениеКомандыСтартовать");
	КонецЕсли;
...
Процедура ПроверитьВозможностьУстановкиПодписей()
	...
	Если ОсновныеПредметы.Количество() = 1 Тогда
		Если Объект.ВидПроцесса = ВидПроцессаПодписание
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Предмет ""%1"" не может быть подписан.'"),
				ОбщегоНазначенияДокументооборотВызовСервера.ПредметСтрокой(ОсновныеПредметы[0]));
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить(КодВозвратаДиалога.ОК, НСтр("ru = 'Очистить предмет'"));
			Кнопки.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru = 'Установить вид процесса ""Утверждение""'"));
...
Процедура СформироватьЗаголовкиИПодсказкиЭлементовФормы()
	...
	Если Объект.Ссылка.Пустая() Тогда 
		Если Объект.ВидПроцесса = ВидПроцессаПодписание Тогда 
			Заголовок = НСтр("ru = 'Подписание (создание)'");
		Иначе 
	...
	Если Объект.ВидПроцесса = ВидПроцессаПодписание Тогда
		Элементы.ГруппаГлавное.Заголовок = НСтр("ru = 'Подписание'");
...
Функция ПредставлениеРезультата() Экспорт
	...
	Если Объект.ВидПроцесса = Перечисления.ВидыБизнесПроцессаУтверждение.Подписание Тогда
		Если Объект.РезультатУтверждения = Перечисления.РезультатыУтверждения.Утверждено Тогда
			Возврат СтрШаблон(ПредставлениеРезультата, НСтр("ru = 'Подписано'"), Формат(Объект.ДатаЗавершения, "ДФ='dd.MM.yyyy HH:mm'"));
		Иначе
			Возврат СтрШаблон(ПредставлениеРезультата, НСтр("ru = 'Не подписано'"), Формат(Объект.ДатаЗавершения, "ДФ='dd.MM.yyyy HH:mm'"));
		КонецЕсли;
	Иначе
