...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПодписыватьЭП = Объект.БизнесПроцесс.ПодписыватьЭП И ЭлектроннаяПодпись.ИспользоватьЭлектронныеПодписи();
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ВыполнитьЗадачуФоново")
		...
		ПараметрыФоновогоВыполнения.Вставить("РезультатУтверждения", ПараметрыЗаписи.РезультатУтверждения);
		Если ПодписыватьЭП
			...
			Для Каждого Данные Из ПодписиКПредметам.МассивДанныхДляЗанесенияВРегистр Цикл
				
				Если ТипЗнч(Данные.СвойстваПодписи) = Тип("Строка") 
				...
				МассивДанныхДляЗанесенияВРегистр.Добавить(Данные);
			КонецЦикла;
			
			ПараметрыФоновогоВыполнения.Вставить("МассивДанныхДляЗанесенияВРегистр", 
				МассивДанныхДляЗанесенияВРегистр);
			ПараметрыФоновогоВыполнения.Вставить("МассивАдресов", ПодписиКПредметам.МассивАдресов);
		КонецЕсли;
...
Процедура Утверждено(Команда)
	...
	Если Подзадачи.Количество() <> 0 Тогда
		...
		Если ЕстьАктивныеПодзадачи Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("УтвержденоПослеВыбораДействияСПодзадачами",
				ЭтотОбъект);
	...
	УтвержденоПослеПодзадач(); 
	
КонецПроцедуры

&НаКлиенте
Процедура УтвержденоПослеВыбораДействияСПодзадачами(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура УтвержденоПослеПодзадач() 
...
Процедура НеУтверждено(Команда)
		...
		Если ЕстьАктивныеПодзадачи Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("НеУтвержденоПослеВыбораДействияСПодзадачами",
				ЭтотОбъект);
			БизнесПроцессыИЗадачиКлиент.ЗадатьВопросОПодзадачахПриВыполненииЗадачи(
				Объект.Ссылка, ОписаниеОповещения);
			Возврат;
	...
	НеУтвержденоПослеПодзадач(); 

КонецПроцедуры

&НаКлиенте
Процедура НеУтвержденоПослеВыбораДействияСПодзадачами(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура НеУтвержденоПослеПодзадач() 
...
Функция ПодписатьПредметыЭП(ОписаниеОповещения)
	...
	РаботаСЭПКлиент.Подписать(МассивПредметовДляПодписания, УникальныйИдентификатор, ОписаниеОповещенияПослеПодписания, Заголовки);
...
Процедура СформироватьПодписиОбъектовПродолжение(Результат, Параметры) Экспорт
	...
	Для Каждого Данные Из Результат.НаборДанных Цикл
		...
		ПодписанныеДанные.Вставить("СвойстваПодписи", Данные.СвойстваПодписи);
	...
	ПодписиКПредметам.МассивДанныхДляЗанесенияВРегистр = МассивДанныхДляЗанесенияВРегистр;
	ПодписиКПредметам.МассивАдресов = МассивАдресов;
...
Процедура СформироватьЗаголовкиИПодсказкиЭлементовФормы()
	...
	Если ВидПроцесса = Перечисления.ВидыБизнесПроцессаУтверждение.Подписание Тогда
		Если ПодписыватьЭП Тогда
			Команды.Утверждено.Заголовок = НСтр("ru = 'Подписать (ЭП)'");
			Команды.Утверждено.Подсказка = НСтр("ru = 'При нажатии кнопки ""Подписать"" потребуется выбрать сертификат для подписи и ввести пароль'");
		Иначе
			Команды.Утверждено.Заголовок = НСтр("ru = 'Подписать'");
			Команды.Утверждено.Подсказка = Команды.Утверждено.Заголовок;
		КонецЕсли;
		
		Команды.НеУтверждено.Заголовок = НСтр("ru = 'Отклонить'");
	...
	Иначе
		...
		Иначе
			Команды.Утверждено.Заголовок = НСтр("ru = 'Утверждено'");
			Команды.Утверждено.Подсказка = Команды.Утверждено.Заголовок;
		КонецЕсли;
		
		Команды.НеУтверждено.Заголовок = НСтр("ru = 'Не утверждено'");
		
	КонецЕсли;
	
	Команды.НеУтверждено.Подсказка = Команды.НеУтверждено.Заголовок;
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	Элементы.Утверждено.Картинка = БиблиотекаКартинок.ЗадачаУспешноеВыполнение;
	Элементы.НеУтверждено.Картинка = БиблиотекаКартинок.ЗадачаНеУспешноеВыполнение;
