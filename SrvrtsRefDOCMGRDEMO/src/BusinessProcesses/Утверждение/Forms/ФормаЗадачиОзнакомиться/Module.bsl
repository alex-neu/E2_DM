...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// заголовки кнопок для мобильных устройств
	Если ЭтоМобильныйКлиент Тогда
		Элементы.Ознакомился.Заголовок = Нстр("ru = 'Ознакомился'");
		Если РезультатУтверждения = Перечисления.РезультатыУтверждения.НеУтверждено Тогда
			Элементы.Повторить.Заголовок = Нстр("ru = 'Повторить'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ВыполнитьЗадачу") И ПараметрыЗаписи.ВыполнитьЗадачу Тогда
		...
		УтверждениеОбъект.ДополнительныеСвойства.Вставить("ПовторитьУтверждение", ПараметрыЗаписи.ПовторитьУтверждение);
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ВыполнитьЗадачуФоново")
		...
		ПараметрыФоновогоВыполнения.Вставить("ПовторитьУтверждение", ПараметрыЗаписи.ПовторитьУтверждение);
		Если ПараметрыЗаписи.Свойство("Исполнитель") Тогда
			ПараметрыФоновогоВыполнения.Вставить("Исполнитель", ПараметрыЗаписи.Исполнитель);
		КонецЕсли;
		...
		Если ПараметрыЗаписи.Свойство("СрокИсполнения") Тогда
			ПараметрыФоновогоВыполнения.Вставить("СрокИсполнения", ПараметрыЗаписи.СрокИсполнения);
		КонецЕсли;
		Если ПараметрыЗаписи.Свойство("СрокИсполненияДни") Тогда
			ПараметрыФоновогоВыполнения.Вставить("СрокИсполненияДни", ПараметрыЗаписи.СрокИсполненияДни);
		КонецЕсли;
		Если ПараметрыЗаписи.Свойство("СрокИсполненияЧасы") Тогда
			ПараметрыФоновогоВыполнения.Вставить("СрокИсполненияЧасы", ПараметрыЗаписи.СрокИсполненияЧасы);
		КонецЕсли;
		Если ПараметрыЗаписи.Свойство("СрокИсполненияМинуты") Тогда
			ПараметрыФоновогоВыполнения.Вставить("СрокИсполненияМинуты", ПараметрыЗаписи.СрокИсполненияМинуты);
		КонецЕсли;
		Если ПараметрыЗаписи.Свойство("ВариантУстановкиСрокаИсполнения") Тогда
			ПараметрыФоновогоВыполнения.Вставить("ВариантУстановкиСрокаИсполнения", ПараметрыЗаписи.ВариантУстановкиСрокаИсполнения);
		КонецЕсли;
...
Процедура РезультатВыполненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	РаботаСБизнесПроцессамиКлиент.ВыбратьШаблонТекстаРеализация(ЭтаФорма, "РезультатВыполнения",
		ПредопределенноеЗначение("Перечисление.ОбластиПримененияШаблоновТекстов.ЗадачаОзнакомитьсяСРезультатомУтверждения"));
...
Процедура Ознакомился(Команда)
	...
	Если Подзадачи.Количество() <> 0 Тогда
		...
		Если ЕстьАктивныеПодзадачи Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ОзнакомилсяПослеВыбораДействияСПодзадачами",
				ЭтотОбъект);
	...
	ОзнакомилсяПослеПодзадач(); 
	
КонецПроцедуры

&НаКлиенте
Процедура ОзнакомилсяПослеВыбораДействияСПодзадачами(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ОзнакомилсяПослеПодзадач() 
...
Процедура ПродолжениеВыполненияЗадачиОзнакомилсяПослеВыбораФактическогоИсполнителя(
	...
	ПараметрыОбработчикаОповещения = Новый Структура;
	ПараметрыОбработчикаОповещения.Вставить("ПовторитьУтверждение", Ложь);
...
&НаКлиенте
Процедура ПродолжениеВыполненияЗадачиПослеПроверкиНаЗанятыеФайлы(Результат, ПараметрыЗаписи) Экспорт
...
Процедура Повторить(Команда)
	...
	Если Подзадачи.Количество() <> 0 Тогда
		...
		Если ЕстьАктивныеПодзадачи Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ПовторитьПослеВыбораДействияСПодзадачами",
				ЭтотОбъект);
			БизнесПроцессыИЗадачиКлиент.ЗадатьВопросОПодзадачахПриВыполненииЗадачи(
				Объект.Ссылка, ОписаниеОповещения);
			Возврат;
	...
	ПовторитьПослеПодзадач(); 
	
КонецПроцедуры

&НаКлиенте
Процедура ПовторитьПослеВыбораДействияСПодзадачами(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ПовторитьПослеПодзадач() 
...
Процедура ПродолжениеВыполненияЗадачиПовторитьПослеВыбораФактическогоИсполнителя(
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПродолжитьВыполнениеЗадачиПослеИзмененияПараметров",
		ЭтотОбъект,
...
&НаКлиенте
Процедура ПродолжитьВыполнениеЗадачиПослеИзмененияПараметров(Результат, Параметры) Экспорт
...
Процедура СформироватьЗаголовкиИПодсказкиЭлементовФормы()
	...
	ВидПроцесса = Объект.БизнесПроцесс.ВидПроцесса;
	
	Если ВидПроцесса = Перечисления.ВидыБизнесПроцессаУтверждение.Подписание Тогда
		Если РезультатУтверждения = Перечисления.РезультатыУтверждения.НеУтверждено Тогда 
			Команды.Ознакомился.Заголовок = НСтр("ru = 'Завершить подписание'");
			Команды.Повторить.Заголовок = НСтр("ru = 'Повторить подписание...'");
		КонецЕсли;
		
		Элементы.РезультатУтверждения.Заголовок = НСтр("ru = 'Результат подписания'");
		Элементы.УтверждающееЛицо.Заголовок = НСтр("ru = 'Подписант'");
		Элементы.ДатаУтверждения.Заголовок = НСтр("ru = 'Дата подписания'");
		
		Для Каждого ЭлементСписка Из Элементы.РезультатУтверждения.СписокВыбора Цикл
			Если ЭлементСписка.Значение = Перечисления.РезультатыУтверждения.Утверждено Тогда
				ЭлементСписка.Представление = НСтр("ru = 'Подписано'");
			Иначе
				ЭлементСписка.Представление = НСтр("ru = 'Не подписано'");
			КонецЕсли;
		КонецЦикла;
		
	Иначе
		Если РезультатУтверждения = Перечисления.РезультатыУтверждения.НеУтверждено Тогда 
			Команды.Ознакомился.Заголовок = НСтр("ru = 'Завершить утверждение'");
			Команды.Повторить.Заголовок = НСтр("ru = 'Повторить утверждение...'");
		КонецЕсли;
		
		Элементы.РезультатУтверждения.Заголовок = НСтр("ru = 'Результат утверждения'");
		Элементы.УтверждающееЛицо.Заголовок = НСтр("ru = 'Утверждающий'");
		Элементы.ДатаУтверждения.Заголовок = НСтр("ru = 'Дата утверждения'");
		
		Для Каждого ЭлементСписка Из Элементы.РезультатУтверждения.СписокВыбора Цикл
			Если ЭлементСписка.Значение = Перечисления.РезультатыУтверждения.Утверждено Тогда
				ЭлементСписка.Представление = НСтр("ru = 'Утверждено'");
			Иначе
				ЭлементСписка.Представление = НСтр("ru = 'Не утверждено'");
			КонецЕсли;
		КонецЦикла;
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	Элементы.Ознакомился.Картинка = БиблиотекаКартинок.ЗадачаУспешноеВыполнение;
	Элементы.Повторить.Картинка = БиблиотекаКартинок.ЗадачаНеУспешноеВыполнение;
	...
	ЭлементыОсновное.Добавить(Элементы.ГруппаРезультатЦикл);
	...
	ПараметрыАдаптациии.ОтображатьКомандыВыполненияБезГруппировки = Истина;
