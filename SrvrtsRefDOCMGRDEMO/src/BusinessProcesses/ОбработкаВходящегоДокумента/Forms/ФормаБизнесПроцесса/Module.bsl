...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_ШаблонПроцесса" Тогда
		Если Параметр = Объект.ШаблонРассмотрения Тогда
...
Процедура ОбработкаПроверкиЗаполненияНаСервереДляФоновогоСтарта(Отказ) Экспорт
	...
	ПроцессОбъект = РеквизитФормыВЗначение("Объект", Тип("БизнесПроцессОбъект.ОбработкаВходящегоДокумента"));
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	// Рассмотрение, без процедуры потому что не нужно отображение выделения.
	Элементы.ГруппаШаблонРассмотрения_БезПредметов.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	Элементы.ШаблонРассмотренияПредставлениеСрока_БезПредметов.Заголовок = НСтр("ru = 'Срок'");
	Элементы.ШаблонРассмотренияПредставлениеСрока_БезПредметов.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
