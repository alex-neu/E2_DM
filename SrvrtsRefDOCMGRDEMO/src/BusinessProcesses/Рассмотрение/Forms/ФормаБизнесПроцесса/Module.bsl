...
Процедура ЗаполнитьПоШаблону(Команда)
	...
	ШаблоныПоПредметам.ЗагрузитьЗначения(
		ШаблоныБизнесПроцессов.ШаблоныПоОбъектам(Предметы, Тип("СправочникСсылка.ШаблоныРассмотрения")));
	
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ЗаполнитьПоШаблонуПродолжение",
		ЭтотОбъект);
	ШаблоныБизнесПроцессовКлиент.ВыбратьШаблонБизнесПроцесса(
		"ШаблоныРассмотрения", ШаблоныПоПредметам, ОписаниеОповещения);
...
Процедура ОбновитьСрокиИсполненияНаСервере() Экспорт
	...
	Если СрокиИсполненияПроцессов.ТребуетсяРасчетСроков(ЭтаФорма)
		...
		СрокиИсполненияПроцессов.РассчитатьСрокиРассмотрения(Объект, ПараметрыДляРасчетаСроков);
...
Процедура ПроверитьИзменениеСроковПроцесса()
	...
	СтруктураСроковДляПроверки.СрокиИсполнителей.Вставить(
		БизнесПроцессы.Рассмотрение.ТочкиМаршрута.Рассмотреть, Объект.СрокИсполнения);
	
	СтруктураСроковДляПроверки.СрокиИсполнителей.Вставить(
		БизнесПроцессы.Рассмотрение.ТочкиМаршрута.Ознакомиться, Объект.СрокОбработкиРезультатов);
...
Процедура ОбработкаПроверкиЗаполненияНаСервереДляФоновогоСтарта(Отказ) Экспорт
	...
	ПроцессОбъект = РеквизитФормыВЗначение("Объект", Тип("БизнесПроцессОбъект.Рассмотрение"));
...
Процедура ОписаниеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	РаботаСБизнесПроцессамиКлиент.ВыбратьШаблонТекстаРеализация(ЭтаФорма, "Описание",
		ПредопределенноеЗначение("Перечисление.ОбластиПримененияШаблоновТекстов.ПроцессРассмотрениеОписание"));
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	Элементы.Переместить(Элементы.Важность, Элементы.ГруппаГлавное, Элементы.ГруппаИсполнительСрокиРезолюция);
	
	// Адаптируем резолюцию.
	Элементы.Резолюция.Ширина = 0;
	Элементы.Резолюция.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	Элементы.ГруппаРезолюция.Заголовок = НСтр("ru = 'Проект резолюции'");
	Элементы.ГруппаРезолюция.ОтображатьЗаголовок = Истина;
	Элементы.ГруппаРезолюция.ШрифтЗаголовка = Новый Шрифт(ШрифтыСтиля.ОбычныйШрифтТекста,,, Истина);
	Элементы.ГруппаРезолюция.Отображение = ОтображениеОбычнойГруппы.ОбычноеВыделение;
	...
	// Нстроим заголовок для исполнители.
	Элементы.ГруппаИсполнитель.Заголовок = НСтр("ru = 'Кому'");
	
	// Уберем отступ между описанием и исполнителем.
	Элементы.ГруппаИсполнитель.Отображение = ОтображениеОбычнойГруппы.Нет;
	Элементы.ГруппаИсполнительСрокиРезолюция.Отображение = ОтображениеОбычнойГруппы.Нет;
	...
	ПараметрыАдаптациии.ЭлементГруппаИсполнители = Элементы.ГруппаИсполнитель;
