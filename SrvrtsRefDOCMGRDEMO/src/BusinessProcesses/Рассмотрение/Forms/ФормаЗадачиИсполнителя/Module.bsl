...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПодписатьРезолюцииЭП Тогда
		...
		Команды.Исполнить.Заголовок = НСтр("ru = 'Исполнить (ЭП)'");
		Команды.Исполнить.Подсказка = НСтр("ru = 'При нажатии кнопки ""Исполнить"" потребуется выбрать сертификат для подписи и ввести пароль'");
		Команды.Ознакомить.Заголовок = НСтр("ru = 'Ознакомить (ЭП)'");
		Команды.Ознакомить.Подсказка = НСтр("ru = 'При нажатии кнопки ""Ознакомить"" потребуется выбрать сертификат для подписи и ввести пароль'");
	КонецЕсли;
	...
	ПоказатьРасширеннуюФорму(ЭтотОбъект);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ПоказатьРасширеннуюФорму(Форма)
...
#КонецЕсли
	...
	ВариантРассмотрения = Форма.ВариантРассмотрения;
	РасширеннаяФорма = Форма.РасширеннаяФорма;
	
	Если ЭтоМобильныйКлиент Тогда
		
		// В мобильном клиенте нет страниц для переключения.
		// Управляем отображением групп.
		Элементы.ГруппаНаИсполнение.Видимость = РасширеннаяФорма;
		Элементы.ГруппаОзнакомление.Видимость = РасширеннаяФорма;
		Элементы.СтраницаЗадача.ОтображатьЗаголовок = РасширеннаяФорма;
		Если РасширеннаяФорма Тогда
			Элементы.СтраницаЗадача.Поведение = ПоведениеОбычнойГруппы.Всплывающая;
		Иначе
			Элементы.СтраницаЗадача.Поведение = ПоведениеОбычнойГруппы.Обычное;
		КонецЕсли;
		
	Иначе
		
		Если Не РасширеннаяФорма 
			И ВариантРассмотрения <> Форма.ВариантРассмотренияНаправитьНаИсполнение 
			И ВариантРассмотрения <> Форма.ВариантРассмотренияНаправитьНаОзнакомление Тогда 
		...
		Если ВариантРассмотрения = Форма.ВариантРассмотренияВвестиТекстРезолюции Тогда
			Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаЗадача;
			Элементы.ГруппаКомандыВыполнения.ТекущаяСтраница = Элементы.Рассмотрение;
		ИначеЕсли ВариантРассмотрения = Форма.ВариантРассмотренияНаправитьНаИсполнение Тогда 
			Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.ГруппаНаИсполнение;
			Элементы.ГруппаКомандыВыполнения.ТекущаяСтраница = Элементы.Исполнение;
		ИначеЕсли ВариантРассмотрения = Форма.ВариантРассмотренияНаправитьНаОзнакомление Тогда 
...
Процедура СформироватьТекстРезолюции()
	...
	Если ВариантРассмотрения = Перечисления.ВариантыРассмотрения.НаправитьНаИсполнение Тогда
		...
		Если ЗначениеЗаполнено(СрокИсполненияПроцессаИсполнения) Тогда
			РезультатВыполнения = РезультатВыполнения + НСтр("ru = 'Срок'") + ": " + Формат(СрокИсполненияПроцессаИсполнения, "ДФ=dd.MM.yyyy") + Символы.ПС;
		КонецЕсли;
	...
	ИначеЕсли ВариантРассмотрения = Перечисления.ВариантыРассмотрения.НаправитьНаОзнакомление Тогда
		...
		Если ЗначениеЗаполнено(СрокИсполненияПроцессаОзнакомления) Тогда
			РезультатВыполнения = РезультатВыполнения + НСтр("ru = 'Срок'") + ": " + Формат(СрокИсполненияПроцессаОзнакомления, "ДФ=dd.MM.yyyy") + Символы.ПС;
		КонецЕсли;
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	Элементы.ЗаполнитьПоШаблону.Заголовок = НСтр("ru = 'Заполнить по шаблону'");
	Элементы.ЗаполнитьПоШаблонуОзнакомление.Заголовок = Элементы.ЗаполнитьПоШаблону.Заголовок;
	Элементы.ЗаполнитьПоШаблонуИсполнение.Заголовок = Элементы.ЗаполнитьПоШаблону.Заголовок;
	...
	// Добавим подсказку ввода для резолюции.
	Элементы.Резолюция.ПодсказкаВвода = НСтр("ru = 'Введите текст или заполните по шаблону'");
	
	// Установим картинки у кнопок выполнения задачи.
	Элементы.Выполнено.Картинка = БиблиотекаКартинок.ЗадачаУспешноеВыполнение;
	Элементы.РассмотретьПоШаблону.Картинка = БиблиотекаКартинок.ЗадачаНейтральноеВыполнение;
	Элементы.РассмотретьПоШаблону0.Картинка = БиблиотекаКартинок.ЗадачаУспешноеВыполнение;
	Элементы.РассмотретьПоШаблону1.Картинка = БиблиотекаКартинок.ЗадачаУспешноеВыполнение;
	...
	// Настроим важность.
	Элементы.Переместить(Элементы.ВажностьИсполнения, Элементы.ГруппаНаИсполнение, Элементы.ИсполнителиИсполненияМК);
	...
	// Также см. процедуру ПоказатьРасширеннуюФорму.
	//  Страница "Ввести резолюцию".
	Элементы.Переместить(Элементы.СтраницаЗадача, Элементы.ГруппаОбластьВыполнения);
	Элементы.СтраницаЗадача.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	Элементы.СтраницаЗадача.Поведение = ПоведениеОбычнойГруппы.Всплывающая;
	Элементы.СтраницаЗадача.Заголовок = НСтр("ru='Ввести резолюцию'");
	...
	Элементы.Переместить(Элементы.Рассмотрение, Элементы.СтраницаЗадача);
	КомандыРассмотрено = Новый Массив;
	КомандыРассмотрено.Добавить(Элементы.Выполнено);
	КомандыРассмотрено.Добавить(Элементы.РассмотретьПоШаблону);
	КомандыРассмотрено.Добавить(Элементы.РассмотретьПоШаблону0);
	КомандыРассмотрено.Добавить(Элементы.РассмотретьПоШаблону1);
	МобильныйКлиентАдаптацияИнтерфейсаСервер.АдаптироватьОбластьВыполненияЗадачи(
