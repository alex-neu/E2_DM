...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МестнаяДатаНачалаМероприятия = РаботаСЧасовымиПоясами.ПривестиКМестномуВремени(ДатаНачалаМероприятия);
	МестнаяДатаОкончанияМероприятия = РаботаСЧасовымиПоясами.ПривестиКМестномуВремени(ДатаОкончанияМероприятия);
...
Процедура Принято(Команда, ОбработанныеВопросы = Неопределено)
	...
	Если Подзадачи.Количество() <> 0 Тогда
		...
		Если ЕстьАктивныеПодзадачи Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ПринятоПослеВыбораДействияСПодзадачами",
	...
	ПринятоПослеПодзадач(Команда, ОбработанныеВопросы); 

КонецПроцедуры

&НаКлиенте
Процедура ПринятоПослеВыбораДействияСПодзадачами(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = "ИгнорироватьПодзадачи" Тогда 
		
		ПринятоПослеПодзадач(ДополнительныеПараметры.Команда, ДополнительныеПараметры.ОбработанныеВопросы);
...
&НаКлиенте
Процедура ПринятоПослеПодзадач(Команда, ОбработанныеВопросы) 
...
Процедура ПродолжениеВыполненияЗадачиПринятоПослеВыбораФактическогоИсполнителя(
	...
	ПараметрыОбработчикаОповещения = Новый Структура();
	ПараметрыОбработчикаОповещения.Вставить("ПриглашениеПринято", Истина);
	ОписаниеОповещения = Новый ОписаниеОповещения(
...
Процедура НеПринято(Команда)
	...
	Если Подзадачи.Количество() <> 0 Тогда
		...
		Если ЕстьАктивныеПодзадачи Тогда
			...
			ДопПараметры = Новый Структура;
			ОписаниеОповещения = Новый ОписаниеОповещения("НеПринятоПослеВыбораДействияСПодзадачами",
				ЭтотОбъект, ДопПараметры);
			БизнесПроцессыИЗадачиКлиент.ЗадатьВопросОПодзадачахПриВыполненииЗадачи(
				Объект.Ссылка, ОписаниеОповещения);
			Возврат;
	...
	НеПринятоПослеПодзадач(); 
	
КонецПроцедуры

&НаКлиенте
Процедура НеПринятоПослеВыбораДействияСПодзадачами(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура НеПринятоПослеПодзадач() 
...
Процедура ПродолжениеВыполненияЗадачиНеПринятоПослеВыбораФактическогоИсполнителя(
	...
	ПараметрыОбработчикаОповещения = Новый Структура();
	ПараметрыОбработчикаОповещения.Вставить("ПриглашениеПринято", Ложь);
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПродолжениеВыполненияЗадачиПослеПроверкиНаЗанятыеФайлы",
		ЭтотОбъект, ПараметрыОбработчикаОповещения);
...
&НаКлиенте
Процедура ПродолжениеВыполненияЗадачиПослеПроверкиНаЗанятыеФайлы(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;
	
	Если ДополнительныеПараметры.ПриглашениеПринято Тогда
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	Элементы.ГруппаМестоПроведенияМК.Видимость = Истина;
	
	Элементы.Переместить(Элементы.МестоПроведения,Элементы.ГруппаМестоПроведенияМК,Элементы.ГруппаМестоПроведенияМК);
	Элементы.Переместить(Элементы.ДекорацияМестоПроведения,Элементы.ГруппаМестоПроведенияМК,Элементы.ГруппаМестоПроведенияМК);
	...
	ЭлементыОсновное.Добавить(Элементы.ГруппаМестоПроведения);
	ЭлементыОсновное.Добавить(Элементы.ГруппаВремяПроведения);
	...
	Элементы.Принято.Картинка = БиблиотекаКартинок.ЗадачаУспешноеВыполнение;
	Элементы.Отклонено.Картинка = БиблиотекаКартинок.ЗадачаНеУспешноеВыполнение;
