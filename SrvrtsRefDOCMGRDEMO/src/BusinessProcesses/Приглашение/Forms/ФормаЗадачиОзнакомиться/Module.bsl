...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоМобильныйКлиент Тогда
			...
			Элементы.Отложить.Заголовок = Нстр("ru = 'Отменить'");
	КонецЕсли;
...
Процедура Подтвердить(Команда)
	...
	Если Подзадачи.Количество() <> 0 Тогда
		...
		Если ЕстьАктивныеПодзадачи Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ПодтвердитьПослеВыбораДействияСПодзадачами",
				ЭтотОбъект);
	...
	ПодтвердитьПослеПодзадач(); 

КонецПроцедуры

&НаКлиенте
Процедура ПодтвердитьПослеВыбораДействияСПодзадачами(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ПодтвердитьПослеПодзадач() 
...
Процедура Отложить(Команда)
	...
	Если Подзадачи.Количество() <> 0 Тогда
		...
		Если ЕстьАктивныеПодзадачи Тогда
			...
			ДопПараметры = Новый Структура;
			ОписаниеОповещения = Новый ОписаниеОповещения("ОтложитьПослеВыбораДействияСПодзадачами",
				ЭтотОбъект, ДопПараметры);
			БизнесПроцессыИЗадачиКлиент.ЗадатьВопросОПодзадачахПриВыполненииЗадачи(
				Объект.Ссылка, ОписаниеОповещения);
			Возврат;
	...
	ОтложитьПослеПодзадач(); 
	
КонецПроцедуры

&НаКлиенте
Процедура ОтложитьПослеВыбораДействияСПодзадачами(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ОтложитьПослеПодзадач() 
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	Элементы.РезультатПриглашения.АвтоМаксимальнаяШирина = Ложь;
	Элементы.РезультатПриглашения.РастягиватьПоГоризонтали = Истина;
	
	// Установим картинки у кнопок выполнения задачи.
	Элементы.Подтвердить.Картинка = БиблиотекаКартинок.ЗадачаУспешноеВыполнение;
	Элементы.Повторить.Картинка = БиблиотекаКартинок.ЗадачаНейтральноеВыполнение;
	Элементы.Отложить.Картинка = БиблиотекаКартинок.ЗадачаНеУспешноеВыполнение;
	...
	ЭлементыОсновное.Добавить(Элементы.ГруппаРезультатИЦикл);
	...
	ПараметрыАдаптациии.СтраницыЭлементы.Добавить(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Элементы.ХодИсполнения));
