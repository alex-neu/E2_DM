...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РезультатыРегистрацииЗарегистрировано = Перечисления.РезультатыРегистрации.Зарегистрировано;
	РезультатыРегистрацииНеЗарегистрировано = Перечисления.РезультатыРегистрации.НеЗарегистрировано;
	...
	// номер цикла
	НайденнаяСтрока = Объект.БизнесПроцесс.РезультатыРегистрации.Найти(Объект.Ссылка, "ЗадачаИсполнителя");
	...
	Если Не ПраваПоОбъекту.Изменение Тогда
		...
		Элементы.Зарегистрировано.Доступность = Ложь;
		Элементы.НеЗарегистрировано.Доступность = Ложь;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("Регистрация") Тогда
		...
		ОсновныеПредметы = МультипредметностьКлиентСервер.ПолучитьМассивПредметовОбъекта(Объект, ТипыДокументов, Истина);
...
&НаКлиенте
Процедура ПередЗаписьюПродолжениеПослеУказанияРегистрационногоНомера(Результат, ПараметрыОповещения) Экспорт
	...
	Если Не ВыполнениеЗадачКлиент.ВыполнитьЗадачуИзФормы(ЭтаФорма, ПараметрыОповещения.ПараметрыЗаписи) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ВыполнитьЗадачу") И ПараметрыЗаписи.ВыполнитьЗадачу Тогда
		...
		РегистрацияОбъект.ДополнительныеСвойства.Вставить("РезультатРегистрации",ПараметрыЗаписи.РезультатРегистрации);
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ВыполнитьЗадачуФоново")
		...
		ПараметрыФоновогоВыполнения.Вставить("РезультатРегистрации", ПараметрыЗаписи.РезультатРегистрации);
	КонецЕсли;
...
&НаКлиенте
Процедура Зарегистрировано(Команда)
	...
	Если Подзадачи.Количество() <> 0 Тогда
		...
		Если ЕстьАктивныеПодзадачи Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ЗарегистрированоПослеВыбораДействияСПодзадачами",
				ЭтотОбъект);
	...
	ЗарегистрированоПослеПодзадач(); 
	
КонецПроцедуры

&НаКлиенте
Процедура ЗарегистрированоПослеВыбораДействияСПодзадачами(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ЗарегистрированоПослеПодзадач() 
...
&НаКлиенте
Процедура ПродолжениеВыполненияЗадачиЗарегистрированоПослеВыбораФактическогоИсполнителя(ВыбранныйФактическийИсполнитель, ДопПараметры) Экспорт
	...
	ПараметрыОбработчикаОповещения = Новый Структура;
	ПараметрыОбработчикаОповещения.Вставить("РезультатРегистрации", РезультатыРегистрацииЗарегистрировано);
...
&НаКлиенте
Процедура НеЗарегистрировано(Команда)
	...
	Если Подзадачи.Количество() <> 0 Тогда
		...
		Если ЕстьАктивныеПодзадачи Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("НеЗарегистрированоПослеВыбораДействияСПодзадачами",
				ЭтотОбъект);
			БизнесПроцессыИЗадачиКлиент.ЗадатьВопросОПодзадачахПриВыполненииЗадачи(
				Объект.Ссылка, ОписаниеОповещения);
			Возврат;
	...
	НеЗарегистрированоПослеПодзадач(); 

КонецПроцедуры

&НаКлиенте
Процедура НеЗарегистрированоПослеВыбораДействияСПодзадачами(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура НеЗарегистрированоПослеПодзадач() 
...
Процедура ПродолжениеВыполненияЗадачиНеЗарегистрированоПослеВыбораФактическогоИсполнителя(
	...
	ПараметрыОбработчикаОповещения = Новый Структура;
	ПараметрыОбработчикаОповещения.Вставить("РезультатРегистрации", РезультатыРегистрацииНеЗарегистрировано);
...
Процедура ПродолжениеВыполненияЗадачиПослеПроверкиНаЗанятыеФайлы(Результат, Параметры) Экспорт
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, "РегистрацияВыполнениеКомандыЗарегистрировано");
	
	ПараметрыЗаписи = Новый Структура;
	ПараметрыЗаписи.Вставить("РезультатРегистрации", Параметры.РезультатРегистрации);
	
	Если Параметры.РезультатРегистрации = РезультатыРегистрацииЗарегистрировано Тогда 
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	Элементы.Зарегистрировано.Картинка = БиблиотекаКартинок.ЗадачаУспешноеВыполнение;
	Элементы.НеЗарегистрировано.Картинка = БиблиотекаКартинок.ЗадачаНеУспешноеВыполнение;
