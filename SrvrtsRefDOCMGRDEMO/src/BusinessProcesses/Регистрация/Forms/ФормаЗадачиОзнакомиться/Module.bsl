...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтрокиИтерации = Объект.БизнесПроцесс.РезультатыРегистрации.НайтиСтроки(Новый Структура("НомерИтерации", НомерИтерации));
	...
	Если РезультатРегистрации = Перечисления.РезультатыРегистрации.НеЗарегистрировано Тогда 
		...
		Элементы.Ознакомился.Заголовок = Нстр("ru = 'Завершить регистрацию'");
	...
	Если ЭтоМобильныйКлиент Тогда
		...
		Если РезультатРегистрации = Перечисления.РезультатыРегистрации.НеЗарегистрировано Тогда
			Элементы.Ознакомился.Заголовок = Нстр("ru = 'Завершить'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ВыполнитьЗадачу") И ПараметрыЗаписи.ВыполнитьЗадачу Тогда
		...
		РегистрацияОбъект = ТекущийОбъект.БизнесПроцесс.ПолучитьОбъект();
		РегистрацияОбъект.ДополнительныеСвойства.Вставить("ТекущаяЗадача",ТекущийОбъект.Ссылка);
		РегистрацияОбъект.ДополнительныеСвойства.Вставить("ПовторитьРегистрацию",ПараметрыЗаписи.ПовторитьРегистрацию);
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ВыполнитьЗадачуФоново")
		...
		ПараметрыФоновогоВыполнения.Вставить("ПовторитьРегистрацию", ПараметрыЗаписи.ПовторитьРегистрацию);
...
Процедура РезультатВыполненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	РаботаСБизнесПроцессамиКлиент.ВыбратьШаблонТекстаРеализация(ЭтаФорма, "РезультатВыполнения",
		ПредопределенноеЗначение("Перечисление.ОбластиПримененияШаблоновТекстов.ЗадачаОзнакомитьсяСРегистрацией"));
...
Процедура История(Команда)
	...
	ОткрытьФорму("БизнесПроцесс.Регистрация.Форма.ФормаИсторияРегистрации", ПараметрыФормы, ЭтаФорма);
...
Процедура ПродолжениеВыполненияЗадачиОзнакомилсяПослеВыбораФактическогоИсполнителя(
	...
	ПараметрыОбработчикаОповещения = Новый Структура;
	ПараметрыОбработчикаОповещения.Вставить("ПовторитьРегистрацию", Ложь);
...
Процедура ПродолжениеВыполненияЗадачиПовторитьПослеВыбораФактическогоИсполнителя(
	...
	ОткрытьФорму(
		"БизнесПроцесс.Регистрация.Форма.ФормаИзменениеПараметров",
		ПараметрыФормы,
		ЭтаФорма,,,,
		ОписаниеОповещения,
...
Процедура ПродолжениеВыполненияЗадачиПослеИзмененияПараметров(Результат, Параметры) Экспорт
	...
	Результат.Вставить("ПовторитьРегистрацию", Истина);
...
Процедура ПродолжениеВыполненияЗадачиПослеПроверкиНаЗанятыеФайлы(Результат, Параметры) Экспорт
	...
	Если Не ВыполнениеЗадачКлиент.ВыполнитьЗадачуИзФормы(ЭтаФорма, Параметры) Тогда
		Возврат;
	КонецЕсли;
