...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СрокиИсполненияПроцессов.КарточкаПроцессаПриСозданииНаСервере(
		ЭтаФорма, БизнесПроцессы.Регистрация.ТочкиМаршрута.Ознакомиться);
...
Процедура НаименованиеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	РаботаСБизнесПроцессамиКлиент.ВыбратьШаблонТекстаРеализация(ЭтаФорма, "Наименование",
		ПредопределенноеЗначение("Перечисление.ОбластиПримененияШаблоновТекстов.ПроцессРегистрацияНаименование"));
...
Процедура ОписаниеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	РаботаСБизнесПроцессамиКлиент.ВыбратьШаблонТекстаРеализация(ЭтаФорма, "Описание",
		ПредопределенноеЗначение("Перечисление.ОбластиПримененияШаблоновТекстов.ПроцессРегистрацияОписание"));
...
Функция ПредставлениеРезультата() Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ПредставлениеРезультата, Объект.РезультатРегистрации, Формат(Объект.ДатаЗавершения, "ДФ='dd.MM.yyyy HH:mm'"));
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	// Выровняем Исполнителя.
	Элементы.Исполнитель.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
	Элементы.Исполнитель.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	Элементы.Исполнитель.РастягиватьПоГоризонтали = Истина;

	// Важность покажем после описания.
	Элементы.Переместить(Элементы.Важность, Элементы.ГруппаИсполнительСрокИсполненияСрокОбработкиРезультатовКонтрольныйСрокПроцесса, Элементы.ГруппаИсполнитель);
	...
	Элементы.ГруппаИсполнитель.Заголовок = НСтр("ru = 'Исполнитель'");
...
Процедура ОбновитьСрокиИсполненияНаСервере() Экспорт
	...
	Если СрокиИсполненияПроцессов.ТребуетсяРасчетСроков(ЭтаФорма)
		...
		СрокиИсполненияПроцессов.РассчитатьСрокиРегистрации(Объект, ПараметрыДляРасчетаСроков);
...
Процедура ПроверитьИзменениеСроковПроцесса()
	...
	СтруктураСроковДляПроверки.СрокиИсполнителей.Вставить(
		БизнесПроцессы.Регистрация.ТочкиМаршрута.Зарегистрировать, Объект.СрокИсполнения);
	
	СтруктураСроковДляПроверки.СрокиИсполнителей.Вставить(
		БизнесПроцессы.Регистрация.ТочкиМаршрута.Ознакомиться, Объект.СрокОбработкиРезультатов);
...
Процедура ОбновитьПризнакиИстекшихСроков()
	...
	Если Объект.Завершен
		И Объект.РезультатРегистрации <> Перечисления.РезультатыРегистрации.НеЗарегистрировано Тогда
...
Процедура ОбработкаПроверкиЗаполненияНаСервереДляФоновогоСтарта(Отказ) Экспорт
	...
	ПроцессОбъект = РеквизитФормыВЗначение("Объект", Тип("БизнесПроцессОбъект.Регистрация"));
...
Процедура ЗаполнитьПоШаблону(Команда)
	...
	ШаблоныПоПредметам.ЗагрузитьЗначения(
		ШаблоныБизнесПроцессов.ШаблоныПоОбъектам(Предметы, Тип("СправочникСсылка.ШаблоныРегистрации")));
	
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ЗаполнитьПоШаблонуПродолжение",
		ЭтотОбъект);
	ШаблоныБизнесПроцессовКлиент.ВыбратьШаблонБизнесПроцесса(
		"ШаблоныРегистрации", ШаблоныПоПредметам, ОписаниеОповещения);
