...
Функция ПодзадачиПроцесса(Процесс) Экспорт
	...
	Если МассивЗадач.Количество() = 0 Тогда // еше не стартован
		...
		СтруктураЗадачи.Участник = РеквизитыПроцессаПоСсылке.Исполнитель;
		СтруктураЗадачи.Срок = РеквизитыПроцессаПоСсылке.СрокИсполнения;
		
		СтруктураЗадачи.ДатаСоздания = РеквизитыПроцессаПоСсылке.ДатаНачала;
...
Функция ПолучитьОписаниеПроцесса() Экспорт
	...
	Возврат НСтр("ru = 'Используется для передачи на регистрацию входящего, исходящего или внутреннего документа.'");
...
Функция ТекстКомпенсацииПредмета(ПроцессСсылка) Экспорт
	...
	Если КоличествоИзмененныхПредметов = 1 Тогда
		...
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'У документа ""%1"" будет очищено состояние регистрации.'"),
			НаименованиеПредмета);
	Иначе
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'У документов (%1) будет очищено состояние регистрации.'"),
...
Функция ВариантыОтветовДляВыполненияЗадачиПоПочте(
	...
	ИначеЕсли ТочкаМаршрута = ТочкиМаршрута.Ознакомиться Тогда
		...
		Если РезультатРегистрации =
			...
			НаименованиеПоложительногоРезультата = НСтр("ru = 'Завершить регистрацию'");
			НаименованиеОтрицательногоРезультата = НСтр("ru = 'Повторить регистрацию'");
