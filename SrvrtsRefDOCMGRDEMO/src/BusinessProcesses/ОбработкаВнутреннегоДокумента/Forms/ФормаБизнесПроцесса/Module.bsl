...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Объект.Стартован И Не Объект.Завершен Тогда 
		Если Не (ЗначениеЗаполнено(Объект.ШаблонРассмотрения) И ЗначениеЗаполнено(Объект.ШаблонИсполненияОзнакомления)) Тогда 
			...
			Элементы.ШаблонРассмотрения_БезПредметов.Доступность = Элементы.ШаблонРассмотрения.Доступность;
			Элементы.ШаблонИсполненияОзнакомления_БезПредметов.Доступность = Элементы.ШаблонИсполненияОзнакомления.Доступность;
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьГрафикиРаботы") Тогда
		...
		Элементы.ШаблонРассмотренияПредставлениеСрока.Подсказка = СтрЗаменить(
			Элементы.ШаблонРассмотренияПредставлениеСрока.Подсказка,
			СтрокаЗаменыДляВсплывающейПодсказки, "");
			
		Элементы.ШаблонИсполненияОзнакомленияПредставлениеСрока.Подсказка = СтрЗаменить(
			Элементы.ШаблонИсполненияОзнакомленияПредставлениеСрока.Подсказка,
			СтрокаЗаменыДляВсплывающейПодсказки, "");
			
		Элементы.ШаблонПорученияПредставлениеСрока.Подсказка = СтрЗаменить(
			Элементы.ШаблонПорученияПредставлениеСрока.Подсказка,
			СтрокаЗаменыДляВсплывающейПодсказки, "");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_ШаблонПроцесса" Тогда
		...
		ИначеЕсли Параметр = Объект.ШаблонРегистрации Тогда
			...
			ОбновитьСрокиИсполненияОтложенно();
		ИначеЕсли Параметр = Объект.ШаблонРассмотрения Тогда
			РеквизитСИзмененнымСроком = "ШаблонРассмотрения";
			ОбновитьСрокиИсполненияОтложенно();
		ИначеЕсли Параметр = Объект.ШаблонИсполненияОзнакомления Тогда
			РеквизитСИзмененнымСроком = "ШаблонИсполненияОзнакомления";
			ОбновитьСрокиИсполненияОтложенно();
		ИначеЕсли Параметр = Объект.ШаблонПоручения Тогда
			РеквизитСИзмененнымСроком = "ШаблонПоручения";
			ОбновитьСрокиИсполненияОтложенно();
		КонецЕсли;
	КонецЕсли;
...
Процедура ШаблонРассмотренияПриИзменении(Элемент)
	...
	РеквизитШаблона = "ШаблонРассмотрения";
	ОчиститьСрокИсполненияШаблона(РеквизитШаблона);
	РеквизитСИзмененнымСроком = РеквизитШаблона;
	ОбновитьСрокиИсполненияОтложенно();
...
Процедура ШаблонИсполненияОзнакомленияПриИзменении(Элемент)
	...
	РеквизитШаблона = "ШаблонИсполненияОзнакомления";
	ОчиститьСрокИсполненияШаблона(РеквизитШаблона);
	РеквизитСИзмененнымСроком = РеквизитШаблона;
	ОбновитьСрокиИсполненияОтложенно();
...
&НаКлиенте
Процедура ШаблонПорученияПриИзменении(Элемент)
	
	РеквизитШаблона = "ШаблонПоручения";
	ОчиститьСрокИсполненияШаблона(РеквизитШаблона);
	РеквизитСИзмененнымСроком = РеквизитШаблона;
	ОбновитьСрокиИсполненияОтложенно();
...
Процедура ЗаполнитьПоШаблону(Команда)
	...
	ШаблоныПоПредметам.ЗагрузитьЗначения(
		ШаблоныБизнесПроцессов.ШаблоныПоОбъектам(Предметы, ИмяБизнесПроцесса));
...
Процедура УстановитьДоступность()
	...
	Если Объект.Стартован Тогда 
		...
		Элементы.ШаблонРассмотрения_БезПредметов.ТолькоПросмотр = Элементы.ШаблонРассмотрения.ТолькоПросмотр;
		Элементы.ШаблонИсполненияОзнакомления_БезПредметов.ТолькоПросмотр = Элементы.ШаблонИсполненияОзнакомления.ТолькоПросмотр;
		Элементы.ШаблонПоручения_БезПредметов.ТолькоПросмотр = Элементы.ШаблонПоручения.ТолькоПросмотр;
...
Процедура ЗаполнитьТаблицуСрокиПроцессов()
	...
	НоваяСтрока = СрокиШаблонов.Добавить();
	НоваяСтрока.Реквизит = "ШаблонРассмотрения";
	
	НоваяСтрока = СрокиШаблонов.Добавить();
	НоваяСтрока.Реквизит = "ШаблонИсполненияОзнакомления";
	
	НоваяСтрока = СрокиШаблонов.Добавить();
	НоваяСтрока.Реквизит = "ШаблонПоручения";
...
Процедура УстановитьУсловноеОформлениеИстекшихСроков()
	...
	СрокиИсполненияПроцессов.УстановитьУсловноеОформлениеИстекшегоСрока(
		ЭтаФорма,
		НСтр("ru = 'Срок согласования истек'"),
		"ШаблонСогласованияСрокИстек",
		"ШаблонСогласованияПредставлениеСрока");
	
	СрокиИсполненияПроцессов.УстановитьУсловноеОформлениеИстекшегоСрока(
		ЭтаФорма,
		НСтр("ru = 'Срок утверждения истек'"),
		"ШаблонУтвержденияСрокИстек",
		"ШаблонУтвержденияПредставлениеСрока");
	
	СрокиИсполненияПроцессов.УстановитьУсловноеОформлениеИстекшегоСрока(
		ЭтаФорма,
		НСтр("ru = 'Срок регистрации истек'"),
		"ШаблонРегистрацииСрокИстек",
		"ШаблонРегистрацииПредставлениеСрока");
	
	СрокиИсполненияПроцессов.УстановитьУсловноеОформлениеИстекшегоСрока(
		ЭтаФорма,
		НСтр("ru = 'Срок рассмотрения истек'"),
		"ШаблонРассмотренияСрокИстек",
		"ШаблонРассмотренияПредставлениеСрока");
	
	СрокиИсполненияПроцессов.УстановитьУсловноеОформлениеИстекшегоСрока(
		ЭтаФорма,
		НСтр("ru = 'Срок исполнения/ознакомления истек'"),
		"ШаблонИсполненияОзнакомленияСрокИстек",
		"ШаблонИсполненияОзнакомленияПредставлениеСрока");
	
	СрокиИсполненияПроцессов.УстановитьУсловноеОформлениеИстекшегоСрока(
		ЭтаФорма,
		НСтр("ru = 'Срок поручения истек'"),
		"ШаблонПорученияСрокИстек",
		"ШаблонПорученияПредставлениеСрока");
...
Процедура ОбработкаПроверкиЗаполненияНаСервереДляФоновогоСтарта(Отказ) Экспорт
	...
	ПроцессОбъект = РеквизитФормыВЗначение("Объект", Тип("БизнесПроцессОбъект.ОбработкаВнутреннегоДокумента"));
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	// Описание оставим одно. Страницы спрячем.
	Элементы.Переместить(Элементы.ГруппаДействия_БезПредметов, Элементы.ГруппаГлавное, Элементы.ГруппаОписаниеИПредметы);
	Элементы.ГруппаОписаниеИПредметы.Видимость = Ложь;
	
	// Уберем отступ перед Наименованием.
	Элементы.ГруппаНаименованиеВажность.Отображение = ОтображениеОбычнойГруппы.Нет;
	
	// Добавим заголовок к группе всех шаблонов.
	Элементы.ГруппаДействия_БезПредметов.ОтображатьЗаголовок = Истина;
	Элементы.ГруппаДействия_БезПредметов.ШрифтЗаголовка = Элементы.ГруппаКонтрольныйСрокПроцесса_БезПредметов.ШрифтЗаголовка;

	// Настроим отображение шаблонов.
	// Согласование, без процедуры потому что не нужно отображение выделения.
	Элементы.ГруппаШаблонСогласования_БезПредметов.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	Элементы.ШаблонСогласованияПредставлениеСрока_БезПредметов.Заголовок = НСтр("ru = 'Срок'");
	Элементы.ШаблонСогласованияПредставлениеСрока_БезПредметов.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	Элементы.ШаблонСогласования_БезПредметов.КнопкаОткрытия = Истина;

	// Утверждения.
	НастройкаСрокаШаблонаИГруппаШаблонМК(
		Элементы.ГруппаШаблонУтверждения_БезПредметов,Элементы.ШаблонУтвержденияПредставлениеСрока_БезПредметов);
	Элементы.ШаблонУтверждения_БезПредметов.КнопкаОткрытия = Истина;
	
	// Регистрации.
	НастройкаСрокаШаблонаИГруппаШаблонМК(
		Элементы.ГруппаШаблонРегистрации_БезПредметов,Элементы.ШаблонРегистрацииПредставлениеСрока_БезПредметов);
	Элементы.ШаблонРегистрации_БезПредметов.КнопкаОткрытия = Истина;
	
	// Рассмотрения.
	НастройкаСрокаШаблонаИГруппаШаблонМК(
		Элементы.ГруппаШаблонРассмотрения_БезПредметов,Элементы.ШаблонРассмотренияПредставлениеСрока_БезПредметов);
	Элементы.ШаблонРассмотрения_БезПредметов.КнопкаОткрытия = Истина;
	
	// Исполнения и Ознакомления.
	НастройкаСрокаШаблонаИГруппаШаблонМК(
		Элементы.ГруппаШаблонИсполненияОзнакомления_БезПредметов,Элементы.ШаблонИсполненияОзнакомленияПредставлениеСрока_БезПредметов);
	Элементы.ШаблонИсполненияОзнакомления_БезПредметов.КнопкаОткрытия = Истина;
	
	// Поручения.
	НастройкаСрокаШаблонаИГруппаШаблонМК(
		Элементы.ГруппаШаблонПоручения_БезПредметов,Элементы.ШаблонПорученияПредставлениеСрока_БезПредметов);
	Элементы.ШаблонПоручения_БезПредметов.КнопкаОткрытия = Истина;
	
	// Выровняем срок исполнения.
	Элементы.СрокИсполненияПроцессаПредставление.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
	Элементы.СрокИсполненияПроцессаПредставление.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	...
	ПараметрыАдаптациии.ЭлементСрокОбработкиРезультатов = Элементы.ДеревоЗадачРезультатВыполнения;
	...
	ПараметрыАдаптациии.ЭлементПредметыКомандаДобавить = Элементы.ПредметыКонтекстноеМеню_ДобавитьПредмет;
...
&НаСервере
Процедура НастройкаСрокаШаблонаИГруппаШаблонМК(ГруппаШаблон, ЭлементШаблонСрок)
	
	ГруппаШаблон.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	ГруппаШаблон.Отображение = ОтображениеОбычнойГруппы.ОбычноеВыделение;
	ЭлементШаблонСрок.Заголовок = НСтр("ru = 'Срок'");
	ЭлементШаблонСрок.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
