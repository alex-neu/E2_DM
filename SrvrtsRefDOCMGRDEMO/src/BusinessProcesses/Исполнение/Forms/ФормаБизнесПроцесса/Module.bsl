...
#Если МобильныйКлиент Тогда
	// При создании нового процесса, активируем поле ввода текста.
	Если ЭтоНовый Тогда
...
Процедура ИсполнителиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если ВсеПройдены И Объект.Стартован Тогда 
		ПоказатьПредупреждение(,
			НСтр("ru = 'Все исполнители завершили свои задачи, добавление нового исполнителя невозможно!'"));
		Отказ = Истина;
	КонецЕсли;
	...
	Если Объект.Стартован Тогда 
		...
		Если ВыделенныеСтроки.Количество() = 1 Тогда 
			...
			Если ДанныеСтроки.Пройден Тогда 
				ПоказатьПредупреждение(,
					НСтр("ru = 'Задача текущего исполнителя уже завершена, удаление исполнителя невозможно!'"));
				Отказ = Истина;
				Возврат;
			КонецЕсли;
		Иначе
		...
		Если ВыделенныеСтроки.Количество() = Объект.Исполнители.Количество() Тогда 
			ПоказатьПредупреждение(,
				НСтр("ru = 'В списке исполнителей не останется ни одного исполнителя, удаление невозможно!'"));
			Отказ = Истина;
			Возврат;
		КонецЕсли;
...
Процедура ПереместитьВверх(Команда)
	...
	Если Объект.Стартован Тогда 
		Если ТекущиеДанные.Пройден 
			...
			ПоказатьПредупреждение(,
				НСтр("ru = 'Задача предыдущего исполнителя уже сформирована, изменение порядка исполнителей невозможно!'"));
...
Процедура ПереместитьВниз(Команда)
	...
	Если Объект.Стартован Тогда 
		Если ТекущиеДанные.Пройден 
			...
			ПоказатьПредупреждение(,
				НСтр("ru = 'Задача текущего исполнителя уже сформирована, изменение порядка исполнителей невозможно!'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	Элементы.ИсполнителиСрокИсполненияПредставление.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
	Элементы.ИсполнителиСрокИсполненияПредставление.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	Элементы.ИсполнителиСрокИсполненияПредставление.РастягиватьПоГоризонтали = Истина;
	
	// Настроим отображение кнопки подобрать и отв. исполнитель.
	Элементы.Подобрать.Отображение = ОтображениеКнопки.Текст;
	Элементы.НазначитьОтветственным.Отображение = ОтображениеКнопки.Текст;
	
	// Важность покажем после описания.
	Элементы.Переместить(Элементы.Важность, Элементы.ГруппаГлавное, Элементы.ГруппаИсполнители);
	...
	Элементы.Переместить(Элементы.ПереместитьВверх, Элементы.Исполнители.КонтекстноеМеню);
	Элементы.Переместить(Элементы.ПереместитьВниз, Элементы.Исполнители.КонтекстноеМеню);
	Элементы.Переместить(Элементы.НазначитьОтветственным, Элементы.Исполнители.КонтекстноеМеню);
	// Управление списком перенесем в ком. панель.
	Элементы.Переместить(Элементы.Подобрать, Элементы.Исполнители.КоманднаяПанель);
	Элементы.Переместить(Элементы.ИсполнителиКонтекстноеМенюОсобыеСрокиИОписаниеДляИсполнителей,
		Элементы.Исполнители.КоманднаяПанель);
	Элементы.Переместить(Элементы.ИсполнителиКонтекстноеМенюВывестиСписок, Элементы.Исполнители.КоманднаяПанель);
	Элементы.Исполнители.ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиЭлементаФормы.Верх;
	
	// Вариант согласования поставим после таблицы исполнителей.
	Элементы.Переместить(Элементы.ВариантИсполнения, Элементы.ГруппаИсполнители);
	...
	Элементы.ГруппаИсполнители.Заголовок =
	...
	// Расстянем группы по горизонтали.
	Элементы.ГруппаГруппаПроверяющийИКонтролерИСрокОбработкиРезультатовПредставление.РастягиватьПоГоризонтали = Истина;
	Элементы.ГруппаПроверяющийСрокОбработкиРезультатовКонтрольныйСрокПроцесса.РастягиватьПоГоризонтали = Истина;
	Элементы.ГруппаПроверяющийСрокОбработкиРезультатов.РастягиватьПоГоризонтали = Истина;
	Элементы.ГруппаКонтрольныйСрокПроцесса.РастягиватьПоГоризонтали = Истина;
	
	// Выровняем проверяюшего и сроки обработки.
	Элементы.Проверяющий.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
	...
	ПараметрыАдаптациии.ЭлементГруппаИсполнители = Элементы.ГруппаИсполнители;
