...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// вывод ответственного исполнителя
	НайденнаяСтрока = Объект.БизнесПроцесс.Исполнители.Найти(Истина, "Ответственный");
	Если НайденнаяСтрока <> Неопределено Тогда 
		ЗадачаОтвИсполнителя = НайденнаяСтрока.ЗадачаИсполнителя;
		Если ЗначениеЗаполнено(ЗадачаОтвИсполнителя.Исполнитель) Тогда 
			ОтветственныйИсполнительСтрокой = ЗадачаОтвИсполнителя.Исполнитель;
		Иначе
			ОтветственныйИсполнительСтрокой = Строка(ЗадачаОтвИсполнителя.РольИсполнителя);
		КонецЕсли;
		ОтветственныйИсполнитель = ЗадачаОтвИсполнителя.Исполнитель;
	Иначе	
		Элементы.ОтветственныйИсполнительСтрокой.Видимость = Ложь;
	КонецЕсли;
...
&НаКлиенте
Процедура ОтветственныйИсполнительСтрокойНажатие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ПоказатьЗначение(, ОтветственныйИсполнитель);
...
Процедура Исполнено(Команда)
	...
	Если Подзадачи.Количество() <> 0 Тогда
		...
		Если ЕстьАктивныеПодзадачи Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ПродолжениеИсполненоПослеВыбораДействияСПодзадачами",
				ЭтотОбъект);
...
&НаКлиенте
Процедура ПродолжениеИсполненоПослеВыбораДействияСПодзадачами(Результат, ДополнительныеПараметры) Экспорт
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	ЭлементыОсновное.Добавить(Элементы.ОтветственныйИсполнительСтрокой);
