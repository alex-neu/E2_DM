...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ВыполнитьЗадачуФоново")
		...
		Если ПараметрыЗаписи.Свойство("РезультатыПроверки") Тогда
			ПараметрыФоновогоВыполнения.Вставить("РезультатыПроверки", ПараметрыЗаписи.РезультатыПроверки);
		КонецЕсли;
...
Процедура ВыполнитьЗадачу(Команда)
	...
	ИсполненоПослеПодзадач(Команда); 
...
Процедура ИсполненоПослеВыбораДействияСПодзадачами(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = "ИгнорироватьПодзадачи" Тогда 
		...
		ИсполненоПослеПодзадач(ДополнительныеПараметры.Команда);
...
&НаКлиенте
Процедура ИсполненоПослеПодзадач(Команда) 

	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПродолжениеВыполненияЗадачиПослеВыбораФактическогоИсполнителя", ЭтаФорма, Команда);
...
Процедура ПродолжениеВыполненияЗадачиПослеВыбораФактическогоИсполнителя(
	ВыбранныйФактическийИсполнитель, Команда) Экспорт
...
Процедура ПродолжениеВыполненияЗадачиПослеИзмененияПараметров(Результат, Параметры) Экспорт
	...
	ПараметрыЗаписи = Новый Структура;
	Если Результат.Свойство("Исполнители") Тогда
		ПараметрыЗаписи.Вставить("Исполнители", Результат.Исполнители);
	КонецЕсли;
	Если Результат.Свойство("РезультатыПроверки") Тогда
		ПараметрыЗаписи.Вставить("РезультатыПроверки", Результат.РезультатыПроверки);
	КонецЕсли;
	
	Если Результат.Свойство("СрокИсполненияПроцесса") Тогда
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	Элементы.Завершить.Картинка = БиблиотекаКартинок.ЗадачаУспешноеВыполнение;
	Элементы.ВернутьНаДоработку.Картинка = БиблиотекаКартинок.ЗадачаУспешноеВыполнение;
