...
Функция ПодзадачиПроцесса(Процесс) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.Текст = РаботаСБизнесПроцессамиВызовСервера.ТекстЗапросаЗадачДляПодзадач();
	Запрос.УстановитьПараметр("БизнесПроцесс", Процесс); 
	МассивТекЗадач = Запрос.Выполнить().Выгрузить();
	
	Для Каждого Стр Из МассивТекЗадач Цикл
		
		СтруктураЗадачи = РаботаСБизнесПроцессамиВызовСервера.СтруктураПодзадачи();
		ЗаполнитьЗначенияСвойств(СтруктураЗадачи, Стр);
		Если ЗначениеЗаполнено(Стр.РольИсполнителя) Тогда
			Если Стр.ПринятаКИсполнению Тогда
				СтруктураЗадачи.Участник = Стр.ТекущийИсполнитель;
			Иначе
				СтруктураЗадачи.Участник = Стр.РольИсполнителя;
			КонецЕсли;	
		Иначе
			СтруктураЗадачи.Участник = Стр.Исполнитель;
		КонецЕсли;	
	...
	Исполнители = РеквизитыПроцессаПоСсылке.Исполнители.Выгрузить();
	Для Каждого Стр Из Исполнители Цикл
		
		Если Не ЗначениеЗаполнено(Стр.ЗадачаИсполнителя) Тогда
			
			СтруктураЗадачи = РаботаСБизнесПроцессамиВызовСервера.СтруктураПодзадачи();
			СтруктураЗадачи.БизнесПроцесс = Процесс;
			СтруктураЗадачи.Наименование = РеквизитыПроцессаПоСсылке.Наименование;
			
			СтруктураЗадачи.Участник = Стр.Исполнитель;
			
			Если ЗначениеЗаполнено(Стр.СрокИсполнения) Тогда
				СтруктураЗадачи.Срок = Стр.СрокИсполнения;
			КонецЕсли;	
			
			СтруктураЗадачи.ДатаСоздания = РеквизитыПроцессаПоСсылке.ДатаНачала + 86400;
...
Процедура ОбработкаПередВыполнениемЗадачи(
	...
	Если ТочкаМаршрутаБизнесПроцесса = БизнесПроцессы.Согласование.ТочкиМаршрута.Согласовать Тогда
		...
		// Выполняем помещение активной визы в историю, аналогично интерактивному
		// вопросу при выполнении задачи согласования.
		Если ПараметрыВыполнения.Свойство("ПоместитьАктивнуюВизуВИсторию")
			И ПараметрыВыполнения.ПоместитьАктивнуюВизуВИсторию Тогда
			
			ИсполнительВизы = Неопределено;
			Если ПараметрыВыполнения.Свойство("ИсполнительЗадачи") Тогда
				ИсполнительВизы = ПараметрыВыполнения.ИсполнительЗадачи;
			КонецЕсли;
			
			Если ПараметрыВыполнения.Свойство("ИсполнительЗадачиПоПочте") Тогда
				ИсполнительВизы = ПараметрыВыполнения.ИсполнительЗадачиПоПочте;
			КонецЕсли;
			
			НайденнаяВиза = Неопределено;
			Если РаботаСВизамиСогласования.УжеЕстьВизаИсполнителяЗадачи(
				ЗадачаСсылка, Неопределено, НайденнаяВиза, ИсполнительВизы) Тогда
				
				Если ЗначениеЗаполнено(НайденнаяВиза) Тогда
					РаботаСВизамиСогласования.ПоместитьВизыВИсторию(
						ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(НайденнаяВиза));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция ДополнительныеДанныеПоЗадаче(Задача) Экспорт
	...
	Если Задача.ДополнительныеСвойства.Свойство("ПредыдущийИсполнитель")
		...
		ПредставлениеИсполнителя = НСтр("ru = '%1 (вместо %2)'");
