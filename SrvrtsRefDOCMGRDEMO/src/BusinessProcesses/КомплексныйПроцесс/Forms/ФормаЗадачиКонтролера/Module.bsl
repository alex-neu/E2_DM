...
Процедура ЗаполнитьСписки()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ЗапущенныйБизнесПроцесс) Тогда
			НовыйЭтап.СрокВыполнения = Выборка.ЗапущенныйБизнесПроцесс.СрокИсполненияПроцесса;
		КонецЕсли;
...
Процедура ЗаполнитьВычисляемыеПоляЭтапов()
	...
	Для каждого РабочийЭтап Из СписокЭтапов Цикл
		...
		РабочийЭтап.ИсполнителиЭтапаСтрокой = МенеджерШаблона.ПолучитьСтроковоеПредставлениеИсполнителей(РеквизитыШаблона);
		
		Длительность = СрокиИсполненияПроцессов.ДлительностьИсполненияПроцесса(РабочийЭтап.ШаблонБизнесПроцесса);
		...
		РабочийЭтап.Срок = ДлительностьСтрокой;
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	УстановитьУсловноеОформлениеСписокЭтаповМК();
	...
	// Скроем ненужные команды.
	Элементы.ИзменитьДействие.Видимость = Ложь;
	Элементы.СписокЭтаповОткрытьБизнесПроцесс.Видимость = Ложь;
	Элементы.СписокЭтаповКонтекстноеМенюИзменитьДействие.Видимость = Ложь;
	Элементы.СписокЭтаповКонтекстноеМенюОткрытьБизнесПроцесс.Видимость = Ложь;
	Элементы.СписокЭтаповОписаниеДействия.Видимость = Ложь;
	Элементы.СписокЭтаповВажность.Видимость = Ложь;
	
	// Настроим поля у СписокЭтапов.
	Элементы.СписокЭтаповНомерЭтапа.Ширина = 1;
	Элементы.СписокЭтаповЗадачаВыполнена.Ширина = 0;
	...
	ПараметрыАдаптациии.СтраницыНазвания.Добавить(НСтр("ru='Этапы'"));
	ПараметрыАдаптациии.СтраницыЭлементы.Добавить(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Элементы.СписокЭтапов));
	...
	Элементы.Проконтролировано.Картинка = БиблиотекаКартинок.ЗадачаУспешноеВыполнение;

КонецПроцедуры

&НаСервере
Процедура УстановитьУсловноеОформлениеСписокЭтаповМК()
	
	// Добавим условное оформление цвет по важности.
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
	
	ЭлементОтбораДанных = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбораДанных.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СписокЭтапов.Важность");
	ЭлементОтбораДанных.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ЭлементОтбораДанных.ПравоеЗначение = 1;
	ЭлементОтбораДанных.Использование = Истина;
	
	ЭлементОбластиОформления = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
	ЭлементОбластиОформления.Поле = Новый ПолеКомпоновкиДанных("СписокЭтаповНомерЭтапа");
