...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "СозданиеДействия" Тогда
		...
		Если ИспользоватьСхемуПроцесса Тогда
			...
			ЗавершитьНастройкуЭлементаСхемыДействия(Параметр.ШаблонБП, Параметр.Предметы, ДатаОтсчетаДляДействия);
		Иначе
...
Процедура ЭтапыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Не РаботаСКомплекснымиБизнесПроцессамиКлиент.
		ЗаписатьНовыйПроцессПередНастройкойПодчиненногоЭлемента(ЭтаФорма) Тогда
...
//
&НаКлиенте
Процедура ЗавершитьНастройкуЭлементаСхемыДействия(Шаблон, Предметы, ДатаОтсчетаДляДействия)
...
Процедура НастроитьДоступностьРедактированияСхемы(Форма)
	...
	Если СхемаДоступнаДляРедактирования Тогда
		...
		ИначеЕсли Форма.СхемаКомплексногоПроцесса.ЭлементыСхемы.Количество() > 0
