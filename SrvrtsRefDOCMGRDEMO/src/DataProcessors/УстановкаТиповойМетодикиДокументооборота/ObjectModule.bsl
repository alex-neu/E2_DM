...
Процедура ЗаполнитьНастройкиПрограммы()
	...
	НаборКонстант.ПрограммаРаспознавания = Перечисления.ПрограммыРаспознавания.СервисРаспознавания;
	...
	НаборКонстант.ПрефиксУзлаРаспределеннойИнформационнойБазы = НСтр("ru = 'ГЛ'");
	...
	НаборКонстант.ПолитикаУчетаРабочегоВремени = Перечисления.ПолитикиУчетаРабочегоВремени.ИспользоватьЕжедневныеИЕженедельныеОтчеты;
...
Процедура ЗагрузитьНастройкиУведомлений()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Настройка = "" Или Выборка.ТипСобытия = "" Или Выборка.Событие = ""
			...
			ОписаниеОшибки = НСтр("ru = 'В макете имеются строки с пустыми полями.'");
...
Процедура ЗаполнитьПривязкиШаблонаКВидамДокументов(СтруктураШаблона, Выборка, ПустаяСтрока)
	...
	Если Выборка.НастройкиШаблонаИмяТипаВидаДокумента <> "" Тогда
		...
		НоваяНастройка.КомуНазначен = ВидДокумента;
...
Процедура ЗагрузитьШаблоныВнутреннихДокументов()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(СтруктураШаблона.Папка) Тогда
			ТекстОшибки = НСтр("ru = 'Не найдена папка внутренних документов ""%1""'");
...
Процедура ЗагрузитьНастройкиДопРеквизитов()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(Набор) Тогда
			Строка = НСтр("ru = 'Не найдено набора доп.реквизитов для свойства ""%1""'");
...
Процедура ЗагрузитьВнутренниеДокументы()
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктуруВнутреннегоДокумента.Источник = Справочники.ИсточникиДанных.УстановкаТиповойМетодики;
...
Функция ЗаполненныеДанныеВ_ИБ() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Псевдоним = "" Тогда
			ОписаниеОшибки = 
				НСтр("ru = 'В строке макета ПроверяемыеДанныеПередУстановкой не заполнено поле Псевдоним.'");
			ВызватьИсключение ОписаниеОшибки;
		КонецЕсли;
