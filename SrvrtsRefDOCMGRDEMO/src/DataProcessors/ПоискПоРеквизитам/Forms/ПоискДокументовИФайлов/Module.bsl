...
Процедура ВидПоискаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если НЕ ПроверитьНаличиеУсловия() Тогда
		ТекстВопроса = НСтр("ru = 'Изменение области поиска очистит все введенные условия поиска. Продолжить?'");
...
Процедура ВидПоискаОбработкаВыбораПродолжение(Ответ, Параметры) Экспорт
	...
	Состояние(НСтр("ru = 'Выполняется настройка формы для выбранной области поиска. Пожалуйста, подождите...'"));
	...
	Если НЕ Параметры.СтандартнаяОбработка Тогда
		Состояние();
		ТекстСообщения = НСтр("ru = 'Недостаточно прав для поиска в указанной области.'");
...
Процедура ВыполнитьПоиск(Команда = Неопределено)
	...
	Если ТипПоиска = "РасширенныйПоиск" И НЕ ЗначениеЗаполнено(ТипОбъекта) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не выбран тип объектов для поиска'"),,,"ТипОбъекта");
		Возврат;
	КонецЕсли;
	
	Состояние(НСтр("ru ='Выполняется поиск. Пожалуйста, подождите...'"));
	Попытка
	...
	Исключение
		...
		Если Инфо.Описание = "НетУсловийПоиска" Тогда
			Состояние();
			ТекстСообщения = НСтр("ru = 'Укажите хотя бы одно условие поиска!'");
			ПоказатьПредупреждение(, ТекстСообщения);
		ИначеЕсли Инфо.Описание = "НичегоНеНайдено" Тогда 
			Состояние();
			ТекстСообщения = НСтр("ru = 'Ничего не найдено. Уточните условия поиска.'");
			ПоказатьПредупреждение(, ТекстСообщения);
		Иначе
			ВызватьИсключение;
		КонецЕсли;
...
Процедура УдалитьСохраненныйПоиск(Команда)
	...
	Если Элементы.СписокСохраненныхПоисков.ВыделенныеСтроки.Количество() = 1 Тогда
		...
		КонецЕсли;
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Удалить сохраненный поиск ""%1""?'"), 
	...
	Иначе
		...
		КонецЕсли;
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Удалить сохраненные поиски (%1)?'"), 
...
Процедура СброситьРезультатыПоиска(ПравоНаВнутренние = Ложь, ПравоНаВходящие = Ложь, ПравоНаИсходящие = Ложь, ПравоНаФайлы = Ложь)
	...
	Если ПравоНаФайлы Тогда
		...
		РезультатФайлы.Параметры.УстановитьЗначениепараметра("ТекущийПользователь", Пользователи.ТекущийПользователь());
	КонецЕсли;
...
Процедура ВыполнитьПоискСервер()
	...
	Если Элементы.Найти(ИмяЗначенияПеречисления + "СодержимоеФайлов") <> Неопределено
		...
		Если Найти(НРег(Строка(ВидПоиска)), "файл") Тогда
			ФайловоеДобавление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = ', содержащие ""%1""'"),
				ЭтаФорма[ИмяЗначенияПеречисления + "СодержимоеФайлов"]);
		Иначе
			ФайловоеДобавление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = ', файлы которых содержат ""%1""'"),
				ЭтаФорма[ИмяЗначенияПеречисления + "СодержимоеФайлов"]);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(УсловияОтбора) Тогда
		Если Найти(НРег(Строка(ВидПоиска)), "файл") Тогда
			СтроковоеОписаниеПоисковогоЗапроса = СтроковоеОписаниеПоисковогоЗапроса + НСтр("ru =', у которых '") + УсловияОтбора;
		Иначе
			Если Элементы.Найти(ИмяЗначенияПеречисления + "СодержимоеФайлов") <> Неопределено
				И ЗначениеЗаполнено(ЭтаФорма[ИмяЗначенияПеречисления + "СодержимоеФайлов"]) Тогда 
	...
	ЭтаФорма.Команды["ВыполнитьПоиск"].Подсказка = НСтр("ru = 'Повторить поиск'");
		...
		Для Каждого Параметр Из ПараметрыОтбора Цикл
			Если Параметр.Использование Тогда
				...
				Если ТипЗнч(Параметр.Значение) = Тип("Массив")
					...
					ВидСравненияЗначений = НСтр("ru = 'в списке'");
				...
				Иначе
					ПравоеЗначение = НСтр("ru = '""не заполнено""'");
				КонецЕсли;
...
Функция ВыполнитьЗапрос(КомпоновщикНастроек, ТипПоиска)
	...
	Элементы.РезультатВнутренниеДокументы.Заголовок = НСтр("ru = 'Внутренние документы'");
	Элементы.РезультатВходящиеДокументы.Заголовок = НСтр("ru = 'Входящие документы'");
	Элементы.РезультатИсходящиеДокументы.Заголовок = НСтр("ru = 'Исходящие документы'");
	Элементы.РезультатФайлы.Заголовок = НСтр("ru = 'Файлы'");
	
	Если КолвоВнутренниеДокументы > 0 Тогда
		Элементы.РезультатВнутренниеДокументы.Заголовок = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Внутренние документы (%1)'"),
				Строка(КолвоВнутренниеДокументы));
	КонецЕсли;
	Если КолвоВходящиеДокументы > 0 Тогда
		Элементы.РезультатВходящиеДокументы.Заголовок = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Входящие документы (%1)'"),
				Строка(КолвоВходящиеДокументы));
	КонецЕсли;
	Если КолвоИсходящиеДокументы > 0 Тогда
		Элементы.РезультатИсходящиеДокументы.Заголовок = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Исходящие документы (%1)'"),
	...
	Если СуммаВсего > 0 Тогда
		Если ТипПоиска = "ПоискФайлы"  Тогда 
			Элементы.ДекорацияКоличество.Заголовок = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Найдено файлов: %1.'"),
					Строка(СуммаВсего));
		Иначе
			Элементы.ДекорацияКоличество.Заголовок = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Найдено документов: %1.'"),
					Строка(СуммаВсего));
		КонецЕсли;
	Иначе
		Элементы.ДекорацияКоличество.Заголовок = НСтр("ru = 'Документы и файлы не найдены.'");
	КонецЕсли;
...
Процедура ОбновитьСоставРеквизитовДляПоиска()
	...
	Если Найти(ИмяЗначенияПеречисления, "Документ") > 0 ИЛИ Найти(ИмяЗначенияПеречисления, "Файл") > 0 Тогда
		...
		Если ПравоДоступа("Чтение", Метаданные.Справочники[ИмяЗначенияПеречисления]) Тогда
			...
			НаборСвойствОбъекта = УправлениеСвойствами.СвойстваОбъекта(Объект);
...
Процедура ПереключитьВидимостьОписанияЗапроса()
	...
	Элементы.ОписаниеПоисковогоЗапроса.Видимость = ОписаниеОтбораПоказывается;
	Если ОписаниеОтбораПоказывается Тогда
		Элементы.РазвернутьСкрытьОписаниеЗапроса.Заголовок = НСтр("ru = 'Скрыть подробности'");
		ЭтаФорма.Команды.Найти("РазвернутьСкрытьОписаниеЗапроса").Подсказка = НСтр("ru = 'Скрыть условия поиска'");
	Иначе
		Элементы.РазвернутьСкрытьОписаниеЗапроса.Заголовок = НСтр("ru = 'Подробнее...'");
		ЭтаФорма.Команды.Найти("РазвернутьСкрытьОписаниеЗапроса").Подсказка = НСтр("ru = 'Показать условия поиска'");
	КонецЕсли;
...
Процедура ИзменитьКоманднуюПанельПриВозвратеКУсловиям()
	...
	ЭтаФорма.Команды["ВыполнитьПоиск"].Подсказка = НСтр("ru = 'Искать'");
...
Процедура ЗагрузитьСписокСохраненныхПараметровПоиска(КлючНастроек = Неопределено)
	...
	НоваяСтрока.ИмяСохраненногоПоиска = НСтр("ru = 'Новый поиск'");
...
Процедура ОткрытьНастройку()
	...
	Если Элементы.ГруппаПараметрыИРезультатыПоиска.ТекущаяСтраница <> Элементы.ГруппаРезультатыПоиска
		...
		Иначе
			...
			Если НЕ ПроверитьНаличиеУсловия() Тогда
				Режим = РежимДиалогаВопрос.ДаНетОтмена;
				ТекстВопроса = НСтр("ru = 'Сохранить введенные условия поиска в качестве шаблона?'");
				ОписаниеОповещения = Новый ОписаниеОповещения(
...
Процедура СохранитьПоискПродолжение(Результат, Параметры) Экспорт
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Текущие условия поиска сохранены как ""%1"".'"), 
...
Процедура УстановитьДоступностьКоманд(РедактируетТекущийПользователь, Редактирует, ПодписанЭП, Зашифрован)
	...
	ИспользоватьImageMagickДляРаспознаванияPDF = 
		ФайловыеФункцииКлиентПовтИсп.ПолучитьПерсональныеНастройкиРаботыСФайлами().ИспользоватьImageMagickДляРаспознаванияPDF;
	...
	Элементы.ФайлыСписокРаспознать.Доступность = 
		ДоступноРаспознаваниеПоЗапросу И РасширениеПоддерживается И Редактирует.Пустая();
	
	Элементы.ФайлыСписокКонтекстноеМеню.ПодчиненныеЭлементы.ФайлыСписокКонтекстноеМенюРаспознать.Доступность = 
		ДоступноРаспознаваниеПоЗапросу И РасширениеПоддерживается И Редактирует.Пустая();
