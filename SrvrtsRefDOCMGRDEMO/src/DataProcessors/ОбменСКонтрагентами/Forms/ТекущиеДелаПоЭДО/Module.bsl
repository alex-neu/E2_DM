...
Процедура ВходящиеЭДПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	|	ВЫБОР
	|		КОГДА Основания.ДокументОснование ССЫЛКА Справочник.ВерсииФайлов
	|			ТОГДА ПРЕДСТАВЛЕНИЕ(ВЫРАЗИТЬ(Основания.ДокументОснование КАК Справочник.ВерсииФайлов).Владелец.ВладелецФайла)
	|		ИНАЧЕ ПРЕДСТАВЛЕНИЕ(Основания.ДокументОснование)
	|	КОНЕЦ КАК УчетныйДокументПредставление
	|ИЗ
	...
	Для каждого ТекущаяСтрока Из Строки Цикл
		...
		Иначе
			Данные.ОтражениеВУчете = НСтр("ru = '<Отсутствует>'");
		КонецЕсли;
...
Процедура ПоставитьВОчередьПриема(Команда)
	...
	ОбменСКонтрагентамиДОСлужебныйКлиент.ПоставитьЭДВОчередьПриема(МассивСсылок);
...
Процедура ПоказатьКомандыРаздела(Знач Раздел)
	...
	КомандыРазделов.Вставить("Исправить", "Обновить, ЗакрытьПринудительно, Перенаправить, КомандыВходящиеЭД, ПоставитьВОчередьПриема");
	...
	ЭлементыКоманд.Вставить("ПоставитьВОчередьПриема", Элементы.КомандаПоставитьВОчередьПриема);
...
Функция УчетныеДокументыЭлектронногоДокумента(Знач ЭлектронныйДокумент)
	...
	|	ВЫБОР
	|		КОГДА Основания.ДокументОснование ССЫЛКА Справочник.ВерсииФайлов
	|			ТОГДА ВЫРАЗИТЬ(Основания.ДокументОснование КАК Справочник.ВерсииФайлов).Владелец.ВладелецФайла
	|		ИНАЧЕ Основания.ДокументОснование
