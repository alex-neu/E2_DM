...
Функция ФайлОписанияВыгрузки(Организация, ТЗОписи, АдресКаталога)
	...
	Попытка
		...
		ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(Файл, "ДатаВыгрузки", Формат(ДатаВыгрузки, "ДФ=dd.MM.yyyy"), Истина, Ошибки);
		ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(Файл, "ВремяВыгрузки", Формат(ДатаВыгрузки, "ДФ=HH:mm:ss"), Истина, Ошибки);
...
Функция ВидДокументаПоВидуЭД(ВидЭД, ТипЭлементаВерсииЭД)
	...
	// "01" - СчетФактура 
	// "02" - АктПриемкиСдачиРабот 
	// "03" - ТоварнаяНакладнаяТОРГ12 
	// "04" - КорректировочныйСчетФактура 
	// "05" - ПередачаТоваров 
	// "06" - ПередачаУслуг 
	// "07" - УПД 
	// "08" - УКД
...
Функция ВидЭДСтрокой(ВидЭД, ТипЭлементаВерсииЭД)
	...
	ИначеЕсли ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.АктНаПередачуПрав")
		И ТипЭлементаВерсииЭД = ПредопределенноеЗначение("Перечисление.ТипыЭлементовВерсииЭД.ДОПУПД") Тогда
		ВидЭДСтрокой = "УПД";
