	...
	Пока НЕ Recordset.EOF Цикл
		...
		Если Группа = 0 Тогда 
			...
			Если НовыйПользователь = Неопределено Или НовыйПользователь.Пустая() Тогда
				...
				Результат = Пользователи.УстановитьСвойстваПользователяИБ(
					ИмяПользователя,
...
Процедура ЗагрузкаВходящиеДокументы(МассивСвязей)
	...
	Пока НЕ Recordset.EOF Цикл
		...
		НовыйДокумент.Источник = Справочники.ИсточникиДанных.ЗагрузкаИз1САрхив;
...
Процедура ЗагрузитьДанные(Команда)
	...
	Состояние(НСтр("ru = 'Выполняется загрузка пользователей'"));
	ЗагрузкаПользователей();
	
	МассивСвязей = Новый Массив();
	Состояние(НСтр("ru = 'Выполняется загрузка входящих документов'"));
	ЗагрузкаВходящиеДокументы(МассивСвязей);
	Состояние(НСтр("ru = 'Выполняется загрузка исходящих документов'"));
	ЗагрузкаИсходящиеДокументы(МассивСвязей);	
	
	Состояние(НСтр("ru = 'Выполняется установка связей между входящими и исходящими документами'"));
	УстановитьСвязи(МассивСвязей);
	
	Состояние(НСтр("ru = 'Выполняется загрузка папок'"));
	ЗагрузкаПапок();
	
	Состояние(НСтр("ru = 'Выполняется загрузка файлов'"));
	ЗагрузкаФайлы();
	
	Состояние(НСтр("ru = 'Выполняется загрузка версий файлов'"));
	ЗагрузкаПодверсии();
	
	Состояние(НСтр("ru = 'Выполняется загрузка рабочих каталогов пользователей'"));
	ЗагрузкаРабочиеКаталогиФайлов();
	
	Если СтрДлина(Ошибки) = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Загрузка данных из 1С:Архива 3.0 успешно завершена!'"));
	Иначе	
