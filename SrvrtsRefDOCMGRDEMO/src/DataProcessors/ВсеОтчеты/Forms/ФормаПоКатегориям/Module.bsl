...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Заголовок = НСтр("ru = 'Панель отчетов'");
	КонецЕсли;
...
Процедура ОбновитьДеревоОтчетов()
	...
	СтрокиПустыхКатегорий = Новый Массив;
	...
	Пока ВыборкаКатегории.Следующий() Цикл
		...
		Если НЕ НеОтображатьИерархию  И ЗначениеЗаполнено(ВыборкаКатегории.Категория)  Тогда
			КоличествоОтчетов = ВетвьОтчет.Строки.Количество();
			ВетвьОтчет.Синоним = ВыборкаКатегории.Категория.Наименование + 
				" (" + Строка(КоличествоОтчетов) + ")";
			Если КоличествоОтчетов = 0 Тогда
				СтрокиПустыхКатегорий.Добавить(ВетвьОтчет);
			КонецЕсли;
		КонецЕсли;
		
	КонецЦикла;
	
	Для Каждого СтрокаКУдалению Из СтрокиПустыхКатегорий Цикл
...
Процедура СформироватьГиперссылкуНаОтчетыРаздела()
	...
	Если ЗначениеЗаполнено(РазделГипперссылка) Тогда
		...
		Элементы.ОтчетыРаздела.Заголовок = СтрШаблон(НСтр("ru = 'Все отчеты раздела %1'"), РазделГипперссылка.Синоним);
	Иначе
