...
&НаКлиенте
Процедура ИспользоватьАвтоматическуюПоддержкуЭДОПриИзменении(Элемент)
	
	ИзменитьИспользованиеЗадания("ПоддержкаЭДО", ИспользоватьАвтоматическуюПоддержкуЭДО);
	Элементы.ПоддержкаЭДО.Доступность = ИспользоватьАвтоматическуюПоддержкуЭДО;
...
&НаКлиенте
Процедура НастроитьПоддержкаЭДО(Команда)
	
	ОткрытьНастройкуРасписанияОбмена("ПоддержкаЭДО");
...
&НаКлиенте
Процедура НастроитьСписокПолучателейОПроблемахЭДО(Команда)
	
	ПараметрыОткрытия = Новый Структура;
	
	НастраиваемыеРазделы = Новый Массив;
	НастраиваемыеРазделы.Добавить(ПредопределенноеЗначение(
		"Перечисление.РазделыУведомленийОПроблемах.ОтправкаПоЭДО"));
	НастраиваемыеРазделы.Добавить(ПредопределенноеЗначение(
		"Перечисление.РазделыУведомленийОПроблемах.ПриемПоЭДО"));
	
	ПараметрыОткрытия.Вставить("НастраиваемыеРазделы", НастраиваемыеРазделы);
	ПараметрыОткрытия.Вставить("ЗаголовокФормы", "Получатели уведомлений о проблемах обмена ЭДО");
	
	ОткрытьФорму("Обработка.НастройкаПрограммы.Форма.ПолучателиУведомленийОПроблемах", ПараметрыОткрытия);
...
Процедура ПрочитатьУстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если ЕстьОбменСКонтрагентами
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ГруппаОтключитьНемедленнуюОтправкуЭД", "Видимость",
			ЭлектроннаяПодпись.СоздаватьЭлектронныеПодписиНаСервере());
...
Процедура УстановитьНастройкиЗаданий()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами") Тогда
		...
		РегламентноеЗадание = РегламентноеЗаданиеПоНаименованию("ПоддержкаЭДО");
		ЭлементНастройкиРасписания = Элементы.ПоддержкаЭДО;
		ВидимостьФлажка = (РегламентноеЗадание <> Неопределено);
		Если ВидимостьФлажка Тогда
			ИспользоватьАвтоматическуюПоддержкуЭДО = РегламентноеЗадание.Использование;
			ЭлементНастройкиРасписания.Заголовок = ТекстРасписанияРегламентногоЗадания(РегламентноеЗадание);
			ЭлементНастройкиРасписания.Доступность = РегламентноеЗадание.Использование;
		КонецЕсли;
		ВидимостьРасписания = ВидимостьФлажка И Не РазделениеВключено И ЭтоАдминистраторСистемы;
