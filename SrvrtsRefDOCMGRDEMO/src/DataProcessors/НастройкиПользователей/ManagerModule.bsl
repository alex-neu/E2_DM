...
Процедура ЗаполнитьДеревоГрупп(ДеревоГрупп, ПользовательВнешний) Экспорт
	...
	Иначе
		ПустаяГруппа = Справочники.РабочиеГруппы.ПустаяСсылка();
	КонецЕсли;
...
Функция ГруппыПользователей(ПользовательВнешний)
	...
	|ИЗ
	|	Справочник.РабочиеГруппы КАК СправочникГруппыПользователей";
	Если ПользовательВнешний Тогда 
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Справочник.РабочиеГруппы", "Справочник.ГруппыВнешнихПользователей");
	КонецЕсли;
...
Функция ПолныйСоставГруппПользователей(ГруппыПользователейСостав)
	...
	Для Каждого СтрокаСостава Из ГруппыПользователейСостав Цикл
		...
		Если ТипЗнч(СтрокаСостава.ГруппаПользователей) = Тип("СправочникСсылка.РабочиеГруппы")
