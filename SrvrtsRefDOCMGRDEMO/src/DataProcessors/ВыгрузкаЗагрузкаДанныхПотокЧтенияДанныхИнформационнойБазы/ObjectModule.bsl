	...
	Если ПотокЧтения.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
		...
		Если ПотокЧтения.Имя = "Artefacts" Тогда
			...
			Пока ПотокЧтения.ТипУзла <> ТипУзлаXML.КонецЭлемента Цикл
				...
				ПотокЧтенияАртефакта = Неопределено;
				...
				Исключение
					...
					Если ПотокЧтенияАртефакта = Неопределено Тогда
						ВызватьИсключениеЧтенияXML(ФрагментАртефакта, ИсходноеИсключение);
					Иначе
						// Пробуем без проверки типа (обход платформенных ошибок сериализации объектов).
						Попытка
							Артефакт = ФабрикаXDTO.ПрочитатьXML(ПотокЧтенияАртефакта);
						Исключение
							ВызватьИсключениеЧтенияXML(ФрагментАртефакта, ИсходноеИсключение);
						КонецПопытки;
					КонецЕсли;
