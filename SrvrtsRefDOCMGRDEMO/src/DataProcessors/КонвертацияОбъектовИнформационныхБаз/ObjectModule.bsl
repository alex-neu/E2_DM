...
Процедура ЗагрузитьТабличнуюЧасть(Объект, ИмяТабличнойЧасти, ОбщаяИнформацияОТипеДокумента, ПараметрыОбъекта, ПКО)
	...
	// Не нужно загружать пустую табличную часть в объект, у которого ее нет.
	Если КоллекцияСортировки.Количество() = 0
		И Не ТабличнаяЧастьИспользуетсяДляОбъекта(Объект, ИмяТабличнойЧасти) Тогда
		Возврат;
	КонецЕсли;
			...
			ИначеЕсли ИмяТипаОбъекта <> "Перечисление" Тогда
				...
				Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
					...
					Если НЕ (ИмяТипаОбъекта = "РегистрСведений"
						...
						Если ЗаписатьОбъект
							И Объект <> Неопределено
							И ИмяТипаОбъекта <> "Константы" // для обмена константами
							И Объект.Ссылка <> Неопределено Тогда
...
Функция ТабличнаяЧастьИспользуетсяДляОбъекта(Объект, ИмяТабличнойЧасти)
	Возврат ЭтоОбъектСИерархиейГрупп(Объект)
		И ТабличнаяЧастьИспользуетсяДляОбъектаСИерархиейГрупп(Объект, ИмяТабличнойЧасти);
КонецФункции

Функция ЭтоОбъектСИерархиейГрупп(Объект)
	Если Метаданные.Справочники.Содержит(Объект.Метаданные()) Тогда
		Возврат Объект.Метаданные().Иерархический
			И Объект.Метаданные().ВидИерархии = Метаданные.СвойстваОбъектов.ВидИерархии.ИерархияГруппИЭлементов;
...
Функция ТабличнаяЧастьИспользуетсяДляОбъектаСИерархиейГрупп(Объект, ИмяТабличнойЧасти)
	Использование = Объект.Метаданные().ТабличныеЧасти[ИмяТабличнойЧасти].Использование;
	Возврат Использование = Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляГруппыИЭлемента
		Или (Объект.ЭтоГруппа И Использование = Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляГруппы)
		Или (Не Объект.ЭтоГруппа И Использование = Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляЭлемента);
КонецФункции
