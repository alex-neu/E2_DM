...
// Открывает форму выбора настроек распознавания.
//
// Параметры:
//  Настройки - Структура - настройки распознавания для программы.
//   * СтратегияРаспознавания - ПеречислениеСсылка.СтратегииРаспознаванияТекста
//   * ЯзыкРаспознавания - Строка - номер языка распознавания.
//  ФормаВладелец - УправляемаяФорма - форма из которой выполняется выбор настроек распознавания.
//  ОбработчикВыбора - ОписаниеОповещения - процедура с кодом, которая обрабатывает
//                     выбранные настройки. В нее в качестве результата передается
//                     структура соответствующая параметру Настройки.
//
Процедура ВыбратьНастройкиРаспознавания(Настройки, ФормаВладелец, ОбработчикВыбора) Экспорт
	
	ПрограммаРаспознавания = РаботаСФайламиКлиентПовтИсп.ПрограммаРаспознавания();
	
	ИмяФормы = Неопределено;
	Если ПрограммаРаспознавания = ПредопределенноеЗначение("Перечисление.ПрограммыРаспознавания.CuneiForm") Тогда
		ИмяФормы = "Справочник.Файлы.Форма.НастройкаРаспознаванияCuneiForm";
	ИначеЕсли ПрограммаРаспознавания = ПредопределенноеЗначение("Перечисление.ПрограммыРаспознавания.СервисРаспознавания") Тогда
		ИмяФормы = "Справочник.Файлы.Форма.НастройкаСервисаРаспознавания";
	КонецЕсли;
...
// Открывает форму выбора настроек распознавания файлов по запросу.
//
// Параметры:
//  ОбработчикВыбора - ОписаниеОповещения - процедура с кодом, которая обрабатывает
//                     выбранные настройки. В нее в качестве результата передается
//                     структура соответствующая параметру Настройки.
//  УточнитьСтратегиюПоВозможности - Булево - определяет будет-ли уточнена стратегии распознавания,
//                                            если это в принципе допустимо.
//
Процедура ВыбратьНастройкиРаспознаванияФайлаПоЗапросу(ОбработчикВыбора, УточнитьСтратегиюПоВозможности = Истина) Экспорт
	...
	ИмяФормы = Неопределено;
	Если ПрограммаРаспознавания = ПредопределенноеЗначение("Перечисление.ПрограммыРаспознавания.CuneiForm") Тогда
		ИмяФормы = "Справочник.Файлы.Форма.НастройкаРаспознаванияПоКомандеCuneiForm";
		ПараметрыФормы.Вставить("УточнитьСтратегию", УточнитьСтратегиюПоВозможности);
	ИначеЕсли ПрограммаРаспознавания = ПредопределенноеЗначение("Перечисление.ПрограммыРаспознавания.СервисРаспознавания") Тогда
		ИмяФормы = "Справочник.Файлы.Форма.НастройкаСервисаРаспознаванияПоКоманде";
	КонецЕсли;
...
Функция ВозможностьОбновитьФайл(ОбъектСсылка, РедактируетТекущийПользователь, Редактирует, СтрокаОшибки = "") Экспорт
	...
	ИначеЕсли Не ЗначениеЗаполнено(Редактирует) Тогда
		СтрокаОшибки = СтрШаблон(
			НСтр("ru = 'Нельзя обновить файл ""%1"" т.к. он никем не занят.'"), Строка(ОбъектСсылка));
		Возврат Ложь;
	Иначе
		Если ФайловыеФункцииСлужебныйКлиентСервер.ПерсональныеНастройкиРаботыСФайлами().ЭтоПолноправныйПользователь Тогда
			Возврат Истина;
		...
		СтрокаОшибки = СтрШаблон(
			НСтр("ru = 'Нельзя обновить файл ""%1"" т.к. он занят пользователем ""%2"".'"),
			Строка(ОбъектСсылка),
			Строка(Редактирует));
		Возврат Ложь;
	КонецЕсли;
...
Функция ВыбратьПутьКРабочемуКаталогу(ИмяКаталога, Заголовок, РабочийКаталогВладельца) Экспорт
	...
	Если ДиалогОткрытияФайла.Выбрать() Тогда
		...
		ИмяКаталога = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ИмяКаталога, ОбщегоНазначенияКлиент.ТипПлатформыКлиента());
...
Процедура ОткрытьФайлПриложением(ДанныеФайла, ИмяОткрываемогоФайла, УникальныйИдентификатор = Неопределено)
	...
	Если НРег(ДанныеФайла.Расширение) = НРег("xml") Тогда
		ПараметрыПросмотра = ОбменСКонтрагентамиДОВызовСервера.ПолучитьПараметрыПросмотраЭД(
			ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ИмяОткрываемогоФайла), УникальныйИдентификатор), 
			ИмяОткрываемогоФайла, 
			УникальныйИдентификатор);
			
		Если ЗначениеЗаполнено(ПараметрыПросмотра) Тогда
			ОткрытьФорму(
				"Обработка.ОбменСКонтрагентами.Форма.ФормаПросмотраЭДДО", 
				Новый Структура("Объект, ИдентификаторФормыВладельца", ДанныеФайла.Владелец, УникальныйИдентификатор),
				,
				ПараметрыПросмотра.УникальныйИдентификатор);
	...
	Если РасширениеПодключено Тогда
		...
		HtmlФайлыСпособОткрытия = ПерсональныеНастройкиРаботыСФайлами.HtmlФайлыСпособОткрытия;
		Если HtmlФайлыСпособОткрытия = ПредопределенноеЗначение("Перечисление.СпособыОткрытияФайлаНаПросмотр.ВоВстроенномРедакторе") Тогда
			
			HtmlФайлыРасширение = ПерсональныеНастройкиРаботыСФайлами.HtmlФайлыРасширение;
			Если ФайловыеФункцииКлиентСервер.РасширениеФайлаВСписке(HtmlФайлыРасширение, ДанныеФайла.Расширение) Тогда
				
				ПараметрыФормы = Новый Структура("Файл, ДанныеФайла, ИмяОткрываемогоФайла, УникальныйИдентификатор", 
					ДанныеФайла.Ссылка, ДанныеФайла, ИмяОткрываемогоФайла, УникальныйИдентификатор);
					
				ОткрытьФорму("Справочник.Файлы.Форма.РедактированиеHtmlФайла", 
					ПараметрыФормы, , ДанныеФайла.КодФайла);
				Возврат;
		...
		ИзображенияСпособОткрытия = ПерсональныеНастройкиРаботыСФайлами.ИзображенияСпособОткрытия;
		Если ИзображенияСпособОткрытия = ПредопределенноеЗначение(
			"Перечисление.СпособыОткрытияФайлаНаПросмотр.ВоВстроенномРедакторе")
			
			И ФайловыеФункцииСлужебныйКлиентСервер.РасширениеФайлаВСписке(
				ИзображенияРасширение, ДанныеФайла.Расширение)
			
			И ДанныеФайла.НаЧтение = Истина Тогда
			
			ПараметрыФормы = Новый Структура(
				"Файл, ДанныеФайла, ИмяОткрываемогоФайла", ДанныеФайла.Ссылка, ДанныеФайла, ИмяОткрываемогоФайла);
			
			ОткрытьФорму("Справочник.Файлы.Форма.ПросмотрИзображений", 
				ПараметрыФормы, УникальныйИдентификатор, ДанныеФайла.КодФайла);
...
Процедура ПриЗавершенииРаботыСистемы(Предупреждения) Экспорт
	...
	ТекстГиперСсылки = СтрШаблон(Нстр("ru='Открыть список редактируемых файлов (%1)'"), 
		КоличествоЗанятыхФайлов);
...
Функция ПроверитьЗанятыеФайлыПриЗавершенииРаботы()
	...
	ТекущийПользователь = СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиента().ТекущийПользователь;
...
Процедура СохранитьФайлНаДиск(ДанныеФайла, ВыбранноеПолноеИмяФайла, АдресФайла, Файл, ПутьКФайлуВКеше, СохранятьСРасшифровкой)
	...
	Если ПутьКФайлуВКеше = "" Тогда
		...
		ПутьКФайлу = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ПутьКФайлу, ОбщегоНазначенияКлиент.ТипПлатформыКлиента());
...
Процедура ОткрытьФайлСОповещением(ОбработчикРезультата, ДанныеФайла, УникальныйИдентификатор = Неопределено) Экспорт
	...
	Если ДанныеФайла.Свойство("ЗанятСДругогоКомпьютера") И ДанныеФайла.ЗанятСДругогоКомпьютера Тогда
		...
		ТекстСообщения = ТекстСообщения + Символы.ВК + Символы.ВК 
			+ НСтр("ru = 'Сейчас файл (предыдущей версии) будет открыт только для просмотра.'");
...
Процедура ОткрытьФайлСОповещениемСРасширениемПослеПолученияВерсииВРабочийКаталог(Результат, ПараметрыВыполнения) Экспорт
	...
	Если Результат.ФайлПолучен = Истина Тогда
		ОткрытьФайлПриложением(ПараметрыВыполнения.ДанныеФайла, Результат.ПолноеИмяФайла, ПараметрыВыполнения.УникальныйИдентификатор);
	КонецЕсли;
...
Процедура ОчиститьРабочийКаталогПриУстаревании() Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Выполняется очистка основного рабочего каталога...'"));
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Завершена очистка основного рабочего каталога.'"));
...
Процедура ОсвободитьФайлыПоСсылкамПослеОтветаНаВопросОтменитьРедактирование(Ответ, ПараметрыВыполнения) Экспорт
	...
	Для Каждого ДанныеФайла Из ПараметрыВыполнения.ДанныеФайлов Цикл
		...
		ВладелецФайлов = ДанныеФайла.Владелец;
...
Процедура ЗанятьФайлыПоСсылкамПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	Для Каждого ДанныеФайла Из ДанныеФайлов Цикл
		...
		Если Не РаботаСФайламиВызовСервера.ЗанятьФайл(ДанныеФайла, СтрокаОшибки, ДатаЗаема) Тогда 
			ОписанияОшибок.Вставить(ДанныеФайла.Ссылка, СтрокаОшибки);
			Продолжить;
		КонецЕсли;
	...
	ТекстОшибок = "";
	Для Каждого КлючИЗначения Из ОписанияОшибок Цикл
		ТекстОшибок = ТекстОшибок + Строка(КлючИЗначения.Ключ) + ": " + КлючИЗначения.Значение;
		ТекстОшибок = ТекстОшибок + Символы.ВК;
	КонецЦикла;	
...
Процедура ЗакончитьРедактированиеБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, ВыбранноеИмяФайла, ПараметрыВыполнения) Экспорт
	...
	// даты нет на руках, если без расширения - ставим текущую
...
Процедура ОбновитьИзФайлаНаДиске(ОбработчикРезультата, ДанныеФайла, ИдентификаторФормы) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		МодульЭлектроннаяПодписьКлиент =
			ОбщегоНазначенияКлиент.ОбщийМодуль("ЭлектроннаяПодписьКлиент");
		
		Если МодульЭлектроннаяПодписьКлиент.ОбщиеНастройки().ИспользоватьШифрование Тогда
			РасширениеДляЗашифрованныхФайлов =
				МодульЭлектроннаяПодписьКлиент.ПерсональныеНастройки(
					).РасширениеДляЗашифрованныхФайлов;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьИзФайлаНаДискеЗапуск(ПараметрыВыполнения) Экспорт
	...
	ПараметрыВыполнения.Вставить("РасширениеДляЗашифрованныхФайлов",
		ЭлектроннаяПодписьКлиент.ПерсональныеНастройки().РасширениеДляЗашифрованныхФайлов);
	...
	Если ПараметрыВыполнения.ДобавлениеОригинала Тогда 
		...
		Если МассивРасширенийСканКопий.Найти(ТолькоРасширение) = Неопределено Тогда 
			...
			ТекстВопроса = СтрШаблон(НСтр("ru = 'Выбранный файл не является скан-копией.
                                 |Необходимо выбрать файл с расширением: %1'"), СтрокаРасширений);
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить(КодВозвратаДиалога.Повторить, НСтр("ru = 'Выбрать другой файл'"));
			Кнопки.Добавить(КодВозвратаДиалога.Отмена);
			Обработчик = Новый ОписаниеОповещения("ДобавитьИзФайловойСистемыБезРасширенияПослеОтветаНаВопросПовторить", ЭтотОбъект, ПараметрыВыполнения);
...
#Если МобильныйКлиент Тогда
	ПолноеИмяФайла = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ВыборФайла.Каталог)
		+ Строка(ИмяСРасширением);
#КонецЕсли
...
Процедура ПроверитьПредельныйРазмерФайлов(ОбработчикРезультата, ПараметрыПроверки) Экспорт
	...
	Если МассивСлишкомБольшихФайлов.Количество() <> 0 Тогда 
		...
		Для Каждого Файл Из МассивСлишкомБольшихФайлов Цикл
			...
			ТекстСтроки = СтрШаблон(НСтр("ru = '%1 (%2 МБ)'"), Строка(Файл), Строка(РазмерФайлаВМб));
...
Процедура ВыбратьРежимИРедактироватьФайл(ОбработчикРезультата, ДанныеФайла, ДоступностьКомандыРедактировать) Экспорт
	...
	СпособОткрытия = ПерсональныеНастройки.HtmlФайлыСпособОткрытия;
	Если СпособОткрытия = ПредопределенноеЗначение("Перечисление.СпособыОткрытияФайлаНаПросмотр.ВоВстроенномРедакторе") Тогда
		
		РасширениеВСписке = ФайловыеФункцииСлужебныйКлиентСервер.РасширениеФайлаВСписке(
			ПерсональныеНастройки.HtmlФайлыРасширение,
			ДанныеФайла.Расширение);
...
Функция ФайлОткрытВнешнейПрограммойДляПросмотра(ПолныйПутьКФайлу) Экспорт
	...
	#Иначе
		...
		Если Расширение <> ".doc" И Расширение <> ".docx" И Расширение <> ".pdf"
			И Расширение <> ".ppt" И Расширение <> ".pptx" Тогда
			Возврат Ложь;
		КонецЕсли;
...
Процедура ОткрытьФормуФайла(Знач ПрисоединенныйФайл, СтандартнаяОбработка = Ложь, ДополнительныеПараметры = Неопределено, 
	...
	Иначе	
		...
		ОткрытьФорму("ОбщаяФорма.ПрисоединенныйФайл", ПараметрыФормы,, ПрисоединенныйФайл,,, ОписаниеОповещенияОЗакрытии);
	КонецЕсли;
...
#Если Не ВебКлиент И Не МобильныйКлиент Тогда
Функция СоздатьВизуализациюЭПВФайлеDoc(ДанныеФайла, ИдентификаторФормы) Экспорт
		
	ВизуализацияСсылка = Неопределено;
		
	Состояние(НСтр("ru = 'Выполняется вставка отметки об электронной подписи...'"));
	...
	ДвоичныеДанныеНовые = ВизуализацияЭПКлиентСервер.ВставитьВизуализациюЭПВместоТэга(
		ДвДанныеФайлаDoc, ДанныеФайла.Расширение, "ВставитьЭП", ДвоичныеДанныеШтампа, 1,
		ДанныеФайла.РасположениеШтампаЭПВPdf);
...
// Открыть файл для просмотра из карточки документа
Процедура ОткрытьИсходныйФайлДокумента(Файл, Форма) Экспорт 
...
// Выполняет автопомещение файлов. Это только для формы ЗадачиМне
//
//Параметры:
//	ЗанятыеФайлы - массив структур 
//   (РедактируетТекущийПользователь, ПолноеИмяФайлаВРабочемКаталоге,  ДатаМодификацииУниверсальнаяВБазе, Размер, Ссылка)
Процедура ВыполнитьАвтопомещениеФайлов(ЗанятыеФайлы) Экспорт
	
	#Если ВебКлиент Тогда
		Возврат;
	#КонецЕсли
	
	МассивЗахваченныхФайлов = ЗанятыеФайлы;
	...
	Если МассивЗахваченныхФайлов.Количество() > 0 Тогда
		...
		ДелопроизводствоКлиент.ИсключитьЗанятыеВнешнимиПрограммамиФайлы(
			МассивЗахваченныхФайлов);

		Если МассивЗахваченныхФайлов.Количество() <> 0 Тогда
				
			НеОсвобождатьЗанятыеWordФайлы = Истина;
			СкрыватьОшибки = Истина;
