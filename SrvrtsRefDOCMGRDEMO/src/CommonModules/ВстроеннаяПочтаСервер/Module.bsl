...
Функция ПолучитьСтруктуруПараметровОтправкиПисьма(ПисьмоОбъект,
	...
	Для каждого ПочтовыйАдресИнфо Из ТаблицаЗначений Цикл
		...
		Если РаботаСоСтроками.ЭтоАдресЭлектроннойПочты(Адрес) Или Не ТолькоАдресаЭлектроннойПочты Тогда
			Если ПочтовыйАдресИнфо.ТипАдреса = НСтр("ru = 'Кому:'") Тогда 
				ПараметрыОтправки.Получатели.Добавить(СтруктураПочтовогоАдреса);
				ЕстьПолучатели = Истина;
			ИначеЕсли ПочтовыйАдресИнфо.ТипАдреса = НСтр("ru = 'Копия:'") Тогда 
				ПараметрыОтправки.Копии.Добавить(СтруктураПочтовогоАдреса);
				ЕстьПолучатели = Истина;
			ИначеЕсли ПочтовыйАдресИнфо.ТипАдреса = НСтр("ru = 'Скрытая копия:'") Тогда
...
Функция ЗаписатьВходящееПисьмо(Сообщение, УчетнаяЗапись, 
	...
	Если ИдентификаторСообщения <> "$null" Тогда
...
Функция ПолучитьТаблицуПолучателейКомуКопияУВходящегоПисьма(Письмо) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", Письмо);
	Запрос.УстановитьПараметр("СтрокаКому", НСтр("ru = 'Кому:'"));
	Запрос.УстановитьПараметр("СтрокаКопия", НСтр("ru = 'Копия:'"));
...
Функция ПолучитьТаблицуПолучателейКомуКопияСкрытаяУИсходящегоПисьма(Письмо,
	...
	Запрос.УстановитьПараметр("СтрокаКому", НСтр("ru = 'Кому:'"));
	Запрос.УстановитьПараметр("СтрокаКопия", НСтр("ru = 'Копия:'"));
	Запрос.УстановитьПараметр("СтрокаСкрытаяКопия", НСтр("ru = 'Скрытая копия:'"));
...
Процедура ПриемПисемПоВнутреннейМаршрутизацииБыстрый() Экспорт 
	...
	Отказ = Ложь;
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ПриемПисемПоВнутреннейМаршрутизацииБыстрый, Отказ);
...
Процедура ПриемПисемПоВнутреннейМаршрутизацииДолгий() Экспорт
	...
	Отказ = Ложь;
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ПриемПисемПоВнутреннейМаршрутизацииДолгий, Отказ);
...
Процедура ПринятьОтправитьПочту(НомерЗадания) Экспорт
	...
	Отказ = Ложь;
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания["ПриемОтправкаПочты"+НомерЗадания], Отказ);
...
Процедура ЗаписьЖурналаРегистрацииСервер(ТекстСообщения) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка орфографии'", Метаданные.ОсновнойЯзык.КодЯзыка), УровеньЖурналаРегистрации.Ошибка, , , ТекстСообщения);
...
Процедура ЗаполнениеДанныхДляПоискаПисем() Экспорт
	...
	Отказ = Ложь;
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ЗаполнениеДанныхДляПоискаПисем, Отказ);
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Поиск по тексту писем.Информация'", Метаданные.ОсновнойЯзык.КодЯзыка),
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru = 'Начато регламентное заполнение данных для поиска писем'", Метаданные.ОсновнойЯзык.КодЯзыка));
	...
	Исключение
		Инфо = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Поиск по тексту писем.Ошибка'", Метаданные.ОсновнойЯзык.КодЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,
			,
			ПодробноеПредставлениеОшибки(Инфо));
		Возврат;
	...
	Если МассивСсылок.Количество() = 0 Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Поиск по тексту писем.Завершена обработка всех новых писем'", Метаданные.ОсновнойЯзык.КодЯзыка),
			УровеньЖурналаРегистрации.Информация);
...
Процедура ПроверитьНаличиеНовыхПисем(ВозвращаемыеПараметры) Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'ПроверитьНаличиеНовыхПисем.ДатаПоследнейПроверки'"),
		УровеньЖурналаРегистрации.Информация,,,
