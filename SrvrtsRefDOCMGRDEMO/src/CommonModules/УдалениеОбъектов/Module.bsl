...
Процедура УдалитьПомеченныеОбъекты(
	...
	УпорядочитьУдаляемыеОбъектыПоПриоритету(Удаляемые);
...
Функция УдалитьОбъект(
	...
	Результат = Истина;
	УдаляемыеПоСсылкам = Новый Массив;
	...
	Для Каждого СтрокаСсылки Из ТаблицаСсылок Цикл    
		...
		Если Результат И ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(СтрокаСсылки.Метаданные) Тогда
			...
			ОбъектНужноУдалитьВЭтойТранзакции = 
				...
				Или ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаСсылки.Данные, "ПометкаУдаления") = Истина
			...
			Если ОбъектНужноУдалитьВЭтойТранзакции Тогда
				УдаляемыеПоСсылкам.Добавить(СтрокаСсылки.Данные);
			Иначе
				Результат = Ложь;
			КонецЕсли;
		Иначе
	...
	// Удаление объектов, ссылающихся на УдаляемыйОбъект.
	// Логику наполнения массива УдаляемыеПоСсылкам см. выше.
	УпорядочитьУдаляемыеОбъектыПоПриоритету(УдаляемыеПоСсылкам);
	Для Каждого УдаляемыйПоСсылке Из УдаляемыеПоСсылкам Цикл
		НайденныйПоСсылкеУдаленУспешно = УдалитьОбъект(
			УдаляемыйПоСсылке, Удаляемые, ОбработанныеУдаляемые, УдаляемыхОбработано, 
		...
		Если НайденныйПоСсылкеУдаленУспешно Тогда
			УспешноУдаленныеВРамкахТранзакции.Добавить(УдаляемыйПоСсылке);
		КонецЕсли;
		Если Не Результат Тогда
			ДанныеОСсылке = Новый Структура("Ссылка", УдаляемыйПоСсылке);
			ЗаполнитьСведенияОНевозможностиУдаления(
				УдаляемыйОбъект, ДанныеОСсылке, Найденные, НеУдаленные, ЭтоОбъектИзСпискаУдаляемых);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СтрокаСсылки Из ТаблицаСсылок Цикл
		Если Не ЗначениеЗаполнено(СтрокаСсылки.ИмяРегистраСведений) Тогда
			Продолжить;
		КонецЕсли;
		// ОтметкиВремениСсылочныхДанных должен хранить ссылки удаленных объектов.
		Если СтрокаСсылки.ИмяРегистраСведений = "ОтметкиВремениСсылочныхДанных" Тогда
			Продолжить;
		КонецЕсли;
...
// Сортирует объекты по порядку, определенному в функции ПорядокУдаленияЭлемента().
// 
Процедура УпорядочитьУдаляемыеОбъектыПоПриоритету(МассивСсылокКУдалению)
	...
	УдаляемыхВсего = МассивСсылокКУдалению.Количество();
	Для Сч = 1 По УдаляемыхВсего Цикл
		ИндексЭлемента = УдаляемыхВсего - Сч;
		ТекущийУдаляемый = МассивСсылокКУдалению[ИндексЭлемента];
		...
		Если Приоритет <> 0 Тогда
			МассивСсылокКУдалению.Удалить(ИндексЭлемента);
	...
	Для Каждого Стр Из ТаблицаУдаляемыхСПриоритетами Цикл
		Если Стр.Приоритет < 0 Тогда
			МассивСсылокКУдалению.Вставить(ИндексДляВставкиВНачало, Стр.Значение);
			ИндексДляВставкиВНачало = ИндексДляВставкиВНачало + 1;
		Иначе
			МассивСсылокКУдалению.Добавить(Стр.Значение);
		КонецЕсли;
	КонецЦикла;
