...
Функция ПараметрыРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Параметры.Вставить("ИспользоватьОбщийОбработчикОжидания", Не ОбщегоНазначения.РазделениеВыключено());
	
	Если НЕ СтандартныеПодсистемыСервер.ДобавитьПараметрыРаботыКлиентаПриЗапуске(Параметры) Тогда
		Если Не Параметры.Свойство("ИспользоватьУведомленияПользователя") Тогда
			Параметры.Вставить("ИспользоватьУведомленияПользователя", Ложь);
		КонецЕсли;
