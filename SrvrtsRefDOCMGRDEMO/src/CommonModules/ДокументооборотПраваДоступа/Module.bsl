...
Функция ПолучитьПраваПользователейПоОднотипнымОбъектам(
	...
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.СамочувствиеСотрудника") Тогда
		...
		Запрос.Текст = 
			...
			|	ИСТИНА КАК Чтение,
			|	ВЫБОР
			|		КОГДА СамочувствиеСотрудника.Сотрудник = СоставСубъектов.Пользователь
			|			ТОГДА ИСТИНА
			|		ИНАЧЕ ЛОЖЬ
			|	КОНЕЦ КАК Добавление,
			|	ВЫБОР
	...
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.КаналыОбсуждений") Тогда
		...
		Запрос.Текст = 
			...
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СоставСубъектовПравДоступа КАК СоставСубъектов
			|		ПО ПользователиВКонтейнерах.Пользователь = СоставСубъектов.Субъект
			|			И СоставСубъектов.ИмяОбластиДелегирования В (&ОбластиДелегирования)
			|			%ОтборПоПользователю%
			|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Обновление прав доступа'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ЕстьПравоРегистрации(Документ, Пользователь = Неопределено) Экспорт
	...
	// Вычисление субъектов прав доступа.
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		...
		|	И СоставСубъектовПравДоступа.ИмяОбластиДелегирования В(&ОбластиДелегирования)");
	ОбластиДелегирования = 
		ДокументооборотПраваДоступаПереопределяемый.ПолучитьОбластиДелегированияПоИОМ(ОбъектМетаданных);
	Запрос.УстановитьПараметр("ОбластиДелегирования", ОбластиДелегирования);
	Запрос.УстановитьПараметр("Пользователь", Пользователь);
	ВсеСубъектыПрав = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Субъект");
	
	// Поиск субъектов со специиальными разрешениями.
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	ПользователиВКонтейнерах.Пользователь
		|ИЗ
		...
		|	И ПользователиВКонтейнерах.Пользователь В (&ВсеСубъектыПрав)");
	Запрос.УстановитьПараметр("ВсеСубъектыПрав", ВсеСубъектыПрав);
	Запрос.УстановитьПараметр("Предмет", ОбъектМетаданных);
	СубъектыПравСоСпецРазрешениями = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Пользователь");
	СубъектыПравБезСпецРазрешений = ОбщегоНазначенияКлиентСервер.РазностьМассивов(
		ВсеСубъектыПрав, СубъектыПравСоСпецРазрешениями);
	
	// Проверка общих разрешений.
	Если СубъектыПравБезСпецРазрешений.Количество() > 0 Тогда
		...
		Запрос = Новый Запрос(
			...
			|ГДЕ
			|	ПользователиВКонтейнерах.Пользователь В (&СубъектыПравБезСпецРазрешений)
			|	И УровниДоступа.Регистрация
			|	И РазрешенияДляРазрезовДоступа.РазрезДоступа = &ЗначениеДоступа");
		
		Запрос.УстановитьПараметр("СубъектыПравБезСпецРазрешений", СубъектыПравБезСпецРазрешений);
	...
	Если СубъектыПравСоСпецРазрешениями.Количество() > 0 Тогда
		...
		Запрос = Новый Запрос(
			...
			|		И ПользователиВКонтейнерах.Пользователь В (&СубъектыПравСоСпецРазрешениями)
		...
		Запрос.УстановитьПараметр("СубъектыПравСоСпецРазрешениями", СубъектыПравСоСпецРазрешениями);
		Запрос.УстановитьПараметр("Предмет", ОбъектМетаданных);
	...
	Если Константы.ИспользоватьЛокальныхАдминистраторов.Получить() Тогда
...
Процедура ОбработкаПравилДелегированияПрав() Экспорт
	...
	Отказ = Ложь;
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбработкаПравилДелегированияПрав, Отказ);
...
Процедура ДокументооборотПередЗаписьюПравообразующихРегистров(Источник, Отказ, Замещение) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("РегистрСведенийНаборЗаписей.СведенияОПользователяхДокументооборот") Тогда
		...
		// Удаление пользователя обрабатывать не нужно, все его права будут удалены автоматически.
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Пользователь, "Ссылка") = Неопределено Тогда
			Возврат;
		КонецЕсли;
...
Процедура РасширитьТаблицуПравНеограниченнымиПравами(
			ТаблицаПрав, ИдентификаторОМ, ОбъектыДоступа, Пользователи = Неопределено) Экспорт
