...
Функция ДанныеПоСертификату(СведенияОЭлектроннойПодписи, УникальныйИдентификатор) Экспорт
	...
	ДанныеСертификата = СведенияОЭлектроннойПодписи.Сертификат;
...
Процедура НастроитьФормуПодписанияШифрованияРасшифровки(Форма, Шифрование = Ложь, Расшифровка = Ложь) Экспорт
	...
	Элементы.СтраницыСертификаты.ТекущаяСтраница = Элементы.СтраницаВыборСертификата;	
	...
	ИначеЕсли Форма.СертификатСписокВыбора.Количество() = 1 Тогда
		...
		Элементы.СтраницыСертификаты.ТекущаяСтраница = Элементы.СтраницаСертификат;
	КонецЕсли;
...
Функция СохраненныеСвойстваСертификата(Отпечаток, Адрес, ПараметрыРеквизитов, ДляШифрования = Ложь) Экспорт
	...
	СохраненныеСвойства.Вставить("ОбязательныйДляШифрования");
	...
	|	Сертификаты.ОбязательныйДляШифрования,
	...
	Если Не ЗначениеЗаполнено(СохраненныеСвойства.Ссылка) Тогда
		...
		ЗаполнитьРеквизит(СохраненныеСвойства, ПараметрыРеквизитов, "ОбязательныйДляШифрования");
	КонецЕсли;
...
Процедура ЗаписатьСертификатВСправочник(Форма, Программа = Неопределено, ДляШифрования = Ложь) Экспорт
	...
	Если Не ЗначениеЗаполнено(Форма.Сертификат) Тогда
		...
		Если Форма.СертификатОбязательныйДляШифрования = Истина Тогда
			ДополнительныеПараметры.Вставить("ОбязательныйДляШифрования", Истина);
		КонецЕсли;
...
// Только для внутреннего использования.
Процедура ДобавитьОбязательныеДляШифрованияСертификаты(Форма, Программа) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	СертификатыШифрования = Форма.СертификатыШифрования;
	
	// Удаляем ранее добавленные обязательные для шифрования сертификаты.
	МассивНаУдаление = Новый Массив;
	Для Каждого Элемент Из СертификатыШифрования Цикл
		Если Элемент.ОбязательныйДляШифрования = Истина Тогда
			МассивНаУдаление.Добавить(Элемент);
		КонецЕсли;
	КонецЦикла;
	Для Каждого Элемент Из МассивНаУдаление Цикл
		СертификатыШифрования.Удалить(Элемент);
	КонецЦикла;
	
	// Добавляем обязательные для шифрования сертификаты для выбранного криптопровайдера.
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	Сертификаты.Ссылка
		...
		|	И Сертификаты.Программа = &Программа
		|	И Сертификаты.Отозван = ЛОЖЬ
		|	И Сертификаты.ПометкаУдаления = ЛОЖЬ";
	Запрос.Параметры.Вставить("Программа", Программа);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		НовыйЭлемент = СертификатыШифрования.Добавить();
		НовыйЭлемент.Сертификат = Выборка.Ссылка;
		НовыйЭлемент.ОбязательныйДляШифрования = Истина;
	КонецЦикла;
