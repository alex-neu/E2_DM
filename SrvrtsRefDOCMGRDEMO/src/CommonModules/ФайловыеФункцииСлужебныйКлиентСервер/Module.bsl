...
// (см. РаботаСФайламиСлужебныйПовтИсп.НастройкиРаботыСФайлами),
...
#КонецЕсли
	...
	Если ПустаяСтрока(ИзвлеченныйТекст) Тогда
		...
		СписокРасширенийDocx = "docx";
		Если РасширениеФайлаВСписке(СписокРасширенийDocx, РасширениеИмениФайла) Тогда
			Возврат ИзвлечьТекстИзФайлаDocx(ПолноеИмяФайла, Отказ);		
		КонецЕсли;	
...
Процедура ЗаполнитьСтатусПодписи(СтрокаПодписи, ТекстОшибки = "") Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ТекстОшибки) Тогда
		СтрокаПодписи.Статус = СтрШаблон(НСтр("ru = 'Неверна. %1'"), ТекстОшибки);
...
// Извлечь текст из файла Docx и возвратить его в виде строки.
//
Функция ИзвлечьТекстИзФайлаDocx(ПутьКФайлу, Отказ)
...
#Если Не ВебКлиент  И Не МобильныйКлиент Тогда
	...
	Попытка
		...
		ИмяФайлаXML = ВременнаяПапкаДляРазархивирования + "/word/document.xml";
		ФайлПроверка = Новый Файл(ИмяФайлаXML);
		Если Не ФайлПроверка.Существует() Тогда
			
			ИмяФайлаXML = ВременнаяПапкаДляРазархивирования + "/word/wordDocument.xml";
			ФайлПроверка = Новый Файл(ИмяФайлаXML);
			Если Не ФайлПроверка.Существует() Тогда
		...
		ИзвлеченныйТекст = ИзвлечьТекстИзDocumentXML(ЧтениеXML);
		ЧтениеXML.Закрыть();
	Исключение
		// Не считаем ошибкой, т.к. например расширение OTF может быть как OpenDocument, так и шрифт OpenType.
		Отказ = Истина;
	...
	Попытка
		УдалитьФайлы(ВременныйZIPФайл);
	Исключение
		// Не считаем ошибкой, т.к. ВременныйZIPФайл может быть занят при неудачной попытке Архив.ИзвлечьВсе
	КонецПопытки;
...
// Извлечь текст из объекта ЧтениеXML (прочитанного из файла docx).
Функция ИзвлечьТекстИзDocumentXML(ЧтениеXML)
...
#Если Не ВебКлиент Тогда
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.Текст Тогда
			...
			Если СтрНайти(ПоследнееИмяТега, "w:t") <> 0 Тогда
				ИзвлеченныйТекст = ИзвлеченныйТекст + ЧтениеXML.Значение;
			КонецЕсли;
