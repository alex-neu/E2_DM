...
Процедура ПриОпределенииОбщихПараметровБазовойФункциональности(ОбщиеПараметры) Экспорт
	...
	ОбщиеПараметры.ЗапрашиватьПодтверждениеПриЗавершенииПрограммы = Ложь;
	//ОбщиеПараметры.МинимальнаяВерсияПлатформы = "8.3.14.1694";
	ОбщиеПараметры.РекомендуемаяВерсияПлатформы = "8.3.16.1814";
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(
		ОбщиеПараметры.МинимальнаяВерсияПлатформы, ОбщиеПараметры.РекомендуемаяВерсияПлатформы) > 0 Тогда
		ОбщиеПараметры.РекомендуемаяВерсияПлатформы = ОбщиеПараметры.МинимальнаяВерсияПлатформы;
	КонецЕсли;
...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Параметры.Вставить("Банки", Новый ФиксированнаяСтруктура("ВыводитьОповещениеОНеактуальности", Ложь));
	
	Обработки.РасчетПравДоступаПослеОбновления.ПараметрыРаботыКлиентаПриЗапуске(Параметры);
	
	Параметры.Вставить("ИспользоватьУведомленияПользователя", ПолучитьФункциональнуюОпцию("ИспользоватьУведомленияПользователя"));
	
	Параметры.Вставить("ЧасовойПоясПользователя",
		РаботаСЧасовымиПоясами.ЧасовойПоясПользователя(
			Пользователи.ТекущийПользователь()));
	
	СВЗарегистрирована = СистемаВзаимодействия.ИнформационнаяБазаЗарегистрирована();
	Параметры.Вставить("СистемаВзаимодействияЗарегистрирована", СВЗарегистрирована);
	
	Если СВЗарегистрирована Тогда 
		
		// Обсуждения для служебных оповещений.
		ИДОбсужденийСтроками = Новый Массив;
		Попытка
			ИДОбсужденийДляСлужебныхОповещений = 
				ОбсужденияДокументооборот.ИДОбсужденийДляСлужебныхОповещенийТекущегоПользователя();
			Для Каждого ИДОбсуждения Из ИДОбсужденийДляСлужебныхОповещений Цикл
				ИДОбсужденийСтроками.Добавить(Строка(ИДОбсуждения));
			КонецЦикла;
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Обсуждения'", Метаданные.ОсновнойЯзык.КодЯзыка),
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
		Параметры.Вставить("ИДОбсужденийДляСлужебныхОповещенийСтроками", ИДОбсужденийСтроками);
		
		// Каналы пользователя.
		ИдентификаторыСВКаналов = КаналыОбсуждений.ИдентификаторыСВКаналовПользователя();
		ИДКаналовСтроками = Новый Массив;
		Для Каждого ИдентификаторСВКанала Из ИдентификаторыСВКаналов Цикл
			ИДКаналовСтроками.Добавить(Строка(ИдентификаторСВКанала));
		КонецЦикла;
		Параметры.Вставить("ИдентификаторыСВКаналовПользователяСтроками", ИДКаналовСтроками);
		
		Попытка
			Параметры.Вставить("НеСоздаватьБота", 
				Не РаботаСЧатБотомПовтИсп.ЕстьАктивныеСостояния() Или РаботаСЧатБотом.ОбсуждениеСБотомСуществует());
		Исключение
			// В случае проблем, бота не активируем
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Запуск чат-бота.Ошибка'", Метаданные.ОсновнойЯзык.КодЯзыка),
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Параметры.Вставить("НеСоздаватьБота", Истина);
		КонецПопытки;
	...
	Если РольДоступна("ПолныеПрава") И ПолучитьФункциональнуюОпцию("РегламентированныйУчетОбращений") Тогда
		Параметры.Вставить("ОбновитьКлассификатор", Константы.АктуализироватьКлассификаторОбращенийГраждан.Получить());
	Иначе 
		Параметры.Вставить("ОбновитьКлассификатор", Ложь);
	КонецЕсли;
	
	// Для случая, когда обсуждения подключены или отключены обработкой, не входящей в состав конфигурации.
	ОбсужденияПодключены = СистемаВзаимодействия.ИнформационнаяБазаЗарегистрирована();
	Если ПолучитьФункциональнуюОпцию("ОбсужденияПодключены") <> ОбсужденияПодключены Тогда
...
Процедура ПриДобавленииПараметровРаботыКлиента(Параметры) Экспорт
	...
	// РаботаСДокументами
	Параметры.Вставить("ПерсональныеНастройкиРаботыСДокументами", 
		Новый ФиксированнаяСтруктура(Делопроизводство.ПолучитьПерсональныеНастройкиРаботыСДокументамиСервер()));
	// Конец РаботаСДокументами
	
	// УчетВремени
	Параметры.Вставить("ПерсональныеНастройкиУчетаВремени", 
		Новый ФиксированнаяСтруктура(УчетВремени.ПолучитьПерсональныеНастройкиУчетаВремениСервер()));
	// Конец УчетВремени
	
	Параметры.Вставить("ВерсияИБ", ОбновлениеИнформационнойБазы.ВерсияИБ(Метаданные.Имя));
	
	Обработки.РасчетПравДоступаПослеОбновления.ПараметрыРаботыКлиента(Параметры);
...
Процедура ПриДобавленииПереименованийОбъектовМетаданных(Итог) Экспорт
	...
	СтароеИмя = "Роль.ЧтениеБазовойНСИ";
	НовоеИмя  = "Роль.ЧтениеБазовойНСИДокументооборот";
	ОбщегоНазначения.ДобавитьПереименование(Итог, "2.0.10.4", СтароеИмя, НовоеИмя, "");
	
	СтароеИмя = "Роль.ДобавлениеИзменениеБазовойНСИ";
	НовоеИмя  = "Роль.ДобавлениеИзменениеБазовойНСИДокументооборот";
	ОбщегоНазначения.ДобавитьПереименование(Итог, "2.0.10.4", СтароеИмя, НовоеИмя, "");
	
	СтароеИмя = "Роль.ДобавлениеИзменениеГруппДоступаКорреспондентов";
	НовоеИмя  = "Роль.ДобавлениеИзменениеГруппДоступаКонтрагентов";
	ОбщегоНазначения.ДобавитьПереименование(Итог, "2.1.0.11", СтароеИмя, НовоеИмя, "");
	
	СтароеИмя = "Роль.ЧтениеГруппДоступаКорреспондентов";
	НовоеИмя  = "Роль.ЧтениеГруппДоступаКонтрагентов";
	ОбщегоНазначения.ДобавитьПереименование(Итог, "2.1.0.11", СтароеИмя, НовоеИмя, "");
	
	СтароеИмя = "Роль.ДобавлениеИзменениеКорреспондентов";
	НовоеИмя  = "Роль.ДобавлениеИзменениеКонтрагентов";
	ОбщегоНазначения.ДобавитьПереименование(Итог, "2.1.0.11", СтароеИмя, НовоеИмя, "");
	
	СтароеИмя = "Роль.ЧтениеКорреспондентов";
	НовоеИмя  = "Роль.ЧтениеКонтрагентов";
	ОбщегоНазначения.ДобавитьПереименование(Итог, "2.1.0.11", СтароеИмя, НовоеИмя, "");
	
	СтароеИмя = "Роль.БазовыеПраваПользователя";
	НовоеИмя  = "Роль.БазовыеПраваДокументооборот";
	ОбщегоНазначения.ДобавитьПереименование(Итог, "2.1.7.7", СтароеИмя, НовоеИмя, "");
...
Процедура ПриОпределенииПараметровФункциональныхОпцийИнтерфейса(ОпцииИнтерфейса) Экспорт
	...
	ОпцииИнтерфейса.Вставить("Пользователи", ПользователиКлиентСервер.ТекущийПользователь());
