...
//                                    (элементу справочника ВнешниеПользователи).
//
Процедура ЗаписатьПользователяИнформационнойБазы(ПользовательИБ, ЭтоВнешнийПользователь = Ложь, СоздатьНового = Неопределено) Экспорт
	
	Если ОбщегоНазначения.РазделениеВключено()
		И ПользовательИБ.Роли.Содержит(Метаданные.Роли.АдминистраторСистемы) Тогда
		// Системный администратор сервиса. Редактируется только в конфигураторе. 
		Возврат;
	КонецЕсли;
	
	ИнтеграцияПодсистемБСП.ПередЗаписьюПользователяИБ(ПользовательИБ);
	
	Если СоздатьНового <> Ложь Тогда
...
Процедура СкопироватьГруппыПользователя(Источник, Приемник) Экспорт
	...
	Иначе
		...
		Запрос.Текст = 
			...
			|	ГруппыПользователейСостав.Ссылка КАК ГруппаПользователей
			|ИЗ
			|	Справочник.РабочиеГруппы.Состав КАК ГруппыПользователейСостав
			|ГДЕ
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			ГруппаПользователейОбъект = Выборка.ГруппаПользователей.ПолучитьОбъект(); // СправочникОбъект.РабочиеГруппы, СправочникОбъект.ГруппыВнешнихПользователей
...
Процедура ПриЗаполненииВидовДоступа(ВидыДоступа) Экспорт
	...
	ВидДоступа.ТипГруппЗначений       = Тип("СправочникСсылка.РабочиеГруппы");
...
Процедура ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание) Экспорт
	...
	|Справочник.РабочиеГруппы.Чтение.Пользователи
...
Процедура КонтрольАктивностиПользователей() Экспорт
	...
	Отказ = Ложь;
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.КонтрольАктивностиПользователей, Отказ);
...
Функция АвторизованныйПользователь() Экспорт
	...
	Если Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных()
		И Не ОбщегоНазначения.ДоступноИзменениеНеРазделенныхДанных() Тогда
...
//
// Параметры:
//  ГруппаПользователей - СправочникСсылка.РабочиеГруппы.
//
...
Процедура ОбновитьСоставыГруппПользователей(Знач ГруппаПользователей,
	...
	НачатьТранзакцию();
	Попытка
		Если ГруппаПользователей = Справочники.РабочиеГруппы.ВсеПользователи Тогда
...
//  ГруппаИсточник      - СправочникСсылка.РабочиеГруппы - группа, из которой происходит
//                        перенос пользователей.
//  ГруппаПриемник      - СправочникСсылка.РабочиеГруппы - группа, в которую переносятся
//                        пользователи.
...
Функция ПеремещениеПользователяВНовуюГруппу(МассивПользователей, ГруппаИсточник,
	...
	Для Каждого ПользовательСсылка Из МассивПользователей Цикл
		...
		Если ГруппаПриемник = Справочники.РабочиеГруппы.ВсеПользователи
...
//
// Параметры:
//  ГруппыПриемник     - СправочникСсылка.РабочиеГруппы, группа, в которую
...
Процедура ДобавитьПользователяВГруппу(ГруппаПриемник, ПользовательСсылка, ТипПользователя) Экспорт
	...
	Попытка
		...
		Иначе
			ИмяТаблицы = "Справочник.РабочиеГруппы";
		КонецЕсли;
...
Функция ФормированиеСообщенияПользователю(МассивПользователей, ГруппаПриемник,
	...
	ИначеЕсли КоличествоПользователей > 1 Тогда
		...
		Если ГруппаПриемник = Справочники.РабочиеГруппы.ВсеПользователи Тогда
...
Процедура ЗаполнитьНаименованиеПредопределенныхГруппПользователей() Экспорт
	...
	ГруппаПользователейОбъект = Справочники.РабочиеГруппы.ВсеПользователи.ПолучитьОбъект();
...
Функция ЗначенияПараметровСеансаТекущийПользователь()
	...
	Если Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		Если ОбщегоНазначения.РазделениеВыключено() Тогда
			...
			Результат = ЗначенияПараметровСеансаТекущийПользователь();
...
Процедура СоздатьТекущегоПользователяВСправочнике(СведенияОПользователе)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СведенияОПользователе.ИдентификаторПользователяИБ) Тогда
			...
			Если ОбщегоНазначения.РазделениеВключено() И РольДоступна("ПолныеПрава") Тогда
				НовыйПользователь.ДополнительныеСвойства.Вставить("СозданиеАдминистратора", Истина);	
			КонецЕсли;
		КонецЕсли;
...
//   * Ссылка - СправочникСсылка.ГруппыВнешнихПользователей, СправочникСсылка.РабочиеГруппы -  ссылка на элемент справочника.
//   * Родитель - СправочникСсылка.ГруппыВнешнихПользователей, СправочникСсылка.РабочиеГруппы - ссылка на родителя
//       элемента справочника.
//
Функция СсылкиВИерархииРодителей(Таблица) Экспорт
...
Процедура ОбновитьСоставыИерархическихГруппПользователей(ГруппаПользователей,
	...
	ОбновитьГруппыВнешнихПользователей =
		ТипЗнч(ГруппаПользователей) <> Тип("СправочникСсылка.РабочиеГруппы");
	...
	|ИЗ
	|	Справочник.РабочиеГруппы КАК ГруппыПользователей
	|ГДЕ
...
Процедура ПроверитьПраваПользователя(ПользовательИБ, РежимПроверки, ЭтоВнешнийПользователь)
	...
	Если РазделениеВключено И
		(РежимПроверки = "ПриЗапуске" Или ПользовательИБ.УникальныйИдентификатор = ПользователиИнформационнойБазы.ТекущийПользователь().УникальныйИдентификатор) Тогда
		Возврат; // Не перепроверяем права для пользователей созданных в конфигураторе.
	КонецЕсли;
...
Процедура ПриПолученииДанных(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, ОтПодчиненного)
	...
	ИначеЕсли ТипЗнч(ЭлементДанных) = Тип("КонстантаМенеджерЗначения.ИспользоватьГруппыПользователей")
	      ...
	      ИЛИ ТипЗнч(ЭлементДанных) = Тип("СправочникОбъект.РабочиеГруппы")
...
Процедура ЗаписатьПользователяИБ(ПользовательОбъект, ПараметрыОбработки)
	...
	Если ПарольЗадан Или Не ХешПароляЗадан И ПроверитьПустойПароль Тогда
		...
		Исключение
			Если ХранимыеСвойства.ВходВПрограммуРазрешен Тогда
