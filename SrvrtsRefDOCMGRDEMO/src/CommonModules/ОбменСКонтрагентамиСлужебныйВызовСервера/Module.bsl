...
Функция ОтправкаПакетовЭД(Знач МассивПакетовЭД, Знач СоотвСоглашенийИПараметровСертификатов,
	ОтправленныеДокументы = Неопределено, КонтекстОперации = Неопределено) Экспорт
	
	РезультатОтправки = ОбменСКонтрагентамиСлужебный.ОтправкаПакетовЭД(
		МассивПакетовЭД, СоотвСоглашенийИПараметровСертификатов, ОтправленныеДокументы,, КонтекстОперации);
...
Процедура ЗакрытьДокументыПринудительно(Знач МассивСсылокНаВладельцев, Знач ПричинаЗакрытия, КоличествоОбработанныхЭД) Экспорт
	...
	// Удалим файлы из очереди формирования документов ДО
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ОчередьПриемаСообщенийЭДО.ЭДПрисоединенныйФайл КАК ЭДПрисоединенныйФайл
		|ИЗ
		|	РегистрСведений.ОчередьПриемаСообщенийЭДО КАК ОчередьПриемаСообщенийЭДО
		|ГДЕ
		|	ОчередьПриемаСообщенийЭДО.ЭДПрисоединенныйФайл.ВладелецФайла В(&ВладельцыФайловЭД)";
	
	Запрос.УстановитьПараметр("ВладельцыФайловЭД", МассивСсылокНаВладельцев);
	
	ФайлыДляУдаленияИзОчереди = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ЭДПрисоединенныйФайл");
	
	Для Каждого ФайлЭД Из ФайлыДляУдаленияИзОчереди Цикл
		РегистрыСведений.ОчередьПриемаСообщенийЭДО.УдалитьЗапись(ФайлЭД);
	КонецЦикла;
	
	// Удалим записи о ошибках при формировании документов ДО
	Для Каждого ЭД Из МассивСсылокНаВладельцев Цикл
...
Функция ЗапуститьПерезаполнениеСловаряСопоставленияНоменклатурыВФоне(УникальныйИдентификатор, Знач ПараметрыПроцедуры) Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйПовтИсп.ПодсистемаСопоставленияНоменклатурыСуществует() Тогда
		Возврат ОбменСКонтрагентамиСлужебныйПовтИсп.МодульСопоставлениеНоменклатурыКонтрагентовСлужебныйВызовСервера().ЗапуститьПерезаполнениеСловаряСопоставленияНоменклатурыВФоне(
			УникальныйИдентификатор, ПараметрыПроцедуры);
	КонецЕсли;
	...
	ИмяПроцедуры        = "ОбменСКонтрагентамиСлужебный.НачатьПерезаполнениеСловаряСопоставленияНоменклатуры";
...
Процедура ЗаполнитьДанныеЗаявки(Знач ДополнительныеПараметры, ДанныеЗаявки) Экспорт
	...
	Ответ = ОбменСКонтрагентамиСлужебный.СведенияОбАдресеПоПредставлению(АдресОрганизации);
...
Функция ИзменятьУстановившегоПодпись(ЭлектронныйДокумент, УстановленныеПодписи) 
	...
	Для каждого УстановленнаяПодпись Из УстановленныеПодписи Цикл
		УсловиеСравнения = ?(УстановленнаяПодпись.Свойство("Объект"),
			ЭлектронныйДокумент = УстановленнаяПодпись.Объект,
			Истина);
...
//
Процедура СохранитьРезультатыПроверкиПодписей(ЭД, МассивРезультатов, Подписи) Экспорт
	...
	Для Каждого Элемент Из МассивРезультатов Цикл
		...
		СведенияОПодписи = Новый Структура("УникальныйИдентификатор, ДатаПодписи, УстановившийПодпись");
		ЗаполнитьЗначенияСвойств(СведенияОПодписи, Подписи[Элемент.НомерСтроки - 1]);
		
		МенеджерЗаписи = РегистрыСведений.ЭлектронныеПодписи.СоздатьМенеджерЗаписи();
		ЗаполнитьЗначенияСвойств(МенеджерЗаписи, СведенияОПодписи);
		МенеджерЗаписи.Объект = ЭД;
		МенеджерЗаписи.Прочитать();
		ЗаполнитьЗначенияСвойств(МенеджерЗаписи, СведенияОПодписи);
		МенеджерЗаписи.Объект = ЭД;
