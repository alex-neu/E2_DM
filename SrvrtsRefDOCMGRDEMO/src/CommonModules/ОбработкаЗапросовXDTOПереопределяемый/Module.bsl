...
Процедура ПриЗаполненииСоответствияСвойствXDTOРезультатамПоиска(Тип, Результат) Экспорт
	...
	Если Тип = "DMIncomingDocument" 
		...
		Или Тип = "DMIncomingDocumentTemplate" 
		Или Тип = "DMInternalDocumentTemplate" 
		Или Тип = "DMOutgoingDocumentTemplate"
		Или СтрНачинаетсяС(Тип, "DMBusinessProcess") Тогда
		Результат.Вставить("project", "Проект");
	КонецЕсли;
	
	Если Тип = "DMInternalDocumentTemplate" Тогда
...
//
Процедура ПередатьОбщиеРеквизитыБизнесПроцесса(БизнесПроцесс, Объект) Экспорт
	
	Объект.parentTaskEnabled = Истина;
	
	Если Объект.parentTaskEnabled И ЗначениеЗаполнено(БизнесПроцесс.ГлавнаяЗадача) Тогда
		...
		ОбработкаЗапросовXDTOБизнесПроцессыИЗадачи.ЗаполнитьОсновныеСвойстваXDTOЗадачи(
			Объект.parentTask, БизнесПроцесс.ГлавнаяЗадача, Ложь, Ложь);
	КонецЕсли;
	
	Объект.leadingTaskEnabled = Истина;
	
	Если Объект.leadingTaskEnabled И ЗначениеЗаполнено(БизнесПроцесс.ВедущаяЗадача) Тогда
		Объект.leadingTask = ОбработкаЗапросовXDTO.СоздатьОбъект("DMBusinessProcessTask");
		ОбработкаЗапросовXDTOБизнесПроцессыИЗадачи.ЗаполнитьОсновныеСвойстваXDTOЗадачи(
			Объект.leadingTask, БизнесПроцесс.ВедущаяЗадача, Ложь, Ложь);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(БизнесПроцесс.Состояние) Тогда
		Объект.state = ОбработкаЗапросовXDTO.СоздатьОбъект(
			ОбработкаЗапросовXDTO.ПолучитьИмяТипаСсылки(БизнесПроцесс.Состояние));
		Объект.state.name = Строка(БизнесПроцесс.Состояние);
		Объект.state.objectId = ОбработкаЗапросовXDTO.ПолучитьObjectIDПоСсылке(БизнесПроцесс.Состояние);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(БизнесПроцесс.Проект) Тогда
		Объект.project = ОбработкаЗапросовXDTO.СоздатьОбъект(
			ОбработкаЗапросовXDTO.ПолучитьИмяТипаСсылки(БизнесПроцесс.Проект));
		Объект.project.name = Строка(БизнесПроцесс.Проект);
		Объект.project.objectId = ОбработкаЗапросовXDTO.ПолучитьObjectIDПоСсылке(БизнесПроцесс.Проект);
	КонецЕсли;
...
Процедура УстановитьУсловиеОтбораВедущейЗадачиПоПустойСсылке(ТекстЗапроса) Экспорт
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "%ОтборВедущаяЗадача%",
		"(БизнесПроцессы.ГлавнаяЗадача = ЗНАЧЕНИЕ(Задача.ЗадачаИсполнителя.ПустаяСсылка)
...
Процедура ЗаписатьБПРешениеВопросовВыполненияЗадач(БПОбъект, Объект, СоздатьНовый) Экспорт
	...
	Если Объект.Установлено("issueType") Тогда
		БПОбъект.ВидВопроса = ОбработкаЗапросовXDTO.ПолучитьСсылкуПоObjectID(Объект.issueType.objectId);
	КонецЕсли;
...
//     БизнесПроцессОбъект.РешениеВопросовВыполненияЗадач
//   ПолучитьПредметы - Булево - Истина, если нужно получить значения предметов бизнес-процесса
//
Процедура ПолучитьБПРешениеВопросовВыполненияЗадач(Ответ, БизнесПроцесс, ПолучитьПредметы = Истина) Экспорт
	
	Если ТипЗнч(БизнесПроцесс) = Тип("БизнесПроцессСсылка.РешениеВопросовВыполненияЗадач") Тогда
		Если БизнесПроцесс.Пустая() Тогда
			Возврат;
		КонецЕсли;
		РеквизитыБП = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(БизнесПроцесс,
			"ВидВопроса,
			|Завершен,
			|Автор,
			|ИнициаторПроцесса,
			|ПредметРассмотрения,
			|НовыйСрок");
		РеквизитыБП.Вставить("Ссылка", БизнесПроцесс);
	ИначеЕсли ТипЗнч(БизнесПроцесс) = Тип("БизнесПроцессОбъект.РешениеВопросовВыполненияЗадач") Тогда
		РеквизитыБП = Новый Структура(
			"Ссылка,
			|ВидВопроса,
			|Завершен,
			|Автор,
			...
			БизнесПроцесс.ВидВопроса,
			БизнесПроцесс.Завершен,
			БизнесПроцесс.Автор,
			БизнесПроцесс.ИнициаторПроцесса,
			БизнесПроцесс.ПредметРассмотрения,
			БизнесПроцесс.НовыйСрок);
	Иначе
		Возврат;
	КонецЕсли;
	
	Ответ.issueType = ОбработкаЗапросовXDTO.СоздатьОбъект("DMIssueType");
	Ответ.issueType.name = Строка(РеквизитыБП.ВидВопроса);
	Ответ.issueType.objectId = ОбработкаЗапросовXDTO.ПолучитьObjectIDПоСсылке(РеквизитыБП.ВидВопроса);
	
	Ответ.initiator = ОбработкаЗапросовXDTO.СоздатьОбъект("DMUser");
	Ответ.initiator.name = Строка(РеквизитыБП.ИнициаторПроцесса);
	Ответ.initiator.objectId = ОбработкаЗапросовXDTO.ПолучитьObjectIDПоСсылке(РеквизитыБП.ИнициаторПроцесса);
	
	Если ЗначениеЗаполнено(РеквизитыБП.ПредметРассмотрения) Тогда
		Ответ.issueTask = ОбработкаЗапросовXDTO.СоздатьОбъект("DMBusinessProcessTask");
		ОбработкаЗапросовXDTOБизнесПроцессыИЗадачи.ЗаполнитьОсновныеСвойстваXDTOЗадачи(
			Ответ.issueTask, РеквизитыБП.ПредметРассмотрения, , ПолучитьПредметы);
	КонецЕсли;
	
	Если РеквизитыБП.ВидВопроса = Перечисления.ВидыВопросовВыполненияЗадач.ПереносСрока Тогда
		
		Ответ.newDueDate = РеквизитыБП.НовыйСрок;
		
		Запрос = Новый Запрос;
		Запрос.Текст =
			"ВЫБРАТЬ
			|	ПереносыСроковВыполненияПроцессовИЗадач.СтарыйСрок КАК СтарыйСрок
			|ИЗ
			|	РегистрСведений.ПереносыСроковВыполненияПроцессовИЗадач КАК ПереносыСроковВыполненияПроцессовИЗадач
			|ГДЕ
			|	ПереносыСроковВыполненияПроцессовИЗадач.БизнесПроцессПереноса = &БизнесПроцессПереноса
			|	И ПереносыСроковВыполненияПроцессовИЗадач.Предмет = &Предмет";
		Запрос.УстановитьПараметр("БизнесПроцессПереноса", РеквизитыБП.Ссылка);
		Запрос.УстановитьПараметр("Предмет", РеквизитыБП.ПредметРассмотрения);
		
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			Ответ.oldDueDate = Выборка.СтарыйСрок;
		КонецЕсли;
...
Функция ОбработатьУниверсальноеСообщение(Сообщение) Экспорт
	...
	ПараметрыСеанса.УзелИнтегрированнойСистемы = 
		ОбработкаЗапросовXDTOКорп.УзелИнтегрированнойСистемыПоСообщению(Сообщение);
...
// Дополнительная обработка при записи объекта.
//
// Параметры:
//   Объект - ОбъектXDTO из коллекции objects запроса DMUpdateRequest - записываемый объект.
//
// Возвращаемое значение:
//   ОбъектXDTO - записанный объект, или
//   Неопределено - признак необходимости продолжить создание объекта.
//
Функция ЗаписатьОбъект(Объект) Экспорт
	
	Если ОбработкаЗапросовXDTO.ПроверитьТип(Объект, "DMCashFlowItem") Тогда
		Возврат ОбработкаЗапросовXDTOКОРП.ИзменитьСтатьюДвиженияДенежныхСредств(Объект);
	КонецЕсли;
...
Функция Создать(Сообщение) Экспорт
	...
	ИначеЕсли ОбработкаЗапросовXDTO.ПроверитьТип(Сообщение.object, "DMCashFlowItem") Тогда
		Возврат ОбработкаЗапросовXDTOКОРП.СоздатьСтатьюДвиженияДенежныхСредств(Сообщение);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
// Дополнительная обработка при поиске или создании внешнего объекта.
//
// Параметры:
//   ЗначениеСвойстваXDTO - ОбъектXDTO - объект требуемого типа.
//
// Возвращаемое значение:
//   ЛюбаяСсылка - ссылка на найденный или вновь созданный объект Документооборота, или
//   Неопределено - признак необходимости продолжить создание объекта.
// 
Функция НайтиСоздатьВнешнийОбъект(ЗначениеСвойстваXDTO) Экспорт
	
	Если ОбработкаЗапросовXDTO.ПроверитьТип(ЗначениеСвойстваXDTO, "DMCashFlowItem") Тогда
		Возврат ОбработкаЗапросовXDTOКОРП.НайтиСоздатьСтатьюДвиженияДенежныхСредств(ЗначениеСвойстваXDTO);
	ИначеЕсли ОбработкаЗапросовXDTO.ПроверитьТип(ЗначениеСвойстваXDTO, "DMProject") Тогда
		Возврат ОбработкаЗапросовXDTOКОРП.НайтиСоздатьПроект(ЗначениеСвойстваXDTO);
	КонецЕсли;
...
//
Функция ПолучитьОбъектПоObjectID(ОбъектИД, НаборКолонок) Экспорт
	
	Если ОбъектИд.type = "DMOutgoingEMail"
		Или ОбъектИд.type = "DMIncomingEMail" Тогда
		
		Возврат ОбработкаЗапросовXDTOПочта.ПолучитьДанныеПисьма(ОбъектИд);
		
	ИначеЕсли ОбъектИд.type = "DMMeetingType" Тогда
		
		Возврат ОбработкаЗапросовXDTOКорп.ПолучитьВидМероприятия(ОбъектИд);
		
	ИначеЕсли ОбъектИд.type = "DMMeeting" Тогда
		
		Возврат ОбработкаЗапросовXDTOКорп.ПолучитьМероприятие(ОбъектИд);
		
	ИначеЕсли ОбъектИд.type = "DMMeetingMinutesItem" Тогда
		
		Возврат ОбработкаЗапросовXDTOКорп.ПолучитьПунктПротоколаМероприятия(ОбъектИд);
		
	ИначеЕсли ОбъектИд.type = "DMForumThread" Тогда
		
		Возврат ОбработкаЗапросовXDTOКорп.ПолучитьТемуОбсуждения(ОбъектИд);
		
	ИначеЕсли ОбъектИд.type = "DMForumMessage" Тогда
		
		Возврат ОбработкаЗапросовXDTOКорп.ПолучитьСообщениеОбсуждения(ОбъектИд);
		
	ИначеЕсли ОбъектИд.type = "DMPremisesReservation" Тогда
		
		Возврат ОбработкаЗапросовXDTOКорп.ПолучитьБронь(ОбъектИд);
		
	ИначеЕсли ОбъектИд.type = "DMApplicationNotification" Тогда
		
		Возврат ОбработкаЗапросовXDTOКорп.ПолучитьУведомлениеПрограммы(ОбъектИд);
		
	ИначеЕсли ОбъектИд.type = "DMCashFlowItem" Тогда
		
		Возврат ОбработкаЗапросовXDTOКорп.ПолучитьСтатьюДвиженияДенежныхСредств(ОбъектИд, НаборКолонок);
...
//   Неопределено - признак необходимости продолжить получение процесса
//
Функция ПолучитьБизнесПроцесс(ОбъектИд, ОбъектБизнесПроцесс, ПолучитьПредметы = Истина) Экспорт
	
	Если ОбъектИд.type = "DMBusinessProcessInvitation" Тогда
		Ответ = ОбработкаЗапросовXDTOКорп.ПолучитьБППриглашение(ОбъектБизнесПроцесс, ПолучитьПредметы);
		
	ИначеЕсли ОбъектИд.type = "DMComplexBusinessProcess" Тогда
		Ответ = ОбработкаЗапросовXDTOКорп.ПолучитьБПКомплексныйПроцесс(ОбъектБизнесПроцесс, ПолучитьПредметы);
...
Функция ПолучитьСостояниеНовогоОбъекта(Сообщение, ТипОтвета) Экспорт
	...
	Если Тип = "DMBusinessProcessInvitation" Тогда
		...
		БПОбъект.Наименование = НСтр("ru = 'Принять участие в'");
		...
		Ответ.object = ОбработкаЗапросовXDTOКорп.ПолучитьБППриглашение(БПОбъект);
		
	ИначеЕсли Тип = "DMComplexBusinessProcess" Тогда
		БПОбъект = БизнесПроцессы.КомплексныйПроцесс.СоздатьБизнесПроцесс();
		БПОбъект.Наименование = НСтр("ru = 'Обработка'");
		ОбработкаЗапросовXDTOБизнесПроцессыИЗадачи.ЗаполнитьНовыйБизнесПроцессПоШаблону(
			БПОбъект, Предмет, "ШаблоныКомплексныхБизнесПроцессов", ШаблонБизнесПроцесса);
			
		Ответ.object = ОбработкаЗапросовXDTOКорп.ПолучитьБПКомплексныйПроцесс(БПОбъект);
...
Функция ЗаписатьБизнесПроцесс(Объект, СоздатьНовый) Экспорт
	...
	Ответ = ОбработкаЗапросовXDTOБизнесПроцессыИЗадачи.ПолучитьБизнесПроцесс(ОбъектИд);
...
// Помещает в Результат XDTO объект соответствующего бизнес-процесса.
//
// Параметры:
//   ТипЗначения - Тип - тип ссылки бизнес-процесса.
//   Результат - ОбъектXDTO типа DMBusinessProcess<...> - неявно возвращаемое значение.
//
Процедура ПриПолученииОбъектаXDTOБизнесПроцессаПоТипу(ТипЗначения, Результат) Экспорт
	
	Если ТипЗначения = Тип("БизнесПроцессСсылка.Приглашение") Тогда
		Результат = ОбработкаЗапросовXDTO.СоздатьОбъект("DMBusinessProcessInvitation");
		
	ИначеЕсли ТипЗначения = Тип("БизнесПроцессСсылка.КомплексныйПроцесс") Тогда
		Результат = ОбработкаЗапросовXDTO.СоздатьОбъект("DMComplexBusinessProcess");
...
// Вызывается при определении имени регистра, хранящего историю состояний документов.
//
// Параметры:
//   ИмяРегистра - Строка - неявно возвращаемое значение, имя регистра сведений в метаданных.
//
Процедура ПриОпределенииИмениРегистраИсторияСостоянийДокументов(ИмяРегистра) Экспорт
	
	ИмяРегистра = "ИсторияСостоянийДокументов";
	
КонецПроцедуры

// Вызвается при определении типа дополнительного реквизита для заполнения объекта DMAdditionalProperty.
//
// Параметры:
//   ТипРеквизита - Тип - тип значения дополнительного реквизита ДО.
//   ТипыЗначенийXDTO - СписокXDTO - заполняемый список объектов типа DMType.
//
Процедура ПриОпределенииТипаДополнительногоРеквизита(ТипРеквизита, ТипыЗначенийXDTO) Экспорт
	
	Если ТипРеквизита = Тип("СправочникСсылка.Номенклатура") Тогда
		ОбъектТипXDTO = ОбработкаЗапросовXDTO.СоздатьОбъект("DMType");
		ОбъектТипXDTO.xdtoClassName = "DMProduct";
		ОбъектТипXDTO.presentation = НСтр("ru = 'Товар, услуга'");
		ТипыЗначенийXDTO.Добавить(ОбъектТипXDTO);
	ИначеЕсли ТипРеквизита = Тип("СправочникСсылка.СтатьиДвиженияДенежныхСредств") Тогда
		ОбъектТипXDTO = ОбработкаЗапросовXDTO.СоздатьОбъект("DMType");
		ОбъектТипXDTO.xdtoClassName = "DMCashFlowItem";
		ОбъектТипXDTO.presentation = НСтр("ru = 'Статья движения денежных средств'");
		ТипыЗначенийXDTO.Добавить(ОбъектТипXDTO);
	КонецЕсли;
