...
Функция ИнформацияОСвязяхЮридическогоЛицаПоИНН(Знач ИНН) Экспорт
	...
	ЗаполнитьИнформациюОПодразделениях(ОбъектXDTO, Прокси.ФабрикаXDTO, ИнформацияОСвязях);
...
Процедура ЗаполнитьИнформациюОПодразделениях(ОбъектXDTO, ФабрикаXDTO, ИнформацияОСвязях)
	...
	Если ОбъектXDTO.СвПодразд <> Неопределено Тогда
		...
		Для каждого СтрокаПодразделения Из ОбъектXDTO.СвПодразд Цикл
			...
			Если СтрокаПодразделения.АдрМНРФ <> Неопределено 
				...
				АдресКИСтруктура = АдресКИ(СтрокаПодразделения.АдрМНРФ, ФабрикаXDTO, АдресныеСокращения);
				ОписаниеПодразделения = АдресКИСтруктура.Представление;
