...
Функция ИзменитьВходящийДокумент(Объект, ВозвращатьДанныеЗаписанногоОбъекта = Истина) Экспорт
	...
	Исключение
		...
		Ошибка = ОбработкаЗапросовXDTO.СоздатьОбъект("DMError");
		Ошибка.subject = НСтр("ru = 'Ошибка при изменении входящего документа'");
		Инфо = ИнформацияОбОшибке();
		Ошибка.description = ОбработкаЗапросовXDTO.ПолучитьОписаниеОшибки(Инфо);
		Возврат Ошибка;
...
Функция ЗаписатьДанныеВходящегоДокумента(Документ, ОбъектXDTO, Ошибка)
	...
	ТекстСообщения = "";
	Если Не ОбработкаЗапросовXDTO.ПроверитьЗаполнение(Документ, ТекстСообщения, ПроверятьСвязи) Тогда
		Ошибка = ОбработкаЗапросовXDTO.СоздатьОбъект("DMError");
		...
		Если ДоступныеПоля <> "" Тогда
			...
			Для Каждого Свойство Из ОбъектXDTO.Свойства() Цикл
				Если Свойство.Имя = "status" Тогда // свойства проверяются только в разрезе их типов
					Продолжить;
				КонецЕсли;
				Если Не ДоступныеПоля.Свойство(Свойство.Имя) И Не СвойстваРавны(ТекущиеДанные, ОбъектXDTO, Свойство) Тогда
					Ошибка = ОбработкаЗапросовXDTO.СоздатьОбъект("DMError");
					Ошибка.subject = НСтр("ru = 'Ошибка при записи входящего документа'");
	...
	Попытка
		...
		// состояние
		ЗаполнитьСостоянияДокумента(Документ, ОбъектXDTO);
...
Функция СоздатьИсходящийДокумент(Сообщение) Экспорт
	...
	Исключение
		...
		Ошибка = ОбработкаЗапросовXDTO.СоздатьОбъект("DMError");
		Ошибка.subject = НСтр("ru = 'Ошибка при изменении исходящего документа'");
		Инфо = ИнформацияОбОшибке();
		Ошибка.description = ОбработкаЗапросовXDTO.ПолучитьОписаниеОшибки(Инфо);
		Возврат Ошибка;
...
Функция ИзменитьВнутреннийДокумент(Объект, ВозвращатьДанныеЗаписанногоОбъекта = Истина) Экспорт
	...
	Исключение
		...
		Ошибка = ОбработкаЗапросовXDTO.СоздатьОбъект("DMError");
		Ошибка.subject = НСтр("ru = 'Ошибка при изменении внутреннего документа'");
		Инфо = ИнформацияОбОшибке();
		Ошибка.description = ОбработкаЗапросовXDTO.ПолучитьОписаниеОшибки(Инфо);
		Возврат Ошибка;
...
Функция ЗаписатьДанныеВнутреннегоДокумента(Документ, ОбъектXDTO, Ошибка)
	...
	ЗаполнитьДанныеВнутреннегоДокумента(Документ, ОбъектXDTO);
...
//
Процедура ЗаполнитьДанныеВнутреннегоДокумента(ДокументОбъект, ОбъектXDTO)
	...
	Если ОбъектXDTO.Свойства().Получить("productRows") <> Неопределено
		...
		Для Каждого СтрокаXDTO Из ОбъектXDTO.productRows.rows Цикл
			...
			ОбработкаЗапросовXDTO.ЗаполнитьРеквизитИзСвойстваXDTO(Строка.ЕдиницаИзмерения, СтрокаXDTO, "measurementUnit");
...
Процедура ЗаполнитьДанныеВидаДокумента(ВидДокумента, ОбъектXDTO) Экспорт
	...
	Если ОбработкаЗапросовXDTO.ПроверитьТип(ОбъектXDTO, "DMInternalDocumentType") Тогда
		...
		Если ОбработкаЗапросовXDTO.СвойствоСуществует(ОбъектXDTO, "useConfirmation") Тогда
			ОбработкаЗапросовXDTO.ЗаполнитьРеквизитИзСвойстваXDTO(
				ВидДокумента.ИспользоватьУтверждение, ОбъектXDTO, "useConfirmation");
		КонецЕсли;
		Если ОбработкаЗапросовXDTO.СвойствоСуществует(ОбъектXDTO, "useSigningByManager") Тогда
			ОбработкаЗапросовXDTO.ЗаполнитьРеквизитИзСвойстваXDTO(
...
Процедура ПолучитьДанныеНоменклатурыДел(НоменклатураДел, ОбъектXDTO, НаборКолонок = Неопределено) Экспорт
	...
	Если ТипЗнч(НоменклатураДел) = Тип("СправочникОбъект.НоменклатураДел")
		...
		ДанныеНоменклатурыДел.Вставить("counterpartiesFilledIn", НоменклатураДел.КонтрагентыЗаполнены);
	...
	Иначе
		...
		ДанныеНоменклатурыДел.Вставить("counterpartiesFilledIn", Данные.КонтрагентыЗаполнены);
...
Процедура ЗаполнитьДанныеНоменклатурыДел(НоменклатураДел, ОбъектXDTO) Экспорт
	...
	ОбработкаЗапросовXDTO.ЗаполнитьРеквизитИзСвойстваXDTO(НоменклатураДел.КонтрагентыЗаполнены, ОбъектXDTO, "counterpartiesFilledIn");
...
//
Процедура ЗаполнитьСостоянияДокумента(Документ, ОбъектXDTO)
...
Процедура ЗаполнитьОбщиеДанныеДокумента(ДокументОбъект, ОбъектXDTO)
	...
	ОбработкаЗапросовXDTO.ЗаполнитьРеквизитИзСвойстваXDTO(ДокументОбъект.ВидДокумента, ОбъектXDTO, "documentType");
