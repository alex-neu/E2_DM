...
Процедура КалендарьПередЗаписьюПредмета(Источник, Отказ) Экспорт
	...
	УстановитьДополнительноеСвойствоПредыдущееЗначениеОтменено(Источник);
...
Процедура КалендарьПередЗаписьюУчастников(Источник, Отказ, Замещение) Экспорт
	...
	Справочники.Мероприятия.КалендарьПередЗаписьюУчастников(Источник, Отказ, Замещение);
...
Процедура КалендарьПриЗаписиУчастников(Источник, Отказ, Замещение) Экспорт
	...
	Справочники.Мероприятия.КалендарьПриЗаписиУчастников(Источник, Отказ, Замещение);
...
// Доступно изменение календаря пользователя.
//
// Параметры:
...
Функция ДоступноИзменениеКалендаря(Пользователь, ПользовательКалендаря) Экспорт
	...
	// Делегаты получают права только если включено использование прав досутпа.
	...
	Запрос.УстановитьПараметр("ОтКого", ПользовательКалендаря);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Связанная
			...
			Если ЗначениеЗаполнено(ПредставлениеМестногоЧасовогоПояса) Тогда
				НоваяСтрока.Наименование = ПредставлениеМестногоЧасовогоПояса + " " + НоваяСтрока.Наименование;
			КонецЕсли;
...
// Устанавилвает предыдущее значение отменен.
//
Процедура УстановитьДополнительноеСвойствоПредыдущееЗначениеОтменено(Объект)
	
	Если ТипЗнч(Объект) = Тип("СправочникОбъект.Мероприятия") Тогда
		ПредыдущееЗначениеОтменено = РегистрыСведений.СостоянияМероприятий.Отменено(Объект.Ссылка);
		Объект.ДополнительныеСвойства.Вставить("ПредыдущееЗначениеОтменено", ПредыдущееЗначениеОтменено);
	КонецЕсли;
