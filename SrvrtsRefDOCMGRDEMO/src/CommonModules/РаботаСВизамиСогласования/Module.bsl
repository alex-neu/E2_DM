...
Процедура ЗаполнитьВизуПоЗадаче(Документ, Задача) Экспорт 
	...
	Если НайденнаяСтрока <> Неопределено 
		...
		Если ЗначениеЗаполнено(НайденнаяСтрока.РезультатСогласования) Тогда
			...
			Задача.ДополнительныеСвойства.Свойство("ПользовательИсполнитель", УстановилРезультат);
...
Процедура ЗаполнитьСписокВиз(ЭтаФорма, ПоказыватьУдаленные = Ложь) Экспорт
	...
	Если Выборка.Следующий() И Выборка.Количество > 0 Тогда 
		...
		ЭтаФорма.Элементы.ИсторияСогласования.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'История согласования (%1)'"), Строка(Выборка.Количество));
	Иначе 
...
Функция УжеЕстьВизаИсполнителяЗадачи(ЗадачаСсылка, НайденныйПредмет, НайденнаяВиза, ИсполнительЗадачи = Неопределено) Экспорт 
	...
	Для Каждого Предмет Из Предметы Цикл
		...
		Если ЗначениеЗаполнено(ИсполнительЗадачи) Тогда
			СтруктураИсполнитель.Вставить("Исполнитель", ИсполнительЗадачи);
		ИначеЕсли ЗначениеЗаполнено(РеквизитыЗадачи.Исполнитель) Тогда 
