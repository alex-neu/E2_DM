...
Функция СформироватьНаборВажныхПолей(Знач ОбъектСсылка, Версия = Неопределено,
	...
	ИсключаемыеКлючевыеРеквизиты = Новый Массив;
	
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура")
		И ДополнительныеПараметры.Свойство("ИсключаемыеКлючевыеРеквизиты")
		И ТипЗнч(ДополнительныеПараметры.ИсключаемыеКлючевыеРеквизиты) = Тип("Массив") Тогда
		
		ИсключаемыеКлючевыеРеквизиты = ОбщегоНазначенияКлиентСервер.СкопироватьМассив(
			ДополнительныеПараметры.ИсключаемыеКлючевыеРеквизиты);
	
	КонецЕсли;
	
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура")
		И ДополнительныеПараметры.Свойство("ДанныеПодписанта") Тогда
		
		ДанныеПодписанта = ДополнительныеПараметры.ДанныеПодписанта;
	Иначе
	...
	Для Каждого ИмяПоля Из МассивИмен Цикл
		...
		Если ИсключаемыеКлючевыеРеквизиты.Найти(ИмяПоля) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Иначе  // табличные части отдельно обработать
			...
			Для Каждого Строка Из ОбъектСсылка[ИмяТабличнойЧасти] Цикл
				...
				Если ДанныеПодписанта <> Неопределено
					И ИмяКолонки = "Подписал"
					И НомерСтроки = 0 Тогда
					
					ЗначениеПоля = ДанныеПодписанта.Подписал;
					
				ИначеЕсли ДанныеПодписанта <> Неопределено
					И ИмяКолонки = "ДатаПодписи"
					И НомерСтроки = 0 Тогда
					
					ЗначениеПоля = НачалоДня(ДанныеПодписанта.ДатаПодписи);
					
				ИначеЕсли ДанныеПодписанта <> Неопределено
					И ИмяКолонки = "Подписан"
					И НомерСтроки = 0 Тогда
					
					ЗначениеПоля = ДанныеПодписанта.Подписан;
	...
	Попытка
		...
		Для Каждого Данные Из ПодписанныеДанные Цикл
			...
			Иначе
				...
				Если ДелопроизводствоКлиентСервер.ЭтоДокумент(ПодписываемыйОбъект)
					...
					Если ДелопроизводствоКлиентСервер.ЭтоВнутреннийДокумент(ПодписываемыйОбъект) Тогда
						...
						Если ТипЗнч(СвойстваПодписи) = Тип("Структура") 
							...
							КонецЕсли;
						ИначеЕсли ТипЗнч(ПараметрыВыполнения) = Тип("Структура") И ПараметрыВыполнения.Свойство("ПодписатьЭД")
								И ПараметрыВыполнения.Свойство("НаправлениеЭД") И ПараметрыВыполнения.ПодписатьЭД = Истина Тогда
								
							ДокументПоставленВОчередь = ОбменСКонтрагентамиДОСервер.ПоставитьДокументВОчередьНаОтправкуПоЭДО(
								ПодписываемыйОбъект.Ссылка, Неопределено, ПараметрыВыполнения.НаправлениеЭД, Ложь, Истина);							
							Если ДокументПоставленВОчередь И ПодписываемыйОбъект.Стороны.Количество() > 0 Тогда								
								...
								Если ТипЗнч(ОписаниеПодписи) = Тип("Структура") И ОписаниеПодписи.Свойство("УстановившийПодпись") Тогда
									УстановившийПодпись = ОписаниеПодписи.УстановившийПодпись;
								КонецЕсли;
								
								ДатаПодписи = ТекущаяДатаСеанса();
								Если ТипЗнч(ОписаниеПодписи) = Тип("Структура") И ОписаниеПодписи.Свойство("ДатаПодписи") Тогда
								...
								СтрокаОрганизации.Подписал = УстановившийПодпись;
							Иначе
								ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				ИначеЕсли ТипЗнч(ПодписываемыйОбъект) = Тип("СправочникСсылка.ЭДПрисоединенныеФайлы")
					Или ТипЗнч(ПодписываемыйОбъект) = Тип("СправочникОбъект.ЭДПрисоединенныеФайлы") Тогда
...
Функция ОписаниеЭПДляОтметки(ЭП) Экспорт
	...
	ОписаниеЭП = Новый Структура("Номер, Владелец, ДатаНачала, ДатаОкончания");
...
//
Процедура ЗаписатьОтметкуЭП(Документ, ФайлВладелец, ПутьКОтметкеЭП) Экспорт
	...
	ОтметкаЭП = ПолучитьОтметкуЭП(Документ, ФайлВладелец);
...
//
Функция ПолучитьОтметкуЭП(Документ, ФайлВладелец) Экспорт
...
//    СправочникСсылка.Файлы - файл-изображение отметки ЭП.
//
Функция ПолучитьПервуюОтметкуЭП(Документ, ФайлВладелец) Экспорт
