...
Функция НайтиПравилоОтправкиСообщенийСВД(Организация, Контрагент, ВидДокумента) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ФорматСообщения = ПредопределенноеЗначение("Справочник.ФорматыСообщенийСВД.ОператорЭДО1СТакском") Тогда
			...
			Запрос2.УстановитьПараметр("СтатусПодключения", Перечисления.СтатусыПриглашений.Принято);
...
Функция ЗаполнитьАдрес(ОбъектСТаблЧастьюКонтактнаяИнформация, ПространствоИмен)
	...
	Для Каждого Строка Из ОбъектСТаблЧастьюКонтактнаяИнформация.КонтактнаяИнформация Цикл
		...
		Если Строка.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			...
			Адрес.__content = Строка.Представление;
			ЗначенияПолей = РаботаСАдресами.СведенияОбАдресе(Строка.ЗначенияПолей);
			Если ТипЗнч(ЗначенияПолей) = Тип("Структура") Тогда
				Для Каждого Поле Из ЗначенияПолей Цикл
					...
					ИначеЕсли Поле.Ключ = "Здание" Тогда
						Адрес.house = Поле.Значение.ТипЗдания;
					ИначеЕсли Поле.Ключ = "Помещения" Тогда
						Если Поле.Значение.Количество() > 0 Тогда
							Адрес.flat = Поле.Значение[0].Номер;
						КонецЕсли;
					ИначеЕсли Поле.Ключ = "НаселенныйПункт" Тогда
						Адрес.settlement = Поле.Значение;
					ИначеЕсли Поле.Ключ = "Район" Тогда
						Адрес.district = Поле.Значение;
					ИначеЕсли Поле.Ключ = "Регион" Тогда
						Адрес.region = Поле.Значение;
					ИначеЕсли Поле.Ключ = "Страна" Тогда
						Адрес.country = Поле.Значение;
					ИначеЕсли Поле.Ключ = "Индекс" Тогда
...
Функция СоздатьXDTOЗаголовок(Организация, Контрагент, Транспорт, ВидСообщенияСВД)
	...
	ОбъектXDTOЗаголовок.standart = НСтр("ru='Стандарт системы управления документами'");
...
Процедура ОтправкаДокументовПоСВД() Экспорт
	...
	Отказ = Ложь;
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОтправкаСообщенийПоСВД, Отказ);
...
Функция СоздатьВходящийИзXDTO(ОбъектXDTOHeader, ПараметрыДокумента, Контрагент, Организация) Экспорт
	...
	Документ.Источник = Справочники.ИсточникиДанных.СВД;
...
Процедура ОбработатьОшибкуПроверкиЭПДокумента(Документ, ВидСообщенияСВД) Экспорт
	...
	Если ВходящееСообщениеСВД.НеобходимостьПосылкиУведомлений = Перечисления.НеобходимостьПосылкиУведомлений.ПосылатьУведомления Тогда
		...
		Если Не ЕстьОшибкиСВДДляДокумента(ВходящееСообщениеСВД.ИдентификаторСообщения) Тогда
			...
			ТекстОшибки = НСтр("ru='Некоторые подписи ЭП не прошли проверку.'");
...
Процедура ЗаполнитьHTMLПоXDTO(ТекстHTML, ОбъектXDTO) Экспорт
	...
	ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Отправитель'"), ОбъектXDTO.from_organization);
	ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Идентификатор сообщения'"), ОбъектXDTO.msg_id);
	ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Дата формирования сообщения'"), 
	РаботаСФайламиКлиентСервер.ПолучитьМестноеВремя(ОбъектXDTO.time));
	ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Получатель'"), ОбъектXDTO.to_organization);
	
	Если ОбъектXDTO.Document <> Неопределено Тогда
		
		ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Наименование документа'"), ОбъектXDTO.Document.title);
		...
		ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Вид документа'"), ОбъектXDTO.Document.kind);
		ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Идентификатор документа'"), ОбъектXDTO.Document.idnumber);
		ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Содержание'"), ОбъектXDTO.Document.annotation);
		
		Для Каждого Адресат Из ОбъектXDTO.Document.Addressee Цикл
			Если Адресат.Organization <> Неопределено Тогда
				ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Адресат'"), Адресат.Organization.fullname);
			КонецЕсли;
			Для Каждого OfficialPerson Из Адресат.Organization.OfficialPerson Цикл
				ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Получатель'"), OfficialPerson.Name.__content);
			КонецЦикла;
		КонецЦикла;	
		...
		ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Гриф доступа'"), ОбъектXDTO.Document.Confident.__content);
		...
		ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Рег. номер'"), ОбъектXDTO.Document.RegNumber.__content);
		ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Дата регистрации'"), Строка(ОбъектXDTO.Document.RegNumber.regdate));
		...
		Если ОбъектXDTO.Document.Writer <> Неопределено Тогда
			Если ОбъектXDTO.Document.Writer.PrivatePerson <> Неопределено Тогда
				ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Составитель документа'"),
	...
	Если ОбъектXDTO.Acknowledgement <> Неопределено Тогда
		ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Идентификатор сессии'"), ОбъектXDTO.Acknowledgement.msg_id);
		...
		Для Каждого Подтверждение Из ОбъектXDTO.Acknowledgement.AckResult Цикл
			Если ЗначениеЗаполнено(Подтверждение.__content) Тогда
				ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Описание ошибки'"), Подтверждение.__content);
			КонецЕсли;	
...
Процедура ЗаполнитьHTMLПоXDTO2013(ТекстHTML, ОбъектXDTO) Экспорт
	...
	ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Отправитель'"), ОбъектXDTO.Header.Sender.Contact.Organization.ShortName);
	ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Идентификатор сообщения'"), ОбъектXDTO.Header.ReturnID);
	ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Дата формирования сообщения'"), 
	...
	ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Получатель'"), ОрганизацияНаименование);
	...
	Для Каждого ОбъектXDTODocument Из ОбъектXDTO.DocumentList.Document Цикл
		...
		Для Каждого ОбъектXDTOAdditionalField из ОбъектXDTODocument.AdditionalField Цикл
			Если ОбъектXDTOAdditionalField.Name = "ЗаголовокДокумента" и ОбъектXDTOAdditionalField.Type = "Text" Тогда
				ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Наименование документа'"),
				ПолучитьЗначениеЭлементаXDTO(ОбъектXDTOAdditionalField, "__content"));
			КонецЕсли;
		КонецЦикла;
		
		ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Вид документа'"), 
			ПолучитьЗначениеЭлементаXDTO(ОбъектXDTODocument, "Group.__content"));
		ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Идентификатор документа'"), ОбъектXDTODocument.UID);
		ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Содержание'"), 
		...
		Для Каждого ОбъектXDTOAddressee из ОбъектXDTODocument.Addressee Цикл 
			...
			ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Получатель'"), ОбъектXDTOAddressee.Contact.Organization.ShortName);
			
			Адресат = "";
			Если ПолучитьЗначениеЭлементаXDTO(ОбъектXDTOAddressee, "Contact.OfficialPerson.FIO", Адресат) <> Неопределено Тогда
				ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='ФИО'"), Адресат);
			...
			Если ПолучитьЗначениеЭлементаXDTO(ОбъектXDTOAddressee, "Contact.Department.Name", Подразделение) <> Неопределено Тогда
				ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Подразделение'"), Подразделение);
			Иначе
				ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Подразделение'"), Подразделение);
			КонецЕсли;	
		...
		Для Каждого ОбъектXDTOAuthor из ОбъектXDTODocument.Author Цикл
			...
			Если ПолучитьЗначениеЭлементаXDTO(ОбъектXDTOAuthor, "Contact.OfficialPerson.FIO", Подписал) <> Неопределено Тогда 
				ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Подписал'"), Подписал);
			ИначеЕсли ПолучитьЗначениеЭлементаXDTO(ОбъектXDTOAuthor, "PrivatePerson.FIO", Подписал) <> Неопределено Тогда
				ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Подписал'"), Подписал);
			КонецЕсли;	 
		КонецЦикла;	
		
		ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Гриф доступа'"), 
		...
		ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Рег. номер'"), ОбъектXDTODocument.RegistrationInfo.Number);
		ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Дата регистрации'"), Строка(ОбъектXDTODocument.RegistrationInfo.Date));
		
		Для Каждого ОбъектXDTOVisa Из ОбъектXDTODocument.Visa Цикл 
			ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Резолюция'"), ОбъектXDTOVisa.VisaInfo);
			ДобавитьСтрокуHTML(ТекстHTML, НСтр("ru='Утвердивший'"),
...
Функция ПредставлениеНаселенногоПункта(ОбъектАдреса) 
	...
	Если ТипЗнч(ОбъектАдреса) = ТипXDTO Тогда
		Адрес = ОбъектАдреса.Состав;
		Если ТипЗнч(Адрес) = ТипXDTO И Адрес.Тип() = ФабрикаXDTO.Тип(
				УправлениеКонтактнойИнформациейСлужебный.ПространствоИмен(), "Адрес") Тогда
			Адрес = Адрес.Состав;
		КонецЕсли;
		Если ТипЗнч(Адрес) = ТипXDTO И Адрес.Тип() = ФабрикаXDTO.Тип(
				РаботаСАдресами.ПространствоИмен(), "АдресРФ") Тогда
			АдресРФ = Адрес;
		КонецЕсли;
	КонецЕсли;
...
//  ПространствоИмен                        - Строка         -  Пространство имен для операций XDTO 
// 
Процедура ЗаполнитьXDTOAddress2013(ОбъектXDTOКонтактAddress, ОбъектСТаблЧастьюКонтактнаяИнформация, ПространствоИмен) Экспорт
	...
	Для Каждого Строка Из ОбъектСТаблЧастьюКонтактнаяИнформация.КонтактнаяИнформация Цикл
		...
		Если Строка.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			...
			ОбъектXDTOАдрес.ZipCode = СведенияОбАдресе.Индекс;
...
Процедура ЗаполнитьEmailИТелефоны2013(Контакты, Ссылка, ПространствоИмен)
	...
	ТаблицаКонтактов = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(МассивОбъектов, 
		МассивТиповКонтактнойИнформации,, ТекущаяДатаСеанса());
...
Функция СформироватьСообщениеОбОшибкеСВД2013(ВходящееСообщениеXDTO, КодОшибки, ТекстОшибки, Кому) Экспорт 
	...
	// Зона заголовка
	ОбъектXDTOReport.Header = СоздатьXDTOЗаголовок2013(НСтр("ru = 'Сервер СВД 1С:Документооборот'"), 
...
// Параметры:
//   ДокументСсылка     - ФормаКлиентскогоПриложения Входящего, Исходящего или Внутреннего документа - форма документа,
