...
Функция ПоказатьОписаниеИзмененийСистемы() Экспорт
	...
	// Предупреждение об обновлении выводим только администратору непосредственно после обновления,
	// функционал по отображению описания изменений не используется.
	Возврат Ложь;
...
Функция ВыполнитьДействияПриОбновленииИнформационнойБазы(ПараметрыОбновления, ДополнительныеПараметры)
	...
	Исключение
		ДокументооборотПраваДоступа.УстановитьИспользованиеПравДоступа(Истина); // Включение механизма прав доступа.
		Если Не БлокировкаУстановленаРанее Тогда
			РазблокироватьИБ(БлокировкаИБ);
		КонецЕсли;
		ВызватьИсключение;
...
Функция НайтиОбработчикОбновления(КонтекстОбработчика, СведенияОбОбновлении, ПараметрыОбновления = Неопределено)
	...
	Пока Истина И ПланОбновления <> Неопределено Цикл
	...
	Если ТекущийЦиклОбновления = Неопределено Тогда
		Если ЕстьВыполняющиеся = Истина Тогда
			Возврат Истина;
		Иначе
