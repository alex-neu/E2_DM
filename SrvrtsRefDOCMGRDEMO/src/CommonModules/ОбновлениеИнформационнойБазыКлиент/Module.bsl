...
Процедура ПоказатьОписаниеИзмененийСистемы()
	...
	Если ПараметрыРаботыКлиента.ПоказатьОписаниеИзмененийСистемы Тогда
		...
		ТекстПриОтсутствииОписания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Конфигурация успешно обновлена на версию %1'"), ПараметрыРаботы.ВерсияКонфигурации);
		ПоказатьПредупреждение(, ТекстПриОтсутствииОписания);
		ОбщегоНазначенияДокументооборотВызовСервера.УстановитьФлагОтображенияОписанийПоТекущуюВерсию();
	КонецЕсли;
