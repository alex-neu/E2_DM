...
Функция ИнтернетПочтовыйПрофиль(УчетнаяЗапись, ДляПолучения = Ложь) Экспорт
	...
	|	УчетныеЗаписиЭлектроннойПочты.СохранятьКопиюВОтправленные КАК СохранятьКопиюВОтправленные,
	|	УчетныеЗаписиЭлектроннойПочты.ИспользоватьOauth КАК ИспользоватьOauth
	|ИЗ
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.ИспользоватьOauth Тогда		
			ТокенДоступа = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(УчетнаяЗапись, "access_token");
			ДействуетДо = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(УчетнаяЗапись, "ДействуетДо");
			
			Если ДействуетДо < ТекущаяДатаСеанса() + 60 Тогда // вот вот истечет. надо получить токен заново
				
				refresh_token = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(УчетнаяЗапись, "refresh_token");
				
				Если ОбновитьТокенOAuth(refresh_token, ТокенДоступа, ДействуетДо, Выборка.АдресСервераSMTP) Тогда
		...
		Иначе
			...
			Если Выборка.Протокол = "IMAP" И Выборка.СохранятьКопиюВОтправленные Тогда
		...
		Если Выборка.ИспользоватьOauth Тогда
			Результат.ТокенДоступа = ТокенДоступа;
			Результат.АутентификацияПоТокену = ИспользованиеАутентификацииИнтернетПочтыПоТокену.Использовать;
			Результат.Пароль = "";
			Результат.ПарольSMTP = "";
			Результат.ПарольIMAP = "";
		КонецЕсли;
...
Функция ОбновитьТокенOAuth(refresh_token, ТокенДоступа, ДействуетДо, АдресСервераSMTP)
	...
	Если РаботаСПочтовымиСообщениямиПереопределяемый.ОбновитьТокенOAuth(refresh_token, ТокенДоступа, ДействуетДо, АдресСервераSMTP,
		OAuth_redirection_uri, OAuthАвторизацияКлиент, OAuthАвторизацияКлиентСекретныйКод) Тогда
	...
	Если СтрНайти(НРег(АдресСервераSMTP), "yandex.ru") <> 0 Тогда
		АдресСайта = "oauth.yandex.ru";
		АдресРесурса = "token";
	ИначеЕсли СтрНайти(НРег(АдресСервераSMTP), "mail.ru") <> 0 Тогда
		АдресСайта = "oauth.mail.ru";
		АдресРесурса = "token";
	ИначеЕсли СтрНайти(НРег(АдресСервераSMTP), "gmail.com") <> 0 Тогда
		АдресСайта = "oauth2.googleapis.com";
		АдресРесурса = "token";
		
	ИначеЕсли СтрНайти(НРег(АдресСервераSMTP), "office365.com") <> 0 Тогда
	...
	// СтрОтвета =  такой вид:
	//{
	//  "access_token": "ya29.a0AfH6SMBk8P4BkQdV84VVBZipSJka6gQKcIeNsqUvJSB-dHgrQjUvwXCIQbPiKaktcvYyWhISufMPUCQgx4xMA8OCkUFrVpbOAfvUJnrPjKbQOuXBHhz7i7b7Wmot_UIn4r8ZdO4EalmrZOnM0swK1hF_NTP3ozgFrm0",
	//  "expires_in": 3599,
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru = 'Ошибка получения токена: '") + ОписаниеОшибки;
		
	КонецПопытки;
	
	Если ЗначенияОтвета.Получить("error") <> Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Ошибка получения токена: '") + ЗначенияОтвета["error"];
	КонецЕсли;	
...
Функция ПривестиВложениеДляВставкиВПисьмо(Вложение) Экспорт 
	...
	ИначеЕсли ТипЗнч(Вложение) = Тип("Строка") И Не ЭтоАдресВременногоХранилища(Вложение) Тогда
		// путь к файлу на диске
		
		ДвоичныеДанные = Новый ДвоичныеДанные(Вложение);
...
Функция ОтправитьПисьма(УчетнаяЗапись, Письма, ТекстИсключения = Неопределено) Экспорт
	...
	РеквизитыОтправителя = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(УчетнаяЗапись, 
		"ИмяПользователя,АдресЭлектроннойПочты,ИспользоватьДляПолучения");
	...
	Исключение
		Если ПротоколПолученияПочты = ПротоколИнтернетПочты.IMAP И Не РеквизитыОтправителя.ИспользоватьДляПолучения Тогда
			ТекстОшибки 
				= НСтр("ru = 'Не удалось подключиться к серверу IMAP. Проверьте настройки учетной записи.'", ОбщегоНазначения.КодОсновногоЯзыка());
