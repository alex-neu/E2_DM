...
Процедура УстановитьПараметрыРаспознаванияCuneiForm(ЯзыкРаспознавания, Пользователь, 
...
// Возвращает возможность начала автораспознавания новых файлов.
//
// Возвращаемое значение:
//  Булево
//
Функция ВозможноНачалоАвтораспознаванияНовыхФайлов() Экспорт
...
//   * ПользовательЗаданияРаспознавания - СправочникСсылка.Пользователи - ссылка на пользователя.
//   * ГраницаОтключенияНачалаАвтораспознаванияВСервисе - Число - максимальное число файлов на распознавании при котором
//                                                        возможно автоматическое начало распознавания новых файлов.
//
Функция ПараметрыРапознаванияВСервисе() Экспорт
	...
	Параметры.Вставить("ПользовательЗаданияРаспознавания",
		РаботаСФайламиВызовСервера.ПолучитьПользователяЗаданияРаспознавания());
...
//
// Параметры:
//  Параметры - Структура - см. ПараметрыРапознаванияВСервисе
//
Процедура УстановитьПараметрыРаспознаванияВСервисе(Параметры) Экспорт
	...
	УстановитьПользователяЗаданияРаспознавания(Параметры.ПользовательЗаданияРаспознавания);
...
Функция РаспознатьФайл(ФайлСсылка, ПараметрыРаспознавания, ОписаниеОшибки, РаспознанныйТекст, 
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЭтоТекущаяВерсия Тогда
				СтатусТекущейВерсии = ВерсияОбъект.СтатусРаспознаванияТекста;
			КонецЕсли;
	...
	Если РаспознатьНемедленно
		И ПрограммаРаспознавания = Перечисления.ПрограммыРаспознавания.СервисРаспознавания Тогда
...
Функция РаспознатьВерсию(ВерсияСсылка, ОписаниеОшибки, РаспознанныйТекст,
	...
	РеквизитыВерсии = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		ВерсияСсылка, "Владелец.СтратегияРаспознавания, СтатусРаспознаванияТекста");
...
Функция ЗаписатьРезультатРаспознаванияИВыполнитьДействия(ВерсияСсылка, СтрокаВозврата, 
	...
	ИначеЕсли СтрокаВозврата = "НачатоРаспознавание" Тогда
		...
		Попытка
			...
			ВерсияОбъект = ВерсияСсылка.ПолучитьОбъект();
			ВерсияОбъект.СтатусРаспознаванияТекста = Перечисления.СтатусыРаспознаванияТекста.Распознается;
			ВерсияОбъект.ДополнительныеСвойства.Вставить("ЗаписьПодписанногоОбъекта", Истина); // чтобы прошла запись ранее подписанного объекта
			ВерсияОбъект.Записать();
		...
		Попытка
			...
			Если ПрограммаРаспознавания = Перечисления.ПрограммыРаспознавания.СервисРаспознавания Тогда
				...
				СтатусРаспознаванияТекста = 
					...
					ВерсияСсылка, "СтатусРаспознаванияТекста");
...
Процедура ВозобновитьРаспознаваниеФайлаВСервисе(Файл) Экспорт
	...
	Попытка
		...
		ВерсияОбъект = Файл.ПолучитьОбъект();
		ВерсияОбъект.СтатусРаспознаванияТекста = Перечисления.СтатусыРаспознаванияТекста.Распознается;
		ВерсияОбъект.ДополнительныеСвойства.Вставить("ЗаписьПодписанногоОбъекта", Истина); // чтобы прошла запись ранее подписанного объекта
		ВерсияОбъект.Записать();
...
// создаст штамп png если его еще нет.
Процедура СоздатьШтампПодписиСервер(ТекущаяВерсия)
	
	УстановитьПривилегированныйРежим(Истина);
	
	НаборЗаписей = РегистрыСведений.ЭлектронныеПодписи.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Объект.Установить(ТекущаяВерсия);
...
Функция ПолучитьФайлВизуализации(Файл, ИдентификаторФормы, ТребуетсяЗаполнитьВизуализацию) Экспорт
	...
	Если Не РеквФайла.ПодписанЭП Тогда
		Возврат Неопределено;
	КонецЕсли;	
	...
	Если Не (НРег(Расширение) = "doc" Или НРег(Расширение) = "docx"
			Или НРег(Расширение) = "odt" Или НРег(Расширение) = "pdf") 
		Или Не ДелопроизводствоКлиентСервер.ЭтоДокумент(ВладелецФайла) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Если Делопроизводство.ФайлЯвляетсяОригиналом(Файл) Тогда // Для скан копий не делаем визуализацию (и для помеченных на удаление файлов).
		Возврат Неопределено;
	КонецЕсли;
	...
	ФайлPNG = РаботаСЭП.ПолучитьПервуюОтметкуЭП(ВладелецФайла, Файл);
		
	Если Не ЗначениеЗаполнено(ФайлPNG) Тогда
		
		СоздатьШтампПодписиСервер(РеквФайла.ТекущаяВерсия); // возможно стара подпись - ту же на лету создаем png
		
		ФайлPNG = РаботаСЭП.ПолучитьПервуюОтметкуЭП(ВладелецФайла, Файл);
		Если Не ЗначениеЗаполнено(ФайлPNG) Тогда
			Возврат Неопределено; // не создан png, может imageMagick нет
		КонецЕсли;
	...
	ДвоичныеДанныеШтампа = ПолучитьДвоичныеДанныеФайла(ФайлPNG);
	ДанныеФайла.Вставить("ДвоичныеДанныеШтампа", ДвоичныеДанныеШтампа);
	
	ФайлВизуализация = ПолучитьВизуализациюЭП(ДанныеФайла, ИдентификаторФормы);
...
Процедура ДобавитьВизуализациюЭП(ДанныеФайла, ИдентификаторФормы) 
	...
	ДанныеФайла.Вставить("РасположениеШтампаЭПВPdf", РасположениеШтампаЭПВPdf);
	
	//ДобавитьВизуализация не используется
	Если Не ПолучитьФункциональнуюОпцию("АвтовизуализацияЭП")
		Или Не ДанныеФайла.ПодписанЭП 
		Или ДанныеФайла.Зашифрован
		Или Не (НРег(ДанныеФайла.Расширение) = "doc" Или НРег(ДанныеФайла.Расширение) = "docx"
			Или НРег(ДанныеФайла.Расширение) = "odt" Или НРег(ДанныеФайла.Расширение) = "pdf") 
		Или Не ДелопроизводствоКлиентСервер.ЭтоДокумент(ДанныеФайла.Владелец) Тогда
		Возврат;
	КонецЕсли;
	
	Если Делопроизводство.ФайлЯвляетсяОригиналом(ДанныеФайла.Ссылка) Тогда
		Возврат;
	КонецЕсли;
	
	ФайлPNG = РаботаСЭП.ПолучитьПервуюОтметкуЭП(ДанныеФайла.Владелец, ДанныеФайла.Ссылка);
		
	Если Не ЗначениеЗаполнено(ФайлPNG) Тогда
		
		СоздатьШтампПодписиСервер(ДанныеФайла.Версия); // возможно старая подпись - ту же на лету создаем png
		
		ФайлPNG = РаботаСЭП.ПолучитьПервуюОтметкуЭП(ДанныеФайла.Владелец, ДанныеФайла.Ссылка);
		Если Не ЗначениеЗаполнено(ФайлPNG) Тогда
			Возврат; // не создан png, может imageMagick нет
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ФайлВизуализация)
...
Функция ПолучитьВизуализациюЭП(ДанныеФайла, ИдентификаторФормы)
	...
	ЧислоПодписей = РаботаСЭП.КоличествоПодписей(ДанныеФайла.Версия);
	РасположениеШтампаЭПВPdf = ПолучитьРасположениеШтампаЭПВPdf();
...
Функция ПроверитьЗанятыеФайлыОтложенно() Экспорт
	...
	Для Каждого Строка Из ТаблицаФайлов Цикл
		...
		Права = ДокументооборотПраваДоступа.ПолучитьПраваПоОбъекту(Строка.Файл);
...
Функция ПолучитьСписокЗанятыхФайлов(ВладелецФайла) Экспорт
	...
	Для Каждого Строка Из ТаблицаЗанятыхФайлов Цикл
		...
		ОписаниеФайла = Новый Структура("Редактирует, Наименование, Ссылка, Размер, ПолноеИмяФайлаВРабочемКаталоге, ДатаМодификацииУниверсальнаяВБазе, Расширение");
