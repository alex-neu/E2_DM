...
// перед интерактивной настройко подчиненного элемента.
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - карточка шаблона комплексного процесса..
//
// Возвращаемое значение:
//  Булево - возвращает Ложь, если в ходе записи возникли ошибки, иначе - Истина.
//
Функция ЗаписатьНовыйШаблонПередНастройкойПодчиненногоЭлемента(Форма) Экспорт
...
// Выполняет запись нового комплексного процесса перед интерактивной настройкой
...
Процедура ОбновитьПризнакПрерыванияДляДействияСхемы(Форма) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Установка/снятие пометки прерывания недоступно в веб-клиенте.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьПризнакТекущегоЭлементаСхемы(Форма) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Установка/снятие текущей позиции недоступно в веб-клиенте.'"));
		Возврат;
	КонецЕсли;
...
Процедура НастроитьЭлементСхемыДействие(Форма)
	...
	Если ЭтоШаблон Тогда
		Если Не ЗаписатьНовыйШаблонПередНастройкойПодчиненногоЭлемента(Форма) Тогда
			Возврат;
		КонецЕсли;
	Иначе
	...
	Иначе // Настройка пустого элемента схемы (действия).
		...
		Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
			...
			ПоказатьПредупреждение(, НСтр("ru = 'Настройка новых действий схемы в веб-клиенте недоступна.'"));
			
		Иначе
			
			Параметры = Новый Структура;
			Параметры.Вставить("ВладелецШаблона", Форма.Объект.Ссылка);
			Параметры.Вставить("Предметы", Форма.Объект.Предметы);
			...
			Если Не УЭлементаНаименованиеПоУмолчанию(Форма.Элементы.Схема.ТекущийЭлемент) Тогда
				...
				Параметры.Вставить("НаименованиеДействия", СтрПолучитьСтроку(НаименованиеЭлемента, 1));
				ОписаниеДействия = СтрЗаменить(НаименованиеЭлемента, Параметры.НаименованиеДействия, "");
				Параметры.Вставить("ОписаниеДействия", ОписаниеДействия);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(Форма.ДатаОтсчетаДляРасчетаСроков) Тогда
				ДатаОтсчета = СрокиИсполненияПроцессовКлиентСерверКОРП.
					ДатаОтсчетаДействияКомплексногоПроцессаВФорме(Форма);
				Параметры.Вставить("ДатаОтсчетаДляРасчетаСроков", ДатаОтсчета);
			КонецЕсли;
			
			ПродолжениеНастройки = Новый ОписаниеОповещения(
				"ПродолжениеНастроитьЭлементСхемыДействие",
			...
			Если ПараметрыЭлемента.Тип = ПредопределенноеЗначение(
				...
				ТипДействия = "Действие";
			ИначеЕсли ПараметрыЭлемента.Тип = ПредопределенноеЗначение(
				"Перечисление.ТипыЭлементовСхемыКомплексногоПроцесса.ВложенныйПроцесс") Тогда
				
				ТипДействия = "ВложенныйПроцесс";
			КонецЕсли;
			...
			ПомощникОтправитьКлиентКОРП.ВыбратьВариантОтправкиДействияКомплексногоПроцесса(
				ТипДействия, ПредметыПроцесса, Форма, ПродолжениеНастройки);
...
// Продолжение НастроитьЭлементСхемыДействие.
//
Процедура ПродолжениеНастроитьЭлементСхемыДействие(ВыбранныйШаблон, Параметры) Экспорт
	
	Если Не ЗначениеЗаполнено(ВыбранныйШаблон)
		Или ВыбранныйШаблон = КодВозвратаДиалога.Отмена Тогда
		
		Возврат;
	КонецЕсли;
	
	Параметры.Вставить("ШаблонБП", ВыбранныйШаблон);
...
Процедура НастроитьЭлементСхемыОбработка(Форма, ОписаниеОповещениеПослеНастройки = Неопределено)
	...
	ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ru = 'Обработка'"));
