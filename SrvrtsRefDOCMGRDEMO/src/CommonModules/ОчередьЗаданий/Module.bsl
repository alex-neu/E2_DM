...
//	 * Идентификатор - СправочникСсылка.ОчередьЗаданий, СправочникСсылка.ОчередьЗаданийОбластейДанных - ссылка на справочник.
//	 ...
//
...
Функция ДобавитьЗадание(ПараметрыЗадания) Экспорт
	...
	ОбластьИзменена = Ложь;
	Если ПараметрыЗадания.Свойство("ОбластьДанных")
		И (Не ПараметрыСеанса.ОбластьДанныхИспользование Или ПараметрыСеанса.ОбластьДанныхЗначение <> ПараметрыЗадания.ОбластьДанных) Тогда
		// смена области данных.
		РаботаВМоделиСервисаПереопределяемый.ВойтиВОбластьДанных(ПараметрыЗадания.ОбластьДанных,
			?(ПараметрыЗадания.Свойство("ОбъектМетаданных"), ПараметрыЗадания.ОбъектМетаданных, Неопределено));
			
		ОбластьИзменена = Истина;
	КонецЕсли;
	
	Попытка
		
		Если ПараметрыЗадания.Свойство("Ключ")
			И ЗначениеЗаполнено(ФоновыеЗадания.ПолучитьФоновыеЗадания(Новый Структура("ИмяМетода,Ключ,Состояние", ПараметрыЗадания.ИмяМетода, ПараметрыЗадания.Ключ, СостояниеФоновогоЗадания.Активно))) Тогда
			Возврат Неопределено; // уже работает.
		КонецЕсли;
		
		ФоновыеЗадания.Выполнить(ПараметрыЗадания.ИмяМетода,
			?(ПараметрыЗадания.Свойство("Параметры"), ПараметрыЗадания.Параметры, Неопределено),
			?(ПараметрыЗадания.Свойство("Ключ"), ПараметрыЗадания.Ключ, Неопределено));
		
	Исключение
		
		Если ОбластьИзменена Тогда
