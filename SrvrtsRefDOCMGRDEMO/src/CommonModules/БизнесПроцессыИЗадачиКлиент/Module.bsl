...
//  ВладелецФорма  - ФормаКлиентскогоПриложения - форма, из которой выполняется перенаправление.
//
// Возвращаемое значение:
//   Булево   - Истина, если перенаправление прошло успешно.
//
Функция ПеренаправитьЗадачу(Задача, ВладелецФорма = Неопределено,
	ЕстьАктивныеПодзадачи = Ложь) Экспорт

	Если ЕстьАктивныеПодзадачи Тогда
		
		ПараметрыОбработчика = Новый Структура;
		ПараметрыОбработчика.Вставить("Задача", Задача);
		ПараметрыОбработчика.Вставить("ВладелецФорма", ВладелецФорма);
		ОписаниеОповещения = Новый ОписаниеОповещения("ПеренаправитьЗадачуПослеВыбораВариантаОбработкиПодзадач",
		...
		Кнопки.Добавить("ИгнорироватьПодзадачи", НСтр("ru = 'Перенаправить'"));
		Кнопки.Добавить("ПрерватьПодзадачи", НСтр("ru = 'Перенаправить, прервав подзадачи'"));
	...
	ПеренаправитьЗадачуПослеПодзадачи(Задача, ВладелецФорма);
	
	Возврат Ложь;
	
КонецФункции

Процедура ПеренаправитьЗадачуПослеВыбораВариантаОбработкиПодзадач(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = "ИгнорироватьПодзадачи" Тогда 
		
		ПеренаправитьЗадачуПослеПодзадачи(ДополнительныеПараметры.Задача, ДополнительныеПараметры.ВладелецФорма);
		
	ИначеЕсли Результат = "ПрерватьПодзадачи" Тогда 
		
		БизнесПроцессыИЗадачиВызовСервера.ПрерватьПодзадачи(ДополнительныеПараметры.Задача);
		ПеренаправитьЗадачуПослеПодзадачи(ДополнительныеПараметры.Задача, ДополнительныеПараметры.ВладелецФорма);
...
Функция ПеренаправитьЗадачуПослеПодзадачи(Задача, ВладелецФорма)
...
Процедура ЗавершениеПеренаправленияЗадачи(Результат, Параметры) Экспорт
	...
	ЗадачаПеренаправлена = БизнесПроцессыИЗадачиСервер.ПеренаправитьЗадачу(
		Параметры.Задача, Результат, Параметры.УникальныйИдентификаторФормыВладельца,, БизнесПроцесс);
	...
	Если ЗадачаПеренаправлена Тогда
		...
		ПеренаправленныеЗадачи.Вставить(Параметры.Задача, 
			Новый Структура("БизнесПроцесс", БизнесПроцесс) );
...
// Проверяет, что это перенаправление относится к текущему процессу
//
// Параметры
//  СоответствиеЗадачи  - Соответствие, ключ - Задача, значение - Структура("БизнесПроцесс")
//  Процесс  - ссылка на бизнес процесс
//
// Возвращаемое значение:
//   Булево   - Истина, если относится к этому процессу
//
Функция ЭтоОповещениеОПеренаправленииЗадачиЭтогоПроцесса(СоответствиеЗадачи, Процесс) Экспорт
	
	Для Каждого КиЗ Из СоответствиеЗадачи Цикл
		Если КиЗ.Значение.БизнесПроцесс = Процесс Тогда
			Возврат Истина;
		КонецЕсли;	
	КонецЦикла;	
