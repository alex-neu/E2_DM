...
Функция ПолучитьНастройкуОтбораБроней(ПараметрПомещение, ДатаНачала, ДатаОкончания,
	...
	НастройкаОтбораБроней.Вставить("Помещения", ПолучитьМассивПомещений(ПараметрПомещение));
...
Функция ПолучитьТаблицуРекомендаций(Бронь,
	...
	Для Каждого СтрокаРекомендации Из ТаблицаРекомендаций Цикл
		...
		Если Часов <> 0 Тогда
			...
			ДобавитьЗначениеКСтрокеЧерезРазделитель(ПодписьЧасовМинут, " ", ПодписьЧасов);
		КонецЕсли;
		
		Если Минут <> 0 Тогда
			ПодписьМинут = Строка(Минут) + " " + ПодписьМинут;
			ДобавитьЗначениеКСтрокеЧерезРазделитель(ПодписьЧасовМинут, " ", ПодписьМинут);
		КонецЕсли;
...
Функция ПолучитьОписаниеБрони(Бронь, ПолноеОписание = Истина) Экспорт
	...
	ДобавитьЗначениеКСтрокеЧерезРазделитель(ОписаниеБрони, Символы.ПС, ОписаниеПомещения);
	ДобавитьЗначениеКСтрокеЧерезРазделитель(ОписаниеБрони, Символы.ПС, ОписаниеКогда);
	...
	Если ПолноеОписание Тогда
		ОписаниеБрони = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ОписаниеКогда);
		ДобавитьЗначениеКСтрокеЧерезРазделитель(ОписаниеБрони, Символы.ПС, ДанныеБрони.Комментарий);
	Иначе
...
Функция ОтобразитьПланировщикБроней(Планировщик, Параметры) Экспорт
	...
	ОтображаемыеБрони = ПолучитьОтображаемыеБрони(
		РаботаСЧасовымиПоясами.ПривестиКВремениСеанса(ОтображаемаяДатаНачала),
		РаботаСЧасовымиПоясами.ПривестиКВремениСеанса(ОтображаемаяДатаОкончания),
	...
	Для Каждого Бронь Из ОтображаемыеБрони Цикл
		...
		Элемент = Планировщик.Элементы.Добавить(
			?(Бронь.ВесьДень, Бронь.ДатаНачала, РаботаСЧасовымиПоясами.ПривестиКМестномуВремени(Бронь.ДатаНачала)),
			?(Бронь.ВесьДень, Бронь.ДатаОкончания, РаботаСЧасовымиПоясами.ПривестиКМестномуВремени(Бронь.ДатаОкончания)));
		
		ПредставлениеМестногоЧасовогоПояса = ?(Бронь.ВесьДень, "", РаботаСЧасовымиПоясами.ПредставлениеМестногоЧасовогоПояса());
		ПредставлениеЭлемента = ПредставлениеМестногоЧасовогоПояса;
		ДобавитьЗначениеКСтрокеЧерезРазделитель(ПредставлениеЭлемента, ", ", Строка(Бронь.Пользователь));
		ДобавитьЗначениеКСтрокеЧерезРазделитель(ПредставлениеЭлемента, ", ", Бронь.Комментарий);
...
Процедура БронированиеПриЗаписиПредмета(Источник, Отказ) Экспорт
	...
	Если УправлениеМероприятиями.ПолучитьСостояниеМероприятия(Источник.Ссылка, "СостояниеМероприятия")
			<> Перечисления.СостоянияМероприятий.МероприятиеОтменено Тогда
		ЗабронироватьПомещение(
			Источник.Помещение,
			Источник.ДатаНачала,
			Источник.ДатаОкончания,
			Источник.Организатор,
			Источник,
...
Функция ДоступноБронированиеПомещения(Помещение) Экспорт
	...
	Возврат ДокументооборотПраваДоступа.ПолучитьПраваПоОбъекту(Помещение).Изменение;
...
Функция СформироватьТаблицуБроней()
	...
	ТаблицаБроней.Колонки.Добавить(
		"Предмет", Новый ОписаниеТипов("СправочникСсылка.Мероприятия"));
...
//
Функция ПолучитьМассивПомещений(ПараметрПомещение)
...
Функция ПолучитьРекомендациюСтрокой(РекомендацияДатаНачала, РекомендацияДатаОкончания)
	...
	МестнаяДатаНачала = РаботаСЧасовымиПоясами.ПривестиКМестномуВремени(РекомендацияДатаНачала);
	МестнаяДатаОкончания = РаботаСЧасовымиПоясами.ПривестиКМестномуВремени(РекомендацияДатаОкончания);
...
//  ДопустимыеПользователи - Массив - Пользователи, брони которых можно использовать.
//
Процедура ЗабронироватьПомещение(Помещение, ДатаНачала, ДатаОкончания, Пользователь, Источник, ДопустимыеПользователи)
...
Функция ОтображаемыеТерриторииПользователяПоУмолчанию()
	...
	ПомещениеПользователя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущийПользователь, "Помещение");
	РодительПервогоУровня = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПомещениеПользователя, "Родитель");
	...
	// Заполняем все территории, в которые входит помещение пользователя.
	ПомещениеРодитель = РодительПервогоУровня;
	Пока ЗначениеЗаполнено(ПомещениеРодитель) Цикл
		ОтображаемыеТерритории.Добавить(ПомещениеРодитель);
		ПомещениеРодитель = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПомещениеРодитель, "Родитель");
	КонецЦикла;
