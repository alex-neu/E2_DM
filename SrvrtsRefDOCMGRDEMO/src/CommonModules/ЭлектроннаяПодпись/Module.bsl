...
//  УникальныйИдентификатор - УникальныйИдентификатор - Уникальный идентификатор подписи.
//
// Возвращаемое значение:
...
//     * УникальныйИдентификатор - УникальныйИдентификатор - идентификатор подписи, по которому ее можно
//
...
//
Функция УстановленныеПодписи(Объект, УникальныйИдентификатор = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	ЭлектронныеПодписи.Объект = &ПодписанныйОбъект
		|	И ИСТИНА
		|
		|УПОРЯДОЧИТЬ ПО
		|	ДатаПодписи";
	...
	Если ЗначениеЗаполнено(УникальныйИдентификатор) Тогда
		Запрос.Текст = СтрЗаменить(
			Запрос.Текст, 
			"И ИСТИНА", 
			"ЭлектронныеПодписи.УникальныйИдентификатор = &УникальныйИдентификатор");
...
Процедура ОбновитьПодпись(Объект, Знач СвойстваПодписи) Экспорт
	...
	Попытка
		Блокировка.Заблокировать();
		ПодписиОбъекта = УстановленныеПодписи(Объект, СвойстваПодписи.УникальныйИдентификатор);
		...
		Для Каждого ПодписьОбъекта Из ПодписиОбъекта Цикл
			...
			Если ДвоичныеДанныеПодписи = СвойстваПодписи.Подпись Тогда
				...
				ОбновляемаяПодпись.УникальныйИдентификатор = СвойстваПодписи.УникальныйИдентификатор;
				ОбновляемаяПодпись.Объект = Объект;
				ОбновляемаяПодпись.Прочитать();
				ЗаполнитьЗначенияСвойств(ОбновляемаяПодпись, СвойстваПодписи, , "Подпись, Сертификат, УникальныйИдентификатор");
...
//  ЗаписанныйОбъект   - Объект - объект, который был получен и записан, если передавалась ссылка.
//
Процедура УдалитьПодпись(Объект, УникальныйИдентификатор, ИдентификаторФормы = Неопределено,
			ВерсияОбъекта = Неопределено, ЗаписанныйОбъект = Неопределено) Экспорт
	...
	Попытка
		...
		Если ТипЗнч(УникальныйИдентификатор) = Тип("Массив") Тогда
			Для Каждого ЭлементСписка Из УникальныйИдентификатор Цикл
				УдалитьСтрокуПодписи(ОбъектДанных, ЭлементСписка, СообщениеЖурналаРегистрации);
			КонецЦикла;			
		Иначе
			УдалитьСтрокуПодписи(ОбъектДанных, УникальныйИдентификатор, СообщениеЖурналаРегистрации);
		КонецЕсли;
		
		ОбъектДанных.ПодписанЭП = РаботаСЭП.КоличествоПодписей(ОбъектДанных) <> 0;
...
Функция СертификатыШифрования(Объект) Экспорт
	...
	Запрос.Текст = 
		...
		|	СертификатыШифрования.УникальныйИдентификатор КАК УникальныйИдентификатор
		|ИЗ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ОтпечатокСтруктура.Вставить("УникальныйИдентификатор", ВыборкаДетальныеЗаписи.УникальныйИдентификатор);
...
Процедура ЗаписатьСертификатыШифрования(Объект, Знач СертификатыШифрования, ИдентификаторФормы = Неопределено,
	...
	Попытка
		...
		Для Каждого СертификатШифрования Из СертификатыШифрования Цикл
			...
			НовыйСертификат.УникальныйИдентификатор = Новый УникальныйИдентификатор;
		КонецЦикла;
...
//    * СертификатДействителен - Булево - Указывает, верен ли сертификат.
...
Функция ПроверитьПодпись(МенеджерКриптографии, ИсходныеДанные, Подпись, ОписаниеОшибки = Null, НаДату = Неопределено) Экспорт
	...
	РезультатПроверки.Вставить("СертификатДействителен", Истина);
	...
	Если МенеджерКриптографииДляПроверки = Неопределено Тогда
		...
		Если МенеджерКриптографииДляПроверки = Неопределено Тогда
			...
			Если Не ИспользоватьЭлектроннуюПодписьВМоделиСервиса Тогда
				...
				РезультатПроверки.ТекстОшибкиПроверкиПодписи = ОписаниеОшибки;
	...
	ВеренСертификат = ПроверитьСертификат(
		МенеджерКриптографииДляПроверки,
		Сертификат,
		РезультатПроверки.ТекстОшибкиПроверкиСертификата,
		ДатаПодписания);
	
	ОписаниеОшибки = РезультатПроверки.ТекстОшибкиПроверкисертификата;
	
	РезультатПроверки.СертификатДействителен = ВеренСертификат;
...
Процедура ДобавитьСтрокуПодписи(ОбъектДанных, СвойстваПодписи, СообщениеЖурналаРегистрации)
	...
	НоваяЗапись.Версия					= РаботаСЭП.АктуальнаяВерсияПодписи();
	...
	ПодписьЗагруженаИзФайла = СвойстваПодписи.Свойство("ПодписьЗагруженаИзФайла") 
		И (СвойстваПодписи.ПодписьЗагруженаИзФайла = Истина);
	
	// Сразу устанавливаем статус проверки подписи, созданной в программе.
	Если Не ПодписьЗагруженаИзФайла Тогда
		НоваяЗапись.ДатаПроверкиПодписи = НоваяЗапись.ДатаПодписи;
		НоваяЗапись.ПодписьВерна = Истина;
		НоваяЗапись.СертификатДействителен = Истина;
	Иначе
		НоваяЗапись.ДатаПроверкиПодписи = ТекущаяДатаСеанса();
		НоваяЗапись.ПодписьВерна = СвойстваПодписи.Свойство("ПодписьВерна") И СвойстваПодписи.ПодписьВерна;
		Если Не СвойстваПодписи.Свойство("СертификатДействителен") Тогда
			НоваяЗапись.СертификатДействителен = НоваяЗапись.ПодписьВерна;
		Иначе
			НоваяЗапись.СертификатДействителен = СвойстваПодписи.СертификатДействителен;
		КонецЕсли;
		Если СвойстваПодписи.Свойство("ТекстОшибкиПроверкиПодписи") Тогда
			НоваяЗапись.ТекстОшибкиПроверкиПодписи = СвойстваПодписи.ТекстОшибкиПроверкиПодписи;
		КонецЕсли;
		Если СвойстваПодписи.Свойство("ТекстОшибкиПроверкиСертификата") Тогда
			НоваяЗапись.ТекстОшибкиПроверкиСертификата = СвойстваПодписи.ТекстОшибкиПроверкиСертификата;
		КонецЕсли;
	КонецЕсли;
...
Процедура УдалитьСтрокуПодписи(ПодписанныйОбъект, УникальныйИдентификатор, СообщениеЖурналаРегистрации)
	...
	|	РегистрСведений.ЭлектронныеПодписи КАК ЭлектронныеПодписи
	|ГДЕ
	|	ЭлектронныеПодписи.УникальныйИдентификатор = &Идентификатор
	|	И ЭлектронныеПодписи.Объект = &Объект";
	
	Запрос.УстановитьПараметр("Идентификатор", УникальныйИдентификатор);
	Запрос.УстановитьПараметр("Объект", ПодписанныйОбъект.Ссылка);
