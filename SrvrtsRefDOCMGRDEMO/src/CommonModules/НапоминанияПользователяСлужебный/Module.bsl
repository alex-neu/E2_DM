...
Процедура ПроверитьИзмененияДатВПредметеПриЗаписи(Источник, Отказ) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНапоминанияПользователя") Тогда
		...
		НапоминанияПользователяДокументооборот.ПроверитьУстановкуПометкиУдаления(Источник.Ссылка);
	КонецЕсли;
...
Функция СоздатьНапоминание(ПараметрыНапоминания, ВремяСобытия = Неопределено) Экспорт
	...
	Если Напоминание.СпособУстановкиВремениНапоминания = Перечисления.СпособыУстановкиВремениНапоминания.ОтносительноВремениПредмета Тогда
		Если ЗначениеЗаполнено(ВремяСобытия) Тогда
			Напоминание.ВремяСобытия = ВремяСобытия;
		ИначеЕсли ТипЗнч(Напоминание.Источник) = Тип("СправочникСсылка.ЗаписиРабочегоКалендаря")
			И Напоминание.ИмяРеквизитаИсточника = "ДатаНачала" Тогда
			Напоминание.ВремяСобытия =
				НапоминанияПользователяДокументооборот.ПолучитьЗначениеДатыНачалаЗаписиКалендаря(Напоминание.Источник);
		Иначе
...
Процедура ОбновитьНапоминанияПоПредметам(Предметы) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРезультата Цикл
		Если ТипЗнч(СтрокаТаблицы.Источник) = Тип("СправочникСсылка.ЗаписиРабочегоКалендаря")
			И СтрокаТаблицы.ИмяРеквизитаИсточника = "ДатаНачала" Тогда
			ДатаПредмета =
				НапоминанияПользователяДокументооборот.ПолучитьЗначениеДатыНачалаЗаписиКалендаря(СтрокаТаблицы.Источник);
		Иначе
