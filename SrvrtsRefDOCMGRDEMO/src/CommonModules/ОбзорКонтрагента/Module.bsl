...
Процедура ДобавитьЗначениеСКартинкой(HTMLТекст, Поле, Картинка, МассивЗначений, СсылкаНаРесурс = "", ВключаяЗначение = Истина)
	...
	Если ЗначениеЗаполнено(МассивЗначений) И ЗначениеЗаполнено(СсылкаНаРесурс) Тогда
		...
		Для Ит = 0 По Количество - 1 Цикл 
			...
			Если ВключаяЗначение = Истина Тогда 
				Ссылка = СсылкаНаРесурс + Значение;
			Иначе 
				Ссылка = СсылкаНаРесурс;
			КонецЕсли;
	...
	Иначе 
		ОбзорОбъектовКлиентСервер.ДобавитьЗначение(HTMLТекст, НСтр("ru = 'не указан'"), "696969");
...
Функция ПолучитьОбзорКонтрагента(Контрагент, ПоказыватьУдаленные) Экспорт 
	...
	ДобавитьЗначениеСКартинкой(HTMLТекст, НСтр("ru = 'E-mail'"),
		БиблиотекаКартинок.НепрочтенноеПисьмо, Емайл, "message");
		
	ДобавитьЗначениеСКартинкой(HTMLТекст, НСтр("ru = 'Телефон'"),
		БиблиотекаКартинок.Телефон16, Телефон);
		
	Если ЗначениеЗаполнено(Факс) Тогда 
		ДобавитьЗначениеСКартинкой(HTMLТекст, НСтр("ru = 'Факс'"),
			БиблиотекаКартинок.Факс16, Факс);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Сайт) Тогда 
		ДобавитьЗначениеСКартинкой(HTMLТекст, НСтр("ru = 'Сайт'"),
	...
	Иначе 
		ОбзорОбъектовКлиентСервер.ДобавитьРеквизит(HTMLТекст, "", НСтр("ru = 'Адрес не указан'"), "696969");
	КонецЕсли;
	HTMLТекст = HTMLТекст + "</td>";
	HTMLТекст = HTMLТекст + "</tr>";
	HTMLТекст = HTMLТекст + "</table>";
	...
	// ИндексыСПАРКРиски
	Если СПАРКРиски.ИспользованиеРазрешено() Тогда 
		ПредставлениеИндексов = СПАРКРиски.ИндексыСПАРККонтрагента(Контрагент, Истина);
		
		Если ЗначениеЗаполнено(ПредставлениеИндексов.СводныйИндикаторГрадация) Тогда
			
			HTMLТекст = HTMLТекст + "<tr align=""left"">";
			HTMLТекст = HTMLТекст + "<td colspan=""3"">";
			
			СводныйИндикатор = Новый Массив;
			СводныйИндикатор.Добавить(ПредставлениеИндексов.СводныйИндикаторГрадация);
			
			Если ПредставлениеИндексов.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.НекорректныйИНН") Тогда
				// Оба состояния должны обрабатываться одинаково (вывести сообщение об ошибке):
				// - пустой ИНН;
				// - некорректный ИНН.
				
				ТекстКартинки = НСтр("ru='Нет информации о контрагенте'");
				СводныйИндикатор = Новый Массив;
				СводныйИндикатор.Добавить(ТекстКартинки);
				
				ДобавитьЗначениеСКартинкой(HTMLТекст, ТекстКартинки,
					БиблиотекаКартинок.СветофорБелый, СводныйИндикатор,
					ПредставлениеИндексов.НетИнформацииНавигационнаяСсылкаПодсказки, Ложь);
					
			ИначеЕсли ПредставлениеИндексов.СводныйИндикатор = 1 Тогда  
				ДобавитьЗначениеСКартинкой(HTMLТекст, СокрЛП(ПредставлениеИндексов.СводныйИндикаторГрадация),
					БиблиотекаКартинок.СветофорЗеленый, СводныйИндикатор,
					ПредставлениеИндексов.СводныйИндикаторАдресКомандыПодсказки, Ложь);
			ИначеЕсли ПредставлениеИндексов.СводныйИндикатор = 2 Тогда  
				ДобавитьЗначениеСКартинкой(HTMLТекст, СокрЛП(ПредставлениеИндексов.СводныйИндикаторГрадация),
					БиблиотекаКартинок.СветофорОражевый, СводныйИндикатор,
					ПредставлениеИндексов.СводныйИндикаторАдресКомандыПодсказки, Ложь);
			ИначеЕсли ПредставлениеИндексов.СводныйИндикатор = 3 Тогда  
				ДобавитьЗначениеСКартинкой(HTMLТекст, СокрЛП(ПредставлениеИндексов.СводныйИндикаторГрадация),
					БиблиотекаКартинок.СветофорКрасный, СводныйИндикатор,
					ПредставлениеИндексов.СводныйИндикаторАдресКомандыПодсказки, Ложь);
					
			ИначеЕсли ПредставлениеИндексов.Активен <> Истина Тогда
				
				ТекстКартинки = СтрШаблон(НСтр("ru='Контрагент прекратил деятельность %1'"),
					Формат(ПредставлениеИндексов.ДатаСтатуса, "ДЛФ=D"));
	...
	Если Не РеквизитыОбъекта.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо Тогда 
		...
		Если КоличествоКонтактныхЛиц = 0 Тогда 
			HTMLТекст = HTMLТекст + "<tr align=""left"">";
			HTMLТекст = HTMLТекст + "<td colspan=""3"">";
			ОбзорОбъектовКлиентСервер.ДобавитьРеквизит(HTMLТекст, "", НСтр("ru = 'не указаны'"), "696969");
			HTMLТекст = HTMLТекст + "</td>";
			HTMLТекст = HTMLТекст + "</tr>";
		КонецЕсли;
		...
		Для Каждого КонтактноеЛицо Из КонтактныеЛица Цикл
			...
			Если КонтактноеЛицо.ПометкаУдаления Тогда 
				ДолжностьКонтактногоЛица = ДолжностьКонтактногоЛица + " " + НСтр("ru = '(помечен на удаление)'");
			КонецЕсли;
		...
		Если КоличествоКонтактныхЛиц > 2 Тогда 
			...
			HTMLТекст = HTMLТекст + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				НСтр("ru = 'Показать всех'"), КоличествоКонтактныхЛиц);
			HTMLТекст = HTMLТекст + "</td>";
			HTMLТекст = HTMLТекст + "</tr>";
		КонецЕсли;
	КонецЕсли;
