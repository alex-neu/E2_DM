...
Функция СписокСвойствДляВидаОбъектов(ВидОбъектов, Знач ВидСвойств) Экспорт
	...
	ИмяПредопределенныхДанных = ВидОбъектов;
	Если ВидОбъектов = "Справочник.ШаблоныВнутреннихДокументов" Тогда 
		ИмяПредопределенныхДанных = "Справочник.ВнутренниеДокументы";
	ИначеЕсли ВидОбъектов = "Справочник.ШаблоныВходящихДокументов" Тогда 
		ИмяПредопределенныхДанных = "Справочник.ВходящиеДокументы";
	ИначеЕсли ВидОбъектов = "Справочник.ШаблоныИсходящихДокументов" Тогда 
		ИмяПредопределенныхДанных = "Справочник.ИсходящиеДокументы";
	ИначеЕсли ВидОбъектов = "Справочник.ШаблоныИсполнения" Тогда 
		ИмяПредопределенныхДанных = "БизнесПроцесс.Исполнение";
	ИначеЕсли ВидОбъектов = "Справочник.ШаблоныКомплексныхБизнесПроцессов" Тогда 
		ИмяПредопределенныхДанных = "БизнесПроцесс.КомплексныйПроцесс";
	ИначеЕсли ВидОбъектов = "Справочник.ШаблоныОзнакомления" Тогда 
		ИмяПредопределенныхДанных = "БизнесПроцесс.Ознакомление";
	ИначеЕсли ВидОбъектов = "Справочник.ШаблоныПоручения" Тогда 
		ИмяПредопределенныхДанных = "БизнесПроцесс.Поручение";
	ИначеЕсли ВидОбъектов = "Справочник.ШаблоныПриглашения" Тогда 
		ИмяПредопределенныхДанных = "БизнесПроцесс.Приглашение";
	ИначеЕсли ВидОбъектов = "Справочник.ШаблоныРассмотрения" Тогда 
		ИмяПредопределенныхДанных = "БизнесПроцесс.Рассмотрение";
	ИначеЕсли ВидОбъектов = "Справочник.ШаблоныРегистрации" Тогда 
		ИмяПредопределенныхДанных = "БизнесПроцесс.Регистрация";
	ИначеЕсли ВидОбъектов = "Справочник.ШаблоныСогласования" Тогда 
		ИмяПредопределенныхДанных = "БизнесПроцесс.Согласование";
	ИначеЕсли ВидОбъектов = "Справочник.ШаблоныСоставныхБизнесПроцессов" Тогда 
		ИмяПредопределенныхДанных = "БизнесПроцесс.ОбработкаВнутреннегоДокумента";
	ИначеЕсли ВидОбъектов = "Справочник.ШаблоныУтверждения" Тогда 
		ИмяПредопределенныхДанных = "БизнесПроцесс.Утверждение";
	КонецЕсли;
	
	ИмяПредопределенныхДанных = СтрЗаменить(ИмяПредопределенныхДанных, ".", "_");
...
Процедура ДополнительныеРеквизитыОбработкаПроверкиЗаполнения(Источник, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если Не УправлениеСвойствами.ОбъектПоддерживаетПроверкуУсловийДопСвойств(Источник) Тогда
		Возврат;
	КонецЕсли;
	...
	ДопРеквизитыОтбораДляУсловий = УправлениеСвойствами.ДопРеквизитыОтбораДляУсловий(Источник);
	...
	Для Каждого Элемент Из Зависимости Цикл
		...
		Иначе
			...
			ЗначенияРеквизитов.Вставить("ДопРеквизитыОтбораДляУсловий", ДопРеквизитыОтбораДляУсловий);
			...
			Для Каждого СтрокаЗависимостей Из ЗависимостиОбязательностиЗаполнения Цикл
				Если ТипЗнч(СтрокаЗависимостей.Реквизит) = Тип("Строка") Тогда
					Если СтрНачинаетсяС(СтрокаЗависимостей.Реквизит, "ДопРеквизитыОтбораДляУсловий") Тогда
						ПутьКРеквизиту = "Параметры.Форма." + СтрокаЗависимостей.Реквизит;
					Иначе
...
Функция ПолучитьОсновнойНаборСвойствДляОбъекта(ВладелецСвойств)
	...
	Если ИмяОбъектаМетаданных = "ШаблоныВнутреннихДокументов" Тогда 
		ИмяОбъектаМетаданных = "ВнутренниеДокументы";
	ИначеЕсли ИмяОбъектаМетаданных = "ШаблоныВходящихДокументов" Тогда 
		ИмяОбъектаМетаданных = "ВходящиеДокументы";
	ИначеЕсли ИмяОбъектаМетаданных = "ШаблоныИсходящихДокументов" Тогда 
		ИмяОбъектаМетаданных = "ИсходящиеДокументы";
	КонецЕсли;
