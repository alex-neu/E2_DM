	...
	Попытка
		...
		Для каждого ОписаниеПользователя Из СписокПользователей Цикл
			...
			Если ЗначениеЗаполнено(ОписаниеПользователя.EMail) Тогда
				ЭлПочта = ОписаниеПользователя.EMail;
			КонецЕсли;
			...
			СтруктураАдресаЭП = СоставПочтовогоАдреса(ЭлПочта);
			...
			ОбновитьАдресЭлектроннойПочты(ПользовательОбъект, ЭлПочта, СтруктураАдресаЭП);
