...
Процедура УстановитьДоступностьПоСостоянию(Форма, Ссылка, ОбщиеДоступныеПоля = "", ОбщиеНедоступныеПоля = "") Экспорт
	...
	// Особая логика доступности для поля "Форма документа"
	Если ПолучитьФункциональнуюОпцию("РаздельныйУчетДокументов")
		И Форма.Элементы.ФормаДокумента.Доступность = Истина 
		И (ЗначениеЗаполнено(Форма.Объект.ДатаРегистрации)
		Или (Форма.Объект.ФормаДокумента = Перечисления.ВариантыФормДокументов.Бумажная
			И Форма.Элементы.ФормаДокумента.СписокВыбора.Количество() > 1)) Тогда 
		Форма.Элементы.ФормаДокумента.ТолькоПросмотр = Истина;
	КонецЕсли;
...
Функция ПолучитьНастройкиСостояний(ДокументОбъект, Знач Пользователь = Неопределено) Экспорт 
	...
	Для Каждого Настройка Из Настройки Цикл
		...
		КонтейнерыПользователейИзНастройки = Новый Массив;
		Для Каждого Строка Из Настройка.ИспользоватьДля Цикл
			...
			Если ПользователиДокументооборот.ЭтоКонтейнер(Строка.Участник) Тогда
				
				КонтейнерыПользователейИзНастройки.Добавить(Строка.Участник);
			...
			ИначеЕсли ТипЗнч(Строка.Участник) = Тип("Строка") Тогда
				...
				Если ПользователиДокументооборот.ЭтоКонтейнер(ЗначениеАвтоподстановки) Тогда
					
					КонтейнерыПользователейИзНастройки.Добавить(ЗначениеАвтоподстановки);
				...
				ИначеЕсли ТипЗнч(ЗначениеАвтоподстановки) = Тип("Массив") Тогда
					...
					Для Каждого ЗначениеАвтоподстановкиЭлемент Из ЗначениеАвтоподстановки Цикл
						Если ПользователиДокументооборот.ЭтоКонтейнер(ЗначениеАвтоподстановкиЭлемент) Тогда
							КонтейнерыПользователейИзНастройки.Добавить(ЗначениеАвтоподстановкиЭлемент);
						Иначе
		...
		Запрос = Новый Запрос(
			...
			|ГДЕ
			|	ПользователиВКонтейнерах.Контейнер В(&КонтейнерыПользователейИзНастройки)
			|	И СоставСубъектовПравДоступа.Пользователь = &Пользователь
			|	И (СоставСубъектовПравДоступа.ОбъектМетаданных = &ОбъектМетаданных
			|			ИЛИ СоставСубъектовПравДоступа.ОбъектМетаданных = ЗНАЧЕНИЕ(Справочник.ИдентификаторыОбъектовМетаданных.ПустаяСсылка))");
		
		ИдентификаторОМ = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ДокументОбъект.Метаданные());
		Запрос.УстановитьПараметр("Пользователь", Пользователь);
		Запрос.УстановитьПараметр("ОбъектМетаданных", ИдентификаторОМ);
		Запрос.УстановитьПараметр("КонтейнерыПользователейИзНастройки", КонтейнерыПользователейИзНастройки);
...
Функция ПолучитьПерсональныеНастройкиРаботыСДокументамиСервер() Экспорт
	...
	Если ОбщегоНазначения.РазделениеВыключено() Тогда
		Настройки.Вставить("ИспользоватьФайлыУВходящихДокументов", Ложь);
		Настройки.Вставить("ИспользоватьФайлыУИсходящихДокументов", Ложь);
	Иначе
...
Процедура ЗаполнитьДеревоНоменклатурыДел(Дерево, Год, Организация) Экспорт 
	...
	КонецЕсли;	
	Если Условие <> "" Тогда 
		ТекстЗапроса = ТекстЗапроса + " ГДЕ " + Лев(Условие, СтрДлина(Условие) - (СтрДлина("И") + 1));
	КонецЕсли;
...
Функция СформироватьДанныеВыбораДелаХранения(Знач Текст, Знач Организация, Подразделение = Неопределено,
	...
	Если ЗначениеЗаполнено(ФормаДокумента) И ПолучитьФункциональнуюОпцию("РаздельныйУчетДокументов") Тогда 
		Запрос.Текст = Запрос.Текст + "
...
Процедура ПроверитьСоответствиеНоменклатурыДел(СправочникОбъект, Отказ) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетПоОрганизациям") Тогда 
		...
		Если ЗначениеЗаполнено(СправочникОбъект.НоменклатураДел) 
			И СправочникОбъект.Организация <> РеквизитыНД.Организация Тогда 
	...
	Если ПолучитьФункциональнуюОпцию("РаздельныйУчетДокументов") 
		И ЗначениеЗаполнено(СправочникОбъект.НоменклатураДел)
...
Процедура ЗаполнитьСписокФайловДокумента(Форма, ЗаполнитьПризнакОригинал = Ложь, 
	...
	ТаблицаФайлы = ПолучитьДанныеФайловДляСписка(Неопределено, 
		Документ.Ссылка, Форма.ПоказыватьУдаленныеФайлы, ОтметитьЗахваченныеФайлы);
...
// Указывает, возможно ли для докумена указание связанного документа, делающего его не действующим
// 
// Параметры:
//  Документ - любая ссылка - проверяемый документ
// 
// Возвращаемое значение:
//  Булево - Истина, если по документу ведется учет недействительных документов
//                          и возможно указание отменяющего документа
//           Ложь в противном случае
Функция ВозможноУказаниеОтменяющегоДокумента(Документ) Экспорт
	...
	РеквизитыВида = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидДокумента,
		"УчитыватьНедействующиеДокументы, ВариантУказанияОтменяющегоДокумента");
	
	Возврат РеквизитыВида.УчитыватьНедействующиеДокументы
		И (РеквизитыВида.ВариантУказанияОтменяющегоДокумента = ПредопределенноеЗначение(
				"Перечисление.ВариантыУказанияОтменяющегоДокумента.Необязательное")
			ИЛИ РеквизитыВида.ВариантУказанияОтменяющегоДокумента = ПредопределенноеЗначение(
...
Процедура ПометитьФайлыНаУдаление(Форма, МассивФайлов, ПометкаУдаления, 
	...
	КоличествоЗанятыхФайлов = Неопределено) Экспорт 
	...
	Для Каждого Файл Из МассивФайлов Цикл 
		...
		Если ЗначениеЗаполнено(Редактирует) И Редактирует = ПользователиКлиентСервер.ТекущийПользователь() Тогда
...
Процедура ПрочитатьЭтапы(Форма) Экспорт 
	...
	Если Форма.КоличествоЭтапов > 0 И КоличествоПройденных > 0 Тогда
		ЗаголовокГруппы = СтрШаблон(НСтр("ru = 'Этапы (%1 из %2)'"),
			КоличествоПройденных, Форма.КоличествоЭтапов);
	Иначе 
		ЗаголовокГруппы = СтрШаблон(НСтр("ru = 'Этапы (%1)'"), Форма.КоличествоЭтапов);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНастройкиЭтапов(Форма, Отказ) Экспорт 
	...
	Для Каждого Строка Из ЭтапыОбработкиДокументов Цикл
		Если Не ЗначениеЗаполнено(Строка.Наименование) Тогда 
			...
			ТекстОшибки = СтрШаблон(НСтр("ru = 'В строке %1 не заполнено поле ""Наименование этапа""'"),
				НомерСтроки+1);
		...
		Если ЕстьСроки И Не ЗначениеЗаполнено(Строка.Срок) Тогда 
			НомерСтроки = ЭтапыОбработкиДокументов.Индекс(Строка);
			ТекстОшибки = СтрШаблон(НСтр("ru = 'В строке %1 не заполнено поле ""Срок (дней)""'"),
				НомерСтроки+1);
...
//					СправочникСсылка.ИсходящиеДокументы,
//					СправочникСсылка.ВходящиеДокументы - регистрируемый документ.
...
//           - Ложь, в противном случае.
//
Функция ПредметМожетБытьЗарегистрирован(Предмет, РегистрационныйНомер = "",
	...
	Если ИспользоватьНоменклатуруДел Тогда 
		Если Не ЗначениеЗаполнено(ПредметОбъект.НоменклатураДел) Тогда 
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В документе ""%1"" не заполнено поле ""Номенклатура дел""'"),
				Строка(Предмет));
			Если ПоказыватьОшибки Тогда	
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					ТекстСообщения,, "ДеревоПриложений");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьМестаХранения") Тогда
		РаздельныйУчетДокументов = ПолучитьФункциональнуюОпцию("РаздельныйУчетДокументов");
		Если (РаздельныйУчетДокументов
			И ПредметОбъект.ФормаДокумента = Перечисления.ВариантыФормДокументов.Бумажная)
			Или Не РаздельныйУчетДокументов Тогда 
...
Функция СтрокаПодписалУтвердил(Документ) Экспорт
	...
	Если ГрифыУтверждения.Количество() > 0
		...
		РезультатУтверждения = ?(ГрифыУтверждения.Количество() > 0, НСтр("ru = 'Утвержден'"), НСтр("ru = 'Не утвержден'"));
	...
	Если Стороны.Количество() > 0 Или РеквизитыВидаДокумента.ВестиУчетСторон Тогда
		...
		Если ПодписанСторонами Тогда
			РезультатПодписи = НСтр("ru = 'Подписан'");
		Иначе
			РезультатПодписи = НСтр("ru = 'Не подписан'");
		КонецЕсли;
	...
	ИначеЕсли Документ.РезультатПодписания = ПредопределенноеЗначение("Перечисление.РезультатыПодписания.Отклонен") Тогда
		РезультатПодписи = НСтр("ru = 'Отклонен'");
	Иначе
		РезультатПодписи = НСтр("ru = 'Не подписан'");
	КонецЕсли;
...
//  Документ                - СправочникСсылка.ВходящиеДокументы,
//                            СправочникСсылка.ИсходящиеДокументы,
//                            СправочникСсылка.ВнутренниеДокументы - Документ, запись которого выполняется.
//  Ответственный           - СправочникСсылка.Пользователи        - Ответственный за документ.
//  ЭтоНовыйДокумент        - Булево                               - Это новый документ.
//  ПредыдущийОтветственный - СправочникСсылка.Пользователи        - Предыдущий ответственный за документ.
//
...
//  Документ - СправочникСсылка.ВнутренниеДокументы, СправочникСсылка.ВнутренниеДокументы,
//  	СправочникСсылка.ВнутренниеДокументы,- ссылка на документ.
//
// Возвращаемое значение:
...
//  ВидДокумента -  СправочникСсылка.ВидыВнутреннихДокументов, 
//					СправочникСсылка.ВидыИсходящихДокументов,
//					СправочникСсылка.ВидыВходящихДокументов - вид документа.
...
Процедура ОбновитьФормуДокументаУШаблонов(ВидДокумента, ФормаДокумента) Экспорт 
	...
	Если ТипЗнч(ВидДокумента) = Тип("СправочникСсылка.ВидыВходящихДокументов") Тогда 
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ШаблоныВнутреннихДокументов", "ШаблоныВходящихДокументов");
	ИначеЕсли ТипЗнч(ВидДокумента) = Тип("СправочникСсылка.ВидыИсходящихДокументов") Тогда 
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ШаблоныВнутреннихДокументов", "ШаблоныИсходящихДокументов");
	КонецЕсли;
...
Процедура ЗаполнитьСписокВыбораФормыДокумента(Форма) Экспорт 
	...
	Если Не ПолучитьФункциональнуюОпцию("РаздельныйУчетДокументов") Тогда 
		Возврат;
	КонецЕсли;
...
//  Форма - ФормаКлиентскогоПриложения - форма документа 
//
Процедура ОбновитьСведенияОПолученииОригинала(Форма) Экспорт 
	
	Объект = Форма.Объект; Элементы = Форма.Элементы;
	...
	Если Не ПолучитьФункциональнуюОпцию("РаздельныйУчетДокументов")
		Или ОбщегоНазначенияДокументооборот.ЗначениеРеквизитаОбъектаВПривилегированномРежиме(
			Объект.ВидДокумента, "КонтролироватьНаличиеОригинала") = Ложь Тогда 
	...
	Запрос.Текст = 
		...
		|	ОбщиеРеквизитыДокументов.ОригиналПолучил.ПредставлениеВДокументах КАК СотрудникПредставлениеВДокументах
		|ИЗ
		|	РегистрСведений.ОбщиеРеквизитыДокументов КАК ОбщиеРеквизитыДокументов
		|ГДЕ
		|	ОбщиеРеквизитыДокументов.Документ = &Документ";
...
Процедура ОбновитьВидимостьРеквизитовХранения(Форма, Параметры = Неопределено) Экспорт 
	...
	Объект = Форма.Объект;
	Если ПолучитьФункциональнуюОпцию("РаздельныйУчетДокументов") 
		И Объект.ФормаДокумента = Перечисления.ВариантыФормДокументов.Электронная Тогда 
...
//
// Параметры:
//  СправочникОбъект     – СправочникОбъект.ВходящиеДокументы,
//                         СправочникОбъект.ИсходящиеДокументы,
//                         СправочникОбъект.ВнутренниеДокументы - реквизиты которого необходимо проверить
...
Процедура ПроверитьЗаполнениеРеквизитовХранения(СправочникОбъект, ПроверяемыеРеквизиты) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("РаздельныйУчетДокументов") 
		И СправочникОбъект.ФормаДокумента = Перечисления.ВариантыФормДокументов.Электронная Тогда 
		Возврат;
	КонецЕсли;
...
//
// Параметры:
//  Форма – УправляемаяФорма - Форма элемента справочника ВнутренниеДокументы, ВходящиеДокументы или ИсходящиеДокументы,
