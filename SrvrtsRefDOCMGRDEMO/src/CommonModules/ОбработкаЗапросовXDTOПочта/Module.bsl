...
Функция СоздатьИсходящееПисьмо(Сообщение) Экспорт
	...
	Попытка 
		...
		Если Не ЗаписатьДанныеИсходящегоПисьма(Письмо, Объект, Ошибка) Тогда 
			Возврат Ошибка;
		КонецЕсли;
	...
	Исключение
		...
		Ошибка = ОбработкаЗапросовXDTO.СоздатьОбъект("DMError");
		Ошибка.subject = НСтр("ru = 'Ошибка при создании исходящего письма'");
...
Функция ПолучитьНовоеПисьмо(Сообщение) Экспорт
	...
	ПисьмоXDTO = ПолучитьДанныеИсходящегоПисьма(ПисьмоОбъект, Предмет);
...
// Перемещает письма в указанную папку.
//
// Параметры:
//   Сообщение - ОбъектXDTO типа DMMoveEMailRequest
//
// Возвращаемое значение:
//   ОбъектXDTO типа DMOk или DMError
//
Функция ПереместитьПисьма(Сообщение) Экспорт
	
	Попытка
		
		Папка = Справочники.ПапкиПисем.ПолучитьСсылку(Новый УникальныйИдентификатор(Сообщение.folder));
		Если Папка.ПолучитьОбъект() = Неопределено Тогда
			Ошибка = ОбработкаЗапросовXDTO.СоздатьОбъект("DMError");
			Ошибка.subject = НСтр("ru = 'Ошибка при перемещении писем'");
			Ошибка.description = НСтр("ru = 'Не задана папка для перемещения'");
			Возврат Ошибка;
		КонецЕсли;
		
		МассивПисем = Новый Массив;
		МассивОшибок = Новый Массив;
		Для Каждого Письмо Из Сообщение.emails Цикл
			ТекущаяСсылка = ОбработкаЗапросовXDTO.ПолучитьСсылкуПоObjectID(Письмо);
			
			Если ТекущаяСсылка = Неопределено
				Или Не ВстроеннаяПочтаКлиентСервер.ЭтоПисьмо(ТекущаяСсылка) Тогда
				МассивОшибок.Добавить(СтрШаблон(
					НСтр("ru = 'Не удалось получить письмо по ObjectID: %1 %2'"),
					Письмо.type, Письмо.id));
				Продолжить;
			КонецЕсли;
			
			МассивПисем.Добавить(ТекущаяСсылка);
		КонецЦикла;
		
		Если МассивОшибок.Количество() = 0 Тогда
			Результат = ВстроеннаяПочтаСервер.ПоместитьПисьмаВПапку(МассивПисем, Папка);
		Иначе
			Ошибка = ОбработкаЗапросовXDTO.СоздатьОбъект("DMError");
			Ошибка.subject = НСтр("ru = 'Ошибка при перемещении писем'");
			Ошибка.description = СтрСоединить(МассивОшибок, Символы.ПС);
			Возврат Ошибка;
		КонецЕсли;
		
		Если Результат.ЕстьОшибки Тогда
			Ответ = ОбработкаЗапросовXDTO.СоздатьОбъект("DMError");
			Ответ.subject = НСтр("ru = 'Ошибка при перемещении писем'");
			Ответ.description = СтрСоединить(Результат.Ошибки, Символы.ПС);
		Иначе
...
Функция ПолучитьОтветНаВходящееПисьмо(Сообщение) Экспорт
	...
	ПисьмоXDTO =  ПолучитьДанныеИсходящегоПисьма(ПисьмоОбъект);
...
//
Функция ПолучитьДанныеПисьма(ОбъектИд) Экспорт
	...
	Если ОбъектИд.type = "DMOutgoingEMail" Тогда
		Ответ = ПолучитьДанныеИсходящегоПисьма(СсылкаНаПисьмо);
		
	ИначеЕсли ОбъектИд.type = "DMIncomingEMail" Тогда
		Ответ = ПолучитьДанныеВходящегоПисьма(СсылкаНаПисьмо);
...
Функция ПолучитьДеревоПисем(Сообщение) Экспорт
	...
	Исключение
		...
		Ошибка = ОбработкаЗапросовXDTO.СоздатьОбъект("DMError");
		Ошибка.subject = НСтр("ru = 'Ошибка при получении списка писем'");
		Инфо = ИнформацияОбОшибке();
		Ошибка.description = ОбработкаЗапросовXDTO.ПолучитьОписаниеОшибки(Инфо);
...
Функция ПолучитьАдреснуюКнигу(Сообщение) Экспорт
	...
	Исключение
		...
		Ошибка = ОбработкаЗапросовXDTO.СоздатьОбъект("DMError");
		Ошибка.subject = НСтр("ru = 'Ошибка при получении адресной книги'");
		Инфо = ИнформацияОбОшибке();
		Ошибка.description = ОбработкаЗапросовXDTO.ПолучитьОписаниеОшибки(Инфо);
...
Процедура ЗаполнитьПользователейВГруппах(Ответ, РодительID)
	...
	Если РодительID = Строка(Справочники.РабочиеГруппы.ВсеПользователи.УникальныйИдентификатор()) Тогда
		Запрос.Текст = 
			...
			|	ВЫБРАТЬ РАЗЛИЧНЫЕ
			|		ИсполнителиЗадач.РольИсполнителя,
			|		ИсполнителиЗадач.РольИсполнителя.Представление,
			|		ЛОЖЬ
			|	ИЗ
			|		РегистрСведений.ИсполнителиЗадач КАК ИсполнителиЗадач
			|	ГДЕ
...
// 
Функция ЗаписатьДанныеИсходящегоПисьма(Письмо, ОбъектXDTO, Ошибка)
	
	Письмо.Заблокировать();
	ЗаполнитьДанныеИсходящегоПисьма(Письмо, ОбъектXDTO);
	
	// проверка заполнения
	ТекстСообщения = "";
	Если Не ОбработкаЗапросовXDTO.ПроверитьЗаполнение(Письмо, ТекстСообщения) Тогда 
		Ошибка = ОбработкаЗапросовXDTO.СоздатьОбъект("DMError");
		Ошибка.subject = НСтр("ru = 'Ошибка при записи исходящего письма'");
...
//
Функция ПолучитьДанныеИсходящегоПисьма(СсылкаНаПисьмо, Предмет = Неопределено)
	...
	ПередатьОбщиеРеквизитыПисьма(СсылкаНаПисьмо, Ответ, Предмет);
...
//
Функция ПолучитьДанныеВходящегоПисьма(СсылкаНаПисьмо)
	
	Ответ = ОбработкаЗапросовXDTO.СоздатьОбъект("DMIncomingEMail");
	Ответ.objectId = ОбработкаЗапросовXDTO.ПолучитьObjectIDпоСсылке(СсылкаНаПисьмо.Ссылка);
	ПередатьОбщиеРеквизитыПисьма(СсылкаНаПисьмо, Ответ);
...
//
Процедура ПередатьОбщиеРеквизитыПисьма(СсылкаНаПисьмо, Ответ, Предмет = Неопределено)
		...
		ИначеЕсли ДелопроизводствоКлиентСервер.ЭтоДокумент(Предмет) 
			...
			Если ДелопроизводствоКлиентСервер.ЭтоВходящийДокумент(Предмет) 
				...
				Если ЗначениеЗаполнено(ПредметПереписки) Тогда
					ЗаполнитьПредметПисьма(Ответ, ПредметПереписки);
				КонецЕсли;
			КонецЕсли;
...
//   ОбъектXDTO - ОбъектXDTO типа DMOutgoingEMail
//
Процедура ЗаполнитьДанныеИсходящегоПисьма(Письмо, ОбъектXDTO) 
	...
	Текст = "";
	ОбработкаЗапросовXDTO.ЗаполнитьРеквизитИзСвойстваXDTO(Текст, ОбъектXDTO, "body"); 
	Письмо.ТекстХранилище = Новый ХранилищеЗначения(Текст);
	
	ОбработкаЗапросовXDTO.ЗаполнитьРеквизитИзСвойстваXDTO(Письмо.Тема, ОбъектXDTO, "subject");
	...
	Если ЗначениеЗаполнено(ОбъектXDTO.senderAddress) Тогда
		...
		Иначе
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не найдено подходящей учетной записи для отправки через адрес ""%1""'"),
	...
	Для каждого Строка Из Получатели Цикл
		...
		Исключение
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Указан некорректный адрес получателя: %1'"),
				Строка);
			ВызватьИсключение(Текст);
		КонецПопытки;
	...
	Для каждого Строка Из ПолучателиКопии Цикл
		...
		Исключение
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Указан некорректный адрес получателя копии: %1'"),
				Строка);
			ВызватьИсключение(Текст);
		КонецПопытки;
	...
	Для каждого Строка Из ПолучателиСкрытойКопии Цикл
		...
		Исключение
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Указан некорректный адрес получателя скрытой копии: %1'"),
				Строка);
			ВызватьИсключение(Текст);
		КонецПопытки; 
...
//
Процедура ЗаполнитьПредметПисьма(ПисьмоXDTO, СсылкаНаПредмет)
