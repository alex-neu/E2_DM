...
Функция РабочийКаталогПользователя() Экспорт
	...
	Возврат ФайловыеФункцииСлужебныйКлиент.РабочийКаталогПользователя();
...
Процедура ЗакончитьРедактированиеСРасширением(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.СоздатьНовуюВерсию = Неопределено Тогда
		...
		ОткрытьФорму("Справочник.Файлы.Форма.ФормаВозвратаФайла", СтруктураПараметров, , , , , Обработчик);
...
Процедура ОбновитьИзФайлаНаДиске(ОбработчикРезультата, ДанныеФайла, ИдентификаторФормы)
	...
	Если ЗначениеЗаполнено(РасширениеДляЗашифрованныхФайлов) Тогда
		...
		Диалог.Фильтр = СтрШаблон(Фильтр, ДанныеФайла.Расширение, РасширениеДляЗашифрованныхФайлов);
	Иначе
		Фильтр = НСтр("ru = 'Файл (*.%1)|*.%1|Все файлы (*.*)|*.*'");
		Диалог.Фильтр = СтрШаблон(Фильтр, ДанныеФайла.Расширение);
	КонецЕсли;
...
Процедура РедактироватьФайлПослеЗанятияФайла(ДанныеФайла, ПараметрыВыполнения) Экспорт
	...
	Если ФайловыеФункцииСлужебныйКлиент.РасширениеРаботыСФайламиПодключено() Тогда
		Обработчик = Новый ОписаниеОповещения(
			"РедактироватьФайлСРасширениемПослеПолученияФайлаВРабочийКаталог", 
			ЭтотОбъект, ПараметрыВыполнения);
		ПолучитьФайлВерсииВРабочийКаталог(
			Обработчик,
			ПараметрыВыполнения.ДанныеФайла,
...
Процедура ОсвободитьФайлПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ХранитьВерсии = Неопределено Тогда
		...
		ПараметрыВыполнения.РедактируетТекущийПользователь     = ДанныеФайла.РедактируетТекущийПользователь;
...
Процедура СохранитьИзмененияФайлаСРасширением(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.СоздатьНовуюВерсию = Неопределено Тогда
		...
		Если ДанныеФайла.ХранитьВерсии Тогда
			...
			// Если автор текущей версии не текущий пользователь, тогда
			// отключается доступность флажка «Не создавать новую версию».
			Если ДанныеФайла.АвторТекущейВерсии <> ПараметрыВыполнения.ДанныеФайла.Редактирует Тогда
				СоздатьНовуюВерсиюДоступность = Ложь;
			Иначе
				СоздатьНовуюВерсиюДоступность = Истина;
			КонецЕсли;
...
//
Процедура УдалитьФайлИзРабочегоКаталога(ОбработчикРезультата, Ссылка, 
	УдалитьВРабочемКаталоге = Ложь, ЗадаватьВопрос = Истина) Экспорт
...
Процедура ПроверитьМаксимальнуюДлинуПолногоПутиВРабочемКаталогеЗапуск(ПараметрыВыполнения)
	...
	Если СтрДлина(ПараметрыВыполнения.НормальноеИмяФайла) > МаксимальнаяДлинаИмениФайла Тогда
		ТекстСообщения = ТекстСообщения + Символы.ВК + Символы.ВК
			+ НСтр("ru = 'Измените имя файла на более короткое.'");
		ВернутьРезультатПослеПоказаПредупреждения(
			ПараметрыВыполнения.ОбработчикРезультата, ТекстСообщения, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура ИмпортФайловПослеПроверкиРазмеров(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.КоличествоСуммарное = 0 Тогда
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, 
			НСтр("ru = 'Нет файлов для добавления'"), Неопределено);
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьНапоминаниеПередПоместитьФайл(ОбработчикРезультата)
	...
	Если ПерсональныеНастройки.ПоказыватьПодсказкиПриРедактированииФайлов = Истина Тогда
		Если Не ФайловыеФункцииСлужебныйКлиент.РасширениеРаботыСФайламиПодключено() Тогда
			ОткрытьФорму("Справочник.Файлы.Форма.ФормаНапоминанияПередПоместитьФайл", , , , , , ОбработчикРезультата);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура Зашифровать(ОбработчикРезультата, ДанныеФайла, УникальныйИдентификатор) Экспорт
	...
	Если ПараметрыВыполнения.ДанныеФайла.КоличествоВерсий > 1 Тогда
		ПредставлениеФайла = ПредставлениеФайла + " (" + СтрШаблон(
...
//           * 3 - со сканера.
//   ПараметрыДобавления - Булево или Структура с полями  МаксимальныйРазмер ФильтрДиалогаВыбора НеОткрыватьКарточку
//
Процедура ДобавитьФайл(
	...
	Если ПараметрыДобавления = Неопределено
		...
		ПараметрыВыполнения.Вставить("НеОткрыватьКарточкуПослеСозданияИзФайла", 
	...
	Иначе
		...
		ПараметрыВыполнения.Вставить("НеОткрыватьКарточкуПослеСозданияИзФайла", 
			ПараметрыДобавления.НеОткрыватьКарточку);
	КонецЕсли;
	...
	Обработчик = Новый ОписаниеОповещения("ДобавитьПослеВыбораРежимаСоздания", 
		ЭтотОбъект, ПараметрыВыполнения);

	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("РежимСоздания", РежимСоздания);
	ПараметрыФормы.Вставить("ДоступнаКомандаСканировать", 
		РаботаСФайламиСлужебныйКлиентПовтИсп.ДоступнаКомандаСканировать());
	
	ОткрытьФорму("Справочник.Файлы.Форма.ФормаНовогоСШаблонами", ПараметрыФормы, , , , , Обработчик);
...
Процедура ДобавитьНаОсновеШаблонаПослеВыбораШаблона(Результат, ПараметрыВыполнения) Экспорт
	...
	ОписаниеОповещенияОЗакрытии = ОбработчикЗавершения(ПараметрыВыполнения.ОбработчикРезультата);
	РаботаСФайламиКлиент.СкопироватьФайл(ПараметрыВыполнения.ВладелецФайла, 
...
Процедура ПослеПодписанияФайла(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	Если ОписаниеДанных.Успех Тогда
		...
		Если ЗначениеЗаполнено(ПараметрыВыполнения.ДанныеФайла.Владелец) Тогда
...
Процедура ОткрытьФайлПриложением(ДанныеФайла, ИмяОткрываемогоФайла, 
	ОбработчикРезультата = Неопределено, ИдентификаторВладельца = Неопределено)
	...
	Если HtmlФайлыСпособОткрытия = ПредопределенноеЗначение("Перечисление.СпособыОткрытияФайлаНаПросмотр.ВоВстроенномРедакторе") Тогда
		...
		Если ФайловыеФункцииКлиентСервер.РасширениеФайлаВСписке(HtmlФайлыРасширение, ДанныеФайла.Расширение) Тогда
			...
			ПараметрыФормы = Новый Структура("Файл, ДанныеФайла, ИмяОткрываемогоФайла, УникальныйИдентификатор", 
				ДанныеФайла.Ссылка, ДанныеФайла, ИмяОткрываемогоФайла, ИдентификаторВладельца);
	...
	Если ИзображенияСпособОткрытия = ПредопределенноеЗначение(
		...
		ОткрытьФорму("Справочник.Файлы.Форма.ПросмотрИзображений", 
			ПараметрыФормы, ИдентификаторВладельца, ДанныеФайла.КодФайла);
...
Функция ПроверитьЗанятыеФайлыПриЗавершенииРаботы()
	...
	ПрикладнаяФормаПредупреждения = "Справочник.Файлы.Форма.СписокЗанятыхФайлов";
	...
	Если ПараметрыВыполнения.ТекущийШаг = 1 Тогда
		Если ПараметрыВыполнения.СпособСравненияВерсийФайлов = Неопределено Тогда
			...
			Если ПараметрыВыполнения.СпособСравненияВерсийФайлов = Неопределено Тогда
				...
				ОткрытьФорму("Справочник.ВерсииФайлов.Форма.ФормаВыбораСпособаСравненияВерсий", ,
	...
	Если ПараметрыВыполнения.ТекущийШаг = 2 Тогда
		...
		Если ПараметрыВыполнения.Свойство("ВладелецВерсии") И ЗначениеЗаполнено(ПараметрыВыполнения.ВладелецВерсии) Тогда
			ПараметрыВыполнения.ДанныеФайла1 = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляОткрытия(
				ПараметрыВыполнения.Ссылка1, ПараметрыВыполнения.УникальныйИдентификатор);
			ПараметрыВыполнения.ДанныеФайла2 = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляОткрытия(
				ПараметрыВыполнения.Ссылка2, ПараметрыВыполнения.УникальныйИдентификатор);
		Иначе
			ПараметрыВыполнения.ДанныеФайла1 = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляОткрытия(
				ПараметрыВыполнения.Ссылка1, ПараметрыВыполнения.УникальныйИдентификатор);
			ПараметрыВыполнения.ДанныеФайла2 = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляОткрытия(
...
// Для работы метода на веб-клиенте необходимо предварительно подключить ВернутьРезультатПослеПоказаПредупреждениярасширение для работы с 1С:Предприятием.
//
...
#Если Не ВебКлиент И НЕ МобильныйКлиент Тогда
	...
	Если РасширениеФайлаВСписке Тогда
		Возврат ИзвлечьТекстИзТекстовогоФайла(
...
#КонецЕсли
	...
	Если ПустаяСтрока(ИзвлеченныйТекст) Тогда
		...
		Если РасширениеФайлаВСписке Тогда
			Возврат ФайловыеФункцииКлиентСервер.ИзвлечьТекстOpenDocument(ПолноеИмяФайла, Отказ);
		КонецЕсли;
...
Процедура УдалитьДанныеПослеОтветаНаВопрос(Ответ, Контекст) Экспорт
	...
	Иначе
		ОповеститьОбУдаленииДанных(Контекст.ОбработчикЗавершения, Результат.ТекстПредупреждения, Контекст);
	КонецЕсли;
...
Процедура ПослеУдаленияФайлаИзКеша(РезультатУдаления, Контекст) Экспорт
	...
	ОповеститьОбУдаленииДанных(Контекст.ОбработчикЗавершения, Контекст.ТекстПредупреждения, Контекст);
	
КонецПроцедуры

Процедура ОповеститьОбУдаленииДанных(ОбработчикЗавершения, ТекстПредупреждения, Контекст)
	
	ФайлСсылка = Контекст.Файлы[Контекст.ИндексУдаляемогоФайла];
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("Владелец", Неопределено);
	...
	Оповестить("Запись_Файл", ПараметрыОповещения, ФайлСсылка);
	ОповеститьОбИзменении(ФайлСсылка);
