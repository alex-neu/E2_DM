...
Функция ПодсистемаСуществует(ПолноеИмяПодсистемы, ПроверятьИсключения = Истина) Экспорт
	...
	Если ПроверятьИсключения Тогда
		Если ПолноеИмяПодсистемы = "СтандартныеПодсистемы.БизнесПроцессыИЗадачи"
			Или ПолноеИмяПодсистемы = "СтандартныеПодсистемы.УправлениеДоступом"
			Или ПолноеИмяПодсистемы = "СтандартныеПодсистемы.КалендарныеГрафики" Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
// Возвращает признак отключения разделителя в режиме разделения данных по областям. 
// 
// Возвращаемое значение:
//  Булево - Истина, если разделение включено для ИБ, но выключено для текущего сеанса.
// 
Функция РазделениеВыключено() Экспорт
	
	Возврат РазделениеВключено() И Не ПараметрыСеанса.ОбластьДанныхИспользование; 
...
// Возвращает признак доступности изменения общих, неразделенных данных ИБ.
// 
// Возвращаемое значение:
//  Булево - Истина, если есть право доступа.
//		   - Ложь, нет доступа.
//
Функция ДоступноИзменениеНеРазделенныхДанных() Экспорт
	
	Возврат СтандартныеПодсистемыПовтИсп.ДоступноИзменениеНеРазделенныхДанных();
	
КонецФункции

Функция ЭтоРазделенныйОбъектМетаданных(ОбъектМетаданных) Экспорт
	
	Возврат РазделениеВыключено() И РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(ОбъектМетаданных);
...
//
Процедура ПриНачалеВыполненияРегламентногоЗадания(РегламентноеЗадание = Неопределено, Отказ = Неопределено) Экспорт
	...
	// Неразделенное регламентное задание в разделенной ИБ.
	Если РегламентноеЗадание <> Неопределено И РазделениеВключено() И РазделениеВыключено()
		И РаботаВМоделиСервисаПереопределяемый.ПриНачалеВыполненияНеразделенногоРегламентногоЗадания(РегламентноеЗадание, Отказ) Тогда
		Возврат;
	КонецЕсли;
