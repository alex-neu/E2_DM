...
Процедура ПроверитьЗаполнениеСторон(СправочникОбъект, Отказ) Экспорт 
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетПоОрганизациям") Тогда
		...
		Если ТипЗнч(СправочникОбъект) = Тип("СправочникОбъект.ВнутренниеДокументы") Тогда
			ЭтоЗаявкаНаОплату = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СправочникОбъект.ВидДокумента,
...
Функция ДанныеПодписанияДокумента(Документ) Экспорт
	...
	Если РеквизитыВидаДокумента.ВестиУчетСторон Тогда
				...
				Для каждого СторонаДокумента Из Документ.Стороны Цикл
					Если РаботаСПодписямиДокументовКлиентСервер.ЭтоОрганизация(СторонаДокумента.Сторона)
						...
						ТекстОшибки = СтрШаблон(
							НСтр("ru = 'Документ ""%1"" уже подписан ответственным лицом ""%2"".'"),
							Строка(Документ),
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Документ ""%1"" не подписывается ответственным лицом с нашей стороны.'"),
				Строка(Документ));
	...
	ИначеЕсли РеквизитыВидаДокумента.ИспользоватьПодписание
		И Документ.РезультатПодписания = Перечисления.РезультатыПодписания.Подписан Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Документ ""%1"" уже подписан ответственным лицом.'"),
				Строка(Документ));
			ТекстОшибки = ТекстОшибки;
			Результат = Истина;
...
Функция ОпределитьСостояниеПодписи(Документ) Экспорт
	...
	Если ДелопроизводствоКлиентСервер.ЭтоВнутреннийДокумент(Документ) Тогда 
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Документ,
			"ВидДокумента.ВестиУчетСторон, ВидДокумента.ВариантПодписания, Стороны");
		
		Если РеквизитыДокумента.ВидДокументаВестиУчетСторон = Истина Тогда
			ДокументПодписан = ДокументПодписанСторонами(РеквизитыДокумента.Стороны.Выгрузить(),
				РеквизитыДокумента.ВидДокументаВариантПодписания);
	...
	Иначе
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Процесс ""Подписания"" не может быть использован для ""%1"". Используйте для этого процесс ""Утверждения"".
