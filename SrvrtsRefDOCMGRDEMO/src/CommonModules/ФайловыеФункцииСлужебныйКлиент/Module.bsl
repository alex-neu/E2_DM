...
Процедура ПолучитьРабочийКаталогДанныхПользователяПослеОшибкиПолучения(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	Результат.Вставить("ОписаниеОшибки", СтрШаблон(
...
//
//  ОбработчикЗавершения - ОписаниеОповещения - Описание оповещения, которое необходимо выполнить по завершению проверки подписей.
//
Процедура ПроверитьПодписи(Форма, СсылкаНаДвоичныеДанные, 
	ВыделенныеСтроки = Неопределено, УникальныйИдентификатор = Неопределено,
...
Процедура ПроверитьПодписиПослеПодготовкиДанных(Данные, ДополнительныеПараметры)
	...
	ПроверятьЭлектронныеПодписиНаСервере = 
		МодульЭлектроннаяПодписьКлиент.ОбщиеНастройки().ПроверятьЭлектронныеПодписиНаСервере;
...
Процедура ПроверитьПодписиПослеПроверкиСтроки(Результат, ДополнительныеПараметры) Экспорт
	...
	СтрокаПодписи.ПодписьВерна = 
		ТипЗнч(Результат) = Тип("Булево") И Результат
		Или ТипЗнч(Результат) = Тип("Структура") И Не Результат.Свойство("ТекстОшибкиПроверкиПодписи");
	СтрокаПодписи.СертификатДействителен = 
		ТипЗнч(Результат) = Тип("Булево") И Результат
		Или ТипЗнч(Результат) = Тип("Структура") И Не Результат.Свойство("ТекстОшибкиПроверкиСертификата");
...
// Возвращает массив уникальных модификаторов подписей из таблицы формы
//
// Параметры:
//  ТаблицаПодписей  - ДанныеФормыКоллекция - Таблица формы, содержащая электронные подписи объекта
//  ВыделенныеСтроки  - Массив из Число, Неопределено - Массив уникальных идентификаторов выделенных строк
//                                                    Если указан, то будут возвращены УИД только этих строк.
//
// Возвращаемое значение:
//   Массив из УникальныйИдентификатор   - Массив, содержащий уникальныйе идентификаторы подписей.
//                          Если указан параметр ВыделенныеСтроки, то будут возвращены УИД только этих строк,
//                          в ином случае - всех строк
//
Функция УникальныеИдентификаторыПодписей(ТаблицаПодписей, ВыделенныеСтроки = Неопределено) Экспорт
	
	УИДПодписей = Новый Массив;
	
	Для Каждого СтрокаПодписи Из ТаблицаПодписей Цикл
		Если ВыделенныеСтроки = Неопределено
			ИЛИ ВыделенныеСтроки.Найти(СтрокаПодписи.ПолучитьИдентификатор()) <> Неопределено Тогда
			
			УИДПодписей.Добавить(СтрокаПодписи.УникальныйИдентификатор);
		КонецЕсли;
	КонецЦикла;
...
//
// Параметры:
//  ТаблицаПодписей  - ДанныеФормыКоллекция - Таблица установленных ЭП объекта
...
//
Процедура ОбработатьПроверкуПодписей(ТаблицаПодписей, УИДПодписей, УникальныйИдентификатор) Экспорт
	...
	ИначеЕсли УИДПодписей.Количество() = 1 Тогда
		...
		Для Каждого СтрокаПодписи Из ТаблицаПодписей Цикл
			Если СтрокаПодписи.УникальныйИдентификатор = УИДПодписей[0] Тогда
				РаботаСЭПКлиент.ОткрытьПодпись(СтрокаПодписи, УникальныйИдентификатор);
				Возврат;
			КонецЕсли;
		КонецЦикла;
