...
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения с виджетами
	...
	Для ИндексСтроки = 1 По КоличествоСтрок Цикл
		...
		Для ИндексСтолбца = 1 По КоличествоСтолбцов Цикл
			...
			ИначеЕсли ТекущийВиджет = Справочники.Виджеты.Отсутствия Тогда
				ЗаполнитьВиджетОтсутствия(Параметры);
			Иначе
				Форма["HTML" + ИндексВиджета] = HTMLПредставлениеПустогоВиджета;
			КонецЕсли;
...
Процедура РасчетПоказателейРаботыПользователей() Экспорт
	...
	Отказ = Ложь;
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.РасчетПоказателейРаботыПользователей, Отказ);
...
// Параметры:
//   Параметры - Структура
//                  - Форма - ФормаКлиентскогоПриложения - форма Текущие дела.
//                  - ИндексВиджета - Строка - индекс виджета в форме.
...
Процедура ЗаполнитьВиджетМоиЗадачи(Параметры)
	...
	МоиЗадачи.Заголовок = НСтр("ru = 'Задачи мне'");
...
Процедура ЗаполнитьВиджетЗадачиОтдела(Параметры)
	...
	ЗадачиОтдела.Заголовок = НСтр("ru = 'Задачи отдела'");
	...
	ЗадачиОтдела_НеПринятые.Заголовок = НСтр("ru = 'Не принято'");
	...
	ЗадачиОтдела_Просроченные.Заголовок = НСтр("ru = 'Просрочено'");
...
Процедура ЗаполнитьВиджетМоиДокументы(Параметры)
	...
	Если ДоступныВнутренниеДокументы Тогда
		Запрос.Текст = Запрос.Текст +
			...
			|			ИЛИ МоиДокументы.Причина = ЗНАЧЕНИЕ(Перечисление.ПричиныДобавленияВМоиДокументы.ЯвляюсьОтветственнымЗаДокумент))
			|	И МоиДокументы.Пользователь = &ТекущийПользователь
			|	И ВнутренниеДокументы.ДатаОкончанияДействия >= НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ)
			...
			|			ИЛИ МоиДокументы.Причина = ЗНАЧЕНИЕ(Перечисление.ПричиныДобавленияВМоиДокументы.ЯвляюсьОтветственнымЗаДокумент))
			|	И МоиДокументы.Пользователь = &ТекущийПользователь
			|	И ВнутренниеДокументы.СрокИсполнения >= НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ)
			|	И ВнутренниеДокументы.СрокИсполнения <= КОНЕЦПЕРИОДА(ДОБАВИТЬКДАТЕ(&ТекущаяДата, ДЕНЬ, 2), ДЕНЬ)
			|	И ВнутренниеДокументы.ВидДокумента.ИспользоватьСрокИсполнения
			|	И НЕ ВнутренниеДокументы.ПометкаУдаления
			|	И НЕ ВнутренниеДокументы.НеДействует
			|	И ТекущиеСостоянияДокументов.Состояние ЕСТЬ NULL 
			|;
	...
	МоиДокументы_Просроченные.Заголовок = НСтр("ru = 'Просрочено'");
	...
	ЗначениеМоиДокументы.Заголовок = НСтр("ru = 'Мои документы'");
	...
	МоиДокументы_ВходящиеБезОтвета.Заголовок = НСтр("ru = 'Без ответа'");
	...
	МоиДокументы_ВнутренниеСИстекающимСрокомДействия_Исполнения.Заголовок = НСтр("ru = 'Истекают'");
...
Процедура ЗаполнитьВиджетСВД(Параметры)
	...
	СВД_ЗагруженныеНаПроверку.Заголовок = НСтр("ru = 'На проверку'");
	...
	СВД_СОшибкой.Заголовок = НСтр("ru = 'С ошибкой'");
...
Процедура ЗаполнитьВиджетМероприятия(Параметры)
	...
	Пока Выборка.Следующий() Цикл
		...
		МестнаяДатаНачала = РаботаСЧасовымиПоясами.ПривестиКМестномуВремени(Выборка.ДатаНачала);
		СтрСписка.Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"<FONT color=""%1"">%2</FONT>",
			ЦветаВиджета.ЦветПодписи,
			Формат(МестнаяДатаНачала, "ДФ=dd.MM"));
...
Процедура ЗаполнитьВиджетФорум(Параметры)
	...
	Форум_НовыеОтветыНаСообщенияПользователя.Заголовок = НСтр("ru = 'Новые ответы'");
...
Процедура ЗаполнитьВиджетКонтроль(Параметры)
	...
	Контроль_ЗаголовокВиджета.Заголовок = НСтр("ru = 'Контроль'");
	...
	Контроль_Просрочено.Заголовок = НСтр("ru = 'Просрочено'");
	...
	Иначе
		...
		ТекстНетОбъектовНаКонтроле = НСтр("ru = 'Нет ничего для контроля'");
...
Процедура ЗаполнитьВиджетЕжедневныеОтчеты(Параметры)
	...
	УчетВремениДиаграмма.ОбластьПостроения.Шрифт = Новый Шрифт("Microsoft Sans Serif", 7);
	...
	УчетВремениДиаграмма.Серии[1].Текст = НСтр("ru = 'Нет отчета'");
...
//                  - ТекущийПользователь - СправочникСсылка.Пользователи - Ссылка на текущего пользователя.
//
Процедура ЗаполнитьВиджетОтсутствия(Параметры)
	...
	Запрос = Новый Запрос(
		...
		|		ПО Отсутствие.Сотрудник = СведенияОПользователяхДокументооборот.Пользователь
		|ГДЕ
		...
		|	И СведенияОПользователяхДокументооборот.Подразделение В ИЕРАРХИИ(&ПодразделениеВИерархии)
		|
		|УПОРЯДОЧИТЬ ПО
		|	БудуРазбиратьЗадачи,
		|	ДатаОкончания");
	...
	Если Не ЗначениеЗаполнено(ОтборПодразделение) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СведенияОПользователяхДокументооборот КАК СведенияОПользователяхДокументооборот", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "		ПО Отсутствие.Сотрудник = СведенияОПользователяхДокументооборот.Пользователь", "");
	КонецЕсли;
	...
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "	И СведенияОПользователяхДокументооборот.Подразделение В ИЕРАРХИИ(&ПодразделениеВИерархии)", "");
	КонецЕсли;
	...
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "	И СведенияОПользователяхДокументооборот.Подразделение = &Подразделение", "");
	КонецЕсли;
	...
	Для Каждого СтрокаОтсутствия Из ТаблицаОтсутствий Цикл
		...
		СтрокаСписка = СтруктураПоказателяДляСписка();
		
		СтрокаСписка.Значение = ПредставлениеЗначения(
			СтрокаОтсутствия.Сотрудник, ДопустимаяДлинаЗначения);
		
		СтрокаСписка.Ссылка = ПолучитьНавигационнуюСсылку(СтрокаОтсутствия.Ссылка);
		
		Если СтрокаОтсутствия.БудуРазбиратьЗадачи Тогда
			КартинкаСтроки = КартинкаЦветаСобытияКалендаря(Перечисления.ЦветаРабочегоКалендаря.Желтый)
		Иначе
			КартинкаСтроки = КартинкаЦветаСобытияКалендаря(Перечисления.ЦветаРабочегоКалендаря.Красный)
		КонецЕсли;
		ОбзорОбъектовКлиентСервер.ДобавитьКартинку(СтрокаСписка.Картинка, КартинкаСтроки);
		СтрокаСписка.Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"<FONT color=""%1"">%2</FONT>",
			ЦветаВиджета.ЦветПодписи,
			Формат(СтрокаОтсутствия.ДатаОкончания, "ДФ=dd.MM"));
		
		СтрокиСписка.Добавить(СтрокаСписка);
	...
	Если СтрокиСписка.Количество() > 0 Тогда
		ТелоВиджета = ТелоВиджетаСписком(СтрокиСписка, Истина, Истина);
	Иначе
		ТекстНовыеОтветы = НСтр("ru = 'Сейчас
			|нет отсутствующих'");
	...
	Показатели = ПоказателиВиджета(Справочники.Виджеты.Отсутствия);
	
	ЦветЗначения = ЦветТекста_Обычный;
	Если Показатели[0].ИмяПоказателя = "Отсутствия" 
		И Показатели[0].ПороговоеЗначение < КоличествоСобытий Тогда
		
		ЦветЗначения = ЦветТекста_ПросроченныеДанные;
	КонецЕсли;
	
	СсылкаНаЗаголовок = "e1cib/list/Документ.Отсутствие";
	...
	ПодвалВиджета = ПодвалВиджетаСписком(КоличествоСтрок, "Отсутствия");
