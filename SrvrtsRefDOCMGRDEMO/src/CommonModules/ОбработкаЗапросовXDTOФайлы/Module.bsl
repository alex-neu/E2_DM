...
Функция ПолучитьСписокФайлов(Сообщение) Экспорт
	...
	Попытка 
		...
		Для Каждого ВнешнийОбъект Из Сообщение.externalObjects Цикл
			...
			МассивОбъектовДО = РегистрыСведений.СвязиОбъектовИнтегрированныхСистем.
...
Функция ЗанестиИнформациюОПодписяхОбъекта(signatures, СсылкаОбъектаД8) Экспорт
	...
	Если ДанныеПодписей.Количество() = 0 Тогда
		РаботаСЭП.СнятьПризнакПодписанЭП(СсылкаОбъектаД8);
	Иначе
...
Функция ДобавитьФайлСДиска(Сообщение) Экспорт
	...
	Попытка
		...
		РегистрыСведений.СвязиОбъектовИнтегрированныхСистем.ДобавитьСвязьОбъектаДОИВнешнегоОбъекта(
			Сообщение.file.externalObject.id, Сообщение.file.externalObject.type, ФайлСсылка);
...
//
Функция ИзменитьФайл(Объект) Экспорт 
	...
	Попытка
		...
		Иначе
			...
			Наименование = "";
			Описание = "";
			ОбработкаЗапросовXDTO.ЗаполнитьРеквизитИзСвойстваXDTO(Наименование, Объект, "name");
			ОбработкаЗапросовXDTO.ЗаполнитьРеквизитИзСвойстваXDTO(Описание, Объект, "description");
			
			Если НаименованиеДоЗаписи <> Наименование ИЛИ ОписаниеДоЗаписи <> Описание
				ИЛИ ОбработкаЗапросовXDTO.ДополнительныеРеквизитыИзменены(Ссылка, Объект)
				...
				Если ЗначениеЗаполнено(Наименование) Тогда
					...
					ОбработкаЗапросовXDTO.ЗаполнитьРеквизитИзСвойстваXDTO(ФайлОбъект.ПолноеНаименование, Объект, "name");
					ОбработкаЗапросовXDTO.ЗаполнитьРеквизитИзСвойстваXDTO(ФайлОбъект.Наименование, Объект, "name");
				...
				ОбработкаЗапросовXDTO.ЗаполнитьРеквизитИзСвойстваXDTO(ФайлОбъект.Описание, Объект, "description");
				...
				Если Объект.Установлено("activeVersion") Тогда
					Если Не ФайлОбъект.Редактирует.Пустая() Тогда
						...
						Возврат Ошибка;
					КонецЕсли;
					ОбработкаЗапросовXDTO.ЗаполнитьРеквизитИзСвойстваXDTO(ФайлОбъект.ТекущаяВерсия, Объект, "activeVersion");
				КонецЕсли;
				...
				ОбработкаЗапросовXDTO.ЗаписатьДополнительныеРеквизиты(ФайлОбъект, Объект);
...
Функция ДобавитьФайлВДокумент(Сообщение) Экспорт
	...
	Попытка 
		...
		Иначе
			...
			Ошибка = ОбработкаЗапросовXDTO.СоздатьОбъект("DMError");
			Ошибка.subject = НСтр("ru = 'Ошибка при добавлении файла'");
			Ошибка.description = НСтр("ru = 'Не заполнен ни шаблон, ни двоичные данные файла'");
			Возврат Ошибка;
		КонецЕсли;
		...
		Если Константы.ИспользоватьСостоянияДокументов.Получить() 
			...
			Если ТипДокумента <> Неопределено Тогда
				...
				Если ИменаПолейИКоманд.Найти("editfile", "ИменаDMService") = Неопределено Тогда
					...
					ТекстСообщенияОбОшибке = 
						НСтр("ru = 'Для текущего состояния документа запрещено изменение файлов.'");
...
Функция ПолучитьПапкуДляХраненияФайлов(ИдентификаторКорневойПапки,
	...
	Если РодительскаяПапка = Неопределено Или РодительскаяПапка.Пустая()
			Или Не ОбщегоНазначения.СсылкаСуществует(РодительскаяПапка) Тогда
	...
	Если НЕ ПустаяСтрока(ИменаРеквизитов) Тогда
		...
		Если ПолучатьЯвляетсяОригиналом Тогда
			...
			Запрос = Новый Запрос(
				...
				|	ЯвляетсяОригиналом
				|ИЗ 
				|	РегистрСведений.СведенияОФайлахДокументооборот
				|ГДЕ
				|	Файл = &Файл
				|");
	...
	Если Не Источник.ТекущаяВерсия.Пустая() Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьШтрихкоды")
			...
			Если ДанныеОШтрихкоде <> Неопределено И ДанныеОШтрихкоде.Свойство("ДвоичныеДанныеФайла") Тогда
				Попытка
					ШтрихкодированиеКлиентСервер.ВставитьШтрихкодВместоТэга(Приемник,
				...
				Исключение
					Ошибка = ОбработкаЗапросовXDTO.СоздатьОбъект("DMError");
					Ошибка.subject = НСтр("ru = 'Ошибка при вставке изображения штрихкода в файл'");
					Инфо = ИнформацияОбОшибке();
					Ошибка.description = ОбработкаЗапросовXDTO.ПолучитьОписаниеОшибки(Инфо);
					Возврат Ошибка;
				КонецПопытки;
