		...
		ИначеЕсли ТипХраненияФайловШаблона = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске И ТипХраненияФайлов = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			...
			Если Не СведенияОФайле.СсылкаНаВерсиюИсточник.Том.Пустая() Тогда
				...
				ФайловыеФункции.ДобавитьНаДиск(					
					...
					СведенияОФайле.СсылкаНаВерсиюИсточник.Зашифрован); 
				Версия.Том = Том;
...
Функция ДанныеФайла(ФайлИлиВерсияСсылка) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДанныеФайла.Зашифрован Тогда
			МассивСертификатовШифрования = ЭлектроннаяПодпись.СертификатыШифрования(ДанныеФайла.Ссылка);
...
Функция ПолучитьДанныеФайлаДляОткрытияИЗанятьФайл(ФайлСсылка,
	...
	Результат.ФайлУжеРедактируетТекущийПользователь = ДанныеФайла.РедактируетТекущийПользователь;
...
// заменяет пути в регистре сведений ФайлыВРабочемКаталогеКомпьютера.
//
// Параметры:
//  СсылкаПапки  - СправочникСсылка.ПапкиФайлов - владелец файла.
...
Функция НайтиВРегистреПоПути(ИмяФайла) Экспорт
	...
	НайденныеСвойства.Вставить("Версия", Справочники.ВерсииФайлов.ПустаяСсылка());
	...
	"ВЫБРАТЬ
	|	ФайлыВРабочемКаталогеКомпьютера.Версия КАК Версия,
	|	ФайлыВРабочемКаталогеКомпьютера.ДатаПомещенияВРабочийКаталог КАК ДатаПомещения,
	|	ФайлыВРабочемКаталогеКомпьютера.НаЧтение КАК ВРегистреНаЧтение,
	|	ФайлыВРабочемКаталогеКомпьютера.Версия.Владелец КАК Владелец,
	|	ФайлыВРабочемКаталогеКомпьютера.Версия.НомерВерсии КАК НомерВерсии,
	|	ФайлыВРабочемКаталогеКомпьютера.Версия.Владелец.Код КАК ВРегистреКодФайла,
	|	ФайлыВРабочемКаталогеКомпьютера.Версия.Владелец.ВладелецФайла КАК ВРегистреПапка
	|ИЗ
	|	РегистрСведений.ФайлыВРабочемКаталогеКомпьютера КАК ФайлыВРабочемКаталогеКомпьютера
	|ГДЕ
...
// Удалить из регистра сведений ФайлыВРабочемКаталогеКомпьютера все записи,
...
Процедура УдалитьПодписиВерсииФайла(ВерсияСсылка, ИндексыСтрок, РеквизитПодписанИзменен,
	...
	Попытка
		...
		КоличествоПодписей = РаботасЭП.КоличествоПодписей(ВерсияСсылка);
...
Процедура ОпределитьНаличиеХранимыхФайлов(ВнешнийОбъект, ЕстьХранимыеФайлы) Экспорт
	...
	Если ТипыВладельцев.Найти(ТипЗнч(ВнешнийОбъект)) <> Неопределено Тогда
		МассивФайлов = РаботаСФайламиСлужебный.ПолучитьВсеПодчиненныеФайлы(ВнешнийОбъект);
...
Функция ТабличныйДокументСоШтампом(СсылкаНаФайл, Ссылка) Экспорт
	...
	ДанныеФайла    = РаботаСФайламиВызовСервера.ДанныеФайла(СсылкаНаФайл);
...
Функция КоличествоПрисоединенныхФайлов(ВладелецФайлов, ВозвращатьДанныеФайлов = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяСправочникаХранения) Тогда
		...
		Если КоличествоФайлов > 0 Тогда
			ФайлыВладельца.ДанныеФайла = ДанныеФайла(ТаблицаФайлов[0].Файл);
		КонецЕсли;
...
Функция ИмяСправочникаХраненияФайлов(ВладелецФайлов, ИмяСправочника = "",
	...
	ИменаСправочников = РаботаСФайламиСлужебный.ИменаСправочниковХраненияФайлов(ВладелецФайлов, НеВызыватьИсключение);
...
Процедура УдалитьДанныеВерсии(ФайлИлиВерсия, УникальныйИдентификатор, Результат)
	...
	Если РезЗапроса.Пустой() Тогда
		Возврат;
	КонецЕсли;	
	ПараметрыТекущейВерсииВладельца = РезЗапроса.Выбрать();
