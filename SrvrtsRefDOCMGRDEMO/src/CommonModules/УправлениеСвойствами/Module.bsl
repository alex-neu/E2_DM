...
Процедура ОбработкаПроверкиЗаполнения(Форма, Отказ, ПроверяемыеРеквизиты, Объект = Неопределено) Экспорт
	...
	Если Не Форма.ОбъектПоддерживаетПроверкуУсловийДляДопСвойств Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьДополнительныеРеквизитыВФорме(Форма, Объект = Неопределено, ПоляНадписей = Ложь, СкрытьУдаленные = Неопределено) Экспорт
	...
	Форма.ОбъектПоддерживаетПроверкуУсловийДляДопСвойств = 
		ОбъектПоддерживаетПроверкуУсловийДопСвойств(ОписаниеОбъекта);
	...
	Для Каждого ОписаниеСвойства Из ОписаниеСвойств Цикл
		...
		Для Каждого СтрокаТаблицы Из ЗависимостиДанногоНабора Цикл
			...
			Если ТипЗнч(СтрокаТаблицы.Реквизит) = Тип("Строка") Тогда
				Если СтрНачинаетсяС(СтрокаТаблицы.Реквизит, "ДопРеквизитыОтбораДляУсловий") Тогда
					ПутьКРеквизиту = "Параметры.Форма." + СтрокаТаблицы.Реквизит;
				Иначе
		...
		Иначе
			...
			Иначе
				Элемент.Вид = ВидПоляФормы.ПолеВвода;
				Элемент.АвтоОтметкаНезаполненного = Форма.ОбъектПоддерживаетПроверкуУсловийДляДопСвойств
					И ОписаниеСвойства.ЗаполнятьОбязательно И НЕ ОписаниеСвойства.Удалено;
			КонецЕсли;
	...
	Если Форма.ОбъектПоддерживаетПроверкуУсловийДляДопСвойств Тогда
		
		Форма.ДопРеквизитыОтбораДляУсловий = ДопРеквизитыОтбораДляУсловий(ОписаниеОбъекта);
...
Процедура УдалитьСтарыеРеквизитыИЭлементы(Форма) Экспорт
	...
	Форма.ОбъектПоддерживаетПроверкуУсловийДляДопСвойств = Ложь;
...
Процедура СоздатьОсновныеОбъектыФормы(Форма, Контекст, СоздатьОписаниеДополнительныхРеквизитов)
	...
	Если ОпцияИспользоватьСвойства Тогда
		...
		Если СоздатьОписаниеДополнительныхРеквизитов Тогда
			...
			Реквизиты.Добавить(Новый РеквизитФормы(
				"ОбъектПоддерживаетПроверкуУсловийДляДопСвойств", Новый ОписаниеТипов("Булево")));
			...
			Реквизиты.Добавить(Новый РеквизитФормы(
				"ДопРеквизитыОтбораДляУсловий", Новый ОписаниеТипов()));
...
Функция ИспользуютсяСвойства(Форма, ДополнительныеПараметры)
	...
	НаборыСвойств = УправлениеСвойствамиСлужебный.ПолучитьНаборыСвойствОбъекта(
		ОписаниеОбъекта);
