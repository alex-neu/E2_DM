		...
		Если ПоОбъектуВедетсяАвтоматическоеЗаполнениеРабочейГруппы(ОбъектДляРасширенияРГ) Тогда
			...
			Попытка
				РаботаСРабочимиГруппами.ДобавитьУчастниковВРабочуюГруппуОбъекта(
					ОбъектДляРасширенияРГ, РабочаяГруппа, Истина, Истина);
			Исключение
				ОбработатьИсключениеПерезаписиРабочейГруппыПредметаПроцесса(ОбъектДляРасширенияРГ);
			КонецПопытки;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбработатьИсключениеПерезаписиРабочейГруппыПредметаПроцесса(Предмет) Экспорт
	...
	ИначеЕсли ТипЗнч(Предмет) = Тип("СправочникСсылка.ПротоколыМероприятий")
		Или ТипЗнч(Предмет) = Тип("СправочникСсылка.Мероприятия") Тогда
...
Процедура ДокументПриКопировании(Форма) Экспорт
	...
	Если ДелопроизводствоКлиентСервер.ЭтоВнутреннийДокумент(ЗначениеКопирования) Тогда
		ИмяТаблицыШаблона = "ШаблоныВнутреннихДокументов";
	ИначеЕсли ДелопроизводствоКлиентСервер.ЭтоВходящийДокумент(ЗначениеКопирования) Тогда
		ИмяТаблицыШаблона = "ШаблоныВходящихДокументов";
	ИначеЕсли ДелопроизводствоКлиентСервер.ЭтоИсходящийДокумент(ЗначениеКопирования) Тогда
		ИмяТаблицыШаблона = "ШаблоныИсходящихДокументов";
	Иначе
		Возврат;
	КонецЕсли;
	...
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("Объект", ЗначениеКопирования);
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ШаблоныВходящихДокументов", ИмяТаблицыШаблона);
	...
	Для НомерСтроки = 1 По КоличествоУчастников Цикл 
		СтрокаУчастник = Участники.Получить(КоличествоУчастников - НомерСтроки);
		
		СтрПоиск = ТаблицаУчастников.НайтиСтроки(Новый Структура("Участник, Изменение", СтрокаУчастник.Участник, СтрокаУчастник.Изменение));
		Если СтрПоиск.Количество() = 0 Тогда 
			Участники.Удалить(СтрокаУчастник);
		КонецЕсли;
	КонецЦикла;
	
	ФормаДокументаДобавитьУчастников(Форма, Участники);
...
Процедура ОбеспечитьУсловиеОбязательногоЗаполненияРабочейГруппы(Форма)
	...
	Участник = Пользователи.ТекущийПользователь();
...
Процедура ПроверитьПраваПередЗаписьюРабочейГруппы(Ссылка, Участники, АвторИзменений) Экспорт
	...
	Если Константы.ПроверятьСоответствиеРабочихГруппОбщимНастройкам.Получить() Тогда
		...
		// Отбор контейнеров, в которых есть действительные пользователи.
		Запрос = Новый Запрос(
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура РабочиеГруппыБизнесПроцессыПриЗаписиПриЗаписи(Источник, Отказ) Экспорт
		...
		Для Каждого Предмет Из Предметы Цикл 
			...
			Попытка
				ДобавитьУчастниковВРабочуюГруппуДокументаИзИсточника(
					ОбъектДляРасширенияРГ, Источник.Ссылка,, Истина);
			Исключение
				ОбработатьИсключениеПерезаписиРабочейГруппыПредметаПроцесса(ОбъектДляРасширенияРГ);
			КонецПопытки;
		КонецЦикла;
	...
	Иначе
		...
		Если ПолучитьФункциональнуюОпцию("ВестиУчетПоПроектам") И ЗначениеЗаполнено(Источник.Проект) Тогда
			Попытка
				Если ПредыдущийПроект = Источник.Проект Тогда // проект не изменился
					Если ПоОбъектуВедетсяАвтоматическоеЗаполнениеРабочейГруппы(Источник.Проект) Тогда
