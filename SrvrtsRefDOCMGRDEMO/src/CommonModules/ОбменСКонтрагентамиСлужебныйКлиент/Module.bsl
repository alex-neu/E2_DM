...
Процедура ОткрытьНастройкуОтраженияВУчете(Организация, Контрагент, ИдентификаторОрганизации, ИдентификаторКонтрагента) Экспорт
	...
	ОткрытьФорму("РегистрСведений.НастройкиПолученияЭлектронныхДокументов.Форма.НастройкиОтраженияДокументовВУчетеДО", ПараметрыФормы);
...
Процедура ОпределитьСтатусОчереднойПодписи(Результат, Параметры) Экспорт
	...
	Если Параметры.Свойство("ПроверяемыйЭД") Тогда
		ОбменСКонтрагентамиСлужебныйВызовСервера.СохранитьРезультатыПроверкиПодписей(Параметры.ПроверяемыйЭД,
			Параметры.РезультатПроверки, Параметры.Подписи);
	КонецЕсли;
...
Процедура ОткрытьФормуНастройкиОтправкиЭДО(ПараметрыФормы, ВладелецФормы, Знач РежимОткрытияОкна = Неопределено) Экспорт
	...
	ОткрытьФорму("РегистрСведений.НастройкиОтправкиЭлектронныхДокументовПоВидам.Форма.НастройкиОтправкиДокументовДО", 
		ПараметрыФормы,
		ВладелецФормы,,,,,
		РежимОткрытияОкна);
...
Процедура ОткрытьФормуНастройкиОтраженияВУчетеЭДО(ПараметрыФормы, ВладелецФормы, Знач РежимОткрытияОкна = Неопределено) Экспорт
	...
	ОткрытьФорму("РегистрСведений.НастройкиПолученияЭлектронныхДокументов.Форма.НастройкиОтраженияДокументовВУчетеДО", 
		ПараметрыФормы,
		ВладелецФормы,,,,,
		РежимОткрытияОкна);
...
Процедура ОткрытьСопоставлениеНоменклатуры(Знач НоменклатураКонтрагентов, Знач Настройки = Неопределено, Знач ОповещениеОЗакрытии = Неопределено) Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйКлиентПовтИсп.ПодсистемаСопоставленияНоменклатурыСуществует() Тогда
		ОбменСКонтрагентамиСлужебныйКлиентПовтИсп.МодульСопоставлениеНоменклатурыКонтрагентовКлиент().ОткрытьСопоставлениеНоменклатуры(
			НоменклатураКонтрагентов, Настройки, ОповещениеОЗакрытии);
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьВыборНоменклатурыКонтрагента(Знач ВладелецНоменклатуры, Знач ВладелецФормы, Знач Идентификатор = Неопределено) Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйКлиентПовтИсп.ПодсистемаСопоставленияНоменклатурыСуществует() Тогда
		ОбменСКонтрагентамиСлужебныйКлиентПовтИсп.МодульСопоставлениеНоменклатурыКонтрагентовКлиент().ОткрытьВыборНоменклатурыКонтрагента(
			ВладелецНоменклатуры, ВладелецФормы, Идентификатор);
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьФормуВыбораНоменклатуры(Знач Параметры, Знач Владелец, Знач Уникальность) Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйКлиентПовтИсп.ПодсистемаСопоставленияНоменклатурыСуществует() Тогда
		ОбменСКонтрагентамиСлужебныйКлиентПовтИсп.МодульСопоставлениеНоменклатурыКонтрагентовКлиент().ОткрытьФормуВыбораНоменклатуры(
...
Процедура ОткрытьСписокНоменклатурыКонтрагентов() Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйКлиентПовтИсп.ПодсистемаСопоставленияНоменклатурыСуществует() Тогда
		ОбменСКонтрагентамиСлужебныйКлиентПовтИсп.МодульСопоставлениеНоменклатурыКонтрагентовКлиент().ОткрытьСписокНоменклатурыКонтрагентов();
		Возврат;
	КонецЕсли;
...
Процедура НачатьСозданиеНоменклатурыПоДаннымКонтрагента(Знач НаборНоменклатурыКонтрагентов, Знач ОповещениеОЗавершении = Неопределено) Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйКлиентПовтИсп.ПодсистемаСопоставленияНоменклатурыСуществует() Тогда
		ОбменСКонтрагентамиСлужебныйКлиентПовтИсп.МодульСопоставлениеНоменклатурыКонтрагентовСлужебныйКлиент().НачатьСозданиеНоменклатурыПоДаннымКонтрагента(
			НаборНоменклатурыКонтрагентов, ОповещениеОЗавершении);
		Возврат;
	КонецЕсли;
...
// См. ОбменСКонтрагентамиКлиентПереопределяемый.ПриОткрытииФормыПодсистемы.
Процедура ПриОткрытииФормыПодсистемы(Контекст, Отказ) Экспорт

	ОбменСКонтрагентамиКлиентПереопределяемый.ПриОткрытииФормыПодсистемы(Контекст, Отказ);
...
Процедура ОтправитьМассивПакетовОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	КонтекстОперации = Результат.КонтекстОперации;
	КонтекстОперации.ЗаголовокОперации = НСтр("ru = 'При отправке пакетов электронных документов'");
	ОтправленныеДокументы = Новый Массив;
	Результат = ОбменСКонтрагентамиСлужебныйВызовСервера.ОтправкаПакетовЭД(МассивПЭД,
		СоотвСоглашенийИСтруктурСертификатов, ОтправленныеДокументы, КонтекстОперации);
	...
	Если Результат = 0 И КонтекстОперации.Диагностика.Ошибки.Количество() > 0 Тогда
		ОбработчикПовторенияДействия = Новый ОписаниеОповещения("ОтправитьМассивПакетовЭДПовторить", ЭтотОбъект, ДополнительныеПараметры);
		ПараметрыОбработкиОшибок = ЭлектронноеВзаимодействиеОбработкаОшибокКлиент.НовыеПараметрыОбработкиОшибок();
		ПараметрыОбработкиОшибок.ОбработчикПовторенияДействия = ОбработчикПовторенияДействия;
...
Процедура ОбработатьОтклонениеАннулированиеЭДЗавершить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	Если ДокументыДляПростогоОтклонения.Количество() > 0 Тогда
		...
		Иначе
			НовыйСтатусЭД = ПредопределенноеЗначение("Перечисление.СтатусыЭД.Аннулирован");
		КонецЕсли;
...
Процедура ЗавершитьОбновлениеСертификата(Знач ПроцессВыполнения)
	...
	ОтобразитьПроцессВыполненияОбновленияСертификата(ПроцессВыполнения, ОперацияВыполнена);
...
Процедура ОтправитьМассивПакетовЭДПовторить(Результат, Контекст) Экспорт
	
	ОтправитьМассивПакетовЭД(Контекст.МассивПЭД, Контекст.ОбработкаОповещения);
