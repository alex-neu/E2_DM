...
Процедура ОбновитьСвязиЭлементовСхемыПроцесса(СхемаПроцесса, ГрафическаяСхема) Экспорт
	...
	Для Каждого ЭлементГрафическойСхемы Из ГрафическаяСхема.ЭлементыГрафическойСхемы Цикл
		...
		Если ТипЭлементаНачала = Тип("ЭлементГрафическойСхемыУсловие") Тогда
			...
			Если ЭлементГрафическойСхемы.Наименование = НСтр("ru = 'Да'") Тогда
...
// Заполняет схему процесса по графической схеме.
// Прежние настройки схемы очищаются.
//
// Параметры:
//  СхемаПроцесса - ДанныеФормыСтруктура,
//                  СправочникОбъект.СхемыКомплексныхПроцессов - схема процесса.
//  ГрафическаяСхема - ГрафическаяСхема - графическая схема процесса.
//
Процедура ЗаполнитьСхемуПроцессаПоГрафическойСхеме(СхемаПроцесса, ГрафическаяСхема) Экспорт
	
	СхемаПроцесса.ПараметрыДействий.Очистить();
	СхемаПроцесса.ПараметрыОбработок.Очистить();
	СхемаПроцесса.ПараметрыУсловий.Очистить();
	СхемаПроцесса.ПредшественникиЭлементовСхемы.Очистить();
	СхемаПроцесса.ЭлементыСхемы.Очистить();
	
	Для Каждого ЭлементГрафическойСхемы Из ГрафическаяСхема.ЭлементыГрафическойСхемы Цикл
		
		ТипЭлементаГрафическойСхемы = ТипЗнч(ЭлементГрафическойСхемы);
		
		// Соединительные линии не добавляются в настройки.
		Если ТипЭлементаГрафическойСхемы = Тип("ЭлементГрафическойСхемыСоединительнаяЛиния") Тогда
			Продолжить;
		КонецЕсли;
		
		ТипЭлементаСхемы = ТипЭлементаСхемы(ТипЭлементаГрафическойСхемы);
		...
		ЭлементСхемыПроцесса = СхемаПроцесса.ЭлементыСхемы.Добавить();
		ЭлементСхемыПроцесса.Имя = ЭлементГрафическойСхемы.Имя;
		ЭлементСхемыПроцесса.Тип = ТипЭлементаСхемы;
		
		Если ТипЭлементаГрафическойСхемы = Тип("ЭлементГрафическойСхемыВложенныйБизнесПроцесс")
			...
			ПараметрыДействия = СхемаПроцесса.ПараметрыДействий.Добавить();
		...
		ИначеЕсли ТипЭлементаГрафическойСхемы = Тип("ЭлементГрафическойСхемыУсловие") Тогда
			...
			ПараметрыУсловия = СхемаПроцесса.ПараметрыУсловий.Добавить();
		...
		ИначеЕсли ТипЭлементаГрафическойСхемы = Тип("ЭлементГрафическойСхемыОбработка") Тогда
			...
			ПараметрыОбработки = СхемаПроцесса.ПараметрыОбработок.Добавить();
	...
	ОбновитьСвязиЭлементовСхемыПроцесса(СхемаПроцесса, ГрафическаяСхема);
	...
	Если ПоказатьТочныеСроки Или ПоказатьОтносительныеСроки Тогда
		...
		Если ЗначениеЗаполнено(СтруктураСроков.СрокИсполненияПроцесса)
			...
			НаименованиеДействия = НаименованиеДействия
				...
				+ СтрШаблон(НСтр("ru = 'Срок: %1'"), СтруктураСроков.СрокИсполненияПроцессаПредставление);
		КонецЕсли;
