...
// Формирует представление навигационной ссылки.
//
// Параметры:
//  НавигационнаяСсылка - Строка - Навигационная ссылка.
// 
// Возвращаемое значение:
//  Строка - Представление навигационной ссылки.
//
Функция ПредставлениеСсылки(НавигационнаяСсылка) Экспорт
	
	Попытка
		
		МассивСсылок = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(НавигационнаяСсылка);
		ПредставленияСсылок = ПолучитьПредставленияНавигационныхСсылок(МассивСсылок);
		Если ПредставленияСсылок.Количество() > 0
			И ПредставленияСсылок[0] <> Неопределено 
			И ЗначениеЗаполнено(ПредставленияСсылок[0].Текст) Тогда
			ПредставлениеСсылки = ПредставленияСсылок[0].Текст;
		Иначе
...
//
Функция ПолучитьHTMLИзТекста_ДляПросмотраТекста(Знач Текст, Знач Кодировка = Неопределено) Экспорт
...
Функция ПолучитьHTMLИзТекста(
	...
	ТекстHTML =
		"<html>
		|<head><meta %content% http-equiv=""Content-Type""></head><body style=""margin-top:1px; padding-top:1px; overflow:auto; "">
		|<div>%Текст%</div>
		|</body>
		|</html>";
...
#КонецЕсли
	...
	УдалитьТегиИзЭлементаHTML(ДокументHTML, "style", 0);
...
Функция ПолучитьТекстHTMLДляИсходящегоПисьмаИзПростогоТекста(Знач Текст, Знач Кодировка = Неопределено) Экспорт
	...
	ТекстHTML =
		"<html>
		|<head><meta content=""text/html; charset=[charset]"" http-equiv=""Content-Type""></head><body>
...
Процедура ДобавитьТегиКСсылкам(ТекстHTML) Экспорт
	...
	// Заполнить tabindex у ссылок для работы команды "Копировать ссылку" - чтобы фокус оставался на ссылке.
	ЗаполнитьСвойствоTabindexУСсылок(ТекстHTML);
	
КонецПроцедуры

// Заполняет tabindex у ссылок для работы команды "Копировать ссылку" - чтобы фокус оставался на ссылке.
Процедура ЗаполнитьСвойствоTabindexУСсылок(ТекстHTML)
	
	ВставляемыйТекстTabindex = " tabindex=""0""";
	Смещение = СтрДлина(ВставляемыйТекстTabindex);
	...
	Пока ЗаполненаСтруктураТега(Тег) <> 0 Цикл
		...
		ТекстTabindex = ПолучитьЗначениеПараметраТега(Тег, "tabindex");
		Если ТекстTabindex = Неопределено Тогда
			// Параметр tabindex не заполнен, добавим его.
			ТекстHTML =
				Лев(ТекстHTML, Тег.ПозицияОкончанияОткрывающегоТега - 1)
				+ ВставляемыйТекстTabindex
				+ Сред(ТекстHTML, Тег.ПозицияОкончанияОткрывающегоТега);
		КонецЕсли;
...
// Дописывает описание стиля отображения в HTML текст.
// Если в переданном тексте не хватает тегов <html>, <head>, <body>, то они будут добавлены.
// Стиль будет добавлен в <head> в тег <style class=v8doc-style type=text/css>.
//
// Параметры:
//  ТекстHTML        - Строка - Текст, в который следует добавить описание стиля.
//  ШрифтОтображения - Шрифт  - Шрифт, который следует использовать в стиле.
//
Процедура ПрименитьНастройкиОтображение(ТекстHTML, ШрифтОтображения) Экспорт
	
	// Если уже есть стиль v8doc-style, то не добавлять его еще раз.
	НРегТекстHTML = НРег(ТекстHTML);
	Если СтрНайти(НРегТекстHTML, "v8doc-style") <> 0 Тогда
		Возврат;
	КонецЕсли;
	
	ОписаниеСтиля = ПолучитьОписаниеСтиля(ШрифтОтображения, Истина, Ложь);
	Если Не ЗначениеЗаполнено(ОписаниеСтиля) Тогда
		Возврат;
	КонецЕсли;
...
Процедура УдалитьТегиИзЭлементаHTML(ЭлементHTML, Тег, Глубина)
	
	Глубина = Глубина + 1;
	Если Глубина > 300  Тогда
		Возврат;
	КонецЕсли;	
	...
	Для каждого Узел Из ЭлементHTML.ДочерниеУзлы Цикл
		...
		Иначе
			// Рекурсия
			УдалитьТегиИзЭлементаHTML(Узел, Тег, Глубина);
		КонецЕсли;
	КонецЦикла;
	...
	Пока ПозицияНачалаОткрывающегоТега <> 0 Цикл
		...
		Если ЗначениеЗаполнено(ПервыйСимволНазванияТега) Тогда
			...
			Иначе
				...
				ВозможныеПозицииНужногоТега = Новый Массив;
				ВозможныеПозицииНужногоТега.Добавить(СтрНайти(НачалоТекстаHTML, НужныйТег + " "));
				ВозможныеПозицииНужногоТега.Добавить(СтрНайти(НачалоТекстаHTML, НужныйТег + Символы.ПС));
				ВозможныеПозицииНужногоТега.Добавить(СтрНайти(НачалоТекстаHTML, НужныйТег + ">"));
				Если ВозможныеПозицииНужногоТега.Найти(1) <> Неопределено Тогда
...
Функция ПолучитьЗначениеПараметраТега(
	СтруктураТег,
	ИмяПараметраТега,
	НайденныйСимволОграничения = Неопределено,
	НайденноеНачалоПараметра = Неопределено,
	НайденныйКонецПараметра = Неопределено) Экспорт
	
	Если Не ЗаполненаСтруктураТега(СтруктураТег) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	НРегПараметрыТега = НРег(СтруктураТег.ПараметрыОткрывающегоТега);
	ДлинаИмениПараметраТега = СтрДлина(ИмяПараметраТега);
	ПоследняяПозиция = СтрДлина(НРегПараметрыТега);
	
	ДопустимыеСимволыОграничения = Новый Массив;
	ДопустимыеСимволыОграничения.Добавить("""");
	ДопустимыеСимволыОграничения.Добавить("'");
	ДопустимыеСимволыОграничения.Добавить("");
	
	// Найдем все возможные позиции параметра - перед именем параметра должен встречаться незначащий символ.
	ВозможныеПозицииПараметра = ВозможныеПозицииПараметра(НРегПараметрыТега, ИмяПараметраТега);
	Для Каждого ВозможнаяПозицияПараметра Из ВозможныеПозицииПараметра Цикл
		
		// Начнем считывать параметр тега.
		ТекущаяПозиция = ВозможнаяПозицияПараметра;
		НачалоПараметра = ТекущаяПозиция;
		
		// Пропустим символы, являющиеся именем параметра.
		ТекущаяПозиция = ВозможнаяПозицияПараметра + ДлинаИмениПараметраТега;
		
		// Найдем следующий значащий символ, это должен быть знак равенства.
		СледующийЗначащийСимвол = СледующийЗначащийСимвол(НРегПараметрыТега, ТекущаяПозиция);
		Если Не СледующийЗначащийСимвол.ЕстьЗначащийСимвол
			Или СледующийЗначащийСимвол.Символ <> "=" Тогда
			Продолжить;
		КонецЕсли;
		ТекущаяПозиция = СледующийЗначащийСимвол.ПозицияСимвола + 1;
		
		// Найдем следующий значащий символ, это должен быть символ ограничения значения тега, либо уже начало значения тега.
		СледующийЗначащийСимвол = СледующийЗначащийСимвол(НРегПараметрыТега, ТекущаяПозиция);
		Если Не СледующийЗначащийСимвол.ЕстьЗначащийСимвол Тогда
			Продолжить;
		КонецЕсли;
		ТекущаяПозиция = СледующийЗначащийСимвол.ПозицияСимвола;
		
		// Определим символ ограничения и начало значения параметра тега.
		ЭтоДопустимыйСимволОграничения =
			ДопустимыеСимволыОграничения.Найти(СледующийЗначащийСимвол.Символ) <> Неопределено;
		Если ЭтоДопустимыйСимволОграничения Тогда
			// Текущий символ - символ ограничения. Требуется смещение на начало значения параметра тега.
			СимволОграничения = СледующийЗначащийСимвол.Символ;
			ТекущаяПозиция = ТекущаяПозиция + СтрДлина(СимволОграничения);
		Иначе
			// Текущий символ - начало значения параметра тега. Символа ограничения нет, смещение не требуется.
			СимволОграничения = "";
		КонецЕсли;
		НачалоЗначенияПараметраТега = ТекущаяПозиция;
		
		// Определим конец значения параметра тега.
		КонецПараметра = 0;
		Если СимволОграничения = "" Тогда
			
			// Символа ограничения нет - первый пустой символ является ограничением.
			СледующийНезначащийСимвол = СледующийНезначащийСимвол(НРегПараметрыТега, ТекущаяПозиция);
			Если СледующийНезначащийСимвол.ЕстьНезначащийСимвол Тогда
				КонецПараметра = СледующийНезначащийСимвол.ПозицияСимвола;
			Иначе
				КонецПараметра = ПоследняяПозиция + 1;
			КонецЕсли;
			
		Иначе
			
			// Символ ограничения задан - следующий является ограничением.
			КонецПараметра = СтрНайти(
				НРегПараметрыТега,
				СимволОграничения,
		...
		ДлинаЗначенияПараметраТега = КонецПараметра - НачалоЗначенияПараметраТега;
		
		ЗначениеПараметраТега = Сред(СтруктураТег.ПараметрыОткрывающегоТега,
			НачалоЗначенияПараметраТега,
			ДлинаЗначенияПараметраТега);
		НайденныйСимволОграничения = СимволОграничения;
		НайденноеНачалоПараметра = НачалоПараметра;
		НайденныйКонецПараметра = КонецПараметра;
...
// Определяет возможные позиции параметра тега.
//
// Параметры:
//  НРегПараметрыТега - Строка - Параметры тега для поиска в нижнем регистре.
//  ИмяПараметраТега - Строка - Имя параметра тега в нижнем регистре.
//
Функция ВозможныеПозицииПараметра(НРегПараметрыТега, ИмяПараметраТега)
	
	ВозможныеПозицииПараметра = Новый Массив;
	ДопустимыеНезначащиеСимволы = НезначащиеСимволы();
	Для Каждого ДопустимыйНезначащийСимвол Из ДопустимыеНезначащиеСимволы Цикл
		
		ПараметрДляПоиска = НРег(ДопустимыйНезначащийСимвол + ИмяПараметраТега);
		
		// Позиция параметра тега за счёт одного незначащего символа в искомой строке.
		СмещеннаяВозможнаяПозицияПараметра = СтрНайти(НРегПараметрыТега, ПараметрДляПоиска);
		Пока СмещеннаяВозможнаяПозицияПараметра <> 0 Цикл
			
			ВозможнаяПозицияПараметра = СмещеннаяВозможнаяПозицияПараметра + 1;
			ВозможныеПозицииПараметра.Добавить(ВозможнаяПозицияПараметра);
			
			СмещеннаяВозможнаяПозицияПараметра = СтрНайти(
				НРегПараметрыТега,
				ПараметрДляПоиска,
				НаправлениеПоиска.СНачала,
				ВозможнаяПозицияПараметра + 1);
			
		КонецЦикла;
		
	КонецЦикла;
	
	Возврат ВозможныеПозицииПараметра;
	
КонецФункции

// Ищет следующий значащий символ в строке.
//
// Параметры:
...
// 
// Возвращаемое значение:
//  Структура - Данные следующего значащего символа.
//  * ЕстьЗначащийСимвол - Булево - Значащий символ есть.
//  * Символ - Строка - Значащий символ.
//  * ПозицияСимвола - Число - Позиция значащего символа.
//
Функция СледующийЗначащийСимвол(Строка, НачальнаяПозиция)
	
	ДанныеЗначащегоСимвола = Новый Структура;
	ДанныеЗначащегоСимвола.Вставить("ЕстьЗначащийСимвол", Ложь);
	ДанныеЗначащегоСимвола.Вставить("Символ", "");
	ДанныеЗначащегоСимвола.Вставить("ПозицияСимвола", 0);
	
	ТекущаяПозиция = НачальнаяПозиция;
	Пока ТекущаяПозиция < СтрДлина(Строка) Цикл
		
		ТекущийСимвол = Символ(КодСимвола(Строка, ТекущаяПозиция));
		Если Не ПустаяСтрока(ТекущийСимвол) Тогда
			ДанныеЗначащегоСимвола.ЕстьЗначащийСимвол = Истина;
			ДанныеЗначащегоСимвола.Символ = ТекущийСимвол;
			ДанныеЗначащегоСимвола.ПозицияСимвола = ТекущаяПозиция;
			Прервать;
		КонецЕсли;
	...
	Возврат ДанныеЗначащегоСимвола;
	
КонецФункции

// Ищет следующий незначащий символ в строке.
//
// Параметры:
//  Строка - Строка - Исходная строка.
//  НачальнаяПозиция - Число - Указывает позицию в строке, с которой начинается поиск.
// 
// Возвращаемое значение:
//  Структура - Данные следующего незначащего символа.
//  * ЕстьНезначащийСимвол - Булево - Незначащий символ есть.
//  * Символ - Строка - Незначащий символ.
//  * ПозицияСимвола - Число - Позиция незначащего символа.
//
Функция СледующийНезначащийСимвол(Строка, НачальнаяПозиция)
	
	ДанныеЗначащегоСимвола = Новый Структура;
	ДанныеЗначащегоСимвола.Вставить("ЕстьНезначащийСимвол", Ложь);
	ДанныеЗначащегоСимвола.Вставить("Символ", "");
	ДанныеЗначащегоСимвола.Вставить("ПозицияСимвола", 0);
	...
	Пока ТекущаяПозиция < СтрДлина(Строка) Цикл
		...
		ТекущийСимвол = Символ(КодСимвола(Строка, ТекущаяПозиция));
		Если ПустаяСтрока(ТекущийСимвол) Тогда
			ДанныеЗначащегоСимвола.ЕстьНезначащийСимвол = Истина;
			ДанныеЗначащегоСимвола.Символ = ТекущийСимвол;
			ДанныеЗначащегоСимвола.ПозицияСимвола = ТекущаяПозиция;
			Прервать;
		КонецЕсли;
...
// Формирует массив незначащих символов.
// 
// Возвращаемое значение:
//  Массив - Незначащие символы.
//
...
Функция ПолучитьТекстHTMLСВыделениемСсылок(Знач ТекстHTML)
	...
	Пока ЗаполненаСтруктураТега(Тег) <> 0 Цикл
		...
		// Выделение ссылки.
		ПредставлениеСсылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '(Ссылка: %1)'"), ТекстHref);
...
Функция ПолучитьОписаниеСтиля(ШрифтОтображения = Неопределено, ОтображатьДругиеШрифты = Неопределено,
	...
	РазмерШрифтаЗаголовка = Формат(РазмерШрифта * 1.1, "ЧДЦ=0; ЧН=0; ЧГ=0"); // Уменьшенный шрифт заголовка
...
Функция СформироватьСсылкуСТегом(Ссылка, Схема, НовоеЗначениеСхемы)
	...
	Если Схема = "e1cib/" <> 0 Тогда
		ПредставлениеСсылки = ПредставлениеСсылки(Ссылка);
	КонецЕсли;
...
Процедура СкорректироватьСсылки(ТекстHTML, Схема, НовоеЗначениеСхемы = "")
	...
	Пока ЗаполненаСтруктураТега(Тег) <> 0 Цикл
		...
		Если ЗначениеЗаполнено(НовоеЗначениеСхемы) Тогда
			...
			// проверка что v8doc:e1cib - он уже преобразован, не надо его больше менять.
			Если СтрНайти(Тег.ПараметрыОткрывающегоТега, "v8doc:e1cib") <> 0 Тогда
				Тег = ПолучитьТегВТекстеHTML(ТекстHTML, "a", Тег.ПозицияОкончанияОткрывающегоТега + 1);
				Продолжить;
			КонецЕсли;
...
Функция ПатчитьТекстИсходящегоПисьмаДляМобКлиента(ТекстПисьма) Экспорт
	
	Если (СтрНайти(ТекстПисьма, "<head") = 0) Тогда 
		Возврат ТекстПисьма;
	КонецЕсли;
	
	Если (СтрНайти(ТекстПисьма, "<meta name=""viewport"" content=""width=device-width,") <> 0) Тогда 
		Возврат ТекстПисьма;
	КонецЕсли;
	
	ЧаститекстаПисьма = Новый Массив();
	ПервоеВхождениеHead = СтрНайти(ТекстПисьма, "<head");
	ПоложениеЗакрытиеHead = СтрНайти(ТекстПисьма, ">", , ПервоеВхождениеHead);
	ЧаститекстаПисьма.Добавить(Лев(ТекстПисьма, ПоложениеЗакрытиеHead));
	ЧаститекстаПисьма.Добавить(
		"<meta name=""viewport"" content=""viewport-fit=cover, width=device-width, initial-scale=1.1"" />");
	ЧаститекстаПисьма.Добавить(Прав(ТекстПисьма, СтрДлина(ТекстПисьма) - ПоложениеЗакрытиеHead));
	
	ТекстПисьма = СтрСоединить(ЧаститекстаПисьма);
	
	Возврат ТекстПисьма;
КонецФункции

Функция ПатчитьТекстИсходящегоПисьма(ТекстПисьма) Экспорт
	
	Если (СтрНайти(ТекстПисьма, "<body") = 0) Тогда 
		Возврат ТекстПисьма;
	КонецЕсли;
	
	Если (СтрНайти(ТекстПисьма, "window.addEventListener(""focus""") <> 0) Тогда 
		Возврат ТекстПисьма;
	КонецЕсли;
	
	ТмпТекстПисьма = ТекстПисьма;
	
	//вставляем обработчик после body
	ТмпТекстПисьма = ДобавитьОбработчикВТекстПисьма(ТекстПисьма);
		
	Если (СтрНайти(ТекстПисьма, "contenteditable=true") <> 0) Тогда 
		Возврат ТмпТекстПисьма;
	КонецЕсли;
	
	//вставляем в body "contenteditable=true"
	ТмпТекстПисьма = ДобавитьContentEditableВТекстПисьма(ТмпТекстПисьма);
	
	Возврат ТмпТекстПисьма;
КонецФункции

Функция ДобавитьОбработчикВТекстПисьма(ТекстПисьма)
	
	ТмпТекстПисьма = ТекстПисьма;
	//вставляем обработчик после body
	ТекстДоОткрТэгаBody = Лев(ТекстПисьма, СтрНайти(ТекстПисьма, "<body") + СтрДлина("<body") - 1);
	ТмпТекстПисьма = Прав(ТмпТекстПисьма, СтрДлина(ТекстПисьма) - СтрНайти(ТекстПисьма, "<body") - СтрДлина("<body") + 1);
	ТекстДоЗакрТэгаBody = Лев(ТмпТекстПисьма, СтрНайти(ТмпТекстПисьма, ">"));
	ТмпТекстПисьма = Прав(ТмпТекстПисьма, СтрДлина(ТмпТекстПисьма) - СтрНайти(ТмпТекстПисьма, ">"));
	
	ТмпТекстПисьма = ТекстДоОткрТэгаBody + ТекстДоЗакрТэгаBody + 
		"
		|<script>
...
Функция ДобавитьContentEditableВТекстПисьма(ТекстПисьма)
	ТмпТекстПисьма = ТекстПисьма;
	
	ТекстДоОткрТэгаBody = Лев(ТмпТекстПисьма, СтрНайти(ТмпТекстПисьма, "<body") + СтрДлина("<body") - 1);
	ТмпТекстПисьма = Прав(ТмпТекстПисьма, СтрДлина(ТмпТекстПисьма) - СтрНайти(ТекстПисьма, "<body") - 
		СтрДлина("<body") + 1);
	ТекстДоЗакрТэгаBody = Лев(ТмпТекстПисьма, СтрНайти(ТмпТекстПисьма, ">"));
	ТмпТекстПисьма = Прав(ТмпТекстПисьма, СтрДлина(ТмпТекстПисьма) - СтрНайти(ТмпТекстПисьма, ">"));
	
	ТмпТекстПисьма = ТекстДоОткрТэгаBody + "  contenteditable=true " +
		ТекстДоЗакрТэгаBody + ТмпТекстПисьма;
